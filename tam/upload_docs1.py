"""
Upload Hanja_Phan_130 - Phan_275 tu docs/1 len Supabase
1. Upload cac tu CHUA CO trong DB (bo qua trung lap)
2. Fill pronunciation cho cac tu dang thieu
3. Fill examples + root_meaning + memory_tip cho cac tu dang thieu
Chay: python upload_docs1.py
"""
import os, re, json, time, requests, openpyxl, openai

FOLDER       = r"C:\Users\hi\Desktop\code\han\docs\1"
...
def upload_excel():
    print("\n=== BUOC 1: Upload Excel tu docs/1 ===", flush=True)
    ...
    for fname in files:
        ...
        try:
            ...
            for row in ws.iter_rows(min_row=2, values_only=True):
                ...
                if korean in existing:
                    continue  # bo qua tu da co
                rows.append({
                    "korean": korean,
                    "hanja": hanja,
                    "vietnamese": viet,
        ...
        for i in range(0, len(rows), 50):
            ...
            else: print(f"    Insert loi {r.status_code}: {r.text[:100]}", flush=True)
        total_new += inserted
        print(f"  {fname}: {len(rows)} tu moi -> insert {inserted}", flush=True)

    print(f"\nUpload xong! Tong them: {total_new} tu moi", flush=True)
    return total_new
...
def fetch_missing_pron():
    ...
    while True:
        r = requests.get(
            ...
            f"?select=id,korean&pronunciation=eq.&limit=1000&offset={offset}&order=id.asc",
            headers=hdrs(), timeout=30)
        data = r.json()
        if not isinstance(data, list) or not data: break
    ...
    for i in range(0, len(rows), 15):
        ...
        for uid, pron in result.items():
            r = requests.patch(
                ...
                headers={**hdrs(), "Prefer": "return=minimal"},
                json={"pronunciation": pron}, timeout=15)
...
# ─── BUOC 3: FILL EXAMPLES + root_meaning + memory_tip ────────────────────────
def fetch_missing_examples():
    """Lay tat ca tu con thieu root_meaning (ke ca tu cu chua duoc fill)"""
    all_rows, offset = [], 0
    while True:
        r = requests.get(
            f"{SUPABASE_URL}/rest/v1/hanja_tree_nodes"
            f"?select=id,korean,hanja,vietnamese,root_char,pronunciation"
            f"&root_meaning=eq.&limit=1000&offset={offset}&order=id.asc",
            headers=hdrs(), timeout=30)
        data = r.json()
        if not isinstance(data, list) or not data: break
        all_rows.extend(data)
...
def fetch_missing_examples_null():
    """Fallback: lay tu co root_meaning IS NULL"""
    all_rows, offset = [], 0
    while True:
        r = requests.get(
            f"{SUPABASE_URL}/rest/v1/hanja_tree_nodes"
            f"?select=id,korean,hanja,vietnamese,root_char,pronunciation"
            f"&root_meaning=is.null&limit=1000&offset={offset}&order=id.asc",
            headers=hdrs(), timeout=30)
        data = r.json()
        if not isinstance(data, list) or not data: break
        all_rows.extend(data)
...
def ai_enrich(words):
    items = [
        f"{i+1}. {w['korean']} ({w['hanja']}) = {w['vietnamese']} | goc: {w['root_char']}"
...
def fill_examples():
    ...
    # fallback: lay them tu co null
    null_rows = fetch_missing_examples_null()
