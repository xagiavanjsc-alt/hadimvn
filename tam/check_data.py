...
r = requests.get('https://dcjofhkdrgbrowabudyt.supabase.co/rest/v1/hanja_pro?select=slug,meaning_vn,hanja_breakdown&order=id', headers=headers)
data = r.json()
for d in data:
    bd = json.dumps(d['hanja_breakdown'], ensure_ascii=False)
    print(f"{d['slug']:20s} | {d['meaning_vn']:30s} | {bd[:80]}")
