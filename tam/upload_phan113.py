"""
Upload Phan_113.md len Supabase, bat dau tu ID 2349
Slug = phien am tieng Han (SEO-friendly)
Chay: python upload_phan113.py
LUU Y: Phan_113 chi co 20 entries (thieu 1 so voi 21 yeu cau)
"""
import re
import logging
import requests
...
# ================== CAU HINH ==================
FILE_INPUT = os.path.join(os.path.dirname(__file__), "fix", "Phan_113.md")
...
def main():
    try:
        ...
        if len(entries) != 20:
            logger.warning(f"CANH BAO: Phan_113 chi co {len(entries)} entries (yeu cau 21)")
