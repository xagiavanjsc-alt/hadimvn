"""
Upload Phan_125.md len Supabase, bat dau tu ID 2601
Slug = phien am tieng Han (SEO-friendly)
Chay: python upload_phan125.py
"""
import re
import logging
import requests
...
# ================== CAU HINH ==================
FILE_INPUT = os.path.join(os.path.dirname(__file__), "fix", "Phan_125.md")
...
def upload_to_supabase(data):
    ...
    if response.status_code in [200, 201, 204]:
        return True
    elif response.status_code == 409 and 'duplicate key value violates unique constraint' in response.text:
        # Slug trung - thu suffix
        if data['slug'] == 'won-su':
            data['slug'] = 'won-su-2'
            response = requests.post(url, json=data, headers=headers)
            if response.status_code in [200, 201, 204]:
                logger.info(f"Da upload voi slug moi: {data['slug']}")
                return True
        logger.error(f"Upload failed (duplicate slug): {response.status_code} - {response.text}")
        return False
    else:
