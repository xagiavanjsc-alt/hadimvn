...
function exportToCSV(transactions: VipTransaction[]) {
  const headers = ["ID", "Thành viên", "Lo?i hành d?ng", "Lo?i VIP", "S? ti?n (VNÐ)", "Admin th?c hi?n", "Ghi chú", "Ngày h?t h?n VIP", "Th?i gian"];
...
export default function AdminVipTransactionsPage() {
  ...
  return (
    <AdminLayout
      ...
      subtitle="Theo dõi chi ti?t t?ng l?n c?p, gia h?n, thu h?i VIP"
      actions={
      ...
      <div className="rounded-2xl border p-5 mb-6" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: "var(--admin-text-muted)" }}>6 tháng g?n nh?t</p>
          </div>
      ...
      <div className="rounded-2xl border overflow-hidden" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-sm" style={{ color: "var(--admin-text-muted)" }}>Không có giao d?ch nào</p>
          </div>
        ) : (
          <>
            {/* Header */}
            ...
            >
              ...
              <span className="text-[10px] tracking-normal font-semibold" style={{ color: "var(--admin-text-faint)" }}>Thành viên & Chi ti?t</span>
            ...
            {hasMore && (
              <div className="px-5 py-4 border-t" style={{ borderColor: "var(--admin-border)" }}>
                <button
                  ...
                  style={{ backgroundColor: "var(--admin-hover)", color: "var(--admin-text-muted)", border: "1px solid var(--admin-border)" }}
                >
                  Xem thêm ({filtered.length - paginated.length} giao d?ch còn l?i)
                </button>
              </div>
            )}
          </>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: "var(--admin-border)" }}>
              <p className="font-bold text-sm" style={{ color: "var(--admin-text)" }}>Chi ti?t thành viên</p>
            ...
            <div className="p-5 space-y-3">
              ...
              {[
                { label: "Tr?ng thái VIP", value: selectedUser.is_vip ? "Ðang ho?t d?ng" : "Không có VIP", color: selectedUser.is_vip ? "#34d399" : "#6b7280" },
                ...
                { label: "Ngày dang ký", value: new Date(selectedUser.created_at).toLocaleDateString("vi-VN"), color: "var(--admin-text)" },
                {
                  label: "S? giao d?ch",
