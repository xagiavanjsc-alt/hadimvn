    ...
    >
      ...
      <div className="flex items-center gap-2 mb-6">
        {([["7days", "7 ngày"], ["30days", "30 ngày"], ["90days", "90 ngày"]] as [Period, string][]).map(([p, label]) => (
          <button
            key={p}
            onClick={() => setPeriod(p)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all cursor-pointer whitespace-nowrap ${period === p ? "bg-[#e8c84a]/15 text-[#e8c84a] border border-[#e8c84a]/25" : "bg-white/3 text-white/40 border border-white/8 hover:text-white/60"}`}
          >
            {label}
          </button>
        ))}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          ...
          <div key={card.label} className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-6">
            ...
            {periodXP === 0 ? (
              <div className="flex flex-col items-center justify-center h-48 text-center">
                ...
                <p className="text-white/30 text-sm">Chưa có dữ liệu XP trong kỳ này</p>
                <p className="text-white/20 text-xs mt-1">Hãy học flashcard, làm bài thi để tích lũy XP!</p>
              </div>
            ) : (
                ...
                {chartData.map((d, i) => {
                  ...
                  return (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1 group relative">
                      <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-[#1a1a2e] border border-white/10 rounded-lg px-2 py-1 text-[10px] text-white whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                        {d.xp} XP
                      </div>
                      <div
                        className={`w-full rounded-t-md transition-all ${isLast ? "bg-[#e8c84a]" : d.xp > 0 ? "bg-[#e8c84a]/40 group-hover:bg-[#e8c84a]/60" : "bg-white/5"}`}
                        style={{ height: `${height}%` }}
                      ></div>
          ...
          <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-6">
            ...
            {xpBySource.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-white/25 text-sm">Chưa có dữ liệu nguồn XP</p>
              </div>
            ) : (
                    ...
                    <div key={type} className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-white/80 text-xs font-semibold">{xp} XP <span className="text-white/30 font-normal">({pct}%)</span></span>
                        </div>
          ...
          <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-6">
            ...
            {xpHistory.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-white/25 text-sm">Chưa có lịch sử XP</p>
              </div>
            ) : (
          ...
          <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
            ...
            <div className="flex items-center gap-3 mb-4 p-3 rounded-xl" style={{ backgroundColor: `${currentRank.color}08`, border: `1px solid ${currentRank.color}20` }}>
              ...
              <div>
                ...
                <p className="text-white/40 text-xs">{currentRank.nameKo} · {totalXP.toLocaleString()} XP</p>
              </div>
            </div>
            ...
            {nextRank && (
              <>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-white/40 text-xs">Tiến độ lên {nextRank.name}</span>
                ...
                <p className="text-white/30 text-xs text-center">Còn <strong className="text-white/60">{xpToNext.toLocaleString()} XP</strong> để lên cấp</p>
              </>
            )}
            {!nextRank && (
              <div className="text-center py-2">
                <i className="ri-vip-crown-fill text-[#e8c84a] text-2xl mb-1"></i>
                <p className="text-[#e8c84a] text-sm font-bold">Cấp tối đa!</p>
              </div>
            )}
          </div>
              ...
              {daysToNextLevel !== null && avgXPPerDay > 0 ? (
                <>
                  <div className="text-center py-3">
                    <p className="text-[#e8c84a] font-bold text-3xl">{daysToNextLevel}</p>
                    <p className="text-white/40 text-xs mt-1">ngày nữa</p>
                  </div>
                ...
                <div className="text-center py-4">
                  <p className="text-white/30 text-sm">Chưa đủ dữ liệu</p>
                  <p className="text-white/20 text-xs mt-1">Hãy học ít nhất 1 ngày để dự đoán</p>
                </div>
              )}
            </div>
          )}
          ...
          <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
            ...
            <div className="flex items-center gap-3 mb-4 p-3 rounded-xl" style={{ backgroundColor: `${COMMUNITY_BENCHMARKS[userBenchmark].color}08`, border: `1px solid ${COMMUNITY_BENCHMARKS[userBenchmark].color}20` }}>
              ...
              <div>
                ...
                <p className="text-white/40 text-xs">{avgXPPerDay} XP/ngày của bạn</p>
              </div>
            </div>
            <div className="space-y-2.5">
              {COMMUNITY_BENCHMARKS.map((b, i) => {
                ...
                return (
                  <div key={b.label} className={`flex items-center gap-3 p-2 rounded-lg ${isUser ? "bg-white/5" : ""}`}>
                    ...
                    <span className="text-[10px] text-white/30 w-16">{b.xpPerDay} XP/ng</span>
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={i} className="flex items-center gap-2">
                  ...
                  <p className="text-white/40 text-xs">{tip.text}</p>
                </div>
              ))}
            </div>
          </div>
