...
function AdminPanel() {
  ...
  const subTabs = [
    ...
    { key: "groups" as const, label: "Nhóm t?", icon: "ri-group-line" },
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-3">
        ...
        <p className="text-amber-600 text-sm font-medium">Ch? d? Admin — Ch? hi?n th? cho qu?n tr? viên</p>
      </div>
      ...
      {adminSubTab === "overview" && (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: "T?ng t? Hán Hàn", value: totalWords.toLocaleString(), icon: "ri-translate-2", color: "app-accent-primary" },
              { label: "Ðã thu?c", value: totalMastered, icon: "ri-check-double-line", color: "#34d399" },
              { label: "Ðang h?c", value: totalLearning, icon: "ri-book-open-line", color: "#fb923c" },
              { label: "T?ng l?n ôn", value: totalReviews.toLocaleString(), icon: "ri-refresh-line", color: "#a78bfa" },
            ].map(s => (
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <h3 className="text-white font-semibold text-sm mb-4 flex items-center gap-2">
              <i className="ri-user-line text-app-text-secondary"></i>Ho?t d?ng ngu?i dùng
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                { label: "XP tích luy", value: xpData.toLocaleString() + " XP", color: "app-accent-primary" },
                { label: "Streak hi?n t?i", value: (streakData?.currentStreak || 0) + " ngày", color: "#fb923c" },
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <h3 className="text-white font-semibold text-sm mb-4 flex items-center gap-2">
              <i className="ri-settings-3-line text-app-text-secondary"></i>Qu?n lý nhanh
            </h3>
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: "Reset d? li?u h?c Hán Hàn", icon: "ri-delete-bin-line", color: "#ef4444", action: () => { if (confirm("Reset toàn b? d? li?u h?c Hán Hàn?")) { localStorage.removeItem("hanja_sr_data"); window.location.reload(); } } },
                ...
                { label: "Xem d? li?u SR (JSON)", icon: "ri-code-line", color: "#a78bfa", action: () => { const d = localStorage.getItem("hanja_sr_data"); alert(d ? `${Object.keys(JSON.parse(d)).length} t? dã h?c` : "Chua có d? li?u"); } },
                { label: "Xu?t báo cáo JSON", icon: "ri-download-line", color: "#34d399", action: () => { const data = { totalWords, totalMastered, totalLearning, totalReviews, groupStats }; const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "hanja-report.json"; a.click(); } },
              ].map(btn => (
      ...
      {adminSubTab === "chart" && (
        <div className="bg-app-bg border border-app-border rounded-2xl p-5">
          <h3 className="text-white font-semibold text-sm mb-1 flex items-center gap-2">
            <i className="ri-bar-chart-2-line text-app-text-secondary"></i>Ho?t d?ng h?c 30 ngày qua
          </h3>
          <p className="text-app-text-muted text-xs mb-5">S? t? ôn t?p m?i ngày (t? d? li?u Spaced Repetition)</p>

          {activityData.every(d => d.count === 0) ? (
            <div className="text-center py-12">
              <i className="ri-bar-chart-2-line text-white/10 text-4xl mb-3 block"></i>
              <p className="text-app-text-muted text-sm">Chua có d? li?u h?c t?p</p>
              <p className="text-app-text-muted text-xs mt-1">B?t d?u ôn t?p Spaced Repetition d? xem bi?u d?</p>
            </div>
          ) : (
            <>
              {/* Bar chart */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-5">
                {[
                  { label: "T?ng t? ôn (30 ngày)", value: activityData.reduce((s, d) => s + d.count, 0), color: "#f43f5e" },
                  { label: "Ngày h?c nhi?u nh?t", value: Math.max(...activityData.map(d => d.count)), color: "#fb923c" },
                  { label: "Ngày có h?c", value: activityData.filter(d => d.count > 0).length, color: "#34d399" },
                ].map(s => (
        ...
        <div className="bg-app-bg border border-app-border rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-app-border">
            <h3 className="text-white font-semibold text-sm flex items-center gap-2">
              <i className="ri-error-warning-line text-amber-400"></i>T? du?c ôn nhi?u nh?t (khó nh?t)
            </h3>
            <p className="text-app-text-muted text-xs mt-0.5">S?p x?p theo s? l?n ôn t?p — t? c?n ôn nhi?u nh?t</p>
          </div>

          {hardestWords.length === 0 ? (
            <div className="text-center py-12">
              <i className="ri-emotion-happy-line text-white/10 text-4xl mb-3 block"></i>
              <p className="text-app-text-muted text-sm">Chua có d? li?u quiz</p>
              <p className="text-app-text-muted text-xs mt-1">Làm quiz và Spaced Repetition d? xem th?ng kê</p>
            </div>
          ) : (
                ...
                <div key={w.korean} className="flex items-center gap-4 px-5 py-3 hover:bg-white/2 transition-colors">
                  ...
                  <div className="flex items-center gap-4 flex-shrink-0">
                    <div className="text-center">
                      ...
                      <p className="text-app-text-muted text-xs">l?n ôn</p>
                    </div>
                    <div className="text-center">
      ...
      {adminSubTab === "groups" && (
        <div className="bg-app-bg border border-app-border rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-app-border">
            <h3 className="text-white font-semibold text-sm">Th?ng kê theo nhóm ch? cái</h3>
            <p className="text-app-text-muted text-xs mt-0.5">T?ng {totalWords} t? · {totalMastered} dã thu?c ({totalWords > 0 ? Math.round((totalMastered / totalWords) * 100) : 0}%)</p>
          </div>
          <div className="divide-y divide-white/3">
            {groupStats.map(g => (
      ...
      {adminSubTab === "content" && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[
              { label: "T? yêu thích", value: contentStats.favCount, icon: "ri-heart-line", color: "#f43f5e" },
              ...
              { label: "L?n luy?n phát âm", value: contentStats.pronunciationSessions, icon: "ri-mic-line", color: "#34d399" },
              { label: "Thách th?c tu?n", value: contentStats.hasWeeklyChallenge ? "Ðang có" : "Chua có", icon: "ri-sword-line", color: "#fb923c" },
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <h3 className="text-white font-semibold text-sm mb-4 flex items-center gap-2">
              <i className="ri-navigation-line text-app-text-secondary"></i>Ði?u hu?ng nhanh d?n các tính nang
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[
                { label: "T? v?ng Hán Hàn", icon: "ri-translate-2", color: "#f43f5e", path: "/hanja-vocab" },
                ...
                { label: "Luy?n phát âm", icon: "ri-mic-line", color: "#34d399", path: "/hanja-vocab" },
                { label: "So sánh Hán Vi?t", icon: "ri-translate-2", color: "#06b6d4", path: "/hanja-vocab" },
                { label: "Câu ví d? th?c t?", icon: "ri-newspaper-line", color: "#fb923c", path: "/hanja-vocab" },
              ].map(item => (
                <button
                  key={item.label}
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <h3 className="text-white font-semibold text-sm mb-4 flex items-center gap-2">
              <i className="ri-database-2-line text-app-text-secondary"></i>Qu?n lý d? li?u ngu?i dùng
            </h3>
            <div className="space-y-2">
              {[
                {
                  label: "Xu?t toàn b? d? li?u h?c",
                ...
                {
                  label: "Reset d? li?u yêu thích",
                  desc: "Xóa t?t c? t? dã luu yêu thích",
                  icon: "ri-heart-3-line",
                  color: "#f43f5e",
                  action: () => { if (confirm("Xóa t?t c? t? yêu thích?")) { localStorage.removeItem("hanja_favorites"); window.location.reload(); } }
                },
                {
                  label: "Reset ghi chú",
                  desc: "Xóa t?t c? ghi chú t? v?ng",
                  icon: "ri-sticky-note-2-line",
                  color: "app-accent-primary",
                  action: () => { if (confirm("Xóa t?t c? ghi chú?")) { localStorage.removeItem("hanja_notes"); window.location.reload(); } }
                },
                {
                  label: "Reset l?ch s? thi TOPIK",
                  desc: "Xóa k?t qu? thi th? TOPIK Hán Hàn",
                  icon: "ri-file-paper-2-line",
                  color: "#a78bfa",
                  action: () => { if (confirm("Xóa l?ch s? thi TOPIK?")) { localStorage.removeItem("hanja_topik_history"); window.location.reload(); } }
                },
              ].map(btn => (
                <button
                  key={btn.label}
...
function RedemptionRequestsPanel() {
  ...
  const handleApprove = async (id: string) => {
    if (!confirm("Duy?t yêu c?u d?i VIP này?")) return;
    try {
  ...
  const handleReject = async (id: string) => {
    const note = prompt("Lý do t? ch?i (tùy ch?n):");
    try {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-app-bg border border-app-border rounded-2xl p-5">
        <h3 className="text-white font-semibold text-sm mb-1 flex items-center gap-2">
          <i className="ri-vip-crown-line text-amber-400"></i>Qu?n lý yêu c?u d?i VIP
        </h3>
        <p className="text-app-text-muted text-xs mb-4">
          Duy?t ho?c t? ch?i yêu c?u d?i VIP t? ngu?i dùng
        </p>
        ...
        <div className="flex gap-2 mb-4">
          {(["all", "pending", "approved", "rejected"] as const).map(f => (
            ...
            >
              {f === "all" ? "T?t c?" : f === "pending" ? "Ch? duy?t" : f === "approved" ? "Ðã duy?t" : "Ðã t? ch?i"}
            </button>
          ))}
        </div>

        {loading ? (
          <div className="text-center py-8 text-app-text-muted text-sm">Ðang t?i...</div>
        ) : requests.length === 0 ? (
          <div className="text-center py-8 text-app-text-muted text-sm">Không có yêu c?u nào</div>
        ) : (
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                      ...
                      {r.status === "pending" ? "Ch? duy?t" : r.status === "approved" ? "Ðã duy?t" : "Ðã t? ch?i"}
                    </span>
                  </div>
...
function GrantXPPanel() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setUsers((data || []).map(u => ({
        id: u.id,
        display_name: u.display_name || "H?c viên",
        avatar_url: u.avatar_url,
        xp: xpMap.get(u.id) || 0,
      })));
    } catch (e) {
      setResult({ ok: false, msg: e instanceof Error ? e.message : "Tìm ki?m th?t b?i" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGrant = async () => {
    ...
    try {
      ...
      setResult({ ok: true, msg: `Ðã ${amount > 0 ? "trao" : "tr?"} ${Math.abs(amount)} XP cho ${selected.display_name}. XP m?i: ${newXP}` });
  ...
  return (
    <div className="space-y-4">
      <div className="bg-app-bg border border-app-border rounded-2xl p-5">
        <h3 className="text-white font-semibold text-sm mb-1 flex items-center gap-2">
          <i className="ri-gift-2-line text-amber-400"></i>Trao XP th? công cho h?c viên
        </h3>
        <p className="text-app-text-muted text-xs mb-4">
          Tìm h?c viên theo tên hi?n th?, sau dó trao bonus XP (positive) ho?c tr? XP (negative). M?i thay d?i tuân th? rule never-decrease ? client; mu?n tr? XP hãy nh?p s? âm.
        </p>
        ...
        <div className="flex gap-2 mb-3">
          <input
            ...
            placeholder="Tìm theo tên hi?n th?..."
        ...
        {selected && (
          <div className="bg-amber-500/5 border border-amber-400/20 rounded-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-app-text-muted text-xs mb-1 block">S? XP (âm = tr?)</label>
                <input
                  type="number"
                  value={amount}
              ...
              <div>
                <label className="text-app-text-muted text-xs mb-1 block">Lý do (tu? ch?n)</label>
                <input
                  type="text"
                  value={reason}
            ...
            <div className="flex gap-2">
              ...
              >
                {submitting ? "Ðang x? lý..." : `${amount > 0 ? "+" : ""}${amount} XP — Xác nh?n`}
              </button>
            </div>
          </div>
        )}
      ...
      {history.length > 0 && (
        <div className="bg-app-bg border border-app-border rounded-2xl p-5">
          <h4 className="text-white font-semibold text-sm mb-3 flex items-center gap-2">
            <i className="ri-history-line text-app-text-muted"></i>L?ch s? trao XP g?n dây
          </h4>
          <div className="space-y-2">
...
const REWARDS: RewardItem[] = [
  { id: "r4", title: "Huy hi?u H?c viên Tích c?c", desc: "Huy hi?u d?c bi?t hi?n th? trên h? so và b?ng x?p h?ng", xpCost: 500, type: "badge", icon: "ri-medal-2-line", color: "#a78bfa", stock: 999 },
  { id: "r6", title: "M? khóa giao di?n t?i", desc: "Dark mode d?c bi?t — ch? dành cho h?c viên tích c?c", xpCost: 1500, type: "feature", icon: "ri-moon-line", color: "#06b6d4", stock: 999 },
  { id: "r1", title: "Gi?m 20% VIP tháng d?u", desc: "Áp d?ng khi dang ký gói VIP l?n d?u tiên", xpCost: 2500, type: "discount", icon: "ri-coupon-3-line", color: "app-accent-primary", stock: 50, popular: true },
  { id: "r2", title: "VIP 7 ngày mi?n phí", desc: "Tr?i nghi?m toàn b? tính nang VIP trong 1 tu?n", xpCost: 5000, type: "vip", icon: "ri-vip-crown-line", color: "#FFD700", stock: 20, popular: true },
  { id: "r3", title: "Gi?m 50% VIP tháng d?u", desc: "Uu dãi l?n cho h?c viên c?c k? tích c?c", xpCost: 8000, type: "discount", icon: "ri-percent-line", color: "#34d399", stock: 10 },
  { id: "r5", title: "VIP 30 ngày mi?n phí", desc: "1 tháng VIP hoàn toàn mi?n phí — ph?n thu?ng cao nh?t", xpCost: 15000, type: "vip", icon: "ri-vip-diamond-line", color: "#fb923c", stock: 5 },
];
...
const XP_SOURCES = [
  // -- H?c t?p ---------------------------------------------------------
  { icon: "ri-login-circle-line", label: "Ðang nh?p h?ng ngày", xp: "+10 XP", color: "app-accent-primary", desc: "1 l?n/ngày — gi? streak d? nh?n bonus" },
  { icon: "ri-fire-line", label: "Streak 7 ngày liên ti?p", xp: "+50 XP", color: "#fb923c", desc: "Bonus 1 l?n khi d?t m?c 7 ngày" },
  { icon: "ri-fire-line", label: "Streak 30 ngày liên ti?p", xp: "+200 XP", color: "#fb923c", desc: "Bonus 1 l?n khi d?t m?c 30 ngày" },
  { icon: "ri-fire-fill", label: "Streak 100 ngày liên ti?p", xp: "+500 XP", color: "#f59e0b", desc: "Bonus 1 l?n khi d?t m?c 100 ngày" },
  { icon: "ri-stack-line", label: "H?c Flashcard m?i", xp: "+5 XP/t?", color: "#a78bfa", desc: "T?i da 100 t?/ngày (500 XP)" },
  { icon: "ri-file-list-3-line", label: "EPS tr? l?i dúng", xp: "+3 XP/câu", color: "#06b6d4", desc: "T?i da 200 câu/ngày (600 XP)" },
  { icon: "ri-timer-line", label: "Hoàn thành thi th? EPS", xp: "+20 XP", color: "#34d399", desc: "T?i da 5 bài/ngày" },
  { icon: "ri-file-paper-2-line", label: "Hoàn thành thi TOPIK", xp: "+25 XP", color: "#f43f5e", desc: "T?i da 5 bài/ngày" },
  { icon: "ri-book-2-line", label: "Hoàn thành luy?n ch? d?", xp: "+15 XP", color: "#22d3ee", desc: "T?i da 10 bài/ngày" },
  { icon: "ri-survey-line", label: "Hoàn thành quiz", xp: "+10 XP", color: "#f472b6", desc: "T?i da 10 quiz/ngày" },
  // -- Hán Hàn ---------------------------------------------------------
  { icon: "ri-translate-2", label: "H?c t? Hán Hàn m?i", xp: "+3 XP/t?", color: "#a78bfa", desc: "T?i da 100 t?/ngày" },
  { icon: "ri-plant-line", label: "Hoàn thành cây Hán", xp: "+30 XP", color: "#34d399", desc: "T?i da 5 cây/ngày" },
  { icon: "ri-question-answer-line", label: "Hoàn thành quiz Hán", xp: "+15 XP", color: "#60a5fa", desc: "T?i da 10 quiz/ngày" },
  // -- C?ng d?ng -------------------------------------------------------
  { icon: "ri-article-line", label: "Ðang bài (du?c duy?t)", xp: "+15 XP", color: "#60a5fa", desc: "T?i da 5 bài/ngày" },
  { icon: "ri-heart-3-line", label: "Bài vi?t nh?n lu?t thích", xp: "+2 XP", color: "#f87171", desc: "T?i da 100 lu?t/ngày" },
];

// Ph?n thu?ng khác (ngoài XP_REWARDS): dùng `addXP(amount, reason)` — t? do
const XP_MANUAL_REWARDS = [
  { icon: "ri-gift-line", label: "Ph?n thu?ng admin trao", xp: "Tu?", color: "#a78bfa", desc: "Admin có th? c?p bonus th? công" },
  { icon: "ri-trophy-line", label: "Hoàn thành bài/lesson", xp: "+5 ? +50 XP", color: "#FFD700", desc: "Tu? lo?i bài: Seoul, EPS, luy?n t?p..." },
];

const XP_NOTES = [
  { icon: "ri-shield-check-line", label: "Anti-cheat", desc: "H? th?ng có gi?i h?n XP m?i ngày theo lo?i ho?t d?ng d? ch?ng spam. M?t s? ho?t d?ng (bonus streak, hoàn thành bài) ch? c?ng 1 l?n duy nh?t.", color: "#34d399" },
  { icon: "ri-refresh-line", label: "Ð?ng b? server", desc: "XP ch? tang, không gi?m. Leaderboard c?p nh?t theo max(công th?c, t?ng local) d? b?o v? di?m c?a b?n.", color: "#60a5fa" },
];
...
export default function RewardsPage() {
  ...
  const handleRedeem = async (reward: RewardItem) => {
    ...
    if (reward.type === "vip") {
      if (!user) {
        alert("Vui lòng dang nh?p d? d?i VIP");
        return;
      }
      ...
      if (error) {
        alert("L?i khi g?i yêu c?u d?i VIP: " + error.message);
        return;
      }
      
      setRedeemMsg({ id: reward.id, msg: `Ðã g?i yêu c?u d?i ${reward.title} - ch? admin duy?t!` });
    } else {
      // Non-VIP rewards are automatic
      setRedeemSpent(prev => prev + reward.xpCost);
      setRedeemedRewards(prev => [...prev, reward.id]);
      setRedeemMsg({ id: reward.id, msg: `Ðã d?i thành công: ${reward.title}!` });
    }
  ...
  return (
    <DashboardLayout
      ...
      subtitle="Tích luy XP t? vi?c h?c, d?i l?y uu dãi VIP và huy hi?u"
    >
      <DailyLoginBonus />
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex-1">
            ...
            <p className="text-app-text-secondary text-sm mb-3">
              {xpToNextReward.affordable > 0 ? `Có th? d?i ${xpToNextReward.affordable} ph?n thu?ng ngay!` : "Ti?p t?c h?c d? tích luy XP"}
              {xpToNextReward.next && ` · C?n thêm ${(xpToNextReward.next.xpCost - displayXp).toLocaleString()} XP cho ph?n thu?ng ti?p theo`}
            </p>
          ...
          <div className="grid grid-cols-2 gap-4 text-center">
            {[
              ...
              { label: "Ðã d?i", value: redeemedRewards.length, color: "#34d399" },
            ].map(s => (
              <div key={s.label}>
      ...
      <div className="flex flex-wrap gap-1 bg-app-surface/50 p-1 rounded-xl mb-6 w-fit">
        {([["rewards", "ri-gift-line", "Ð?i thu?ng"], ["earn", "ri-add-circle-line", "Cách ki?m XP"], ["history", "ri-history-line", "L?ch s?"]] as const).map(([tab, icon, label]) => (
          <button
            key={tab}
      ...
      {showAdminLogin && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setShowAdminLogin(false)}>
          <div className="bg-app-bg border border-app-border rounded-2xl p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
            <h3 className="text-white font-bold mb-4 flex items-center gap-2"><i className="ri-shield-keyhole-line text-amber-400"></i>Xác th?c Admin</h3>
            <input type="password" value={adminCode} onChange={e => setAdminCode(e.target.value)}
              placeholder="Nh?p mã admin..."
            ...
            <div className="flex gap-3">
              ...
              }} className="flex-1 py-2.5 bg-amber-500 text-white rounded-xl text-sm font-semibold cursor-pointer hover:bg-amber-600 transition-colors">Xác nh?n</button>
            </div>
          </div>
        </div>
      )}
              ...
              >
                ...
                {redeemed ? (
                  <div className="w-full py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-app-accent-success text-xs font-bold text-center">
                    <i className="ri-checkbox-circle-fill mr-1"></i>Ðã d?i
                  </div>
                ) : (
                  <button
                  ...
                  >
                    {canAfford ? "Ð?i ngay" : `Thi?u ${(reward.xpCost - displayXp).toLocaleString()} XP`}
                  </button>
                )}
              </div>
            );
        ...
        <div className="space-y-4 max-w-2xl">
          ...
          <div className="bg-gradient-to-r from-app-surface to-[#0f1117] border border-app-accent-primary/20 rounded-2xl p-5">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <p className="text-white font-bold text-base">Ðang nh?p hàng ngày</p>
                  <span className="bg-app-accent-primary/15 text-app-accent-primary text-[10px] font-bold px-2 py-0.5 rounded-full">M?i!</span>
                </div>
                <p className="text-white/50 text-sm">Nh?n <span className="text-app-accent-primary font-bold">1–5 XP ng?u nhiên</span> m?i ngày khi m? app. Hôm nay b?n dã nh?n r?i!</p>
              </div>
              <div className="text-center">
          ...
          <div className="bg-white/2 border border-app-border rounded-2xl overflow-hidden">
            <div className="px-5 py-3 border-b border-app-border bg-app-surface/30">
              <h3 className="text-white font-semibold text-sm">T?t c? cách ki?m XP</h3>
            </div>
          ...
          <div className="bg-white/2 border border-[#a78bfa]/15 rounded-2xl overflow-hidden">
            <div className="px-5 py-3 border-b border-[#a78bfa]/20 bg-[#a78bfa]/5">
              <h3 className="text-white font-semibold text-sm flex items-center gap-2">
                <i className="ri-gift-line text-[#a78bfa]"></i>Ph?n thu?ng bonus — Ngoài công th?c c? d?nh
              </h3>
            </div>
            <div className="overflow-x-auto">
          ...
          <div className="bg-app-accent-primary/5 border border-app-accent-primary/15 rounded-xl p-4">
            <p className="text-app-accent-primary/80 text-xs font-semibold mb-1">M?o tích XP nhanh nh?t</p>
            <p className="text-app-text-secondary text-xs leading-relaxed">
              Duy trì streak m?i ngày là cách ki?m XP hi?u qu? nh?t. K?t h?p h?c flashcard, luy?n EPS/TOPIK, quiz Hán Hàn và dóng góp bài c?ng d?ng d? leo h?ng. XP ch? tang — không bao gi? gi?m!
            </p>
            <button onClick={() => navigate("/daily-plan")} className="mt-2 flex items-center gap-1.5 text-app-accent-primary text-xs font-semibold cursor-pointer whitespace-nowrap hover:text-app-accent-primary/80">
              <i className="ri-route-line"></i>Xem l? trình hôm nay ?
            </button>
          </div>
        </div>
      )}
                  ...
                  <div key={id} className="flex items-center gap-4 px-4 py-4 bg-app-bg border border-app-border rounded-xl">
                    ...
                    <div className="text-right">
                      ...
                      <div className="flex items-center gap-1 justify-end mt-0.5">
                        ...
                        <span className="text-app-accent-success text-[10px]">Ðã d?i</span>
                      </div>
                    </div>
                  </div>
                );
