...
const mockVocab: VocabCard[] = [
  { id: "m1", korean: "??", vietnamese: "Tình yêu", example: "????.", topic: "C?m xúc", level: "A1" },
  { id: "m2", korean: "??", vietnamese: "H?nh phúc", example: "?????!", topic: "C?m xúc", level: "A1" },
  { id: "m3", korean: "????", vietnamese: "H?c bài", example: "??? ????.", topic: "Giáo d?c", level: "A1" },
  { id: "m4", korean: "??", vietnamese: "B?n bè", example: "??? ????.", topic: "Xã h?i", level: "A1" },
  ...
  { id: "m9", korean: "??", vietnamese: "Phát tri?n", example: "?? ?????.", topic: "Xã h?i", level: "B1" },
  { id: "m10", korean: "??", vietnamese: "Môi tru?ng", example: "??? ????.", topic: "Xã h?i", level: "B1" },
  { id: "m11", korean: "??", vietnamese: "Van hóa", example: "?? ??? ???.", topic: "Xã h?i", level: "A2" },
  ...
  { id: "m14", korean: "??", vietnamese: "Thành công", example: "???? ???.", topic: "Tr?u tu?ng", level: "A2" },
  { id: "m15", korean: "??", vietnamese: "Thói quen", example: "?? ??? ????.", topic: "Tr?u tu?ng", level: "B1" },
  { id: "m16", korean: "??", vietnamese: "Thái d?", example: "?? ??? ????.", topic: "Tr?u tu?ng", level: "B1" },
  { id: "m17", korean: "??", vietnamese: "S? th?u hi?u", example: "??? ???.", topic: "Tr?u tu?ng", level: "A2" },
  { id: "m18", korean: "??", vietnamese: "Bi?u hi?n/Di?n d?t", example: "??? ????.", topic: "Ngôn ng?", level: "B1" },
  { id: "m19", korean: "??", vietnamese: "Ð? xu?t/Gi?i thi?u", example: "? ?? ????.", topic: "Ho?t d?ng", level: "A2" },
  { id: "m20", korean: "??", vietnamese: "Gi?i thích", example: "??? ???.", topic: "Ngôn ng?", level: "A2" },
];
...
function FlashCard({ card, onRate, cardIndex, total }: {
  ...
  const ratings: { label: string; value: Rating; color: string; desc: string }[] = [
    { label: "L?i", value: 1, color: "bg-rose-500/15 border-rose-500/30 text-rose-400 hover:bg-rose-500/25", desc: "Không nh?" },
    { label: "Khó", value: 2, color: "bg-amber-500/15 border-amber-500/30 text-amber-400 hover:bg-amber-500/25", desc: "Nh? m?" },
    ...
    <div className={`transition-opacity duration-200 ${animating ? "opacity-0" : "opacity-100"}`}>
        ...
        <div className="flex flex-col items-center justify-center p-8 min-h-[260px]">
          {!flipped ? (
            <div className="text-center">
              <p className="text-app-text-muted text-xs mb-4 tracking-normal">Ti?ng Hàn</p>
      ...
      {!flipped && (
        <button onClick={handleFlip}
          ...
          <i className="ri-refresh-line mr-2"></i>L?t th? d? xem dáp án
        </button>
      )}
    </div>
  );
...
function StatsPanel({ memories, vocab }: { memories: Record<string, CardMemory>; vocab: VocabCard[] }) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
      {[
        { label: "C?n ôn hôm nay", value: dueCount, color: "#f87171", icon: "ri-alarm-line" },
        { label: "T? m?i", value: newCount, color: "app-accent-primary", icon: "ri-add-circle-line" },
        { label: "Ðã thu?c", value: learnedCount, color: "#34d399", icon: "ri-checkbox-circle-line" },
        { label: "T?ng l?n ôn", value: totalReviews, color: "#a78bfa", icon: "ri-repeat-line" },
        { label: "Ð? chính xác", value: `${Math.round(avgAccuracy * 100)}%`, color: "#fbbf24", icon: "ri-percent-line" },
      ].map(s => (
  ...
  return (
    <div className="rounded-2xl border border-app-accent-primary/15 bg-app-accent-primary/3 p-5 mb-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          <p className="text-app-accent-primary font-bold text-sm">AI Phân tích h?c t?p</p>
          <p className="text-app-text-secondary text-xs">D?a trên thu?t toán SM-2 và l?ch s? ôn t?p c?a b?n</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          <p className="text-rose-400 text-xs font-semibold mb-2 flex items-center gap-1">
            <i className="ri-error-warning-line"></i> T? c?n chú ý ({weakCards.length})
          </p>
          {weakCards.length > 0 ? weakCards.map(s => (
            <div key={s.vocab.id} className="flex items-center justify-between py-1.5">
              ...
              <span className="text-rose-400 text-xs">{Math.round(s.accuracy * 100)}% dúng</span>
            </div>
        ...
        <div>
          <p className="text-amber-400 text-xs font-semibold mb-2 flex items-center gap-1">
            <i className="ri-time-line"></i> Ð?n h?n ôn ({dueCards.length})
          </p>
          ...
          )) : <p className="text-app-text-muted text-xs">Không có t? d?n h?n</p>}
        </div>

        {/* Streak cards */}
        <div>
          <p className="text-app-accent-success text-xs font-semibold mb-2 flex items-center gap-1">
            <i className="ri-fire-line"></i> Ðang streak ({streakCards.length})
          </p>
          ...
          )) : <p className="text-app-text-muted text-xs">Chua có streak</p>}
        </div>
      </div>
    </div>
  );
...
function SessionComplete({ results, onRestart }: {
  ...
  return (
    <div className="text-center py-10">
      ...
      <h2 className="text-white font-bold text-2xl mb-2">Phiên h?c hoàn thành!</h2>
      ...
      <p className="text-app-text-secondary text-sm mb-6">
        AI dã c?p nh?t l?ch ôn t?p cho {results.length} t?. T? khó s? xu?t hi?n s?m hon!
      </p>
    ...
    <DashboardLayout>
      <div className="max-w-3xl mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-white/50 text-sm">AI g?i ý t? c?n ôn d?a trên du?ng cong quên lãng — h?c dúng lúc, nh? lâu hon</p>
        </div>
        ...
        {mode === "overview" && (
          <>
            ...
            <div className="rounded-2xl border border-app-border bg-app-surface/50 p-5 mb-5">
              <h3 className="text-white font-bold text-base mb-4">B?t d?u phiên h?c</h3>
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  {[
                    { value: "due", label: "Ð?n h?n ôn", icon: "ri-alarm-line", color: "#f87171", count: dueCount },
              ...
              {/* Session size */}
              <div className="mb-5">
                <p className="text-white/50 text-xs mb-2">S? th? m?i phiên: <span className="text-white font-bold">{sessionSize}</span></p>
                ...
                {suggestions.slice(0, 15).map((s, i) => (
                  <div key={s.vocab.id} className="flex items-center gap-3 py-2 border-b border-app-border last:border-0">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {s.isDue && !s.isNew && <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-rose-500/15 text-rose-400 font-bold">Ð?N H?N</span>}
