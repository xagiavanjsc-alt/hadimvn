        ...
        {filtered.map(lesson => {
          ...
          return (
            <button
              ...
              className="text-left p-4 rounded-xl border border-white/8 bg-white/2 hover:border-white/18 hover:bg-white/4 transition-all cursor-pointer group"
            >
              <div className="flex items-start gap-3">
                <div className="w-9 h-9 flex items-center justify-center rounded-xl flex-shrink-0 text-sm font-bold bg-white/5 text-white/30 group-hover:bg-white/8 transition-colors">
                  {lesson.id}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    ...
                    {bestScore !== null && (
                      <span className={`text-[9px] font-bold ${bestScore >= 80 ? "text-emerald-400" : bestScore >= 60 ? "text-[#e8c84a]" : "text-red-400"}`}>
                        Cao nhất: {bestScore}%
                      </span>
                    )}
                  </div>
                </div>
                <i className="ri-arrow-right-s-line text-white/20 group-hover:text-white/50 transition-colors flex-shrink-0 mt-1"></i>
              </div>
            </button>
          );
        })}
...
function QuizScreen({
  ...
  const timerColor = timeLeft > 15 ? "text-emerald-400" : timeLeft > 7 ? "text-[#e8c84a]" : "text-red-400";
  const timerBg = timeLeft > 15 ? "bg-emerald-400" : timeLeft > 7 ? "bg-[#e8c84a]" : "bg-red-400";
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-5">
        <button onClick={onBack} className="flex items-center gap-2 text-white/40 hover:text-white/70 text-sm transition-colors cursor-pointer whitespace-nowrap">
      ...
      <div className="h-1.5 bg-white/5 rounded-full overflow-hidden mb-5">
        <div className="h-full rounded-full bg-[#e8c84a] transition-all duration-300" style={{ width: `${progress}%` }} />
      </div>
      ...
      {/* Question */}
      <div className="bg-[#0f1117] border border-white/8 rounded-2xl p-6 mb-5">
        ...
        <p className="text-white/40 text-sm italic">{getQuestionSubtext()}</p>
        {current.type === "meaning_kr_to_vi" && (
          <button
            onClick={() => speakKorean(current.korean)}
            className="mt-3 flex items-center gap-2 text-[#e8c84a]/70 hover:text-[#e8c84a] text-xs transition-colors cursor-pointer"
          >
      ...
      <div className="space-y-2.5 mb-5">
        {current.options.map((opt, i) => {
          let cls = "border-white/8 bg-white/2 hover:border-white/18 hover:bg-white/4 cursor-pointer";
          if (showAnswer) {
          ...
          return (
            ...
            >
              <span className="w-7 h-7 flex items-center justify-center rounded-lg bg-white/5 text-white/30 text-xs font-bold flex-shrink-0">
                {["A","B","C","D"][i]}
              </span>
              <span className="text-white/80 text-sm flex-1">{opt}</span>
              {showAnswer && i === current.correctIndex && <i className="ri-checkbox-circle-fill text-emerald-400 text-lg flex-shrink-0"></i>}
      ...
      {showAnswer && (
        <div className={`p-4 rounded-xl border mb-4 ${isCorrect ? "border-emerald-500/25 bg-emerald-500/8" : "border-red-500/25 bg-red-500/8"}`}>
          <div className="flex items-start gap-3">
            <i className={`${isCorrect ? "ri-checkbox-circle-fill text-emerald-400" : "ri-close-circle-fill text-red-400"} text-lg flex-shrink-0 mt-0.5`}></i>
            <div>
              ...
              <p className="text-white/40 text-xs mt-1 italic">{current.example} — {current.exampleVi}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {showAnswer && (
        <button
          onClick={handleNext}
          className="w-full py-3.5 rounded-xl bg-[#e8c84a] hover:bg-[#d4b43a] text-[#0f1117] font-bold text-sm transition-colors cursor-pointer whitespace-nowrap"
        >
...
function ResultScreen({
  ...
  return (
    <div className="max-w-lg mx-auto text-center">
      ...
      <p className="text-white/40 text-sm mb-6">Bài {lesson.id}: {lesson.titleVi}</p>
      ...
      <div className="relative w-32 h-32 mx-auto mb-6">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <p className="text-white/40 text-xs">{result.score}/{result.total}</p>
        </div>
      </div>
      ...
      {/* XP earned */}
      <div className="p-3 rounded-xl bg-[#e8c84a]/8 border border-[#e8c84a]/20 mb-6">
        <p className="text-[#e8c84a] font-bold text-sm">
      ...
      <div className="space-y-2.5">
        ...
        <button
          ...
          className="w-full py-3 rounded-xl border border-white/10 bg-white/3 hover:bg-white/6 text-white/70 text-sm font-semibold transition-colors cursor-pointer whitespace-nowrap"
        >
          <i className="ri-list-check mr-2"></i>Chọn bài khác
        </button>
        <button
          onClick={onBack}
          className="w-full py-3 rounded-xl border border-white/8 bg-white/2 hover:bg-white/4 text-white/40 text-sm transition-colors cursor-pointer whitespace-nowrap"
        >
          ...
          <div className="w-full lg:w-72 space-y-4">
              ...
              <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-4">
                ...
                <div className="space-y-2">
                  {recentHistory.map((h, i) => {
                    const pctColor = h.percentage >= 80 ? "text-emerald-400" : h.percentage >= 60 ? "text-[#e8c84a]" : "text-red-400";
                    return (
                      <button
                        key={i}
                      ...
                      >
                        <div className="w-8 h-8 flex items-center justify-center rounded-lg bg-white/5 text-white/30 text-xs font-bold flex-shrink-0">
                          {h.lesson.id}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-white/70 text-xs truncate">{h.lesson.titleVi}</p>
                          <p className="text-white/30 text-[10px]">{h.score}/{h.total} câu đúng</p>
                        </div>
            ...
            <div className="bg-[#a78bfa]/5 border border-[#a78bfa]/15 rounded-xl p-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex justify-between text-xs">
                  <span className="text-white/40">Điểm &ge;80%</span>
                  <span className="text-[#a78bfa] font-bold">+20 XP bonus</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-white/40">Điểm 100%</span>
