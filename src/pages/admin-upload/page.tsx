    ...
    <div className={`bg-app-bg border rounded-xl p-4 transition-all ${item.status === "done" ? "border-emerald-500/20" : item.status === "error" ? "border-red-500/20" : "border-app-border"}`}>
      ...
      {assignedQ && (
        <div className="bg-app-surface/50 rounded-lg px-2.5 py-2 mb-3">
          <p className="text-app-text-muted text-[9px] mb-0.5">URL dích:</p>
...
function AudioGenerator() {
  ...
  return (
    <div className="bg-app-bg border border-app-border rounded-2xl p-5">
      <h3 className="text-white font-semibold text-sm mb-4"><i className="ri-volume-up-line text-rose-400 mr-2"></i>T?o âm thanh TTS</h3>
...
interface VocabItem {
  id: string; korean: string; meaning: string; phonetic: string;
...
async function generatePhonetic(korean: string, meaning: string, apiKey: string, model: string): Promise<string> {
  const prompt = `B?n là chuyên gia ti?ng Hàn. T?o phiên âm ti?ng Vi?t cho t?: ${korean} (nghia: ${meaning}). Ch? tr? v? phiên âm, không gi?i thích.`;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5">
      <div className="space-y-4">
        <div className="bg-app-bg border border-app-border rounded-2xl p-5">
          <h3 className="text-white font-semibold text-sm mb-3"><i className="ri-list-unordered text-rose-400 mr-2"></i>Nh?p danh sách t? v?ng</h3>
          <p className="text-app-text-muted text-xs mb-3">M?i dòng m?t t?. Ð?nh d?ng: <code className="bg-app-card/50 px-1 rounded text-rose-400">ti?ng_hàn,nghia_vi?t</code></p>
              ...
              {items.map(item => (
                <div key={item.id} className="grid grid-cols-[1fr_1fr_1fr_32px] gap-3 px-5 py-3 items-center">
                  ...
                  <div>
                    ...
                    {item.status === "generating" && <span className="flex items-center gap-1.5 text-app-text-muted text-xs"><i className="ri-loader-4-line animate-spin text-rose-400"></i>Ðang t?o...</span>}
...
export default function AdminUploadPage() {
  ...
  const [activeTab, setActiveTab] = useState<"upload" | "phonetic" | "audio">("upload");
  ...
  const handleAssign = (itemId: string, questionId: string) => setUploads(prev => prev.map(u => u.id === itemId ? { ...u, questionId } : u));
  const handleRemove = (itemId: string) => {
    setUploads(prev => { const item = prev.find(u => u.id === itemId); if (item) URL.revokeObjectURL(item.preview); return prev.filter(u => u.id !== itemId); });
  };
