    ...
    <div className="max-w-xl mx-auto space-y-4">
      <div className="bg-gradient-to-r from-[#1a0800] to-[#0f1117] border border-[#fb923c]/20 rounded-2xl p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-white font-bold text-sm">Th? thách t? {challenge.creatorName}</p>
            <p className="text-app-text-secondary text-xs">Ði?m c?n vu?t: <span className="text-[#fb923c] font-bold">{challenge.creatorScore}/{challenge.creatorTotal}</span> ({Math.round((challenge.creatorScore / challenge.creatorTotal) * 100)}%)</p>
          </div>
        </div>
      </div>
    ...
    <div className="max-w-lg mx-auto">
      <div className="bg-app-bg border border-app-border rounded-2xl p-6 space-y-5">
        <div className="text-center mb-2">
          ...
          <h2 className="text-white font-bold text-lg">T?o th? thách m?i</h2>
          <p className="text-app-text-secondary text-sm mt-1">Làm quiz tru?c, r?i g?i link cho b?n bè thách d?u!</p>
        </div>

        <div>
          <label className="text-app-text-secondary text-xs font-medium block mb-2">Tên c?a b?n</label>
          <input value={creatorName} onChange={e => setCreatorName(e.target.value)}
            placeholder="Nh?p tên d? b?n bè bi?t ai thách d?u..."
        ...
        <div>
          <label className="text-app-text-secondary text-xs font-medium block mb-2">Ch? d? EPS</label>
        ...
        {topic && (
          <div className="flex items-center gap-2 p-3 rounded-xl border" style={{ backgroundColor: `${topic.color}08`, borderColor: `${topic.color}20` }}>
            ...
            <span className="text-white/60 text-xs">Ch? d?: <strong style={{ color: topic.color }}>{topic.label}</strong> · {questionCount} câu</span>
          </div>
        )}
        ...
        <button onClick={handleStartQuiz} disabled={!creatorName.trim()}
          ...
          <i className="ri-play-line"></i>B?t d?u làm quiz
        </button>
      </div>
    </div>
  );
...
function ChallengeResult({ challenge, onNew }: { challenge: ChallengeRecord; onNew: () => void }) {
  ...
  const handleCopy = async () => {
    const text = `?? ${challenge.creatorName} thách d?u b?n!\n\nMình v?a d?t ${pct}% (${challenge.creatorScore}/${challenge.creatorTotal} câu) trong quiz EPS-TOPIK.\nB?n có th? vu?t qua không?\n\n?? ${challengeUrl}\n\n#HànQu?cOi #EPSTopik #Th?Thách`;
  ...
  return (
    <div className="max-w-lg mx-auto space-y-4">
      <div className="bg-app-bg border border-app-border rounded-2xl p-8 text-center">
        ...
        <h2 className="text-white font-bold text-2xl mb-1">Ði?m c?a b?n</h2>
        <p className="text-5xl font-black mb-2" style={{ color }}>{pct}%</p>
        <p className="text-app-text-secondary text-sm mb-4">{challenge.creatorScore}/{challenge.creatorTotal} câu dúng</p>
        <div className="w-full max-w-xs mx-auto h-2 bg-app-card/50 rounded-full overflow-hidden mb-6">
          <div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: color }} />
        </div>
        <p className="text-white/50 text-sm">Th? thách dã du?c t?o! G?i link cho b?n bè d? h? thách d?u.</p>
      </div>

      <div className="bg-app-bg border border-app-border rounded-2xl p-5">
        <h3 className="text-white font-bold text-sm mb-3">Chia s? th? thách</h3>
        ...
        <button onClick={handleCopy}
          ...
          {copied ? "Ðã sao chép!" : "Sao chép link + n?i dung"}
        </button>
      </div>
  ...
  if (!challenge) {
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        <p className="text-app-text-secondary text-sm">Không tìm th?y th? thách này</p>
        <p className="text-app-text-muted text-xs mt-2">Link có th? dã h?t h?n ho?c không h?p l?</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-lg mx-auto">
        <div className="bg-gradient-to-br from-[#1a0800] to-[#0f1117] border border-[#fb923c]/20 rounded-2xl p-8 text-center mb-4">
          ...
          <h2 className="text-white font-black text-2xl mb-2">{challenge.creatorName} thách d?u b?n!</h2>
          <p className="text-white/50 text-sm mb-5">H? dã d?t <span className="text-[#fb923c] font-bold">{creatorPct}%</span> ({challenge.creatorScore}/{challenge.creatorTotal} câu). B?n có th? vu?t qua không?</p>
          {topic && (
          ...
          <button onClick={() => setPhase("quiz")} disabled={!myName.trim()}
            ...
            <i className="ri-sword-line mr-2"></i>Ch?p nh?n th? thách!
          </button>
        </div>
      </div>
    );
    ...
    <div className="max-w-lg mx-auto space-y-4">
      <div className="bg-app-bg border border-app-border rounded-2xl p-8 text-center">
        ...
        <h2 className="text-white font-black text-2xl mb-4" style={{ color: resultColor }}>
          {won ? "B?n th?ng!" : tied ? "Hòa nhau!" : "Thua r?i!"}
        </h2>
        ...
        <p className="text-app-text-secondary text-sm">
          {won ? `Xu?t s?c! B?n vu?t qua ${challenge.creatorName} r?i!` : tied ? "Hai bên ngang tài ngang s?c!" : `C? lên! Luy?n thêm d? vu?t qua ${challenge.creatorName}!`}
        </p>
      </div>
    </div>
  );
...
export default function ChallengePage() {
  ...
  if (challengeId) {
    return (
      <DashboardLayout title="Th? thách b?n bè" subtitle="Ai s? d?t di?m cao hon?">
  ...
  return (
    <DashboardLayout
      title="Th? thách b?n bè"
      subtitle="T?o quiz thách d?u — g?i link cho b?n bè so sánh di?m"
      actions={
        phase === "list" ? (
          <button onClick={() => setPhase("create")}
            ...
            <i className="ri-add-line"></i>T?o th? thách
          </button>
        ) : undefined
      }
    >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-[#1a0800] to-[#0f1117] border border-[#fb923c]/15 rounded-2xl p-6">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h2 className="text-white font-bold text-lg">Th? thách b?n bè</h2>
                <p className="text-app-text-secondary text-sm">Làm quiz EPS-TOPIK ? g?i link ? so sánh di?m</p>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[
                { icon: "ri-play-circle-line", color: "#fb923c", label: "Bu?c 1", desc: "Làm quiz tru?c" },
                { icon: "ri-share-line", color: "app-accent-primary", label: "Bu?c 2", desc: "G?i link cho b?n" },
                { icon: "ri-trophy-line", color: "#34d399", label: "Bu?c 3", desc: "So sánh di?m s?" },
              ].map(s => (
            ...
            <button onClick={() => setPhase("create")}
              ...
              <i className="ri-sword-line"></i>T?o th? thách ngay
            </button>
          </div>
          ...
          {challenges.length > 0 && (
            <div className="bg-app-bg border border-app-border rounded-2xl p-5">
              <h3 className="text-white font-semibold text-sm mb-4">L?ch s? th? thách ({challenges.length})</h3>
          ...
          {challenges.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-app-text-muted text-sm">Chua có th? thách nào</p>
              <p className="text-app-text-muted text-xs mt-1">T?o th? thách d?u tiên và g?i cho b?n bè!</p>
            </div>
          )}
        </div>
      )}
