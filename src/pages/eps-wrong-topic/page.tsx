    ...
    return (
      <div className="text-center py-16 max-w-md mx-auto">
        ...
        <p className="text-white/50 text-sm mb-6">
          Ðã nh?: <span className="text-app-accent-success font-bold">{masteredSet.size}</span> t? &nbsp;|&nbsp;
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      ...
      <div className="flex gap-4 justify-center text-sm">
        <span className="text-app-accent-success">? {masteredSet.size} dã nh?</span>
      ...
      <div className="flex gap-3">
        ...
        >
          <i className="ri-check-line mr-1"></i> Ðã nh? r?i!
        </button>
      </div>
    </div>
  );
  ...
  if (finished) {
    ...
    return (
      <div className="text-center py-16 max-w-md mx-auto">
        ...
        <p className="text-white/50 text-sm mb-6">{score}/{questions.length} câu dúng</p>
    ...
    <div className="max-w-lg mx-auto space-y-6">
      ...
      <div className="bg-app-card/50 border border-app-border rounded-2xl p-8 text-center">
        <div className="flex justify-center mb-3">
          <span className="text-xs px-3 py-1 rounded-full bg-red-500/10 text-red-400 border border-red-500/20">
            Ðã sai {q.question.wrongCount} l?n
          </span>
        </div>
  ...
  if (finished) {
    ...
    return (
      <div className="text-center py-16 max-w-md mx-auto">
        ...
        <p className="text-white/50 text-sm mb-6">{score}/{cards.length} câu dúng</p>
        <button
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      ...
      <div className="bg-app-card/50 border border-app-border rounded-2xl p-8 text-center space-y-3">
        ...
        <p className="text-app-text-secondary text-xs">Ði?n t? ti?ng Hàn có nghia:</p>
          ...
          <div className={`p-4 rounded-xl border text-center ${isCorrect ? "bg-emerald-500/8 border-emerald-500/20" : "bg-red-500/8 border-red-500/20"}`}>
            {isCorrect ? (
              ...
              <>
                ...
                <p className="text-white/60 text-sm">Ðáp án dúng: <span className="text-white font-bold">{card.korean}</span></p>
...
function ListenFillReview({ items }: { items: WrongItem[] }) {
  ...
  if (finished) {
    ...
    return (
      <div className="text-center py-16 max-w-md mx-auto">
        ...
        <h3 className="text-white text-2xl font-bold mb-2">K?t qu? Nghe & Ði?n</h3>
        <p className="text-5xl font-black mb-2" style={{ color: pct >= 80 ? "#34d399" : pct >= 60 ? "app-accent-primary" : "#f87171" }}>
          {pct}%
        </p>
        <p className="text-white/50 text-sm mb-6">{score}/{cards.length} câu dúng</p>
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      ...
      <div className="flex gap-4 justify-center text-sm">
        <span className="text-app-accent-success">? {score} dúng</span>
      ...
      <div className="bg-app-card/50 border border-app-border rounded-2xl p-8 text-center space-y-5">
        <p className="text-white/50 text-sm">Nghe phát âm r?i di?n t? ti?ng Hàn</p>
        ...
        {playCount > 0 && !submitted && (
          <p className="text-app-text-muted text-xs">Ðã nghe {playCount} l?n — nh?n l?i d? nghe thêm</p>
        )}
        {playCount === 0 && (
          <p className="text-app-text-muted text-xs">Nh?n nút loa d? nghe phát âm</p>
        )}
        ...
        {!submitted && playCount >= 2 && (
          ...
          >
            ...
            {showHint ? "?n g?i ý" : "Xem g?i ý (ch? cái d?u)"}
          </button>
        )}
        {showHint && !submitted && (
      ...
      <div className="space-y-3">
        <input
          ...
          onKeyDown={e => { if (e.key === "Enter" && !submitted) handleSubmit(); }}
          disabled={submitted}
          placeholder="Ði?n t? ti?ng Hàn b?n v?a nghe..."
          className={`w-full rounded-xl px-5 py-4 text-lg font-bold text-center border transition-all focus:outline-none ${
            submitted
              ? isCorrect
                ? "bg-emerald-500/10 border-emerald-500/40 text-app-accent-success"
        ...
        {submitted && (
          <div className={`p-4 rounded-xl border text-center ${isCorrect ? "bg-emerald-500/8 border-emerald-500/20" : "bg-red-500/8 border-red-500/20"}`}>
            {isCorrect ? (
              ...
              <>
                <p className="text-red-400 font-bold mb-1">Sai r?i!</p>
                <p className="text-white/60 text-sm">Ðáp án dúng: <span className="text-white font-bold text-xl">{card.korean}</span></p>
                <p className="text-white/35 text-xs mt-1">[{card.pronunciation}]</p>
                {card.example && (
...
function ExportModal({ items, onClose }: { items: WrongItem[]; onClose: () => void }) {
  ...
  const generateContent = () => {
    if (format === "csv") {
      const header = ["Ti?ng Hàn", "Ti?ng Vi?t", includePronunciation ? "Phát âm" : null, includeExample ? "Ví d?" : null, includeExample ? "D?ch ví d?" : null, "S? l?n sai", "Ch? d?"]
        .filter(Boolean).join(",");
    ...
    if (format === "txt") {
      return items.map((item, i) => {
        const lines = [
          ...
          includeExample && item.example ? `   Ví d?: ${item.example}` : null,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-app-bg border border-app-border rounded-2xl w-full max-w-md">
        <div className="flex items-center justify-between p-5 border-b border-app-border">
          <div>
            <h3 className="text-white font-bold">Xu?t danh sách t? sai</h3>
            <p className="text-app-text-secondary text-xs mt-0.5">{items.length} t? c?n ôn l?i</p>
          </div>
        ...
        <div className="p-5 space-y-5">
          {/* Format */}
          <div>
            <p className="text-white/50 text-xs font-semibold mb-3">Ð?nh d?ng xu?t</p>
          ...
          {/* Options */}
          <div>
            <p className="text-white/50 text-xs font-semibold mb-3">Tùy ch?n n?i dung</p>
            <div className="space-y-2">
              {[
                { label: "Bao g?m phát âm", value: includePronunciation, setter: setIncludePronunciation },
                { label: "Bao g?m câu ví d?", value: includeExample, setter: setIncludeExample },
              ].map(opt => (
                <button
                  key={opt.label}
      ...
      <DashboardLayout>
        <div className="p-6 max-w-2xl mx-auto">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h1 className="text-white text-xl font-bold">Ôn t?p sai theo ch? d?</h1>
              <p className="text-app-text-secondary text-sm">L?c và ôn l?i t? v?ng dã sai theo nhóm ch? d?</p>
            </div>
          </div>
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-white text-xl font-bold mb-2">Chua có t? sai nào!</h3>
            <p className="text-white/50 text-sm mb-6 max-w-sm mx-auto">
              Hãy làm bài t?p trong các bài h?c EPS d? h? th?ng ghi l?i t? b?n tr? l?i sai.
            </p>
            <div className="flex gap-3 justify-center flex-wrap">
              ...
              >
                <i className="ri-book-open-line mr-2"></i>Ð?n 60 Bài H?c EPS
              </button>
              <button
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-app-text-secondary text-sm">
              {totalWrong} t? c?n ôn l?i · {masteredWords.length} t? dã nh?
            </p>
          </div>
          <div className="ml-auto flex items-center gap-2">
            ...
            {masteredWords.length > 0 && !started && (
              ...
              >
                <i className="ri-refresh-line mr-1"></i>Reset dã nh?
              </button>
            )}
          </div>
        </div>
        ...
        {!started ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="text-white font-semibold">Ch?n ch? d? d? ôn</h2>
                      ...
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-app-text-secondary text-xs">
                            {count > 0 ? `${count} t? c?n ôn` : "Không có t? sai"}
                          </p>
                        </div>
                        {isSelected && count > 0 && (
            ...
            <div className="space-y-4">
              ...
              <div className="bg-red-500/5 border border-red-500/15 rounded-xl p-4">
                <p className="text-white/50 text-xs mb-1">T? c?n ôn l?i</p>
              ...
              <div className="bg-app-surface/50 border border-app-border rounded-xl p-4 space-y-2">
                <p className="text-white/50 text-xs mb-3">Ch? d? ôn t?p</p>
                {(
                  [
                    { id: "flashcard", icon: "ri-stack-line", label: "Flashcard", desc: "L?t th? — nh?/chua nh?" },
                    { id: "quiz", icon: "ri-survey-line", label: "Tr?c nghi?m", desc: "Ch?n dáp án dúng" },
                    { id: "fill", icon: "ri-edit-line", label: "Ði?n t?", desc: "Gõ t? ti?ng Hàn" },
                    { id: "listen", icon: "ri-headphone-line", label: "Nghe & Ði?n", desc: "Nghe phát âm r?i di?n" },
                  ] as const
                ).map(m => (
                  <button
                    key={m.id}
              ...
              >
                ...
                B?t d?u ôn ({filteredWrongItems.length} t?)
              </button>

              {/* Quick links */}
              <div className="bg-app-surface/50 border border-app-border rounded-xl p-4 space-y-2">
                <p className="text-app-text-secondary text-xs mb-2">Liên k?t nhanh</p>
                <button
                  onClick={() => navigate("/eps-lessons")}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-1 bg-app-surface/50 border border-app-border rounded-xl p-1 w-fit flex-wrap">
              {(
                [
                  ...
                  { id: "fill", icon: "ri-edit-line", label: "Ði?n t?" },
                  { id: "listen", icon: "ri-headphone-line", label: "Nghe & Ði?n" },
                ] as const
              ).map(m => (
                <button
                  key={m.id}
            ...
            <div className="flex items-center gap-4 text-xs text-app-text-muted">
              ...
              <span>•</span>
              <span>{filteredWrongItems.length} t? c?n ôn</span>
            </div>
