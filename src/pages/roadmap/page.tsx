...
const ROADMAP_LEVELS: RoadmapLevel[] = [
  {
    ...
    subtitle: "Làm quen b?ng ch? cái, phát âm co b?n",
    color: "#34d399",
    icon: "ri-seedling-line",
    badge: "B?t d?u t?i dây",
    skills: ["Ð?c du?c Hangul", "Phát âm 40 âm co b?n", "Chào h?i don gi?n", "S? d?m 1-100"],
    vocabTarget: 100,
    grammarPoints: ["??/??? (là/không là)", "?/? (ch? ng?)", "?/? (ch? th?)", "?/? (tân ng?)"],
    recommendedLessons: ["Bài hát có giai di?u ch?m, t? v?ng don gi?n"],
  },
  {
    id: "elementary",
    level: "elementary",
    ...
    subtitle: "Câu don gi?n, t? v?ng hàng ngày",
    color: "app-accent-primary",
    icon: "ri-plant-line",
    badge: "TOPIK 1",
    skills: ["Gi?i thi?u b?n thân", "Mua s?m, an u?ng", "H?i du?ng", "Nói v? s? thích"],
    vocabTarget: 800,
    grammarPoints: ["?/?? (kính ng?)", "? ?? (mu?n)", "? ?? (di d?n)", "-(?)? ? ?? (có th?)"],
    recommendedLessons: ["K-pop ballad ch?m", "OST phim Hàn"],
  },
  {
    id: "intermediate",
    level: "intermediate",
    topik: "TOPIK I — C?p 2",
    title: "Di?n d?t ý ki?n",
    subtitle: "Câu ph?c t?p, ng? pháp trung c?p",
    color: "#fb923c",
    icon: "ri-tree-line",
    badge: "TOPIK 2",
    skills: ["K? chuy?n quá kh?", "Di?n d?t c?m xúc", "Ð?c hi?u do?n ng?n", "Vi?t email don gi?n"],
    vocabTarget: 1500,
    grammarPoints: ["-(?)? (n?u)", "-? ??? (vì)", "-(?)?/?? (nhung/mà)", "-?/?? (vì/nên)"],
    recommendedLessons: ["K-pop idol group", "Nh?c có lyric k? chuy?n"],
  },
  {
    id: "upper-intermediate",
    level: "upper-intermediate",
    topik: "TOPIK II — C?p 3",
    title: "Ti?ng Hàn t? nhiên",
    subtitle: "Ng? pháp nâng cao, d?c báo don gi?n",
    color: "#a78bfa",
    icon: "ri-leaf-line",
    badge: "TOPIK 3",
    skills: ["Ð?c tin t?c don gi?n", "Th?o lu?n ch? d? xã h?i", "Vi?t do?n van", "Nghe hi?u 70%"],
    vocabTarget: 3000,
    grammarPoints: ["-(?)? ?? ??? (không ch?...mà còn)", "-? ??? (theo)", "-(?)??? (càng...càng)", "-?? (d?/cho d?n khi)"],
    recommendedLessons: ["Nh?c Hàn có lyric sâu s?c", "Tin t?c Naver don gi?n"],
  },
  {
    id: "advanced",
    level: "advanced",
    topik: "TOPIK II — C?p 4-6",
    title: "Thành th?o ti?ng Hàn",
    subtitle: "Ð?c báo, xem phim không ph? d?",
    color: "#f472b6",
    icon: "ri-award-line",
    badge: "TOPIK 4-6",
    skills: ["Ð?c báo Hàn t? nhiên", "Xem phim không ph? d?", "Vi?t lu?n van", "Ph?ng v?n xin vi?c"],
    vocabTarget: 6000,
    grammarPoints: ["Van phong trang tr?ng", "Thành ng? & t?c ng?", "T? Hán-Hàn nâng cao", "Ng? pháp h?c thu?t"],
    recommendedLessons: ["Nh?c rap Hàn", "Tin t?c Naver d?y d?", "Podcast ti?ng Hàn"],
  },
];
    ...
    >
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-app-text-muted text-[10px]">M?c tiêu: {level.vocabTarget.toLocaleString()} t? v?ng</span>
          </div>
        </div>
      ...
      {isActive && (
        <div className="mt-5 pt-5 border-t border-app-border space-y-4">
          ...
          {/* Grammar */}
          <div>
            <p className="text-app-text-muted text-[10px] tracking-normal font-semibold mb-2">Ng? pháp tr?ng tâm</p>
          ...
          {matchedLessons.length > 0 && (
            <div>
              <p className="text-app-text-muted text-[10px] tracking-normal font-semibold mb-2">Bài h?c g?i ý t? kho c?a b?n</p>
...
export default function RoadmapPage() {
  ...
  return (
    <DashboardLayout
      title="L? trình h?c ti?ng Hàn"
      subtitle="T? Hangul d?n TOPIK — theo t?ng bu?c rõ ràng"
    >
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    {completedLevels.includes(currentLevel) ? "Ðã hoàn thành" : "Ðánh d?u hoàn thành"}
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <h3 className="text-white font-semibold text-sm mb-4">Th?ng kê c?a b?n</h3>
            <div className="space-y-3">
              {[
                { label: "Bài h?c dã t?o", value: approvedLessons.length, icon: "ri-book-2-line", color: "app-accent-primary" },
                { label: "T? v?ng tích luy", value: totalVocab, icon: "ri-translate-2", color: "#34d399" },
                { label: "C?p d? hoàn thành", value: `${completedLevels.length}/${ROADMAP_LEVELS.length}`, icon: "ri-award-line", color: "#a78bfa" },
              ].map(stat => (
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            ...
            <div className="space-y-2">
              {[
                { icon: "ri-stack-line", label: "Ôn flashcard", sub: "Luy?n t? v?ng", path: "/flashcard", color: "app-accent-primary" },
                { icon: "ri-music-2-line", label: "K-pop Lesson", sub: "T?o bài h?c m?i", path: "/melon", color: "#34d399" },
          ...
          <div className="bg-app-accent-primary/5 border border-app-accent-primary/15 rounded-xl p-4">
            ...
            <p className="text-app-text-secondary text-xs leading-relaxed">
              H?c 15-20 phút/ngày d?u d?n hi?u qu? hon h?c 2 ti?ng/tu?n. K?t h?p K-pop + flashcard + d?c tin t?c d? ti?n b? nhanh nh?t!
            </p>
          </div>
        </div>
      </div>
