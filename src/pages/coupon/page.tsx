...
const CHANNEL_OPTIONS = [
  "Zalo OA", "Facebook Group", "TikTok", "Instagram", "YouTube", "Gumroad", "B?n bè gi?i thi?u", "Khác"
];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-app-bg border border-app-border rounded-2xl w-full max-w-md">
        ...
        <div className="p-5 space-y-4">
          ...
          {/* Discount */}
          <div>
            <label className="text-app-text-secondary text-xs font-medium block mb-1.5">Gi?m giá *</label>
            <div className="flex gap-2">
              <input
                type="number"
                value={discount}
          ...
          {/* Series */}
          <div>
            <label className="text-app-text-secondary text-xs font-medium block mb-1.5">Áp d?ng cho</label>
            <select
              value={seriesId}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-app-text-secondary text-xs font-medium block mb-1.5">Gi?i h?n dùng (d? tr?ng = không gi?i h?n)</label>
              <input
                type="number"
                value={maxUsage}
            ...
            <div>
              ...
              <input
                ...
                placeholder="Chi?n d?ch tháng 4..."
                className="w-full bg-app-card/50 border border-app-border rounded-lg px-4 py-2.5 text-white text-sm placeholder-white/20 focus:outline-none focus:border-app-accent-primary/40 transition-colors"
              />
            </div>
          </div>
      ...
      <div className="bg-app-bg border border-app-border rounded-2xl w-full max-w-sm">
        ...
        <div className="p-5">
          <p className="text-white/50 text-xs mb-4">
            Mã: <span className="text-app-accent-primary font-mono font-bold">{coupon.code}</span> · Ðã dùng: {coupon.usageCount} l?n
          </p>
          <div>
            <label className="text-app-text-secondary text-xs font-medium block mb-1.5">S? l?n s? d?ng thêm</label>
            <input
              type="number"
              min={1}
              value={times}
...
export default function CouponPage() {
  ...
  const handleSave = (coupon: Coupon) => {
    ...
    showToast(editingCoupon ? "Ðã c?p nh?t coupon!" : "Ðã t?o coupon m?i!");
  };
  ...
  return (
    <DashboardLayout
      title="Coupon & Mã gi?m giá"
      subtitle="Theo dõi kênh bán hàng hi?u qu? nh?t qua mã coupon"
      actions={
        <button
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: "Ðang ho?t d?ng", value: coupons.filter(c => c.active).length, icon: "ri-checkbox-circle-line", color: "#34d399" },
          { label: "T?ng lu?t dùng", value: totalUsage, icon: "ri-user-received-line", color: "#fb923c" },
          { label: "Kênh dang dùng", value: channelStats.length, icon: "ri-broadcast-line", color: "#a78bfa" },
        ].map(stat => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-5">
        ...
        <div className="bg-app-bg border border-app-border rounded-2xl p-5">
          <h3 className="text-white font-semibold text-sm mb-1">Kênh hi?u qu? nh?t</h3>
          <p className="text-app-text-muted text-xs mb-4">Theo s? l?n dùng coupon</p>
        ...
        <div className="col-span-2 bg-app-bg border border-app-border rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            >
              {channels.map(c => (
                <option key={c} value={c} className="bg-app-bg">{c === "all" ? "T?t c? kênh" : c}</option>
              ))}
            </select>
          </div>
          ...
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-app-text-muted text-sm mb-1">Chua có coupon nào</p>
              <button
                onClick={() => { setEditingCoupon(null); setShowForm(true); }}
      ...
      <div className="bg-app-accent-primary/5 border border-app-accent-primary/15 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-app-accent-primary/80 text-xs font-semibold mb-1">Cách dùng coupon d? theo dõi kênh</p>
          <p className="text-app-text-secondary text-xs leading-relaxed">
            T?o mã riêng cho t?ng kênh: <strong className="text-white/60">ZALO20</strong> cho Zalo, <strong className="text-white/60">FB15</strong> cho Facebook, <strong className="text-white/60">TT10</strong> cho TikTok.
            Khi khách dùng mã ? ghi nh?n th? công ? bi?u d? t? c?p nh?t ? bi?t ngay kênh nào hi?u qu? nh?t!
          </p>
        </div>
      </div>
