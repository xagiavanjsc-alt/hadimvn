...
const POS_LABELS: Record<string, string> = {
  noun: "Danh t?", verb: "Ð?ng t?", adjective: "Tính t?", adverb: "Tr?ng t?", expression: "Bi?u d?t",
};
    ...
    <div className={`bg-app-bg border rounded-xl p-4 transition-all ${isMastered ? "border-emerald-500/20" : isFavorite ? "border-app-accent-primary/20" : "border-app-border hover:border-app-border"}`}>
      <div className="flex items-start justify-between mb-2">
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          ...
          {/* Favorite button */}
          <button onClick={() => onFavorite(item.id)} title={isFavorite ? "B? yêu thích" : "Luu t? khó"} className={`w-7 h-7 flex items-center justify-center rounded-lg cursor-pointer transition-colors ${isFavorite ? "bg-app-accent-primary/15" : "bg-app-card/50 hover:bg-app-card/70"}`}>
...
export default function VocabularyPage() {
  ...
  return (
    <DashboardLayout
      ...
      subtitle="Phân lo?i theo ch? d? và c?p d? TOPIK — h?c flashcard ngay t?i ch?"
      actions={
        <div className="flex items-center gap-3">
          ...
          {!user && <span className="text-app-text-muted text-xs"><i className="ri-cloud-off-line mr-1"></i>Ðang nh?p d? luu cloud</span>}
          {totalFavorites > 0 && (
            <button
            ...
            >
              <i className="ri-bookmark-fill"></i>Ôn t? yêu thích ({totalFavorites})
            </button>
          )}
          <button
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: "T? yêu thích", value: totalFavorites, icon: "ri-bookmark-fill", color: "#f472b6" },
      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        ...
        <div className="flex items-center bg-app-card/50 rounded-xl p-1">
          {([["all", "T?t c?"], ["unmastered", "Chua thu?c"], ["favorites", `Yêu thích (${totalFavorites})`]] as [string, string][]).map(([f, label]) => (
        ...
        <div className="flex items-center gap-2 bg-app-card/50 border border-app-border rounded-xl px-3 py-2 flex-1 min-w-[180px]">
          ...
          <input type="text" placeholder="Tìm t? v?ng..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="flex-1 bg-transparent text-white/70 text-sm outline-none placeholder-white/20" />
        </div>
      ...
      {filterMode === "favorites" && filteredItems.length === 0 && (
        <div className="text-center py-16 bg-app-bg border border-app-border rounded-2xl">
          ...
          <p className="text-app-text-muted text-sm">Chua có t? yêu thích nào</p>
          <p className="text-app-text-muted text-xs mt-1">Nh?n nút <i className="ri-bookmark-line"></i> trên th? t? d? luu t? khó</p>
        </div>
      )}
