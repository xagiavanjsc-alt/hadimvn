...
const LEVELS = [
  { id: "A1", label: "A1 - So c?p 1", color: "#34d399", desc: "Giao ti?p co b?n hàng ngày" },
  { id: "A2", label: "A2 - So c?p 2", color: "#38bdf8", desc: "Tình hu?ng quen thu?c" },
  { id: "B1", label: "B1 - Trung c?p 1", color: "#fb923c", desc: "Ch? d? ph? bi?n, công vi?c" },
    ...
    <DashboardLayout>
      <div className="p-6 max-w-5xl mx-auto">
        ...
        <div className="flex items-start justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-white mb-1">Th?ng kê TOPIK</h1>
            <p className="text-app-text-secondary text-sm">Theo dõi ti?n d? h?c t? v?ng TOPIK I/II theo c?p d? A1–B2</p>
          </div>
          {user && (
            <button
            ...
            >
              ...
              {syncStatus === "syncing" ? "Ðang d?ng b?..." : syncStatus === "done" ? "Ðã d?ng b?!" : "Ð?ng b? lên cloud"}
            </button>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
          {[
            ...
            { label: "Ðã h?c", value: totalStudied, icon: "ri-eye-line", color: "#38bdf8" },
            { label: "Ðã thu?c", value: totalMastered, icon: "ri-check-double-line", color: "#34d399" },
            { label: "C?n ôn hôm nay", value: totalDue, icon: "ri-alarm-line", color: "#fb923c" },
          ].map((kpi) => (
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-xs font-medium transition-all cursor-pointer whitespace-nowrap ${
                  activeTab === tab ? "bg-app-accent-primary/15 text-app-accent-primary" : "text-app-text-secondary hover:text-white/70"
                }`}
              >
                {labels[tab]}
              </button>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="bg-app-surface/50 border border-app-border rounded-2xl p-6 flex flex-col items-center justify-center">
                ...
                <div className="relative">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <p className="text-app-text-muted text-xs">dã thu?c</p>
                  </div>
                </div>
                <div className="mt-4 grid grid-cols-2 gap-3 w-full">
                  <div className="text-center">
                    <p className="text-lg font-bold text-[#34d399]">{totalMastered}</p>
                    <p className="text-app-text-muted text-xs">Ðã thu?c</p>
                  </div>
                  <div className="text-center">
              ...
              <div className="space-y-3">
                {levelStats.map((lv) => (
                  <div key={lv.id} className="bg-app-surface/50 border border-app-border rounded-xl p-4">
                    ...
                    <div className="flex justify-between mt-1.5">
                      <span className="text-app-text-muted text-xs">{lv.mastered} dã thu?c</span>
                      {lv.due > 0 && (
                        <span className="text-[#fb923c] text-xs">{lv.due} c?n ôn</span>
                      )}
                    </div>
                  </div>
                ))}
            ...
            <div className="bg-app-surface/50 border border-app-border rounded-2xl p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="text-center">
                  ...
                  <p className="text-app-text-muted text-xs">Bài dã làm</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-[#34d399]">{avgScore}%</p>
                  <p className="text-app-text-muted text-xs">Ði?m trung bình</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-[#38bdf8]">{bestScore}%</p>
                  <p className="text-app-text-muted text-xs">Ði?m cao nh?t</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-app-surface/50 border border-app-border rounded-2xl p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="text-center">
                  ...
                  <p className="text-app-text-muted text-xs">Th? dã xem</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-[#34d399]">{studyDays}</p>
                  <p className="text-app-text-muted text-xs">Ngày h?c</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-[#fb923c]">{totalDue}</p>
                  <p className="text-app-text-muted text-xs">C?n ôn hôm nay</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === "levels" && (
          <div className="space-y-4">
            {levelStats.map((lv) => (
              <div key={lv.id} className="bg-app-surface/50 border border-app-border rounded-2xl p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                  {[
                    ...
                    { label: "Ðã h?c", value: lv.studied, color: "text-[#38bdf8]" },
                    { label: "Ðã thu?c", value: lv.mastered, color: "text-[#34d399]" },
                    { label: "C?n ôn", value: lv.due, color: "text-[#fb923c]" },
                  ].map((stat) => (
                ...
                <div className="space-y-2">
                  <div>
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-app-text-secondary">Ðã h?c</span>
                  ...
                  <div>
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-app-text-secondary">Ðã thu?c</span>
        ...
        {activeTab === "categories" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {categoryStats.map((cat) => (
              <div key={cat.id} className="bg-app-surface/50 border border-app-border rounded-xl p-4">
                ...
                <div className="flex justify-between mt-1.5">
                  <span className="text-app-text-muted text-xs">{cat.mastered} dã thu?c</span>
                  <span className="text-app-text-muted text-xs">{cat.total - cat.mastered} còn l?i</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div>
            ...
            {!user && (
              <div className="mb-4 p-3 bg-app-accent-primary/5 border border-app-accent-primary/15 rounded-xl flex items-center gap-3">
                ...
                <p className="text-app-accent-primary/60 text-xs">Ðang nh?p d? luu l?ch s? quiz lên cloud và xem trên m?i thi?t b?</p>
              </div>
            )}
            {user && loadingCloud && (
              <div className="mb-4 p-3 bg-app-surface/50 border border-app-border rounded-xl flex items-center gap-3">
                ...
                <p className="text-app-text-muted text-xs">Ðang t?i l?ch s? t? cloud...</p>
              </div>
            )}
            {quizHistory.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="text-app-text-muted text-xs mt-1">Làm bài quiz TOPIK d? xem k?t qu? ? dây</p>
              </div>
            ) : (
              <div className="space-y-2">
                {quizHistory.map((q, i) => {
                  ...
                  const typeLabel = q.quiz_type === "reading" ? "Ð?c hi?u" : q.quiz_type === "listening" ? "Nghe" : "T? v?ng";
                  return (
                    <div key={i} className="bg-app-surface/50 border border-app-border rounded-xl p-4 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-app-text-muted text-xs">{q.level} · {q.score}/{q.total} câu dúng</p>
                      </div>
