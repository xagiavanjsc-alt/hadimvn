        ...
        return (
          <div key={i} className="flex-1 flex flex-col items-center gap-1 group">
            <div className="relative w-full flex items-end justify-center" style={{ height: "100px" }}>
              {val > 0 && (
                <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-[#1a1600] border border-[#e8c84a]/20 rounded px-1.5 py-0.5 text-[9px] text-[#e8c84a] whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity z-10">
                  {val} XP
                </div>
              )}
              <div
            ...
            <span className="text-[9px] text-white/25 whitespace-nowrap">{labels[i]}</span>
          </div>
        );
      })}
    </div>
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="w-7 h-7 flex items-center justify-center rounded-lg bg-white/5 text-white/40 hover:text-white/70 cursor-pointer"
          >
            ...
            >
              ...
              {xp > 0 && (
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none">
                  <div className="bg-[#1a1600] border border-[#e8c84a]/20 rounded px-2 py-1 text-[9px] text-[#e8c84a] whitespace-nowrap">
                    {xp} XP
                  </div>
                </div>
              )}
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          <div key={s.label} className="bg-[#0f1117] border border-white/5 rounded-2xl p-5 text-center">
            ...
            {topicStats.slice(0, 8).map(t => {
              ...
              return (
                <div key={t.topic} className="flex items-center gap-3">
                  ...
                  <div className="text-[10px] text-white/25 w-16 text-right flex-shrink-0">
          ...
          <div className="grid grid-cols-2 gap-3">
            {EPS_TOPICS.map(t => (
              <div key={t.name} className="flex items-center gap-3 px-4 py-3 rounded-xl" style={{ backgroundColor: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.05)" }}>
                ...
                <span className="text-[10px] text-white/20">Chưa có dữ liệu</span>
              </div>
            ))}
          </div>
          <p className="text-white/20 text-xs text-center mt-4">Làm bài thi để xem phân tích điểm yếu chi tiết</p>
        </div>
      )}

      {/* Recommendations */}
      {weakTopics.length > 0 && (
        <div className="bg-[#0f1117] border border-rose-500/15 rounded-2xl p-6">
            ...
            {weakTopics.slice(0, 3).map(t => (
              <div key={t.topic} className="flex items-center gap-3 px-4 py-3 rounded-xl" style={{ backgroundColor: "rgba(248,113,113,0.06)", border: "1px solid rgba(248,113,113,0.12)" }}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-white/30 text-[10px] mt-0.5">Độ chính xác hiện tại: {t.accuracy}% — cần đạt 80%+</p>
                </div>
      ...
      <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-6">
        ...
        {xpByType.length > 0 ? (
              ...
              return (
                <div key={type} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-white/25 text-[10px] mt-0.5">{amount.toLocaleString()} XP</p>
                  </div>
                </div>
              );
            })}
          ...
          <div className="text-center py-8">
            ...
            <p className="text-white/25 text-sm">Chưa có dữ liệu XP. Hãy bắt đầu học!</p>
          </div>
        )}
      </div>
          ...
          {xpByDow.map((xp, i) => {
            ...
            return (
              <div key={i} className="flex-1 flex flex-col items-center gap-1.5 group">
                <div className="relative w-full flex items-end justify-center" style={{ height: 80 }}>
                  {xp > 0 && (
                    <div className="absolute -top-6 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none">
                      <div className="bg-[#1a1600] border border-[#e8c84a]/20 rounded px-1.5 py-0.5 text-[9px] text-[#e8c84a] whitespace-nowrap">
                ...
                <span className="text-[10px] text-white/30">{dowLabels[i]}</span>
              </div>
            );
          })}
        </div>
    ...
    >
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { icon: "ri-flashlight-line", color: "#e8c84a", bg: "bg-[#e8c84a]/10", label: "Tổng XP", value: totalXP.toLocaleString(), sub: `Hạng ${currentRank.name}` },
          ...
          {TABS.map(tab => (
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-medium cursor-pointer whitespace-nowrap transition-all ${
                activeTab === tab.id ? "bg-[#e8c84a] text-[#0f1117]" : "text-white/40 hover:text-white/60"
              }`}
            >
              ...
              <div className="flex items-center justify-between mb-5">
                <div>
                  ...
                  <p className="text-white/30 text-xs mt-0.5">
                    ...
                    {prevXPTotal > 0 && (
                      <span className={`ml-2 font-semibold ${xpChange >= 0 ? "text-emerald-400" : "text-red-400"}`}>
                ...
                <div className="flex gap-1 bg-white/5 p-1 rounded-xl">
                  {(["week", "month", "3months"] as Period[]).map(p => (
                    <button
                      key={p}
                      onClick={() => setPeriod(p)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer whitespace-nowrap transition-all ${period === p ? "bg-[#e8c84a] text-[#0f1117]" : "text-white/40 hover:text-white/60"}`}
                    >
              ...
              <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-white/30 text-xs mt-0.5">10 lần thi gần nhất</p>
                  </div>
                  {examTrend.length >= 2 && (
                    <div className={`flex items-center gap-1 text-sm font-bold px-3 py-1.5 rounded-xl ${examImprovement >= 0 ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"}`}>
                ...
                {examTrend.length >= 2 ? (
                  <>
                    ...
                    <div className="mt-3 grid grid-cols-3 gap-3 pt-3 border-t border-white/5">
                      <div className="text-center">
                        <p className="text-[#e8c84a] font-bold text-lg">{latestExamScore}%</p>
                        <p className="text-white/30 text-[10px]">Lần gần nhất</p>
                      </div>
                      <div className="text-center">
                        <p className="text-white font-bold text-lg">{avgExamScore}%</p>
                        <p className="text-white/30 text-[10px]">Trung bình</p>
                      </div>
                      <div className="text-center">
                        <p className="text-emerald-400 font-bold text-lg">{Math.max(...examTrend)}%</p>
                        <p className="text-white/30 text-[10px]">Cao nhất</p>
                      </div>
                    </div>
                    <div className="mt-3 flex items-center gap-2 px-3 py-2 rounded-xl bg-white/3">
                      <div className="w-3 h-0.5 bg-[#e8c84a]"></div>
                      <span className="text-white/30 text-[10px]">Ngưỡng đậu EPS: 80%</span>
                      {latestExamScore >= 80 ? (
                        <span className="ml-auto text-emerald-400 text-[10px] font-bold">Đã đạt!</span>
                      ) : (
                        <span className="ml-auto text-white/30 text-[10px]">Còn {80 - latestExamScore}%</span>
                      )}
                    </div>
                  </>
                ) : (
                  <div className="flex flex-col items-center justify-center py-10 text-center">
                    ...
                    <p className="text-white/25 text-sm">Cần ít nhất 2 lần thi để hiển thị biểu đồ</p>
                    <button onClick={() => navigate("/eps-exam")} className="mt-3 text-[#e8c84a] text-xs cursor-pointer">Thi thử ngay →</button>
                  </div>
                )}
              </div>
                ...
                <div className="flex items-center justify-center mb-5">
                  <div className="relative w-32 h-32">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      ...
                      <p className="text-white/30 text-[10px]">Điểm hiện tại</p>
                    </div>
                  </div>
                </div>
                {latestExamScore >= 80 ? (
                  <div className="text-center p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
                    ...
                    <p className="text-emerald-400 font-bold text-sm">Đã đạt ngưỡng đậu EPS!</p>
                    <p className="text-white/40 text-xs mt-1">Tiếp tục duy trì và ôn luyện</p>
                  </div>
                ) : daysToGoal !== null ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between px-3 py-2.5 bg-white/3 rounded-xl">
                      <span className="text-white/40 text-xs">Cần đạt thêm</span>
                      <span className="text-[#e8c84a] font-bold text-sm">{80 - (latestExamScore || 0)}%</span>
                    </div>
                    <div className="flex items-center justify-between px-3 py-2.5 bg-[#e8c84a]/5 border border-[#e8c84a]/15 rounded-xl">
                      <span className="text-[#e8c84a]/70 text-xs">Dự đoán đạt mục tiêu</span>
                      <span className="text-[#e8c84a] font-bold text-sm">~{daysToGoal} lần thi nữa</span>
                    </div>
                  </div>
                ) : (
                  <div className="text-center py-4">
                    <p className="text-white/25 text-sm">Thi thêm để có dự đoán chính xác hơn</p>
                    <button onClick={() => navigate("/eps-exam")} className="mt-3 text-[#e8c84a] text-xs cursor-pointer">Thi thử ngay →</button>
                  </div>
                )}
              </div>
