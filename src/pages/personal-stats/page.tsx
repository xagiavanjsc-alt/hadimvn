      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-app-text-muted text-xs mt-0.5">
            {activeDaysThisMonth} ngày h?c · {totalXPThisMonth.toLocaleString()} XP tháng này
          </p>
        </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: "Ði?m TB g?n dây", value: `${avgScore}%`, color: avgScore >= 80 ? "#34d399" : avgScore >= 60 ? "app-accent-primary" : "#f87171", icon: "ri-bar-chart-line" },
      ...
      {topicStats.length > 0 ? (
        <div className="bg-app-bg border border-app-border rounded-2xl p-6">
          <h3 className="text-white font-semibold text-sm mb-5">Phân tích theo ch? d?</h3>
        ...
        <div className="bg-app-bg border border-app-border rounded-2xl p-6">
          <h3 className="text-white font-semibold text-sm mb-4">Phân tích di?m y?u theo ch? d? EPS</h3>
          <div className="grid grid-cols-2 gap-3">
            {EPS_TOPICS.map(t => (
              <div key={t.name} className="flex items-center gap-3 px-4 py-3 rounded-xl" style={{ backgroundColor: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.05)" }}>
                ...
                <span className="text-[10px] text-app-text-muted">Chua có d? li?u</span>
              </div>
            ))}
          </div>
          <p className="text-app-text-muted text-xs text-center mt-4">Làm bài thi d? xem phân tích di?m y?u chi ti?t</p>
        </div>
      )}
      ...
      {weakTopics.length > 0 && (
        <div className="bg-app-bg border border-rose-500/15 rounded-2xl p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-white font-semibold text-sm">G?i ý c?i thi?n</h3>
          </div>
          <div className="space-y-2">
            {weakTopics.slice(0, 3).map(t => (
              <div key={t.topic} className="flex items-center gap-3 px-4 py-3 rounded-xl" style={{ backgroundColor: "rgba(248,113,113,0.06)", border: "1px solid rgba(248,113,113,0.12)" }}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-white/70 text-xs">Ôn luy?n ch? d? <strong className="text-rose-400">{t.topic}</strong></p>
                  <p className="text-app-text-muted text-[10px] mt-0.5">Ð? chính xác hi?n t?i: {t.accuracy}% — c?n d?t 80%+</p>
                </div>
...
function XPBreakdown({ xpLog }: { xpLog: XPEntry[] }) {
  const typeLabels: Record<string, { label: string; color: string; icon: string }> = {
    ...
    daily_study: { label: "H?c hàng ngày", color: "#06b6d4", icon: "ri-book-open-line" },
  ...
  return (
    <div className="space-y-5">
      {/* XP by activity */}
      <div className="bg-app-bg border border-app-border rounded-2xl p-6">
        <h3 className="text-white font-semibold text-sm mb-5">Phân b? XP theo ho?t d?ng</h3>
        {xpByType.length > 0 ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-app-text-muted text-sm">Chua có d? li?u XP. Hãy b?t d?u h?c!</p>
          </div>
        )}
      </div>
      ...
      {/* XP by day of week */}
      <div className="bg-app-bg border border-app-border rounded-2xl p-6">
        <h3 className="text-white font-semibold text-sm mb-5">XP theo ngày trong tu?n</h3>
        ...
        <p className="text-app-text-muted text-xs mt-3 text-center">
          {xpByDow.indexOf(Math.max(...xpByDow)) >= 0 && Math.max(...xpByDow) > 0
            ? `B?n h?c nhi?u nh?t vào ${dowLabels[xpByDow.indexOf(Math.max(...xpByDow))]}`
            : "Chua có d? li?u"}
        </p>
      </div>
    </div>
...
export default function PersonalStatsPage() {
  ...
  const TABS: { id: StatsTab; label: string; icon: string }[] = [
    ...
    { id: "weakness", label: "Ði?m y?u", icon: "ri-focus-3-line" },
    { id: "xp", label: "Phân tích XP", icon: "ri-bar-chart-2-line" },
  ];

  return (
    <DashboardLayout
      title="Th?ng kê h?c t?p cá nhân"
      subtitle="Bi?u d? ti?n d?, streak calendar và phân tích di?m y?u"
    >
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            { icon: "ri-fire-line", color: "#fb923c", bg: "bg-[#fb923c]/10", label: "Streak hi?n t?i", value: `${streak.count} ngày`, sub: "Liên ti?p" },
            ...
            { icon: "ri-stack-line", color: "#a78bfa", bg: "bg-[#a78bfa]/10", label: "T? dã thu?c", value: Object.values(flashcardProgress).filter(Boolean).length, sub: "Qua Flashcard" },
          ].map(s => (
        ...
        {activeTab === "overview" && (
          <>
            ...
            <div className="bg-app-bg border border-app-border rounded-2xl p-6">
              <div className="flex items-center justify-between mb-5">
                <div>
                  ...
                  <p className="text-app-text-muted text-xs mt-0.5">
                    {totalXPInPeriod.toLocaleString()} XP trong k? này
                    {prevXPTotal > 0 && (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-5 pt-4 border-t border-app-border">
                {[
                  { label: "T?ng XP k? này", value: totalXPInPeriod.toLocaleString(), color: "app-accent-primary" },
                  { label: "Ngày ho?t d?ng", value: `${activeDays}/${dateRange.length}`, color: "#34d399" },
                  { label: "TB XP/ngày h?c", value: avgXPPerDay.toLocaleString(), color: "#a78bfa" },
                ].map(s => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                {examTrend.length >= 2 ? (
                  <>
                    ...
                    <div className="mt-3 flex items-center gap-2 px-3 py-2 rounded-xl bg-app-surface/50">
                      ...
                      {latestExamScore >= 80 ? (
                        <span className="ml-auto text-app-accent-success text-[10px] font-bold">Ðã d?t!</span>
                      ) : (
                  ...
                  <div className="flex flex-col items-center justify-center py-10 text-center">
                    ...
                    <p className="text-app-text-muted text-sm">C?n ít nh?t 2 l?n thi d? hi?n th? bi?u d?</p>
              ...
              {/* Goal prediction */}
              <div className="bg-app-bg border border-app-border rounded-2xl p-6">
                <h3 className="text-white font-semibold text-sm mb-4">D? doán d?t m?c tiêu EPS</h3>
                <div className="flex items-center justify-center mb-5">
                  <div className="relative w-32 h-32">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      ...
                      <p className="text-app-text-muted text-[10px]">Ði?m hi?n t?i</p>
                    </div>
                  </div>
                </div>
                {latestExamScore >= 80 ? (
                  <div className="text-center p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
                    ...
                    <p className="text-app-accent-success font-bold text-sm">Ðã d?t ngu?ng d?u EPS!</p>
                    <p className="text-app-text-secondary text-xs mt-1">Ti?p t?c duy trì và ôn luy?n</p>
                  </div>
                ) : daysToGoal !== null ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between px-3 py-2.5 bg-app-surface/50 rounded-xl">
                      <span className="text-app-text-secondary text-xs">C?n d?t thêm</span>
                    ...
                    <div className="flex items-center justify-between px-3 py-2.5 bg-app-accent-primary/5 border border-app-accent-primary/15 rounded-xl">
                      <span className="text-app-accent-primary/70 text-xs">D? doán d?t m?c tiêu</span>
                      <span className="text-app-accent-primary font-bold text-sm">~{daysToGoal} l?n thi n?a</span>
                    </div>
                  </div>
                ) : (
                  <div className="text-center py-4">
                    <p className="text-app-text-muted text-sm">Thi thêm d? có d? doán chính xác hon</p>
                    <button onClick={() => navigate("/eps-exam")} className="mt-3 text-app-accent-primary text-xs cursor-pointer">Thi th? ngay ?</button>
                  </div>
                )}
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                { label: "Phân tích di?m y?u", icon: "ri-bar-chart-2-line", color: "#a78bfa", route: "/eps-weakness-analysis" },
        ...
        {activeTab === "streak" && (
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                { label: "Streak hi?n t?i", value: `${streak.count} ngày`, color: "#fb923c", icon: "ri-fire-line" },
                { label: "T?ng ngày h?c", value: new Set(xpLog.map(e => e.date?.split("T")[0]).filter(Boolean)).size, color: "#34d399", icon: "ri-calendar-check-line" },
