...
const EXAM_DURATION = 40 * 60; // 40 phút tính b?ng giây
...
function selectExamQuestions(): EpsQuestion[] {
  // Uu tiên câu có ?nh và audio, sau dó fill d? 40 câu
  ...
  // L?y t?i da 10 câu có ?nh
  selected.push(...shuffleArray(withImage).slice(0, 10));
  // L?y t?i da 15 câu có audio
  ...
  return (
    <div className="flex items-center gap-3 p-4 bg-[#06b6d4]/8 border border-[#06b6d4]/20 rounded-xl mb-4">
      ...
      <div className="flex-1">
        <p className="text-[#06b6d4] text-xs font-semibold mb-1">Câu nghe hi?u</p>
        ...
        <p className="text-app-text-muted text-[10px] mt-1">Nh?n d? nghe • Ti?ng Hàn chu?n</p>
      </div>
      <button
        onClick={playAudio}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {question.imageUrl && (
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#a78bfa]/15 text-[#a78bfa] font-medium flex items-center gap-1">
              <i className="ri-image-line text-[10px]"></i> Có ?nh
            </span>
          )}
          {isListening && (
          ...
          {isReading && (
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#ec4899]/15 text-[#ec4899] font-medium flex items-center gap-1">
              <i className="ri-book-open-line text-[10px]"></i> Ð?c hi?u
            </span>
          )}
        ...
        <div className="rounded-xl overflow-hidden border border-app-border">
          <div className="w-full h-48 bg-app-card/50">
            <img
              ...
              alt={question.imageAlt || "Hình minh h?a"}
      ...
      {showResult && (
        <div className="bg-app-accent-primary/5 border border-app-accent-primary/15 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-app-accent-primary text-xs font-semibold">Gi?i thích</span>
          </div>
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="text-center mb-8">
        ...
        <h1 className="text-2xl font-bold text-white mb-2">Thi th? EPS mô ph?ng th?t</h1>
        <p className="text-white/50 text-sm">Ð? thi gi?ng format EPS-TOPIK chính th?c nh?t</p>
      </div>

      {/* Exam info */}
      <div className="grid grid-cols-2 gap-4 mb-6">
        {[
          { icon: "ri-file-list-3-line", label: "S? câu h?i", value: "40 câu", color: "app-accent-primary" },
          { icon: "ri-timer-line", label: "Th?i gian", value: "40 phút", color: "#34d399" },
          { icon: "ri-image-line", label: "Câu có ?nh", value: "~10 câu", color: "#a78bfa" },
          { icon: "ri-headphone-line", label: "Câu nghe hi?u", value: "~15 câu", color: "#06b6d4" },
        ].map((item, i) => (
      ...
      <div className="bg-app-surface/50 border border-app-border rounded-xl p-5 mb-6 space-y-3">
        ...
        {[
          { icon: "ri-image-2-line", text: "Câu h?i có ?nh minh h?a — nhìn hình ch?n dáp án dúng", color: "#a78bfa" },
          { icon: "ri-volume-up-line", text: "Câu nghe hi?u — nh?n nút phát d? nghe ti?ng Hàn chu?n", color: "#06b6d4" },
          { icon: "ri-book-open-line", text: "Câu d?c hi?u — d?c do?n van và ch?n dáp án", color: "#ec4899" },
          { icon: "ri-chat-smile-2-line", text: "Câu giao ti?p — tình hu?ng th?c t? t?i noi làm vi?c", color: "#34d399" },
          { icon: "ri-scales-3-line", text: "Câu pháp lu?t — quy?n l?i và nghia v? ngu?i lao d?ng", color: "#f59e0b" },
        ].map((item, i) => (
      ...
      <div className="bg-app-accent-primary/5 border border-app-accent-primary/15 rounded-xl p-4 mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            { label: "Ð?u", score: "= 80 di?m", color: "#34d399" },
            { label: "Trung bình", score: "60-79 di?m", color: "app-accent-primary" },
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className={`rounded-2xl p-6 text-center border ${passed ? "bg-emerald-500/8 border-emerald-500/20" : "bg-rose-500/8 border-rose-500/20"}`}>
        ...
        <p className="text-white/50 text-sm">{correct}/{questions.length} câu dúng • {formatTime(timeUsed)} dã dùng</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {[
          { label: "Câu dúng", value: correct, color: "#34d399", icon: "ri-check-line" },
          { label: "Câu sai", value: questions.length - correct, color: "#f87171", icon: "ri-close-line" },
          { label: "Ði?m s?", value: `${score}/100`, color: "app-accent-primary", icon: "ri-trophy-line" },
        ].map((item, i) => (
        ...
        <DashboardLayout>
          <div className="p-6 md:p-8 max-w-2xl mx-auto">
            ...
            <div className={`mb-4 px-3 py-2 rounded-lg text-xs font-medium flex items-center gap-2 ${r.isCorrect ? "bg-emerald-500/10 text-app-accent-success" : "bg-rose-500/10 text-rose-400"}`}>
              ...
              {r.isCorrect ? "Tr? l?i dúng" : "Tr? l?i sai"}
            </div>
            <QuestionCard
              question={q}
              index={reviewIndex}
      ...
      <div className="flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex-shrink-0 bg-app-bg border-b border-app-border px-4 py-3 flex items-center gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-app-text-secondary text-xs">{answered}/{questions.length} dã tr? l?i</span>
            </div>
            <div className="h-1.5 bg-white/8 rounded-full overflow-hidden">
              <div
        ...
        <div className="flex-shrink-0 bg-app-bg border-t border-app-border px-4 py-3">
          <div className="max-w-2xl mx-auto flex items-center gap-3">
            ...
            >
              {currentIndex === questions.length - 1 ? "N?p bài" : "Câu ti?p"}
