...
export default function ShareFlashcardModal({ cards, onClose }: ShareFlashcardModalProps) {
  ...
  const [shareTitle, setShareTitle] = useState("B? flashcard K-pop c?a tôi");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-md bg-[#1a1d27] border border-app-border rounded-2xl overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          {/* Title input */}
          <div>
            <label className="text-xs font-semibold text-white/50 mb-1.5 block">Tên b? flashcard</label>
            <input
              type="text"
              value={shareTitle}
          ...
          <div className="bg-app-surface/50 border border-app-border rounded-xl p-3">
            ...
            <div className="space-y-1.5">
              ...
              {cards.length > 3 && (
                <p className="text-app-text-muted text-[10px]">+{cards.length - 3} t? khác...</p>
              )}
            </div>
          </div>
          ...
          <div className="flex items-start gap-2 px-3 py-2.5 bg-app-accent-primary/5 border border-app-accent-primary/10 rounded-xl">
            ...
            <p className="text-app-text-secondary text-xs leading-relaxed">
              Link chia s? cho phép b?n bè xem và h?c b? flashcard này. T?i da 100 t? du?c chia s?.
            </p>
          </div>
          ...
          <div className="flex gap-3">
            ...
            >
              {copied ? (
                <><i className="ri-checkbox-circle-line mr-1.5" />Ðã sao chép!</>
              ) : (
                <><i className="ri-link mr-1.5" />T?o & Sao chép link</>
              )}
            </button>
          </div>
        </div>
  ...
  if (notFound) {
    return (
      <div className="min-h-screen bg-app-bg flex flex-col items-center justify-center text-center px-6">
        ...
        <p className="text-white/50 text-sm font-medium mb-1">Link không t?n t?i</p>
        <p className="text-app-text-muted text-xs">Link này dã h?t h?n ho?c không h?p l?</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-app-bg flex flex-col">
      ...
      <header className="h-14 flex items-center px-4 gap-3 border-b border-app-border">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-app-text-muted text-xs">{data.cards.length} t? · Ðu?c chia s? t? KTS</p>
        </div>
        <a
