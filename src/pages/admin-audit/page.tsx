...
const ACTION_CONFIG: Record<string, { color: string; icon: string; label: string }> = {
  ...
  bulk_vip_granted: { color: "app-accent-primary", icon: "ri-vip-crown-2-line", label: "C?p VIP hàng lo?t" },
  ...
  email_bulk_sent: { color: "#38bdf8", icon: "ri-mail-send-line", label: "Email hàng lo?t" },
  content_deleted: { color: "#f87171", icon: "ri-delete-bin-line", label: "Xóa n?i dung" },
  ...
  settings_updated: { color: "#fb923c", icon: "ri-settings-3-line", label: "C?p nh?t cài d?t" },
  backup_created: { color: "#34d399", icon: "ri-save-line", label: "T?o backup" },
  admin_login: { color: "#a78bfa", icon: "ri-login-circle-line", label: "Ðang nh?p Admin" },
  user_joined: { color: "#34d399", icon: "ri-user-add-line", label: "Ðang ký" },
  exam_taken: { color: "#a78bfa", icon: "ri-file-list-3-line", label: "Thi th?" },
  post_created: { color: "#fb923c", icon: "ri-article-line", label: "Bài vi?t" },
};
...
function timeAgo(dateStr: string) {
  ...
  if (d < 30) return `${d} ngày tru?c`;
  return `${Math.floor(d / 30)} tháng tru?c`;
}
...
export default function AdminAuditPage() {
    ...
    try {
      ...
      profiles.forEach(p => {
        events.push({
          id: `uj_${p.id}`,
          type: "user_joined",
          label: "Thành viên m?i dang ký",
          detail: `${p.display_name || "Ngu?i dùng"} dã dang ký tài kho?n`,
          actor: p.display_name || "Ngu?i dùng",
      ...
      (examRes.data || []).forEach(e => {
        ...
        events.push({
          ...
          type: "exam_taken",
          label: "Thi th? EPS",
          detail: `${profile?.display_name || "H?c viên"} — ${e.score}/${e.total} (${pct}%)`,
          actor: profile?.display_name || "H?c viên",
      ...
      (postRes.data || []).forEach(p => {
        ...
        events.push({
          id: `po_${p.id}`,
          type: "post_created",
          label: "Bài vi?t c?ng d?ng",
          detail: `${profile?.display_name || "Thành viên"}: ${p.title || "Bài vi?t m?i"}`,
  ...
  const handleExport = () => {
    const csv = [
      ["Th?i gian", "Lo?i", "Hành d?ng", "Chi ti?t", "Ngu?i th?c hi?n", "IP", "Ngu?n"].join(","),
      ...filtered.map(e => [
  ...
  return (
    <AdminLayout
      title="Audit Log"
      subtitle="L?ch s? hành d?ng admin + ho?t d?ng h? th?ng t? Supabase th?c"
      actions={
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-5">
        {[
          ...
          { label: "Hành d?ng Admin", value: adminActionsCount, color: "#f87171", icon: "ri-shield-keyhole-line" },
          ...
          { label: "Email dã g?i", value: emailCount, color: "#38bdf8", icon: "ri-mail-send-line" },
        ].map(s => (
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 rounded-xl px-3 py-2 flex-1 min-w-[200px] border"
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Tìm s? ki?n, ngu?i dùng, IP..."
        ...
        <div className="rounded-2xl border p-5 mb-5" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-xs" style={{ color: "var(--admin-text-muted)" }}>Theo dõi email t? d?ng g?i b?i VIP Expiry Scheduler</p>
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
            {[
              { label: "T?ng email dã g?i", value: emailCount, color: "#38bdf8", icon: "ri-mail-check-line" },
              { label: "Broadcast", value: typeCount("broadcast_sent"), color: "#a78bfa", icon: "ri-broadcast-line" },
              { label: "Email hàng lo?t", value: typeCount("email_bulk_sent"), color: "#fb923c", icon: "ri-mail-send-line" },
            ].map(s => (
          ...
          <div className="px-4 py-3 rounded-xl border" style={{ backgroundColor: "var(--admin-card2)", borderColor: "var(--admin-border)" }}>
            ...
            <div className="flex items-center gap-3">
              ...
              <p className="text-xs" style={{ color: "var(--admin-text-muted)" }}>vip-expiry-scheduler — Ch?y m?i ngày lúc 08:00 UTC</p>
            ...
            <p className="text-[10px] mt-2" style={{ color: "var(--admin-text-faint)" }}>
              G?i nh?c nh? gia h?n VIP: 7 ngày, 3 ngày, 1 ngày tru?c khi h?t h?n
            </p>
          </div>
        </div>
      )}
          ...
          <div className="rounded-2xl border overflow-hidden" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
            {paginated.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="text-sm" style={{ color: "var(--admin-text-muted)" }}>Không tìm th?y s? ki?n nào</p>
              </div>
            ) : (
          ...
          {hasMore && (
            <div className="text-center mt-4">
              <button onClick={() => setPage(p => p + 1)}
                ...
                T?i thêm ({filtered.length - paginated.length} còn l?i)
              </button>
            </div>
          )}
        </>
