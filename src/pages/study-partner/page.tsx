...
const VOCAB_CHALLENGES = [
  { korean: "????", vietnamese: "H?nh phúc", hint: "C?m xúc tích c?c" },
  { korean: "????", vietnamese: "Cham ch?", hint: "Tính cách t?t" },
  { korean: "????", vietnamese: "Ði làm", hint: "Sinh ho?t hàng ngày" },
  { korean: "???", vietnamese: "Chênh l?ch nhi?t d?", hint: "Th?i ti?t" },
  { korean: "????", vietnamese: "V?ch k? du?ng", hint: "Giao thông" },
];

const STATUS_CONFIG = {
  online: { color: "#34d399", label: "Online", dot: "bg-emerald-400" },
  studying: { color: "app-accent-primary", label: "Ðang h?c", dot: "bg-yellow-400" },
  away: { color: "#fb923c", label: "V?ng m?t", dot: "bg-orange-400" },
};
  ...
  return (
    <div className="bg-app-bg border border-app-border rounded-2xl p-5 hover:border-app-border transition-all">
      ...
      <div className="flex items-center gap-3 mb-4 text-[10px] text-app-text-muted">
        ...
        <span className="flex items-center gap-1"><i className="ri-calendar-line"></i>T? {partner.joinedAt}</span>
      </div>
      ...
      >
        {partner.status === "away" ? "V?ng m?t" : "K?t n?i h?c cùng"}
      </button>
    </div>
  );
}
...
function ChatSession({ partner, onClose }: ChatSessionProps) {
  ...
  const sendChallenge = () => {
    ...
    setMessages(prev => [...prev, {
      ...
      sender: "me",
      text: `?? Th? thách t? v?ng: "${challenge.korean}" có nghia là gì? (G?i ý: ${challenge.hint})`,
      time: now,
      type: "question",
    }]);
  };
  ...
  const checkAnswer = () => {
    ...
    setMessages(prev => [...prev, {
      ...
      sender: "partner",
      text: isCorrect
        ? `? Ðúng r?i! "${currentChallenge.korean}" = "${currentChallenge.vietnamese}". Gi?i l?m!`
        : `? Chua dúng. "${currentChallenge.korean}" = "${currentChallenge.vietnamese}". C? g?ng nhé!`,
      time: now,
      type: "vocab",
    }]);
        ...
        {showChallenge && (
          <div className="px-4 pb-2 flex-shrink-0">
            <div className="flex gap-2 p-3 rounded-xl bg-app-accent-primary/8 border border-app-accent-primary/20">
              <input
                type="text"
                placeholder={`Nghia c?a "${currentChallenge.korean}" là...`}
        ...
        <div className="p-4 border-t border-app-border flex-shrink-0">
          <div className="flex gap-2">
            <button
              ...
              title="G?i th? thách t? v?ng"
            >
              <i className="ri-sword-line text-sm"></i>
            </button>
            <input
              type="text"
              placeholder="Nh?n tin v?i d?i tác h?c t?p..."
              value={input}
...
export default function StudyPartnerPage() {
  ...
  const fetchPartners = useCallback(async () => {
    ...
    try {
      ...
      const mapped: Partner[] = lbRows.map((r: { user_id: string; streak: number; total_xp: number }) => {
        const name = nameMap[r.user_id] || "H?c viên";
        return {
          id: r.user_id,
          name,
          level: "",
  ...
  return (
    <DashboardLayout
      title="Ð?i tác h?c t?p"
      subtitle="Ghép c?p ng?u nhiên d? luy?n nói, h?i dáp t? v?ng và h?c cùng nhau"
    >
      {/* Stats bar */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          { label: "H?c viên online", value: onlineCount, icon: "ri-user-line", color: "#34d399" },
          { label: "Ðang h?c cùng", value: partners.filter(p => p.status === "studying").length, icon: "ri-group-line", color: "app-accent-primary" },
          { label: "Phiên h?c hôm nay", value: 3, icon: "ri-history-line", color: "#fb923c" },
          { label: "Th?i gian h?c cùng", value: "47 phút", icon: "ri-time-line", color: "#a78bfa" },
        ].map(s => (
      ...
      <div className="bg-gradient-to-r from-app-surface to-[#0f1117] border border-app-accent-primary/20 rounded-2xl p-5 mb-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h3 className="text-white font-bold text-base mb-1">Ghép c?p ng?u nhiên</h3>
            <p className="text-app-text-secondary text-sm">AI s? tìm d?i tác phù h?p v?i trình d? và m?c tiêu c?a b?n</p>
          </div>
          {matchedPartner ? (
            <div className="flex items-center gap-3">
              <div className="text-right">
                <p className="text-white/50 text-xs">Ðã tìm th?y!</p>
            ...
            >
              {isMatching ? (
                <><div className="w-4 h-4 border-2 border-app-accent-primary/30 border-t-[app-accent-primary] rounded-full animate-spin"></div>Ðang tìm...</>
              ) : (
                <><i className="ri-shuffle-line"></i>Ghép ng?u nhiên</>
              )}
            </button>
          )}
        </div>
      ...
      <div className="flex items-center gap-3 mb-5">
        <div className="flex-1 relative">
          ...
          <input
            type="text"
            placeholder="Tìm theo tên, ch? d?, mô t?..."
            value={searchQuery}
        ...
        <div className="flex gap-1.5">
          {[["all", "T?t c?"], ["online", "Online"], ["studying", "Ðang h?c"]].map(([val, label]) => (
            <button
              key={val}
      ...
      {filtered.length === 0 && (
        <div className="bg-app-bg border border-app-border rounded-2xl p-16 text-center">
          ...
          <p className="text-app-text-muted text-sm">Không tìm th?y d?i tác phù h?p</p>
        </div>
      )}
      ...
      <div className="mt-6 bg-app-bg border border-app-border rounded-2xl p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-white font-semibold text-sm">M?o h?c cùng d?i tác hi?u qu?</h3>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs text-app-text-secondary leading-relaxed">
          <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Dùng tính nang "Th? thách t? v?ng" d? ki?m tra nhau</p>
          <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Luy?n nói b?ng cách d?c to các câu trong transcript</p>
          <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Ð?t l?ch h?c c? d?nh m?i ngày d? duy trì streak</p>
        </div>
      </div>
