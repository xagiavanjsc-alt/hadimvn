...
const TOPICS = [
  ...
  { id: "greeting", label: "Chào h?i", icon: "ri-hand-heart-line", color: "#34d399" },
  { id: "workplace", label: "Noi làm vi?c", icon: "ri-building-line", color: "#fb923c" },
  { id: "safety", label: "An toàn lao d?ng", icon: "ri-shield-check-line", color: "#f87171" },
  { id: "law", label: "Pháp lu?t", icon: "ri-scales-3-line", color: "#a78bfa" },
  { id: "daily", label: "Sinh ho?t hàng ngày", icon: "ri-home-smile-line", color: "#38bdf8" },
];

const QUESTION_COUNTS = [5, 10, 15, 20];

// Challenge history luu trong localStorage, m?c d?nh r?ng (không còn fake)
  ...
  if (showResult) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center max-w-md mx-auto">
        ...
        <p className="text-white font-bold text-2xl mb-1">{score}/{questions.length} câu dúng</p>
    ...
    <div className={`bg-app-bg border rounded-2xl p-5 ${isWaiting ? "border-app-accent-primary/20" : isWin ? "border-emerald-500/20" : isDraw ? "border-app-border" : "border-red-500/15"}`}>
      <div className="flex items-start justify-between mb-4">
        ...
        {isWaiting ? (
          <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-app-accent-primary/15 text-app-accent-primary flex items-center gap-1">
            <i className="ri-time-line"></i>Ch? b?n bè
          </span>
        ) : isWin ? (
      ...
      <div className="flex items-center gap-4">
        ...
        <div className={`flex-1 text-center p-3 rounded-xl ${isWaiting ? "bg-app-accent-primary/5 border border-dashed border-app-accent-primary/20" : "bg-app-surface/50"}`}>
          ...
          {isWaiting ? (
            <p className="text-app-accent-primary/50 text-sm font-medium">Chua làm</p>
          ) : (
            <>
...
export default function FriendChallengePage() {
  ...
  const handleJoinChallenge = () => {
    ...
    if (!found) {
      setJoinError("Không tìm th?y thách d?u v?i mã này. Ki?m tra l?i link ho?c mã.");
      return;
    }
  ...
  if (view === "quiz") {
    return (
      <DashboardLayout title="Thách d?u b?n bè" subtitle="Làm bài quiz — sau dó chia s? link cho b?n bè">
        <div className="mb-6">
  ...
  if (view === "result" && activeChallenge && finalScore) {
    ...
    const formatTime = (s: number) => `${Math.floor(s / 60)}:${String(s % 60).padStart(2, "0")}`;
    return (
      <DashboardLayout title="K?t qu? thách d?u" subtitle="Chia s? link d? b?n bè cùng làm bài">
        <div className="max-w-lg mx-auto space-y-5">
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <p className="text-white font-semibold text-sm mb-3">Chia s? thách d?u</p>
            <div className="flex gap-2 mb-3">
              ...
              >
                {linkCopied ? <><i className="ri-check-line mr-1"></i>Ðã sao chép</> : <><i className="ri-file-copy-line mr-1"></i>Sao chép</>}
              </button>
            </div>
            <p className="text-app-text-muted text-xs">G?i link này cho b?n bè — h? s? làm cùng b? câu h?i và so sánh di?m v?i b?n!</p>
          </div>
  ...
  return (
    <DashboardLayout
      title="Thách d?u b?n bè"
      subtitle="T?o quiz, chia s? link — b?n bè làm cùng b? câu h?i và so sánh di?m"
    >
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6">
        ...
        <div className="space-y-5">
          ...
          {view === "create" ? (
            <div className="bg-app-bg border border-app-border rounded-2xl p-6 space-y-5">
              <div className="flex items-center justify-between">
                <p className="text-white font-semibold">T?o thách d?u m?i</p>
              ...
              {/* Creator name */}
              <div>
                <label className="text-app-text-secondary text-xs mb-2 block">Tên c?a b?n</label>
                <input
                  type="text"
                  value={creatorName}
                  onChange={e => setCreatorName(e.target.value)}
                  placeholder="Nh?p tên..."
              ...
              {/* Topic */}
              <div>
                <label className="text-app-text-secondary text-xs mb-2 block">Ch? d? câu h?i</label>
              ...
              {/* Question count */}
              <div>
                <label className="text-app-text-secondary text-xs mb-2 block">S? câu h?i</label>
              ...
              >
                <i className="ri-sword-line mr-2"></i>B?t d?u làm bài
              </button>
            </div>
          ) : view === "join" ? (
            <div className="bg-app-bg border border-app-border rounded-2xl p-6 space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-white font-semibold">Tham gia thách d?u</p>
                <button onClick={() => setView("home")} className="text-app-text-muted hover:text-white/60 cursor-pointer">
                  <i className="ri-close-line text-lg"></i>
                </button>
              </div>
              <p className="text-app-text-secondary text-sm">Nh?p mã thách d?u ho?c dán link t? b?n bè</p>
              <input
                type="text"
                value={joinCode}
                onChange={e => { setJoinCode(e.target.value); setJoinError(""); }}
                placeholder="Nh?p mã ho?c link thách d?u..."
          ...
          {/* History */}
          <div>
            <p className="text-app-text-secondary text-xs font-semibold tracking-normal mb-3">L?ch s? thách d?u</p>
            <div className="space-y-3">
              ...
              {challenges.length === 0 && (
                <div className="text-center py-10 text-app-text-muted">
                  ...
                  <p className="text-sm">Chua có thách d?u nào. T?o thách d?u d?u tiên!</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="space-y-4">
          {/* How it works */}
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <p className="text-white font-semibold text-sm mb-4">Cách th?c ho?t d?ng</p>
            <div className="space-y-4">
              {[
                { step: "1", icon: "ri-add-circle-line", color: "app-accent-primary", title: "T?o thách d?u", desc: "Ch?n ch? d? và s? câu h?i, sau dó làm bài" },
                { step: "2", icon: "ri-share-line", color: "#34d399", title: "Chia s? link", desc: "Sao chép link và g?i cho b?n bè qua Zalo/Facebook" },
                { step: "3", icon: "ri-user-received-line", color: "#a78bfa", title: "B?n bè tham gia", desc: "B?n bè m? link và làm cùng b? câu h?i" },
                { step: "4", icon: "ri-trophy-line", color: "#fb923c", title: "So sánh di?m", desc: "Xem ai dúng nhi?u hon và nhanh hon!" },
              ].map(s => (
          ...
          <div className="bg-[#a78bfa]/5 border border-[#a78bfa]/15 rounded-xl p-4">
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs">
                <span className="text-app-text-secondary">Tham gia thách d?u</span>
              ...
              <div className="flex justify-between text-xs">
                <span className="text-app-text-secondary">Th?ng thách d?u</span>
          ...
          {/* My stats */}
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <p className="text-white font-semibold text-sm mb-3">Th?ng kê c?a b?n</p>
            <div className="space-y-2">
              <div className="flex justify-between items-center py-1.5 border-b border-app-border">
                <span className="text-app-text-secondary text-xs">T?ng thách d?u</span>
