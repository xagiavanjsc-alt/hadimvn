        ...
        <div className="flex items-center gap-2">
          ...
          {answered !== null && (
            <div className={`flex items-center gap-1 text-xs font-bold ${isCorrect ? "text-app-accent-success" : "text-red-400"}`}>
              ...
              {isCorrect ? "Ðúng!" : `Ðáp án: ${ex.answer}`}
            </div>
          )}
        </div>
      )}
...
function RoadmapPanel({
  ...
  // Xác d?nh level hi?n t?i (level d?u tiên chua hoàn thành 80%)
  const currentLevelIdx = levelStats.findIndex(s => s.pct < 80);
  const currentLevel = currentLevelIdx >= 0 ? LEVELS[currentLevelIdx] : "B2";

  // G?i ý m?u câu ti?p theo: chua làm bài t?p nào, uu tiên level hi?n t?i
  ...
  return (
    <div className="bg-app-bg border border-app-border rounded-2xl p-5 mb-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <p className="text-white font-semibold text-sm">H?c theo l? trình</p>
          <p className="text-app-text-muted text-xs">G?i ý d?a trên ti?n d? c?a b?n</p>
        </div>
        <div className="ml-auto flex items-center gap-2 bg-app-card/50 rounded-xl px-3 py-1.5">
          ...
          <span className="text-xs font-bold" style={{ color: levelColors[currentLevel] }}>Ðang h?c: {currentLevel}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {levelStats.map(({ lv, pct, doneEx, totalEx }) => (
          <div key={lv} className={`rounded-xl p-3 border transition-all ${lv === currentLevel ? "border-white/12 bg-app-surface/50" : "border-app-border"}`}>
            ...
            <p className="text-app-text-muted text-[9px]">{doneEx}/{totalEx} bài t?p</p>
      ...
      {hasSuggestions ? (
          ...
          {suggestions.needReview.length > 0 && (
            <div>
              <p className="text-red-400/70 text-[10px] tracking-normal mb-2 flex items-center gap-1.5">
                <i className="ri-refresh-line"></i>C?n ôn l?i ({suggestions.needReview.length})
              </p>
          ...
          {suggestions.inProgress.length > 0 && (
            <div>
              <p className="text-app-accent-primary/70 text-[10px] tracking-normal mb-2 flex items-center gap-1.5">
                <i className="ri-loader-line"></i>Ðang h?c d? ({suggestions.inProgress.length})
              </p>
              <div className="flex gap-2 flex-wrap">
        ...
        <div className="text-center py-4 bg-emerald-500/5 border border-emerald-500/15 rounded-xl">
          ...
          <p className="text-app-accent-success text-sm font-semibold">Xu?t s?c! B?n dã hoàn thành t?t c? m?u câu!</p>
        </div>
      )}
    </div>
  );
    ...
    <div className={`bg-app-bg border rounded-2xl overflow-hidden transition-all ${isExpanded ? "border-white/12" : "border-app-border"}`}>
      ...
      >
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          {doneCount > 0 && (
            <span className="text-[10px] text-app-accent-success/70">{doneCount}/{pattern.exercises.length} bài t?p</span>
          )}
      ...
      {isExpanded && (
        <div className="px-5 pb-5 border-t border-app-border pt-4">
          ...
          {/* Examples */}
          <div className="mb-5">
            <p className="text-app-text-muted text-[10px] tracking-normal mb-2">Ví d?</p>
          ...
          {/* Exercises */}
          <div>
            <p className="text-app-text-muted text-[10px] tracking-normal mb-3">Bài t?p luy?n t?p</p>
            <div className="space-y-3">
  ...
  return (
    <div className="absolute top-full left-0 right-0 mt-1.5 bg-[#1a1d27] border border-app-border rounded-xl shadow-2xl z-50 overflow-hidden max-h-80 overflow-y-auto">
      <div className="px-3 py-2 border-b border-app-border">
        <p className="text-app-text-muted text-[10px] tracking-normal">{suggestions.length} m?u câu phù h?p</p>
      </div>
      {suggestions.map(p => {
...
export default function GrammarPage() {
  ...
  return (
    <DashboardLayout
      title="Ng? pháp ti?ng Hàn"
      subtitle="M?u câu t? co b?n d?n nâng cao — gi?i thích rõ ràng, luy?n t?p ngay"
      actions={
        <div className="flex items-center gap-3">
          ...
          {!user && <span className="text-app-text-muted text-xs"><i className="ri-cloud-off-line mr-1"></i>Ðang nh?p d? luu ti?n d?</span>}
          <button
          ...
          >
            ...
            {showRoadmap ? "?n l? trình" : "H?c theo l? trình"}
          </button>
        </div>
      }
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          { label: "M?u câu", value: grammarPatterns.length, icon: "ri-book-open-line", color: "app-accent-primary" },
          { label: "Bài t?p", value: totalExercises, icon: "ri-pencil-line", color: "#34d399" },
          { label: "Ðã làm", value: doneExercises, icon: "ri-checkbox-circle-line", color: "#a78bfa" },
          { label: "Làm dúng", value: correctExercises, icon: "ri-trophy-line", color: "#fb923c" },
        ].map(stat => (
      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        ...
        <div className="relative flex items-center gap-2 bg-app-card/50 border border-app-border rounded-xl px-3 py-2 flex-1 min-w-[200px]">
          ...
          <input
            type="text"
            placeholder="Tìm m?u câu ti?ng Hàn ho?c ti?ng Vi?t..."
            value={searchQuery}
        ...
        <p className="text-app-text-muted text-xs whitespace-nowrap">{filteredPatterns.length} m?u câu</p>
      </div>
      ...
      <div className="space-y-3">
        ...
        {filteredPatterns.length === 0 && (
          <div className="text-center py-16 bg-app-bg border border-app-border rounded-2xl">
            ...
            <p className="text-app-text-muted text-sm">Không tìm th?y m?u câu nào</p>
          </div>
        )}
      </div>
    </DashboardLayout>
