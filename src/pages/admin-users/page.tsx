...
function BulkActionModal({
  ...
  const actions = [
    { id: "vip_grant" as const, icon: "ri-vip-crown-line", label: "C?p VIP hàng lo?t", color: "app-accent-primary", desc: `C?p VIP cho ${selectedUsers.length} thành viên` },
    { id: "vip_revoke" as const, icon: "ri-close-circle-line", label: "H?y VIP hàng lo?t", color: "#f87171", desc: `H?y VIP c?a ${selectedUsers.length} thành viên` },
    ...
    { id: "email_bulk" as const, icon: "ri-broadcast-line", label: "G?i email thông báo", color: "#a78bfa", desc: "G?i email tùy ch?nh" },
  ];
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: "var(--admin-border)" }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs" style={{ color: "var(--admin-text-muted)" }}>{selectedUsers.length} thành viên du?c ch?n</p>
            </div>
          </div>
        ...
        {done ? (
          <div className="p-8 text-center">
            ...
            <p className="text-sm mb-6" style={{ color: "var(--admin-text-muted)" }}>
              Ðã th?c hi?n thành công cho {selectedUsers.length} thành viên
            </p>
          ...
          <div className="p-5 space-y-4">
            ...
            {/* Action selector */}
            <div>
              <p className="text-xs font-semibold mb-2" style={{ color: "var(--admin-text-muted)" }}>Ch?n hành d?ng</p>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {([
                    ...
                    { type: "year" as const, label: "VIP Nam", sub: "365 ngày", color: "app-accent-primary" },
                  ]).map(opt => (
            ...
            {action === "email_bulk" && (
              <div className="space-y-3">
                <div>
                  <label className="text-xs font-semibold mb-1.5 block" style={{ color: "var(--admin-text-muted)" }}>Tiêu d? email</label>
                  <input value={bulkTitle} onChange={e => setBulkTitle(e.target.value)} placeholder="VD: Tính nang m?i dã ra m?t!"
            ...
            {saving && (
              <div>
                <div className="flex items-center justify-between mb-1">
                  <p className="text-xs" style={{ color: "var(--admin-text-muted)" }}>Ðang x? lý...</p>
            ...
            <div className="flex gap-3 pt-1">
              ...
              <button onClick={handleExecute} disabled={!action || saving || (action === "email_bulk" && (!bulkTitle.trim() || !bulkBody.trim()))}
                ...
                {saving ? "Ðang x? lý..." : `Th?c hi?n (${selectedUsers.length} ngu?i)`}
              </button>
            </div>
          </div>
        )}
  ...
  const handleGrant = async () => {
    ...
    try {
      ...
      if (sendEmail && user.email && user.email.includes("@")) {
        ...
        if (emailErr) {
          console.warn("Email g?i không thành công:", emailErr);
        }
      }
      onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-md rounded-2xl border overflow-hidden"
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <p className="text-xs font-semibold mb-1.5" style={{ color: "var(--admin-text-muted)" }}>Ngày h?t h?n tùy ch?nh <span style={{ color: "var(--admin-text-faint)" }}>(d? tr?ng = t? d?ng)</span></p>
          ...
          <div className="px-4 py-3 rounded-xl" style={{ backgroundColor: "rgba(232,200,74,0.06)", border: "1px solid rgba(232,200,74,0.15)" }}>
            <p className="text-xs" style={{ color: "var(--admin-text-muted)" }}>
              <span className="font-bold" style={{ color: "var(--admin-text)" }}>{user.display_name}</span> ? VIP {vipType === "month" ? "Tháng" : "Nam"} d?n{" "}
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-xs" style={{ color: "var(--admin-text-muted)" }}>G?i email thông báo VIP cho thành viên</span>
          </label>
    ...
    <div className="rounded-2xl border p-4 mb-4 space-y-4" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border2)" }}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold" style={{ color: "var(--admin-text)" }}>B? l?c nâng cao</span>
          {hasActiveFilters(filters) && <span className="text-[10px] px-2 py-0.5 rounded-full bg-rose-500/15 text-rose-400 font-bold">Ðang l?c · {resultCount} k?t qu?</span>}
        </div>
        {hasActiveFilters(filters) && <button onClick={onReset} className="text-xs cursor-pointer whitespace-nowrap" style={{ color: "var(--admin-text-muted)" }}><i className="ri-refresh-line mr-1"></i>Xóa b? l?c</button>}
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div>
          <p className="text-[10px] font-semibold mb-2 tracking-normal" style={{ color: "var(--admin-text-faint)" }}>Ngày dang ký</p>
        ...
        <div>
          <p className="text-[10px] font-semibold mb-2 tracking-normal" style={{ color: "var(--admin-text-faint)" }}>XP tích luy</p>
          <div className="space-y-2">
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            <select value={filters.activeWithin} onChange={e => set("activeWithin", e.target.value)} className={inputCls + " cursor-pointer"} style={inputStyle}>
              ...
              <option value="inactive">Không ho?t d?ng &gt;30 ngày</option>
            </select>
          </div>
        </div>
        <div>
          ...
          <select value={filters.vipType} onChange={e => set("vipType", e.target.value)} className={inputCls + " cursor-pointer"} style={inputStyle}>
            <option value="all">T?t c?</option>
            <option value="none">Chua có VIP</option>
      ...
      <div className="flex items-center gap-2 flex-wrap pt-1 border-t" style={{ borderColor: "var(--admin-border)" }}>
        ...
        {[
          ...
          { label: "Không ho?t d?ng", action: () => onChange({ ...filters, activeWithin: "inactive" }) },
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {user.is_vip ? (
              <span className="text-xs px-3 py-1 rounded-full font-medium"
                ...
                <i className="ri-vip-crown-line mr-1"></i>VIP {vipType === "year" ? "Nam" : "Tháng"}
              </span>
            ) : (
          ...
          {user.is_vip && user.vip_expires_at && (
            <div className="px-4 py-3 rounded-xl space-y-2"
              ...
              <div className="flex items-center justify-between">
                <p className="text-xs font-semibold" style={{ color: vipType === "year" ? "app-accent-primary" : "#34d399" }}>VIP {vipType === "year" ? "Nam" : "Tháng"}</p>
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${daysLeft !== null && daysLeft <= 7 ? "bg-rose-500/15 text-rose-400" : "bg-app-accent-success/15 text-app-accent-success"}`}>
                  {daysLeft !== null ? (daysLeft <= 0 ? "Ðã h?t h?n" : `Còn ${daysLeft} ngày`) : ""}
                </span>
              </div>
              ...
              {daysLeft !== null && daysLeft <= 7 && daysLeft > 0 && (
                <button onClick={handleSendExpiryEmail} disabled={sendingEmail || emailSent}
                  ...
                  {emailSent ? "Ðã g?i email nh?c gia h?n" : sendingEmail ? "Ðang g?i..." : "G?i email nh?c gia h?n"}
                </button>
              )}
            </div>
          )}
          ...
          {/* Stats */}
          <div>
            <p className="text-xs font-semibold mb-3" style={{ color: "var(--admin-text-muted)" }}>Th?ng kê h?c t?p</p>
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                { label: "T? dã h?c", value: (user.words_learned || 0).toLocaleString(), icon: "ri-book-open-line", color: "#34d399" },
          ...
          <div className="space-y-2 pt-2 border-t" style={{ borderColor: "var(--admin-border)" }}>
            <p className="text-xs font-semibold" style={{ color: "var(--admin-text-muted)" }}>Qu?n lý quy?n</p>
            <div className="grid grid-cols-2 gap-2">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {[
          { label: "T?ng phiên", value: sessions.length, icon: "ri-login-circle-line", color: "#34d399" },
      ...
      {suspicious.length > 0 && (
        <div className="flex items-start gap-3 px-4 py-3 rounded-xl text-xs"
          ...
          <div>
            <p className="font-bold mb-1">{suspicious.length} phiên dang nh?p b?t thu?ng</p>
            <p className="text-rose-300/70">Có th? là dang nh?p t? nhi?u thi?t b? cùng lúc ho?c nhi?u l?n trong th?i gian ng?n. Ki?m tra ngay.</p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <p className="text-xs font-semibold" style={{ color: "var(--admin-text-muted)" }}>L?ch s? dang nh?p g?n dây</p>
          <button onClick={refetch} className="text-xs cursor-pointer" style={{ color: "var(--admin-text-faint)" }}>
            <i className="ri-refresh-line mr-1"></i>Làm m?i
          </button>
        </div>
        {loading ? (
          ...
          <div className="text-center py-10">
            ...
            <p className="text-sm" style={{ color: "var(--admin-text-muted)" }}>Chua có d? li?u dang nh?p</p>
            <p className="text-xs mt-1" style={{ color: "var(--admin-text-faint)" }}>D? li?u s? du?c ghi l?i khi thành viên dang nh?p</p>
          </div>
        ) : (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <p className="text-xs font-semibold" style={{ color: "var(--admin-text)" }}>
                    {s.device_type === "mobile" ? "Ði?n tho?i" : s.device_type === "tablet" ? "Máy tính b?ng" : "Máy tính"}
                  </p>
                  {s.is_suspicious && (
      ...
      <div className="rounded-xl p-4 space-y-2" style={{ backgroundColor: "rgba(232,200,74,0.05)", border: "1px solid rgba(232,200,74,0.12)" }}>
        <p className="text-xs font-bold" style={{ color: "app-accent-primary" }}><i className="ri-shield-check-line mr-1.5"></i>G?i ý b?o m?t</p>
        <ul className="space-y-1.5">
          {[
            "N?u th?y dang nh?p b?t thu?ng, liên h? thành viên d? xác nh?n",
            "Tài kho?n VIP không nên chia s? — 1 tài kho?n ch? dùng cho 1 ngu?i",
            "Ðang nh?p t? nhi?u IP khác nhau trong th?i gian ng?n = d?u hi?u chia s? tài kho?n",
            "Có th? h?y VIP n?u phát hi?n vi ph?m di?u kho?n s? d?ng",
          ].map((tip, i) => (
...
export default function AdminUsersPage() {
  ...
  const handleToggleVip = useCallback(async (id: string, cur: boolean) => {
    try {
      ...
      showMsg(!cur ? "Ðã c?p VIP" : "Ðã h?y VIP");
  ...
  const handleToggleAdmin = useCallback(async (id: string, cur: boolean) => {
    try {
      ...
      if (res.error || res.data?.error) throw new Error(res.error?.message || res.data?.error || "L?i");
      showMsg(!cur ? "Ðã c?p Admin" : "Ðã h?y Admin");
  ...
  const handleBulkGrantVip = useCallback(async (userIds: string[], type: "month" | "year", expiresAt: string) => {
    ...
    showMsg(`Ðã c?p VIP cho ${granted}/${userIds.length} thành viên!`);
    refetch();
  ...
  const handleBulkEmail = useCallback(async (targetUsers: AdminUser[], type: "vip_expiry_reminder" | "bulk_notification", extra?: Record<string, unknown>) => {
    ...
    showMsg(`Ðã g?i email cho ${targetUsers.length} thành viên!`);
    setSelectedIds(new Set());
  }, []);
  ...
  return (
    <AdminLayout
      title="Qu?n lý Thành viên"
      subtitle={`${users.length} thành viên · ${vipCount} VIP (${vipYearCount} nam, ${vipMonthCount} tháng) · ${adminCount} admin`}
      actions={
        <div className="flex items-center gap-2">
          {selectedIds.size > 0 && (
    ...
    >
      ...
      <div className="flex bg-app-card/50 rounded-xl p-1 mb-5 border" style={{ borderColor: "var(--admin-border)" }}>
        ...
        >
          <i className="ri-shield-keyhole-line mr-1.5"></i>Ðang nh?p & B?o m?t
        </button>
      </div>
      ...
      <div className="grid grid-cols-3 lg:grid-cols-6 gap-3 mb-5">
        {[
          { label: "T?ng thành viên", value: users.length, icon: "ri-user-line", color: "#f87171" },
          ...
          { label: "Ho?t d?ng hôm nay", value: activeToday, icon: "ri-pulse-line", color: "#34d399" },
      ...
      {vipExpiringSoon > 0 && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl mb-4 text-xs"
          ...
          <span><strong>{vipExpiringSoon} thành viên VIP</strong> s?p h?t h?n trong 7 ngày. G?i email nh?c gia h?n hàng lo?t?</span>
          <button onClick={() => {
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {selectedIds.size > 0 && <span className="text-xs font-semibold" style={{ color: "#f87171" }}>{selectedIds.size} dã ch?n</span>}
        </div>
        ...
        <div className="flex items-center gap-2 rounded-xl px-3 py-2 flex-1 min-w-[200px] border"
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Tìm theo tên ho?c ID..."
        ...
        <select value={sortBy} onChange={e => setSortBy(e.target.value as typeof sortBy)}
          ...
          <option value="active">Ho?t d?ng g?n dây</option>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm" style={{ color: "var(--admin-text-muted)" }}>Ðang t?i t? Supabase...</p>
          </div>
        </div>
      ) : error ? (
        ...
        <VirtualList
          ...
          emptyState={
            <div className="text-center py-16">
              ...
              <p className="text-sm" style={{ color: "var(--admin-text-muted)" }}>Không tìm th?y thành viên nào</p>
            </div>
          }
        />
      )}
