...
function MC4ImageQuestion({ q, selected, onAnswer, showResult }: { q: T2Question; selected: number | null; onAnswer: (i: number) => void; showResult: boolean }) {
  return (
    <div className="grid grid-cols-2 gap-3 mt-3">
      {(q.options || []).map((opt, i) => {
        let cls = "border-white/8 bg-white/3 hover:border-white/20 cursor-pointer";
        if (showResult) {
        ...
        } else if (selected === i) cls = "border-[#e8c84a]/40 bg-[#e8c84a]/8 cursor-pointer";
        return (
          <button key={i} onClick={() => !showResult && onAnswer(i)} disabled={showResult}
            ...
            <p className={`text-xs font-medium text-center ${showResult && i === q.correctIndex ? "text-emerald-400" : showResult && i === selected ? "text-red-400" : selected === i ? "text-[#e8c84a]" : "text-white/50"}`}>{opt}</p>
            {showResult && i === q.correctIndex && <i className="ri-checkbox-circle-fill text-emerald-400 text-sm"></i>}
      ...
      {(q.options || []).map((opt, i) => {
        ...
        return (
          <button key={i} onClick={() => !showResult && onAnswer(i)} disabled={showResult}
            ...
            <span className={`w-6 h-6 flex items-center justify-center rounded-full text-[11px] font-bold flex-shrink-0 ${showResult && i === q.correctIndex ? "bg-emerald-500/20 text-emerald-400" : showResult && i === selected ? "bg-red-500/20 text-red-400" : selected === i ? "bg-[#e8c84a]/20 text-[#e8c84a]" : "bg-white/5 text-white/30"}`}>
              {["①","②","③","④"][i]}
            </span>
            <div className="flex-1 min-w-0">
              <p className={`text-sm ${showResult && i === q.correctIndex ? "text-emerald-400 font-medium" : showResult && i === selected && i !== q.correctIndex ? "text-red-400" : selected === i ? "text-[#e8c84a]" : "text-white/60"}`}>{opt}</p>
...
function FillBlankQuestion({ q, value, value2, onChange, onChange2, showResult }: {
  ...
  return (
    <div className="mt-3 space-y-3">
      <div>
        <label className="text-white/30 text-[10px] uppercase tracking-widest mb-1.5 block">① Chỗ trống thứ nhất</label>
        ...
        {showResult && <p className="text-white/30 text-xs mt-1">Gợi ý: {q.correctAnswer}</p>}
      </div>
      {q.type === "fill_blank2" && (
        <div>
          <label className="text-white/30 text-[10px] uppercase tracking-widest mb-1.5 block">② Chỗ trống thứ hai</label>
          ...
          {showResult && <p className="text-white/30 text-xs mt-1">Gợi ý: {q.correctAnswer2}</p>}
        </div>
      )}
      {showResult && q.sampleAnswer && (
        <div className="bg-emerald-500/5 border border-emerald-500/15 rounded-xl p-3">
          <p className="text-emerald-400/70 text-[10px] uppercase tracking-widest mb-1">Đáp án mẫu</p>
    ...
    <div className="mt-3 space-y-3">
      {q.writingGuide && (
        <div className="bg-[#e8c84a]/5 border border-[#e8c84a]/15 rounded-xl p-3">
          <p className="text-[#e8c84a]/70 text-[10px] uppercase tracking-widest mb-1.5">Hướng dẫn viết</p>
      ...
      <div>
        <textarea
          ...
          className={`w-full bg-white/5 border rounded-xl px-4 py-3 text-sm outline-none resize-none transition-all leading-relaxed ${showResult ? "border-white/10 text-white/50 cursor-default" : "border-white/10 text-white/70 focus:border-[#e8c84a]/30"}`}
        />
        <div className="flex items-center justify-between mt-1.5">
          <p className={`text-xs ${isInRange ? "text-emerald-400" : charCount > maxChars ? "text-red-400" : "text-white/30"}`}>
            {charCount} / {minChars}-{maxChars} chữ
            {isInRange && " ✓"}
          </p>
          {!showResult && <p className="text-white/20 text-[10px]">Nhấn Tab để thụt đầu dòng</p>}
        </div>
      </div>
      {showResult && q.sampleAnswer && (
        <div className="bg-white/3 border border-white/8 rounded-xl p-4">
          <p className="text-white/30 text-[10px] uppercase tracking-widest mb-2">Bài mẫu tham khảo</p>
...
function QuestionCard({
  ...
  return (
    <div id={`q2-${q.number}`} className="bg-[#0f1117] border border-white/5 rounded-2xl p-5 scroll-mt-4">
      {/* Header */}
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        ...
        <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/5 text-white/30">{typeLabels[q.type] || q.type}</span>
        <span className="text-[10px] text-white/20 ml-auto">{q.points}đ</span>
      </div>
      ...
      {q.audioScript && (
        <div className="bg-[#38bdf8]/5 border border-[#38bdf8]/15 rounded-xl p-3 mb-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-[#38bdf8]/70 text-[10px] uppercase tracking-widest">Đoạn hội thoại / Âm thanh</p>
          </div>
      ...
      <p className="text-white/30 text-xs italic mb-3">{q.questionVi}</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#0f1117] border border-white/8 rounded-2xl p-8 text-center">
        ...
        <div className="mt-5 bg-white/5 rounded-full h-3 overflow-hidden">
          <div className="h-full rounded-full transition-all duration-1000" style={{ width: `${pct}%`, backgroundColor: level.color }}></div>
        </div>
        <p className="text-white/25 text-xs mt-2">{pct}% tổng điểm</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={sec.label} className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
      ...
      <button onClick={onRetry} className="w-full py-4 rounded-2xl bg-[#e8c84a] hover:bg-[#d4b43a] text-[#0f1117] font-bold text-base transition-colors cursor-pointer whitespace-nowrap">
...
export default function Topik2TestPage() {
          ...
          <div className="bg-[#0f1117] border border-white/8 rounded-2xl p-8 text-center">
            ...
            {bestScore > 0 && (
              <div className="inline-flex items-center gap-2 bg-[#e8c84a]/8 border border-[#e8c84a]/15 rounded-xl px-4 py-2">
                ...
                <span className="text-[#e8c84a] text-sm font-semibold">Điểm cao nhất: {bestScore}</span>
                <span className="text-white/30 text-xs">({attempts} lần)</span>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={s.label} className="bg-[#0f1117] border border-white/5 rounded-xl p-4 text-center">
                ...
                <p className="text-white/30 text-[10px] mt-0.5">{s.value}</p>
              </div>
            ))}
          </div>
          ...
          <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <div key={t.label} className="flex items-center gap-2 bg-white/3 rounded-lg px-3 py-2">
          ...
          <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={l.level} className="flex items-center justify-between px-3 py-2 rounded-lg bg-white/3">
                  <span className="text-xs font-bold" style={{ color: l.color }}>{l.level}</span>
                  <span className="text-white/40 text-xs">{l.range}</span>
                </div>
              ))}
            </div>
          </div>

          <button onClick={handleStart} className="w-full py-4 rounded-2xl bg-[#e8c84a] hover:bg-[#d4b43a] text-[#0f1117] font-black text-lg transition-colors cursor-pointer whitespace-nowrap">
  ...
  return (
    <DashboardLayout
      ...
      actions={
        <button onClick={handleSubmit} className="flex items-center gap-2 bg-[#e8c84a] hover:bg-[#d4b43a] text-[#0f1117] font-bold text-sm px-5 py-2.5 rounded-xl transition-colors cursor-pointer whitespace-nowrap">
    ...
    >
      ...
      <div className={`sticky top-0 z-10 bg-[#0a0c10]/95 backdrop-blur-sm border-b mb-5 py-3 px-1 -mx-1 ${isDanger ? "border-red-500/30" : isWarning ? "border-[#e8c84a]/20" : "border-white/5"}`}>
        <div className="flex items-center gap-4">
          ...
          <span className="text-white/25 text-xs whitespace-nowrap">180 phút</span>
        </div>
      </div>
      ...
      <div className="flex items-center bg-white/5 rounded-xl p-1 mb-5 w-fit">
        {sectionTabs.map(tab => (
          <button key={tab.key} onClick={() => setCurrentSection(tab.key)}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all cursor-pointer whitespace-nowrap ${currentSection === tab.key ? "text-[#0f1117]" : "text-white/40 hover:text-white/60"}`}
      ...
      <div className="mt-8 flex justify-center">
        <button onClick={handleSubmit} className="flex items-center gap-2 bg-[#e8c84a] hover:bg-[#d4b43a] text-[#0f1117] font-bold text-base px-10 py-4 rounded-2xl transition-colors cursor-pointer whitespace-nowrap">
          <i className="ri-send-plane-fill"></i>Nộp bài
        </button>
      </div>
    </DashboardLayout>
