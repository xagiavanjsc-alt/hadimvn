...
const FEATURED_TOPICS = [
  { id: "health", label: "Y t? & S?c kh?e", icon: "ri-heart-pulse-line", color: "#f43f5e", desc: "T? v?ng khám b?nh, thu?c, tri?u ch?ng" },
  { id: "transport", label: "Giao thông & Di chuy?n", icon: "ri-bus-line", color: "#22d3ee", desc: "Tàu di?n ng?m, xe buýt, taxi, du?ng ph?" },
  { id: "housing", label: "Nhà ? & Sinh s?ng", icon: "ri-home-4-line", color: "#84cc16", desc: "Thuê nhà, hóa don, hàng xóm, khu ph?" },
  { id: "safety", label: "An toàn lao d?ng", icon: "ri-shield-check-line", color: "#fb923c", desc: "Thi?t b? b?o h?, quy trình an toàn" },
  { id: "workplace", label: "Noi làm vi?c", icon: "ri-briefcase-line", color: "#38bdf8", desc: "Van phòng, nhà máy, d?ng nghi?p" },
  { id: "daily", label: "Sinh ho?t hàng ngày", icon: "ri-home-smile-line", color: "app-accent-primary", desc: "Mua s?m, an u?ng, gi?i trí" },
  { id: "law", label: "Pháp lu?t lao d?ng", icon: "ri-scales-3-line", color: "#f59e0b", desc: "H?p d?ng, luong, quy?n l?i" },
  { id: "greeting", label: "Giao ti?p co b?n", icon: "ri-chat-smile-2-line", color: "#34d399", desc: "Chào h?i, xã giao, di?n tho?i" },
  { id: "culture", label: "Van hóa Hàn Qu?c", icon: "ri-building-2-line", color: "#a78bfa", desc: "Phong t?c, l? h?i, ?m th?c" },
];
  ...
  return (
    <div className="flex flex-col gap-6 max-w-lg mx-auto">
      <div className="flex items-center justify-between">
        ...
        <span className="text-app-accent-primary text-sm font-bold">{score} dúng</span>
      </div>
      <div className="bg-app-bg border border-app-border rounded-2xl p-8 text-center">
        <p className="text-app-text-muted text-xs mb-3">Nghia ti?ng Vi?t c?a t? này là gì?</p>
  ...
  return (
    <div className="flex flex-col items-center gap-6 py-8 max-w-md mx-auto text-center">
      ...
      <div>
        ...
        <p className="text-app-text-secondary text-sm mt-1">{score}/{total} câu dúng</p>
      </div>
      ...
      <div className="flex gap-3 w-full">
        {unknownCount > 0 && (
          <button onClick={onReviewUnknown} className="flex-1 py-3 rounded-xl border border-app-accent-primary/20 bg-app-accent-primary/5 text-app-accent-primary text-sm font-medium cursor-pointer whitespace-nowrap hover:bg-app-accent-primary/10 transition-colors">
            <i className="ri-refresh-line mr-2"></i>Ôn t? chua nh? ({unknownCount})
          </button>
        )}
  ...
  if (!isStudying) {
    return (
      <DashboardLayout
        ...
        subtitle="H?c t? v?ng theo t?ng ch? d? v?i flashcard l?t th? và quiz"
      >
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6">
          ...
          <div className="space-y-4">
            <div className="bg-app-bg border border-app-border rounded-2xl p-5">
              <h3 className="text-white font-semibold text-sm mb-4"><i className="ri-bar-chart-line text-app-accent-primary mr-2"></i>Th?ng kê h?c t?p</h3>
              <div className="space-y-3">
                {[
                  { label: "T?ng t? v?ng EPS", value: epsVocabulary.length, color: "app-accent-primary" },
                  { label: "Ch? d? có s?n", value: FEATURED_TOPICS.length, color: "#34d399" },
                  { label: "Ch? d? dã h?c", value: Object.keys(progress).filter(k => progress[k] > 0).length, color: "#60a5fa" },
                ].map(s => (
            ...
            <div className="bg-app-bg border border-app-border rounded-2xl p-5">
              ...
              <div className="space-y-2.5 text-app-text-secondary text-xs leading-relaxed">
                <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Nh?n vào th? d? l?t và xem nghia</p>
                <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Ch?n "Ðã nh?" ho?c "Chua nh?" d? phân lo?i</p>
                <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Ôn l?i t? chua nh? sau khi hoàn thành</p>
                <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Dùng Quiz d? ki?m tra sau khi h?c flashcard</p>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-app-surface to-[#0f1117] border border-app-accent-primary/15 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-white font-semibold text-sm">Ch? d? uu tiên EPS</p>
              </div>
              <p className="text-app-text-secondary text-xs leading-relaxed">
                T?p trung vào <strong className="text-white/60">Y t?</strong>, <strong className="text-white/60">Giao thông</strong> và <strong className="text-white/60">Nhà ?</strong> — 3 ch? d? m?i du?c b? sung cho k? thi EPS-TOPIK g?n dây.
              </p>
            </div>
          </div>
        </div>
    ...
    <DashboardLayout
      ...
      actions={
        <button onClick={handleBack} className="flex items-center gap-2 px-4 py-2 rounded-xl border border-app-border text-white/50 text-sm cursor-pointer whitespace-nowrap hover:bg-app-card/50 transition-colors">
          <i className="ri-arrow-left-line"></i>Ch?n ch? d? khác
        </button>
      }
    >
