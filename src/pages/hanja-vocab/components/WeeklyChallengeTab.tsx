  ...
  if (done) {
    ...
    return (
      <div className="text-center py-8">
        ...
        <p className="text-gray-500 mb-2">Ðúng {score}/{questions.length} câu</p>
        <p className={`text-sm font-semibold mb-6 ${passed ? "text-green-600" : "text-red-500"}`}>
          {passed ? "Xu?t s?c! B?n dã vu?t thách th?c tu?n này!" : "Chua d?t 70% — Hãy ôn thêm và th? l?i!"}
        </p>
  ...
  return (
    <div className="max-w-lg mx-auto">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-sm font-semibold text-green-600">Ðúng: {score}</span>
      </div>
      ...
      {answered && (
        <button onClick={next} className="w-full py-3 bg-rose-500 text-white rounded-xl font-semibold cursor-pointer hover:bg-rose-600 transition-colors">
          {idx + 1 >= questions.length ? "Xem k?t qu?" : "Câu ti?p ?"}
        </button>
      )}
    </div>
  );
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <div className="bg-white rounded-3xl p-10 text-center max-w-sm w-full mx-4 shadow-2xl">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Thách th?c hoàn thành!</h2>
          <p className="text-gray-500 mb-4">B?n dã vu?t qua thách th?c tu?n {weekId}</p>
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 mb-6">
            <p className="text-4xl font-bold text-amber-600">+{earnedXp} XP</p>
            <p className="text-sm text-amber-500 mt-1">Ðã c?ng vào tài kho?n c?a b?n!</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6 text-center">
            {[
              { label: "T? dã h?c", value: learnedCount, color: "text-green-600" },
    ...
    return (
      <div className="max-w-lg mx-auto">
        <div className="flex items-center justify-between mb-5">
          ...
          <span className="text-xs text-green-600 font-medium">{learnedCount} dã h?c</span>
        </div>
        ...
        <div className="flex gap-3 mt-5">
          <button
            ...
            <i className="ri-check-line mr-1"></i>Ðã thu?c
          </button>
          <button
            onClick={() => {
              setStudyFlipped(false);
        ...
        <div className="mt-6 bg-gray-50 rounded-xl p-4">
          <p className="text-xs font-semibold text-gray-500 mb-3">50 t? tu?n này</p>
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-rose-500 to-orange-400 rounded-2xl p-6 mb-6 text-white relative overflow-hidden">
        ...
        <div className="relative">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-white/80 text-sm font-medium">Thách th?c hàng tu?n</span>
            <span className="bg-app-border/200 text-white text-xs font-bold px-2 py-0.5 rounded-full">{weekId}</span>
          </div>
          <h2 className="text-2xl font-bold mb-1">H?c 50 t? Hán-Hàn</h2>
          <p className="text-white/70 text-sm">Hoàn thành quiz d?t 70%+ d? nh?n XP bonus</p>
          <div className="flex items-center gap-4 mt-4">
            <div className="flex items-center gap-1.5">
              <i className="ri-time-line text-white/70"></i>
              <span className="text-sm">{daysLeft} ngày còn l?i</span>
            </div>
            <div className="flex items-center gap-1.5">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-5 mb-5">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-lg font-bold text-gray-900">{learnedCount} / {totalWords} t? dã h?c</p>
            <p className="text-sm text-gray-500">
              {learnedCount >= 50 ? "Hoàn thành! S?n sàng làm quiz!" : `Còn ${totalWords - learnedCount} t? n?a`}
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
        {[
          { label: "Ðã h?c", value: learnedCount, icon: "ri-check-double-line", color: "text-green-600", bg: "bg-green-50" },
      ...
      {challenge.quizPassed && (
        <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl px-4 py-3 mb-5">
          ...
          <div>
            <p className="text-green-700 font-semibold text-sm">Ðã hoàn thành thách th?c tu?n này!</p>
            <p className="text-green-500 text-xs">Nh?n +{challenge.xpEarned} XP · Quay l?i tu?n sau nhé!</p>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-3 mb-6">
        ...
        <button
          ...
          {challenge.quizPassed ? "Ðã hoàn thành" : learnedCount >= 20 ? "Làm Quiz ngay!" : `H?c thêm ${20 - learnedCount} t?`}
        </button>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400">{learnedCount} dã h?c</span>
        </div>
      ...
      <div className="mt-5 bg-amber-50 border border-amber-200 rounded-xl p-4">
        <p className="text-sm font-semibold text-amber-700 mb-3 flex items-center gap-2">
          <i className="ri-star-line"></i>Cách nh?n XP bonus
        </p>
        <div className="space-y-2">
          {[
            { label: "Hoàn thành quiz d?t 70%+", xp: "+500 XP", done: challenge.quizPassed },
            { label: "H?c d? 50 t? trong tu?n", xp: "+200 XP", done: learnedCount >= 50 },
            { label: "Hoàn thành tru?c th? 5", xp: "+100 XP", done: false },
          ].map(r => (
