    ...
    return (
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center gap-3 mb-5">
          ...
          <h2 className="text-base font-bold text-gray-900">L?ch s? phát âm</h2>
        </div>

        {history.length === 0 ? (
          <div className="text-center py-16 text-gray-400">
            <i className="ri-mic-line text-4xl"></i>
            <p className="mt-2 text-sm">Chua có l?ch s? phát âm</p>
          </div>
        ) : (
            ...
            {history.map((h, i) => (
              <div key={i} className="flex items-center gap-4 px-4 py-3 bg-white border border-gray-100 rounded-xl">
                ...
                <div className={`text-xs px-2 py-0.5 rounded-full font-bold ${
                  ...
                  {h.score >= 80 ? "T?t" : h.score >= 60 ? "Khá" : "C?n luy?n"}
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
        {[
          { label: "Ðã luy?n", value: history.length, icon: "ri-mic-line", color: "#f43f5e" },
          { label: "Ði?m TB", value: avgScore ? `${avgScore}%` : "—", icon: "ri-bar-chart-line", color: "#fb923c" },
          { label: "Phát âm t?t", value: perfectCount, icon: "ri-star-line", color: "#34d399" },
        ].map(s => (
      ...
      {notSupported && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-amber-700">Trình duy?t không h? tr? nh?n d?ng gi?ng nói</p>
            <p className="text-xs text-amber-600 mt-0.5">Vui lòng dùng Chrome ho?c Edge d? s? d?ng tính nang ghi âm. B?n v?n có th? nghe phát âm chu?n TTS.</p>
          </div>
        </div>
      )}
        ...
        <div className="bg-white border-2 border-gray-100 rounded-2xl overflow-hidden mb-4">
          ...
          <div className="flex gap-3 p-4 border-b border-gray-100">
            <button onClick={handleSpeak} disabled={isSpeaking}
              ...
              {isSpeaking ? "Ðang phát..." : "Nghe phát âm chu?n"}
            </button>
          ...
          <div className="p-6">
            {recordState === "idle" && (
              <div className="text-center">
                <p className="text-sm text-gray-500 mb-4">Nghe phát âm chu?n, sau dó nh?n ghi âm d? luy?n t?p</p>
                ...
                {notSupported && (
                  <p className="text-xs text-gray-400 mt-3">Tính nang ghi âm không kh? d?ng trên trình duy?t này</p>
                )}
              </div>
            )}
            ...
            {recordState === "recording" && (
              <div className="text-center py-4">
                ...
                <p className="text-sm font-semibold text-red-500">Ðang ghi âm... Hãy d?c to t? trên!</p>
                <p className="text-xs text-gray-400 mt-1">T? d?ng d?ng sau 3 giây</p>
              </div>
            )}
            ...
            {recordState === "processing" && (
              <div className="text-center py-4">
                ...
                <p className="text-sm text-gray-500">Ðang phân tích phát âm...</p>
              </div>
            )}
              ...
              <div>
                ...
                <div className="flex items-center gap-6 mb-4">
                  ...
                  <div className="flex-1">
                    <p className={`text-lg font-bold mb-1 ${score >= 80 ? "text-green-600" : score >= 60 ? "text-amber-600" : "text-red-500"}`}>
                      {score >= 80 ? "Phát âm t?t!" : score >= 60 ? "Khá t?t!" : score >= 40 ? "C?n luy?n thêm" : "Th? l?i nhé!"}
                    </p>
                    {recognized && (
                    ...
                    {!recognized && (
                      <p className="text-sm text-gray-400">Không nh?n d?ng du?c gi?ng nói</p>
                    )}
                ...
                <div className="bg-gray-50 rounded-xl p-3 mb-4">
                  <p className="text-xs font-semibold text-gray-600 mb-1">
                    {score >= 80 ? "Xu?t s?c! Ti?p t?c duy trì!" :
                     score >= 60 ? "M?o c?i thi?n:" : "C?n chú ý:"}
                  </p>
                  <p className="text-xs text-gray-500">
                    {score >= 80 ? "Phát âm c?a b?n r?t chu?n. Hãy th? t? khó hon!" :
                     score >= 60 ? "Nghe l?i phát âm chu?n và chú ý âm di?u. Luy?n t?p ch?m tru?c." :
                     "Hãy nghe phát âm chu?n nhi?u l?n, sau dó d?c theo t?ng âm ti?t."}
                  </p>
                </div>
      ...
      <div className="flex gap-3">
        <div className="flex-1 bg-rose-50 border border-rose-100 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-semibold text-rose-700">Hu?ng d?n luy?n phát âm</span>
          </div>
          <ul className="space-y-1 text-xs text-rose-600">
            <li className="flex items-start gap-1.5"><i className="ri-arrow-right-s-line mt-0.5 flex-shrink-0"></i>Nghe phát âm chu?n TTS tru?c khi ghi âm</li>
            <li className="flex items-start gap-1.5"><i className="ri-arrow-right-s-line mt-0.5 flex-shrink-0"></i>Ð?c to, rõ ràng trong môi tru?ng yên tinh</li>
            <li className="flex items-start gap-1.5"><i className="ri-arrow-right-s-line mt-0.5 flex-shrink-0"></i>Ð?t 80+ di?m d? nh?n +10 XP m?i t?</li>
          </ul>
        </div>
