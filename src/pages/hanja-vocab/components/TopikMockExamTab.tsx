...
function getQuestionPrompt(q: ExamQuestion): { main: string; sub: string; label: string } {
  switch (q.type) {
    case "ko2vi":
      return { main: q.entry.korean, sub: q.entry.hanja, label: "T? ti?ng Hàn này có nghia là gì?" };
    case "vi2ko":
      return { main: q.entry.vietnamese, sub: "", label: "Nghia ti?ng Vi?t này là t? Hàn nào?" };
    case "hanja2ko":
      return { main: q.entry.hanja, sub: "", label: "Hán t? này d?c là gì trong ti?ng Hàn?" };
    case "context":
      return { main: q.entry.korean, sub: q.entry.hanja, label: "Ch?n nghia dúng c?a t? này:" };
    default:
      return { main: q.entry.korean, sub: q.entry.hanja, label: "T? ti?ng Hàn này có nghia là gì?" };
  }
}
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden mb-5">
          ...
          <div className="bg-gradient-to-r from-rose-500 to-orange-500 p-6 text-white text-center">
            ...
            <h2 className="text-xl font-bold mb-1">Thi th? TOPIK Hán Hàn</h2>
            <p className="text-white/80 text-sm">30 câu tr?c nghi?m · 30 phút · Theo chu?n d? thi TOPIK</p>
          </div>
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              {[
                { icon: "ri-question-line", label: "S? câu", value: "30 câu", color: "#f43f5e" },
                { icon: "ri-timer-line", label: "Th?i gian", value: "30 phút", color: "#fb923c" },
                { icon: "ri-bar-chart-line", label: "Ði?m d?t", value: "70%+", color: "#34d399" },
              ].map(s => (
            ...
            {/* Question types */}
            <div className="mb-5">
              <p className="text-sm font-semibold text-gray-700 mb-3">D?ng câu h?i trong d? thi</p>
              <div className="space-y-2">
                {[
                  { label: "Hàn ? Vi?t", desc: "Ch?n nghia ti?ng Vi?t dúng", pct: "40%" },
                  { label: "Vi?t ? Hàn", desc: "Ch?n t? ti?ng Hàn dúng", pct: "40%" },
                  { label: "Hán t? ? Hàn", desc: "Ð?c Hán t? thành ti?ng Hàn", pct: "20%" },
                ].map(t => (
            ...
            {/* Filter by group */}
            <div className="mb-5">
              <p className="text-sm font-semibold text-gray-700 mb-2">Ch?n nhóm t? v?ng</p>
              ...
              <p className="text-xs text-gray-400 mt-2">Ð? thi s? l?y ng?u nhiên {TOTAL_QUESTIONS} câu t? {pool.length} t? dã ch?n</p>
            </div>
        ...
        {history.length > 0 && (
          <div className="bg-white border border-gray-100 rounded-xl p-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-gray-900">Ðúng {history[0].score}/{history[0].total} câu</p>
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-4 bg-white border border-gray-100 rounded-xl px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-gray-400">({answeredCount} dã tr? l?i)</span>
          </div>
          ...
          <button onClick={finishExam}
            ...
            <i className="ri-flag-line"></i>N?p bài
          </button>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={goNext}
            ...
            {currentIdx + 1 >= questions.length ? (
              ...
              <><span>Câu ti?p theo</span><i className="ri-arrow-right-line"></i></>
            )}
          </button>
        </div>
        ...
        <div className="mt-4 bg-white border border-gray-100 rounded-xl p-3">
          <p className="text-xs text-gray-400 mb-2">Ði?u hu?ng câu h?i</p>
          <div className="flex flex-wrap gap-1.5">
            {questions.map((q, i) => (
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className={`rounded-2xl p-6 mb-5 text-center ${passed ? "bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200" : "bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200"}`}>
          ...
          <p className={`text-lg font-semibold mb-2 ${passed ? "text-green-600" : "text-amber-600"}`}>
            {passed ? "Ð?t! Xu?t s?c!" : "Chua d?t — C?n ôn thêm"}
          </p>
          <p className="text-gray-500 text-sm mb-4">Ðúng {examResult.score}/{examResult.total} câu · Th?i gian: {formatTime(examResult.timeUsed)}</p>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {[
              { label: "Ðúng", value: examResult.score, color: "text-green-600", bg: "bg-green-100" },
              { label: "Sai", value: examResult.total - examResult.score, color: "text-red-500", bg: "bg-red-100" },
              { label: "Ði?m", value: `${pct}%`, color: passed ? "text-green-600" : "text-amber-600", bg: passed ? "bg-green-100" : "bg-amber-100" },
            ].map(s => (
          ...
          <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden mb-5">
            <div className="px-5 py-4 border-b border-gray-100">
              <h3 className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                ...
                Câu sai c?n ôn l?i ({wrongItems.length} câu)
              </h3>
            </div>
                ...
                <div key={i} className="px-5 py-3 hover:bg-red-50/30 transition-colors">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-green-600 font-medium">? Ðúng: {item.entry.vietnamese}</p>
                      {item.userAnswer && (
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {history.length === 0 ? (
          <div className="text-center py-16 text-gray-400">
            ...
            <p className="mt-2 text-sm">Chua có l?n thi nào</p>
            ...
            {history.map((h, i) => {
              ...
              return (
                <div key={i} className="bg-white border border-gray-100 rounded-xl p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-gray-900">Ðúng {h.score}/{h.total} câu</p>
