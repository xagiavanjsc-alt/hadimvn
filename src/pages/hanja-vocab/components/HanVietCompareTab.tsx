...
const HANVIET_MAP: Record<string, string> = {
  ...
  "低": "đê", "大": "đại", "小": "tiểu", "多": "đa", "少": "thiểu",
  ...
  "書": "thư", "話": "thoại", "聴": "thính", "視": "thị", "観": "quan",
  "察": "sát", "研": "nghiên", "究": "cứu", "発": "phát", "明": "minh",
  "作": "tác", "品": "phẩm", "術": "thuật", "技": "kỹ", "能": "năng",
  "力": "lực", "才": "tài", "能": "năng", "芸": "nghệ", "術": "thuật",
  "音": "âm", "楽": "nhạc", "歌": "ca", "舞": "vũ", "劇": "kịch",
  "映": "ánh", "画": "họa", "写": "tả", "真": "chân", "像": "tượng",
  "体": "thể", "健": "kiện", "康": "khang", "病": "bệnh", "医": "y",
  "薬": "dược", "治": "trị", "療": "liệu", "手": "thủ", "術": "thuật",
  "科": "khoa", "学": "học", "研": "nghiên", "究": "cứu", "実": "thực",
  "験": "nghiệm", "証": "chứng", "明": "minh", "確": "xác", "認": "nhận",
  "合": "hợp", "同": "đồng", "共": "cộng", "協": "hiệp", "力": "lực",
    ...
    <div className="space-y-5">
      ...
      <div className="bg-gradient-to-r from-emerald-500/10 to-[#0f1117] border border-emerald-500/20 rounded-2xl p-5">
        <div className="flex items-start gap-4">
          <div className="w-12 h-12 flex items-center justify-center bg-emerald-500/15 rounded-xl flex-shrink-0">
            <i className="ri-lightbulb-line text-emerald-400 text-xl"></i>
          </div>
          <div className="flex-1">
            <h3 className="text-white font-bold text-base mb-1">Người Việt học tiếng Hàn nhanh hơn nhờ Hán tự chung!</h3>
            <p className="text-white/50 text-sm leading-relaxed">
              Tiếng Hàn và tiếng Việt đều có nguồn gốc từ chữ Hán. Khoảng <span className="text-emerald-400 font-bold">60-70%</span> từ vựng tiếng Hàn là từ Hán-Hàn, 
              và nhiều từ trong số đó <span className="text-emerald-400 font-bold">đọc gần giống</span> tiếng Việt!
            </p>
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4">
          {[
            ...
            <div key={s.label} className="bg-white/3 rounded-xl p-3 text-center">
              ...
              <p className="text-white/20 text-[10px]">{s.pct}% tổng số</p>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setShowSpotlight(false)} className="text-white/20 hover:text-white/40 cursor-pointer text-xs">
          ...
          {quizMode ? (
            <div className="max-w-md mx-auto">
              <div className="flex items-center justify-between mb-3">
                <span className="text-white/40 text-xs">Câu {(quizIdx % quizPool.length) + 1}/{quizPool.length}</span>
                <span className="text-emerald-400 text-xs font-bold">✓ {quizScore.correct}/{quizScore.total}</span>
              </div>
              <div className="bg-white/3 rounded-xl p-5 text-center mb-4">
                <p className="text-white/40 text-xs mb-2">Từ Hán-Hàn này đọc Hán Việt là gì?</p>
                ...
                <p className="text-white/40 text-sm mt-2">{currentQuiz.vietnamese}</p>
              </div>
              {!quizRevealed ? (
                  ...
                  {[currentQuiz.hanviet, ...SPOTLIGHT_EXAMPLES.filter(e => e.korean !== currentQuiz.korean).slice(0, 3).map(e => e.hanviet)]
                    ...
                    .map((ans, i) => (
                      <button
                        ...
                        className="px-4 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-white/70 text-sm cursor-pointer transition-all"
                      >
                        {ans}
                      </button>
                    ))}
                ...
                <div className="text-center">
                  <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 mb-3">
                    <p className="text-emerald-400 font-bold text-lg">{currentQuiz.hanviet}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {SPOTLIGHT_EXAMPLES.map((ex, i) => (
                <div key={i} className="bg-white/3 border border-emerald-500/15 rounded-xl p-4 hover:border-emerald-500/30 transition-all">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400 font-bold whitespace-nowrap">Giống!</span>
                  </div>
                  <div className="flex items-center gap-2 mb-2">
                    <div className="flex-1 bg-white/5 rounded-lg px-2.5 py-1.5 text-center">
                      <p className="text-white/30 text-[9px] mb-0.5">Hán Việt</p>
                      <p className="text-emerald-400 font-bold text-sm">{ex.hanviet}</p>
                    </div>
                    <i className="ri-arrow-right-line text-white/20 text-xs"></i>
                    <div className="flex-1 bg-white/5 rounded-lg px-2.5 py-1.5 text-center">
                      <p className="text-white/30 text-[9px] mb-0.5">Tiếng Việt</p>
                      <p className="text-white/70 font-bold text-sm">{ex.vietnamese}</p>
                    </div>
                  </div>
                  <p className="text-white/30 text-[10px] italic">{ex.note}</p>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400/30 placeholder-white/20"
          />
        </div>
        <div className="flex gap-1 bg-white/3 p-1 rounded-xl">
          {(["all", "identical", "similar", "different"] as const).map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer whitespace-nowrap transition-all ${filter === f ? "bg-white/10 text-white" : "text-white/30 hover:text-white/50"}`}
            >
      ...
      <div className="bg-[#0f1117] border border-white/5 rounded-2xl overflow-hidden">
        <div className="grid grid-cols-[1fr_1fr_1fr_1fr_auto] bg-white/3 px-5 py-3 text-xs font-semibold text-white/30 border-b border-white/5">
          <span>Tiếng Hàn</span>
          <span>Hán tự</span>
          <span>Hán Việt</span>
          ...
          {filtered.map((item, i) => {
            ...
            return (
              <div key={i} className="grid grid-cols-[1fr_1fr_1fr_1fr_auto] px-5 py-3 hover:bg-white/2 transition-colors items-center gap-3">
                ...
                <span className="text-emerald-400 font-semibold text-sm">{item.hanviet || "—"}</span>
