...
const MOOD_CONFIG = {
  ...
  okay: { icon: "ri-emotion-normal-line", label: "Bình thu?ng", color: "text-amber-500", bg: "bg-amber-50" },
  bad: { icon: "ri-emotion-unhappy-line", label: "Khó khan", color: "text-red-500", bg: "bg-red-50" },
};
...
export default function StudyDiaryTab() {
  ...
  const buildCSVContent = (entries: DiaryEntry[]) => {
    const header = "Ngày,T? dã h?c,Quiz dúng,T?ng câu,Ð? chính xác,SR dã ôn,Streak,C?m xúc,Ghi chú\n";
    const rows = entries.map(e => {
      const acc = e.quizTotal > 0 ? Math.round((e.quizScore / e.quizTotal) * 100) : 0;
      const moodLabel = { great: "Tuy?t v?i", good: "T?t", okay: "Bình thu?ng", bad: "Khó khan" }[e.mood ?? "good"];
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-lg font-bold text-gray-900">Nh?t ký h?c t?p</h2>
          <p className="text-sm text-gray-500">{diary.length} ngày dã ghi · Theo dõi ti?n d? m?i ngày</p>
        </div>
        <div className="flex gap-2">
          {diary.length > 0 && (
            ...
            >
              ...
              {isVipYear ? "Xu?t CSV" : isVipMonth ? "Xu?t CSV (50 ngày)" : "VIP Nam"}
            </button>
          )}
          <button
          ...
          >
            ...
            {todayEntry ? "C?p nh?t hôm nay" : "Ghi hôm nay"}
          </button>
        </div>
      ...
      <VipUpgradeModal
        ...
        featureName="Xu?t CSV nh?t ký h?c t?p"
      />
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-gray-900">Ghi nh?t ký — {formatDateShort(getToday())}</h3>
            ...
            {/* Mood */}
            <div className="mb-4">
              <p className="text-xs font-semibold text-gray-600 mb-2">C?m giác h?c hôm nay?</p>
              <div className="flex gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
              <div>
                <label className="text-xs font-semibold text-gray-600 block mb-1">T? dã h?c</label>
                <input
              ...
              <div>
                <label className="text-xs font-semibold text-gray-600 block mb-1">Quiz dúng</label>
                <input
              ...
              <div>
                <label className="text-xs font-semibold text-gray-600 block mb-1">T?ng câu</label>
                <input
            ...
            <div className="mb-4">
              <label className="text-xs font-semibold text-gray-600 block mb-1">Ghi chú (tùy ch?n)</label>
              <textarea
                value={form.note}
                onChange={e => setForm(f => ({ ...f, note: e.target.value }))}
                placeholder="Hôm nay h?c du?c gì? Khó khan g?p ph?i?..."
                rows={3}
                maxLength={300}
      ...
      {diary.length === 0 && (
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="font-medium text-gray-500 mb-1">Chua có nh?t ký nào</p>
          <p className="text-sm mb-6">Nh?n "Ghi hôm nay" d? b?t d?u theo dõi ti?n d? h?c t?p</p>
      ...
      {viewMode === "calendar" && diary.length > 0 && (
        <div>
          <div className="bg-white border border-gray-100 rounded-2xl p-5 mb-4">
            <p className="text-sm font-semibold text-gray-700 mb-4">30 ngày g?n nh?t</p>
      ...
      {viewMode === "stats" && stats && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: "T?ng ngày h?c", value: stats.totalDays, icon: "ri-calendar-check-line", color: "text-rose-600", bg: "bg-rose-50" },
              { label: "T?ng t? dã h?c", value: stats.totalWords, icon: "ri-book-open-line", color: "text-amber-600", bg: "bg-amber-50" },
              { label: "TB t?/ngày", value: stats.avgWords, icon: "ri-bar-chart-line", color: "text-indigo-600", bg: "bg-indigo-50" },
              { label: "Ð? chính xác quiz", value: `${stats.accuracy}%`, icon: "ri-gamepad-line", color: "text-green-600", bg: "bg-green-50" },
            ].map(s => (
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-5">
            <p className="text-sm font-semibold text-gray-700 mb-4">T? h?c du?c — 7 ngày g?n nh?t</p>
          ...
          {stats.bestDay && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-4">
              ...
              <div>
                <p className="font-semibold text-amber-800">Ngày h?c t?t nh?t</p>
          ...
          {/* Mood distribution */}
          <div className="bg-white border border-gray-100 rounded-2xl p-5">
            <p className="text-sm font-semibold text-gray-700 mb-4">Phân b? c?m xúc h?c t?p</p>
