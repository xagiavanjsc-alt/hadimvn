  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 gap-3">
      <div className="flex items-center gap-2 text-gray-400">
        ...
        <span className="text-sm">Ðang t?i...</span>
      </div>
      {slow && (
        <button
        ...
        >
          T?i quá lâu? Nh?n d? t?i l?i trang
        </button>
      )}
    </div>
  );
...
function exportCSV(
  ...
  const cols = ["Ti?ng Hàn", "Hán t?", "Nghia ti?ng Vi?t"];
  if (hasMastery) cols.push("M?c d?");
  if (hasNotes) cols.push("Ghi chú");
  const header = cols.join(",") + "\n";
  const masteryLabel: Record<string, string> = { new: "M?i", learning: "Ðang h?c", mastered: "Ðã thu?c" };
      ...
      <div className="w-full max-w-sm cursor-pointer" style={{ perspective: "1000px" }} onClick={() => setFlipped(f => !f)}>
        <div style={{ transition: "transform 0.5s", transformStyle: "preserve-3d", transform: flipped ? "rotateY(180deg)" : "rotateY(0deg)", position: "relative", height: "240px" }}>
          <div style={{ backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden" }}
            ...
            <p className="text-xs text-gray-400 tracking-normal mb-3">Ti?ng Hàn</p>
            ...
            <button onClick={handleSpeak}
              ...
              {speaking ? "Ðang phát..." : "Nghe phát âm"}
            </button>
          </div>
    ...
    <div className="max-w-lg mx-auto">
      <div className="flex flex-wrap gap-2 mb-5 items-center">
        <button onClick={() => setOnlyFavs(f => !f)}
          ...
          <i className="ri-heart-line"></i>Ch? t? yêu thích ({favs.size})
        </button>
      ...
      {pool.length === 0 ? (
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="mt-2 text-sm">Chua có t? yêu thích nào</p>
        </div>
      ) : (
        <>
  ...
  if (mode === "session" && !sessionDone && currentCard) {
    return (
      <div className="max-w-lg mx-auto">
        ...
        {revealed && (
          <div>
            <p className="text-center text-xs text-gray-400 mb-3">B?n nh? t? này nhu th? nào?</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {[
                { q: 0, label: "Quên", sub: "Không nh?", color: "border-red-300 bg-red-50 text-red-700 hover:bg-red-100" },
                { q: 2, label: "Khó", sub: "Nh? m?", color: "border-orange-300 bg-orange-50 text-orange-700 hover:bg-orange-100" },
                { q: 3, label: "Ðu?c", sub: "Nh? du?c", color: "border-yellow-300 bg-yellow-50 text-yellow-700 hover:bg-yellow-100" },
                { q: 5, label: "D?", sub: "Nh? rõ", color: "border-green-300 bg-green-50 text-green-700 hover:bg-green-100" },
              ].map(r => (
    ...
    return (
      <div className="max-w-lg mx-auto">
        <div className="bg-white border border-gray-100 rounded-2xl p-8 text-center">
          ...
          <h3 className="text-xl font-bold text-gray-900 mb-1">Hoàn thành phiên h?c!</h3>
          <p className="text-gray-500 mb-3">Ðúng {sessionResults.correct} · Sai {sessionResults.wrong} / {sessionCards.length} t?</p>
          <div className="flex items-center justify-center gap-4 mb-6">
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-orange-50 rounded-full">
              ...
              <span className="text-sm font-bold text-orange-600">{streak.currentStreak} ngày liên ti?p</span>
            </div>
          ...
          <div className="flex gap-3">
            <button onClick={startSession} className="flex-1 py-3 bg-rose-500 text-white rounded-xl font-semibold cursor-pointer hover:bg-rose-600 transition-colors">Ti?p t?c ôn</button>
            <button onClick={() => setMode("stats")} className="flex-1 py-3 border border-gray-200 text-gray-700 rounded-xl font-semibold cursor-pointer hover:bg-gray-50 transition-colors">Xem th?ng kê</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6 bg-gradient-to-r from-orange-50 to-amber-50 rounded-2xl p-4">
        ...
        <div className="flex-1">
          <p className="text-2xl font-bold text-orange-600">{streakData.currentStreak} ngày liên ti?p</p>
          <p className="text-xs text-gray-500">K? l?c: {streakData.longestStreak} ngày · Hôm nay: {streakData.history[getToday()] || 0} t?</p>
        </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-5 mb-5">
        <p className="text-sm font-semibold text-gray-700 mb-4">Ho?t d?ng 14 ngày qua</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {[
          { label: "C?n ôn hôm nay", value: stats.due, color: "text-rose-600", bg: "bg-rose-50" },
          { label: "T? m?i", value: stats.newCards, color: "text-amber-600", bg: "bg-amber-50" },
          { label: "Ðang h?c", value: stats.learning, color: "text-orange-600", bg: "bg-orange-50" },
          { label: "Ðã thu?c", value: stats.mastered, color: "text-green-600", bg: "bg-green-50" },
        ].map(s => (
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-4 mb-5">
        <div className="flex justify-between text-xs text-gray-500 mb-2">
          ...
          <span>{stats.mastered} / {stats.total} t? dã thu?c ({stats.total > 0 ? Math.round((stats.mastered / stats.total) * 100) : 0}%)</span>
        </div>
      ...
      <button onClick={startSession} disabled={dueCards.length === 0}
        ...
        {dueCards.length > 0 ? `B?t d?u ôn t?p (${Math.min(20, dueCards.length)} câu)` : "Không có t? c?n ôn hôm nay!"}
      </button>
      {dueCards.length === 0 && <p className="text-center text-sm text-green-600">Tuy?t v?i! Hãy quay l?i vào ngày mai.</p>}

      <div className="mt-5 bg-gray-50 rounded-xl p-4">
        <p className="text-xs font-semibold text-gray-600 mb-2">Cách ho?t d?ng (SM-2)</p>
        <div className="grid grid-cols-2 gap-2 text-xs text-gray-500">
          <div><span className="text-red-400 font-bold">Quên</span> ? Ôn l?i ngay hôm sau</div>
          <div><span className="text-orange-400 font-bold">Khó</span> ? Ôn l?i ngay hôm sau</div>
          <div><span className="text-yellow-500 font-bold">Ðu?c</span> ? Ôn sau vài ngày</div>
          <div><span className="text-green-500 font-bold">D?</span> ? Kho?ng cách tang d?n</div>
        </div>
      </div>
    </div>
  );
    ...
    <div>
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Tìm t? Hàn, Hán t?, nghia ti?ng Vi?t..." value={search} onChange={e => setSearch(e.target.value)}
            ...
            {showExportMenu && (
              <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg z-20 min-w-[180px] py-1">
                {([
                  ...
                  { key: "learning" as MasteryFilter, label: "Ch? Ðang h?c", icon: "ri-book-open-line" },
                  { key: "mastered" as MasteryFilter, label: "Ch? Ðã thu?c", icon: "ri-check-double-line" },
                ]).map(opt => (
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {([
          ...
          { key: "learning", label: "Ðang h?c", count: masteryStats.learning, color: "bg-amber-500 text-white", inactive: "bg-amber-50 text-amber-600 hover:bg-amber-100", icon: "ri-book-open-line" },
          { key: "mastered", label: "Ðã thu?c", count: masteryStats.mastered, color: "bg-green-500 text-white", inactive: "bg-green-50 text-green-600 hover:bg-green-100", icon: "ri-check-double-line" },
      ...
      <p className="text-xs text-gray-400 mb-4">Hi?n th? {filtered.length} / {HANJA_DATA.length} t? · {favs.size} t? yêu thích</p>
                ...
                <div className="flex items-center gap-1 flex-wrap">
                  ...
                  {mastery !== "mastered" ? (
                    <button onClick={() => markAsLearned(item.korean)}
                      ...
                      <i className="ri-check-line text-xs"></i>Ðã thu?c
                    </button>
                  ) : (
      ...
      {viewMode === "list" && (
        <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
          <div className="grid grid-cols-[1fr_1fr_1fr_120px_80px_32px] bg-gray-50 px-4 py-2 text-xs font-semibold text-gray-500 border-b border-gray-100">
            <span>Ti?ng Hàn</span><span>Hán t?</span><span>Nghia ti?ng Vi?t</span><span>M?c d?</span><span>Ðánh d?u</span><span></span>
          </div>
                ...
                <div key={i} className="grid grid-cols-[1fr_1fr_1fr_120px_80px_32px] px-4 py-3 hover:bg-rose-50/30 transition-colors items-center">
                  ...
                  {mastery !== "mastered" ? (
                    <button onClick={() => markAsLearned(item.korean)}
                      ...
                      <i className="ri-check-line"></i>Ðã thu?c
                    </button>
                  ) : (
                    <button onClick={() => resetToNew(item.korean)}
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="mt-2 text-sm">Không tìm th?y t? nào</p>
        </div>
      )}
      ...
      <VipUpgradeModal
        ...
        featureName="Xu?t CSV t? v?ng Hán-Hàn"
      />
    </div>
  );
}
  ...
  if (favList.length === 0) {
    return (
      <div className="text-center py-16 text-gray-400">
        ...
        <p className="font-medium text-gray-500 mb-1">Chua có t? yêu thích</p>
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <p className="text-sm text-gray-500">{favList.length} t? dã luu · {Object.keys(notes).length} ghi chú</p>
        <div className="flex gap-2 flex-wrap">
          <button onClick={handleExportCSV}
            ...
            <i className={getExportBtnIcon(isLoggedIn, isVip, isVipYear)}></i>
            {getExportBtnLabel(isLoggedIn, isVip, isVipYear, "Xu?t CSV yêu thích")}
          </button>
          <button onClick={handleExportAll}
            className={`flex items-center gap-2 px-4 py-2 border rounded-lg text-sm font-medium cursor-pointer transition-colors whitespace-nowrap ${
          ...
          <button onClick={onStartFlashcard}
            ...
            <i className="ri-play-circle-line"></i>Ôn t?p Flashcard
          </button>
        </div>
      </div>
      ...
      <VipUpgradeModal
        ...
        onClose={closeModal}
        reason={modalReason ?? "not_vip_year"}
        featureName="Xu?t CSV t? yêu thích"
      />
...
function QuizTab({ favs }: { favs: Set<string> }) {
  ...
  const getQuestionSub = () => {
    ...
    if (mode === "listen") return "Nghe và ch?n t? Hàn dúng";
      ...
      <div className="max-w-lg mx-auto">
        <div className="bg-white border border-gray-100 rounded-2xl p-8">
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-gray-500">Ch?n ch? d? và b?t d?u luy?n t?p</p>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-5">
            {([
              { key: "ko2vi", label: "Hàn ? Vi?t", icon: "ri-arrow-right-line" },
              { key: "vi2ko", label: "Vi?t ? Hàn", icon: "ri-arrow-left-line" },
          ...
          <div className="mb-5">
            <p className="text-xs text-gray-500 mb-2">L?c theo ch? cái d?u</p>
          ...
          >
            B?t d?u Quiz ({Math.min(20, filteredPool.length)} câu)
          </button>
        </div>
      </div>
    );
    ...
    return (
      <div className="max-w-lg mx-auto">
        <div className="bg-white border border-gray-100 rounded-2xl p-8 text-center">
          ...
          <p className="text-gray-500 mb-2">Ðúng {score} / {pool.length} câu</p>
          <p className="text-sm text-gray-400 mb-8">
            {pct >= 80 ? "Xu?t s?c! B?n n?m r?t t?t Hán-Hàn!" : pct >= 50 ? "Khá t?t! Ti?p t?c luy?n t?p nhé!" : "C?n ôn thêm! Ð?ng n?n lòng!"}
          </p>
          <div className="flex gap-3">
            <button onClick={startQuiz} className="flex-1 py-3 bg-rose-500 text-white rounded-xl font-semibold cursor-pointer hover:bg-rose-600 transition-colors">Làm l?i</button>
            <button onClick={() => setStarted(false)} className="flex-1 py-3 border border-gray-200 text-gray-700 rounded-xl font-semibold cursor-pointer hover:bg-gray-50 transition-colors">Ð?i ch? d?</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-lg mx-auto">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-sm font-semibold text-rose-600">Ðúng: {score}</span>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-8 text-center mb-4">
        <p className="text-xs text-gray-400 mb-2 tracking-wide">
          {mode === "ko2vi" ? "T? ti?ng Hàn này có nghia là gì?" : mode === "vi2ko" ? "T? ti?ng Vi?t này là t? Hàn nào?" : mode === "listen" ? "Nghe và ch?n t? Hàn dúng" : "Hán t? này d?c là gì?"}
        </p>
        {mode === "listen" ? (
      ...
      {question.answered && (
        <button onClick={nextQuestion} className="w-full py-3 bg-rose-500 text-white rounded-xl font-semibold cursor-pointer hover:bg-rose-600 transition-colors">
          {current + 1 >= pool.length ? "Xem k?t qu?" : "Câu ti?p theo ?"}
        </button>
      )}
    </div>
  );
      ...
      <div className="lg:w-80 flex-shrink-0">
        <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
          <div className="p-4 border-b border-gray-100">
            <p className="text-sm font-semibold text-gray-700 mb-3">Ch?n Hán t? d? xem t? d?ng g?c</p>
            <div className="relative">
              <i className="ri-search-line absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm"></i>
      ...
      <div className="flex-1">
        {!selectedChar ? (
          <div className="bg-white border border-gray-100 rounded-2xl p-12 text-center">
            ...
            <p className="text-gray-500 font-medium mb-1">Ch?n m?t Hán t?</p>
            <p className="text-sm text-gray-400">Xem t?t c? t? ti?ng Hàn có ch?a ch? Hán dó</p>
          ...
          <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
            <div className="p-5 border-b border-gray-100 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">{relatedWords.length} t? ch?a ch? này</p>
              </div>
...
export default function HanjaVocabPage() {
  ...
  const tabs: { key: TabType; label: string; icon: string; badge?: number; isNew?: boolean }[] = [
    ...
    { key: "synonym", label: "Ð?ng nghia/âm", icon: "ri-git-merge-line" },
    { key: "antonym", label: "Ð?i nghia", icon: "ri-arrow-left-right-line" },
    { key: "hanviet", label: "So sánh Hán Vi?t", icon: "ri-translate-2", isNew: true },
    { key: "examples", label: "Câu ví d?", icon: "ri-newspaper-line", isNew: true },
    ...
    { key: "roots", label: "Ð?ng g?c", icon: "ri-links-line" },
    { key: "topics", label: "Ch? d?", icon: "ri-apps-line" },
    { key: "ranking", label: "X?p h?ng", icon: "ri-medal-line" },
    { key: "stats", label: "Th?ng kê", icon: "ri-bar-chart-line" },
    ...
    { key: "weekly", label: "Thách th?c tu?n", icon: "ri-sword-line" },
    { key: "leaderboard", label: "B?ng x?p h?ng", icon: "ri-bar-chart-horizontal-line" },
    { key: "homophone", label: "Ð?ng âm khác nghia", icon: "ri-sound-module-line" },
    { key: "topik-exam", label: "Thi th? TOPIK", icon: "ri-file-paper-2-line" },
    { key: "pronunciation", label: "Luy?n phát âm", icon: "ri-mic-line" },
    { key: "smart-review", label: "Ôn t?p thông minh", icon: "ri-brain-line", isNew: true },
    { key: "advanced-topics", label: "Ch? d? nâng cao", icon: "ri-graduation-cap-line", isNew: true },
    { key: "diary", label: "Nh?t ký h?c t?p", icon: "ri-book-2-line", isNew: true },
    { key: "word-match", label: "Ghép c?p", icon: "ri-drag-drop-line", isNew: true },
  ];
      ...
      <div className="p-4 md:p-6 max-w-6xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">T? v?ng Hán-Hàn</h1>
              <p className="text-sm text-gray-500">{HANJA_DATA.length} t? · {favs.size} yêu thích</p>
            </div>
          </div>
        </div>
