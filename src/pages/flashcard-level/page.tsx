...
const RATING_LABELS = [
  { value: 1, label: "Không nh?", color: "#f87171", icon: "ri-close-circle-line" },
    ...
    <div className="flex flex-col items-center">
      ...
      >
        ...
        >
          {/* Front */}
          <div
            className="absolute inset-0 bg-app-bg border border-app-border rounded-3xl p-8 flex flex-col items-center justify-center"
          ...
          >
            ...
            {card.hanja && card.hanja !== card.korean && (
              <p className="text-app-text-muted text-sm mb-3">{card.hanja}</p>
            )}
          ...
          >
            ...
            <p className="text-app-text-secondary text-sm mb-4">[{card.pronunciation}]</p>
            {card.examples?.[0] && (
              <div className="w-full bg-app-surface/50 rounded-2xl p-4 mb-4">
          ...
          <div className="flex gap-2">
            {RATING_LABELS.map(r => (
              <button
                ...
                className="flex-1 flex flex-col items-center gap-1 py-3 rounded-xl border border-app-border hover:border-white/20 cursor-pointer transition-all hover:scale-105"
...
export default function FlashcardLevelPage() {
  ...
  return (
    <DashboardLayout
      ...
      subtitle="L?t th? t? v?ng A1?C1 v?i thu?t toán SRS (Spaced Repetition)"
    >
      {!sessionStarted ? (
        <>
          {/* Stats */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
            {[
              ...
              { label: "C?n ôn hôm nay", value: dueCards.length, icon: "ri-calendar-check-line", color: "#fb923c" },
              { label: "Ðã ôn t?p", value: totalReviewed, icon: "ri-refresh-line", color: "#34d399" },
              { label: "Ðã thu?c (3+ l?n)", value: masteredCount, icon: "ri-check-double-line", color: "#a78bfa" },
            ].map(s => (
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5 mb-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {([["all", "T?t c?", "app-accent-primary"], [1, "A1 · Co b?n", "#34d399"], [2, "A2-B1 · Trung c?p", "app-accent-primary"], [3, "B2-C1 · Nâng cao", "#f87171"]] as const).map(([val, label, color]) => (
                    <button
                      key={String(val)}
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer transition-all whitespace-nowrap border ${
                        selectedDifficulty === val ? "border-white/20 text-white" : "border-app-border text-app-text-secondary hover:text-white/60"
                      }`}
            ...
            <div className="mt-4 pt-4 border-t border-app-border flex items-center justify-between">
              <p className="text-app-text-secondary text-sm">
                <span className="text-white font-bold">{filteredCards.length}</span> t? · <span className="text-[#fb923c] font-bold">{dueCards.length}</span> c?n ôn hôm nay
              </p>
              <div className="flex gap-2">
                <button
              ...
              return (
                <div key={diff} className={`bg-app-bg border ${cfg.border} rounded-2xl p-5`}>
                  ...
                  <div className="space-y-1.5 text-xs">
                    ...
                    <div className="flex justify-between">
                      <span className="text-app-text-secondary">C?n ôn hôm nay</span>
                  ...
                  <div className="mt-3 h-1.5 bg-app-card/50 rounded-full overflow-hidden">
        ...
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <h2 className="text-white font-bold text-2xl mb-2">Hoàn thành phiên h?c!</h2>
          <p className="text-app-text-secondary text-sm mb-8">B?n dã ôn t?p {sessionStats.total} t? v?ng</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
            {[
              { label: "Nh? du?c", value: sessionStats.correct, color: "#34d399", icon: "ri-check-line" },
              { label: "C?n ôn thêm", value: sessionStats.hard, color: "#f87171", icon: "ri-close-line" },
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-app-text-secondary text-sm whitespace-nowrap">{currentIndex + 1} / {sessionCards.length}</span>
          </div>
          ...
          {/* SRS info */}
          <div className="mt-6 flex items-center justify-center gap-4 text-app-text-muted text-xs">
            {RATING_LABELS.map(r => (
