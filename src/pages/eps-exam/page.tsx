...
function ShareResultCard({ pct, correct, total, examResults }: { pct: number; correct: number; total: number; examResults: ExamResult[] }) {
  ...
  const grade = pct >= 80 ? "Ð?U ?" : pct >= 60 ? "G?n d?u ??" : "C?n ôn thêm ??";
  const emoji = pct >= 80 ? "??" : pct >= 60 ? "??" : "??";
  const shareText = `${emoji} K?t qu? thi th? EPS-TOPIK trên Hàn Qu?c Oi!\n\n?? Ði?m: ${correct}/${total} câu dúng (${pct}%)\n?? Th?i gian: ${Math.floor(timeUsed / 60)} phút ${timeUsed % 60} giây\n??? Ðánh giá: ${grade}\n\n${pct >= 80 ? "Vu?t ngu?ng di?m d?u EPS-TOPIK! S?n sàng thi th?t r?i!" : "Ðang ôn luy?n m?i ngày — c? lên nhé!"}\n\n#HànQu?cOi #EPSTOPIK #H?cTi?ngHàn #TàuLáChu?i`;
    ...
    <div className="rounded-2xl overflow-hidden border border-app-border">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-white/60 text-xs font-semibold">Hàn Qu?c Oi!</span>
            </div>
            <span className="text-app-text-muted text-xs">EPS-TOPIK Thi th?</span>
          </div>

          <div className="flex items-center gap-6">
            <div>
              <p className="text-5xl font-extrabold" style={{ color: gradeColor }}>{pct}%</p>
              <p className="text-white/50 text-xs mt-1">{correct}/{total} câu dúng</p>
            </div>
            <div className="flex-1">
              ...
              <div className="flex items-center justify-between text-[10px] text-app-text-muted">
                <span>0%</span>
                <span style={{ color: gradeColor }}>Ngu?ng d?u 80%</span>
                <span>100%</span>
              </div>
              <p className="text-sm font-bold mt-2" style={{ color: gradeColor }}>{grade}</p>
              <p className="text-app-text-secondary text-xs">?? {Math.floor(timeUsed / 60)} phút {timeUsed % 60} giây</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-app-bg p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          >
            {showCard ? "?n n?i dung" : "Xem n?i dung chia s?"}
          </button>
        </div>
...
export default function EpsExamPage() {
  ...
  const startExam = useCallback(() => {
    // Anti-cheat: cooldown d? ch?ng spam
    ...
    if (inCooldown) {
      alert(`Vui lòng ch? ${remainingSec}s tru?c khi làm bài m?i.`);
      return;
    }
  ...
  const submitExam = useCallback(() => {
    ...
    // Anti-cheat: n?u submit quá nhanh ? k?t qu? v?n luu nhung KHÔNG c?ng XP,
    // không sync leaderboard. Ngu?i dùng th?y k?t qu? nhung không ?nh hu?ng rank.
    ...
    if (tooFast) {
      // V?n hi?n th? k?t qu?, nhung c?nh báo và không thu?ng XP
      setMode("result");
      return;
    }

    // Ghi timestamp cooldown — ngan spam submit liên ti?p
  ...
  const topicLabels: Record<string, string> = {
    greeting: "Giao ti?p", safety: "An toàn LÐ", culture: "Van hóa",
    workplace: "Noi làm vi?c", daily: "Sinh ho?t", emergency: "Kh?n c?p",
    listening: "Nghe hi?u", reading: "Ð?c hi?u", law: "Pháp lu?t",
  };

  // -- INTRO ------------------------------------------------------------------
  if (mode === "intro") {
    return (
      <DashboardLayout title="Thi th? EPS-TOPIK" subtitle="Mô ph?ng d? thi th?t — 40 câu · 50 phút">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-8 mb-5">
            ...
            <h2 className="text-white font-bold text-xl text-center mb-2">Ð? thi th? EPS-TOPIK</h2>
            <p className="text-app-text-secondary text-sm text-center mb-6">Câu h?i du?c ch?n ng?u nhiên t? ngân hàng d?. Làm bài nghiêm túc nhu thi th?t!</p>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              {[
                { icon: "ri-survey-line", color: "app-accent-primary", label: "S? câu h?i", value: `${Math.min(EXAM_QUESTION_COUNT, epsQuestions.length)} câu` },
                { icon: "ri-timer-line", color: "#34d399", label: "Th?i gian", value: "50 phút" },
                { icon: "ri-trophy-line", color: "#a78bfa", label: "Ði?m d?u", value: "= 80/200" },
              ].map(item => (
            ...
            <div className="bg-app-accent-primary/5 border border-app-accent-primary/15 rounded-xl p-4 mb-6">
              <p className="text-app-accent-primary/80 text-xs font-semibold mb-2">Luu ý tru?c khi thi</p>
              <ul className="space-y-1.5">
                {[
                  "Câu h?i du?c ch?n ng?u nhiên t? t?t c? ch? d?",
                  "Có th? dánh d?u câu d? xem l?i sau",
                  "H?t gi? s? t? d?ng n?p bài",
                  "K?t qu? du?c luu vào h? so h?c viên",
                ].map(note => (
          ...
          {examResults.length > 0 && (
            <div className="bg-app-bg border border-app-border rounded-2xl p-5">
              <h3 className="text-white font-semibold text-sm mb-4">L?ch s? thi ({examResults.length} l?n)</h3>
    ...
    return (
      ...
      >
        ...
        <div className="hidden md:flex items-center gap-3 px-8 py-3 bg-app-bg border-b border-app-border">
          ...
          <span className="text-app-text-secondary text-sm">{answeredCount}/{examQuestions.length} dã tr? l?i</span>
          <button
            onClick={() => setShowReview(!showReview)}
        ...
        <div className="p-4 md:p-6">
          <div className="grid grid-cols-1 md:grid-cols-[1fr_260px] gap-4 md:gap-5 max-w-5xl mx-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-app-bg border border-app-border rounded-2xl p-4 md:p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">{flagged.has(currentQ.id) ? "Ðã dánh d?u" : "Ðánh d?u"}</span>
                  </button>
                  {currentQ.audioText && (
                    <button
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  <span className="hidden sm:inline">Câu tru?c</span>
                </button>
                <div className="flex-1 text-center text-app-text-muted text-xs">
                  {answers[currentQ.id] !== undefined ? (
                    <span className="text-app-accent-primary/60">Ðã tr? l?i</span>
                  ) : (
                    <span>Chua tr? l?i</span>
                  )}
                </div>
                <button
                ...
                >
                  <span className="hidden sm:inline">Câu ti?p</span>
              ...
              <div className="md:hidden">
                {showReview && (
                  <div className="bg-app-bg border border-app-border rounded-2xl p-4">
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-white/50 text-xs font-medium">Danh sách câu h?i</p>
                    ...
                    <div className="flex items-center gap-4 pt-2 border-t border-app-border">
                      <div className="flex items-center gap-1.5 text-[10px] text-app-text-muted">
                        <div className="w-3 h-3 rounded bg-emerald-500/20"></div>{answeredCount} dã tr? l?i
                      </div>
                      <div className="flex items-center gap-1.5 text-[10px] text-app-text-muted">
                        <div className="w-3 h-3 rounded bg-app-card/50"></div>{examQuestions.length - answeredCount} chua tr? l?i
                      </div>
                      {flagged.size > 0 && (
                        <div className="flex items-center gap-1.5 text-[10px] text-app-text-muted">
                          <div className="w-3 h-3 rounded bg-[#fb923c]/20"></div>{flagged.size} dánh d?u
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="hidden md:block space-y-4">
              <div className="bg-app-bg border border-app-border rounded-2xl p-4">
                <p className="text-app-text-secondary text-xs font-medium mb-3">Danh sách câu h?i</p>
                ...
                <div className="mt-3 pt-3 border-t border-app-border space-y-1.5">
                  <div className="flex items-center gap-2 text-[10px] text-app-text-muted">
                    <div className="w-3 h-3 rounded bg-emerald-500/20"></div>Ðã tr? l?i ({answeredCount})
                  </div>
                  <div className="flex items-center gap-2 text-[10px] text-app-text-muted">
                    <div className="w-3 h-3 rounded bg-app-card/50"></div>Chua tr? l?i ({examQuestions.length - answeredCount})
                  </div>
                  <div className="flex items-center gap-2 text-[10px] text-app-text-muted">
                    <div className="w-3 h-3 rounded bg-[#fb923c]/30 relative"><span className="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 rounded-full bg-[#fb923c]"></span></div>Ðánh d?u ({flagged.size})
                  </div>
                </div>
              </div>
              <div className="bg-app-bg border border-app-border rounded-2xl p-4 text-center">
                <p className="text-app-text-muted text-xs mb-1">Th?i gian còn l?i</p>
  ...
  if (mode === "result" && resultStats) {
    ...
    const grade = pct >= 80 ? { label: "Ð?u!", color: "#34d399", icon: "ri-trophy-line", desc: "Xu?t s?c! B?n dã vu?t ngu?ng di?m d?u EPS-TOPIK." }
      : pct >= 60 ? { label: "G?n d?u", color: "app-accent-primary", icon: "ri-medal-line", desc: "Khá t?t! C?n ôn thêm m?t chút d? d?t di?m d?u." }
      : { label: "C?n ôn thêm", color: "#f87171", icon: "ri-refresh-line", desc: "Hãy ôn luy?n thêm các ch? d? y?u và th? l?i." };
    ...
    return (
      <DashboardLayout title="K?t qu? thi th? EPS-TOPIK" subtitle="Phân tích chi ti?t k?t qu? bài thi">
        <div className="max-w-3xl mx-auto space-y-5">
          ...
          {flaggedTooFast && (
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-amber-300 text-sm font-semibold mb-1">Bài thi không h?p l?</p>
                <p className="text-amber-200/70 text-xs leading-relaxed">
                  Th?i gian làm bài quá ng?n (du?i {MIN_EPS_EXAM_TIME_SEC}s cho {examQuestions.length} câu).
                  K?t qu? v?n du?c luu d? b?n xem l?i, nhung <strong>không du?c c?ng XP</strong> và
                  <strong> không dua lên b?ng x?p h?ng</strong>. Hãy làm bài nghiêm túc d? có k?t qu? chính xác.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-8 text-center">
            ...
            <div className="flex items-center justify-center gap-8 mb-5">
              <div>
                ...
                <p className="text-app-text-muted text-xs mt-1">T? l? dúng</p>
              </div>
              ...
              <div>
                ...
                <p className="text-app-text-muted text-xs mt-1">Câu dúng</p>
              </div>
              <div className="w-px h-12 bg-app-card/70"></div>
              <div>
                <p className="text-white font-bold text-2xl">{Math.floor(timeUsed / 60)}:{String(timeUsed % 60).padStart(2, "0")}</p>
                <p className="text-app-text-muted text-xs mt-1">Th?i gian làm</p>
              </div>
            </div>
          ...
          {/* By topic */}
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <h3 className="text-white font-semibold text-sm mb-4">K?t qu? theo ch? d?</h3>
              ...
              {Object.entries(byTopic).map(([topicId, data]) => {
                ...
                return (
                  <div key={topicId} className="bg-app-surface/50 rounded-xl p-3">
                    ...
                    <p className="text-app-text-muted text-[10px]">{data.correct}/{data.total} câu dúng</p>
                  </div>
                );
              })}
            </div>
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <h3 className="text-white font-semibold text-sm mb-4">
              Câu tr? l?i sai ({examQuestions.filter(q => answers[q.id] !== undefined && answers[q.id] !== q.correctIndex).length} câu)
            </h3>
              ...
              {examQuestions.filter(q => answers[q.id] !== undefined && answers[q.id] !== q.correctIndex).map((q, i) => (
                <div key={q.id} className="bg-red-500/5 border border-red-500/15 rounded-xl p-4">
                  ...
                  <div className="flex items-center gap-2 text-[10px] mt-2">
                    ...
                    <span className="text-app-accent-success/70">Ðáp án: {q.optionsVi[q.correctIndex]}</span>
                  </div>
                </div>
              ))}
          ...
          {user && (
            <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border text-xs ${syncing ? "bg-app-accent-primary/5 border-app-accent-primary/15 text-app-accent-primary/70" : "bg-emerald-500/5 border-emerald-500/15 text-app-accent-success/70"}`}>
              ...
              {syncing ? "Ðang d?ng b? k?t qu? lên cloud..." : "K?t qu? dã du?c luu lên cloud và c?p nh?t b?ng x?p h?ng!"}
            </div>
          )}
