...
function generateMockHistory(): QuizAttempt[] {
  const modes = [
    ...
    { mode: "daily_challenge", label: "Th? thách hàng ngày" },
  ];

  const sampleQuestions: QuizQuestionRecord[] = [
    { question: "\"????\" có nghia là gì?", userAnswer: "Vui v?", correctAnswer: "H?nh phúc", isCorrect: false, explanation: "???? = H?nh phúc. Vui v? là ???.", korean: "????", category: "Tính cách & C?m xúc" },
    { question: "\"????\" có nghia là gì?", userAnswer: "Ði làm", correctAnswer: "Ði làm", isCorrect: true, explanation: "???? = Ði làm (d?n noi làm vi?c).", korean: "????", category: "Gia dình & Sinh ho?t" },
    { question: "\"???\" là gì?", userAnswer: "Máy gi?t", correctAnswer: "T? l?nh", isCorrect: false, explanation: "??? = T? l?nh. Máy gi?t là ???.", korean: "???", category: "Gia dình & Sinh ho?t" },
    { question: "\"????\" có nghia là gì?", userAnswer: "Thân thi?n/T? t?", correctAnswer: "Thân thi?n/T? t?", isCorrect: true, explanation: "???? = Thân thi?n, t? t?.", korean: "????", category: "Tính cách & C?m xúc" },
    { question: "\"????\" là gì?", userAnswer: "Ðèn giao thông", correctAnswer: "V?ch k? du?ng cho ngu?i di b?", isCorrect: false, explanation: "???? = V?ch k? du?ng. Ðèn giao thông là ???.", korean: "????", category: "Giao thông" },
    { question: "\"????\" có nghia là gì?", userAnswer: "Cham ch?/C?n cù", correctAnswer: "Cham ch?/C?n cù", isCorrect: true, explanation: "???? = Cham ch?, c?n cù, siêng nang.", korean: "????", category: "Tính cách & C?m xúc" },
    { question: "\"????\" có nghia là gì?", userAnswer: "Xu?ng xe", correctAnswer: "Chuy?n tuy?n/Ð?i xe", isCorrect: false, explanation: "???? = Chuy?n tuy?n. Xu?ng xe là ???.", korean: "????", category: "Giao thông" },
    { question: "\"???\" là gì?", userAnswer: "Chênh l?ch nhi?t d? trong ngày", correctAnswer: "Chênh l?ch nhi?t d? trong ngày", isCorrect: true, explanation: "??? = S? chênh l?ch nhi?t d? gi?a ngày và dêm.", korean: "???", category: "Th?i ti?t" },
    { question: "\"????\" có nghia là gì?", userAnswer: "V?i vàng", correctAnswer: "C?n th?n/T? m?", isCorrect: false, explanation: "???? = C?n th?n, t? m?. V?i vàng là ???.", korean: "????", category: "Tính cách & C?m xúc" },
    { question: "\"?????\" có nghia là gì?", userAnswer: "R?a bát", correctAnswer: "R?a bát", isCorrect: true, explanation: "????? = R?a bát dia sau b?a an.", korean: "?????", category: "Gia dình & Sinh ho?t" },
  ];
...
function AttemptCard({ attempt, isSelected, onClick }: {
  ...
  const label = pct >= 80 ? "Xu?t s?c" : pct >= 60 ? "T?t" : pct >= 40 ? "Trung bình" : "C?n c? g?ng";
    ...
    <div className={`rounded-2xl border overflow-hidden transition-all ${q.isCorrect ? "border-emerald-500/15" : "border-red-500/15"}`}>
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-app-border pt-3 space-y-2">
          ...
          {!q.isCorrect && (
            <div className="flex items-start gap-2">
              <span className="text-app-text-muted text-xs w-20 flex-shrink-0">Ðáp án dúng:</span>
              <span className="text-app-accent-success text-xs font-medium">{q.correctAnswer}</span>
            </div>
          )}
          <div className="flex items-start gap-2">
            <span className="text-app-text-muted text-xs w-20 flex-shrink-0">Gi?i thích:</span>
...
export default function QuizHistoryDetailPage() {
  ...
  const modeLabels: Record<string, string> = { all: "T?t c?", eps_vocab: "EPS", topik1: "TOPIK I", mixed: "T?ng h?p", daily_challenge: "Th? thách" };
  ...
  return (
    <DashboardLayout
      ...
      subtitle="Xem l?i t?ng câu dã làm, dáp án dúng/sai và gi?i thích"
    >
      {/* Stats */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          { label: "T?ng l?n làm", value: totalAttempts, icon: "ri-history-line", color: "app-accent-primary" },
          { label: "Ði?m trung bình", value: `${avgScore}%`, icon: "ri-bar-chart-line", color: "#34d399" },
          { label: "Ði?m cao nh?t", value: `${bestScore}%`, icon: "ri-trophy-line", color: "#fb923c" },
          { label: "T?ng câu sai", value: totalWrong, icon: "ri-close-circle-line", color: "#f87171" },
        ].map(s => (
          ...
          {selectedAttempt ? (
            <>
              ...
              <div className="flex items-center justify-between mb-3">
                <p className="text-app-text-secondary text-xs">{displayedQuestions.length} câu h?i</p>
                <button
                ...
                >
                  ...
                  {showOnlyWrong ? "Ðang l?c câu sai" : "Ch? xem câu sai"}
            ...
            <div className="flex items-center justify-center h-64">
              <p className="text-app-text-muted text-sm">Ch?n m?t l?n làm quiz d? xem chi ti?t</p>
            </div>
          )}
        </div>
      </div>
      ...
      {topWrong.length > 0 && (
        <div className="mt-6 bg-app-bg border border-app-border rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-app-text-muted text-xs">— C?n ôn t?p uu tiên</span>
          </div>
