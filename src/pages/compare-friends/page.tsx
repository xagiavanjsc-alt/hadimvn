...
// Friend data gi? du?c fetch th?t t? Supabase (không còn mock)

const COMPARE_METRICS = [
  { key: "xp", label: "T?ng XP", icon: "ri-star-line", color: "app-accent-primary", format: (v: number) => v.toLocaleString() },
  { key: "streak", label: "Streak hi?n t?i", icon: "ri-fire-line", color: "#fb923c", format: (v: number) => `${v} ngày` },
  { key: "epsAccuracy", label: "Ð? chính xác EPS", icon: "ri-percent-line", color: "#34d399", format: (v: number) => `${v}%` },
  { key: "epsDone", label: "Câu EPS dã làm", icon: "ri-file-list-3-line", color: "#06b6d4", format: (v: number) => `${v} câu` },
  { key: "flashcardKnown", label: "T? v?ng dã thu?c", icon: "ri-translate-2", color: "#a78bfa", format: (v: number) => `${v} t?` },
  ...
  return (
    <div className="bg-app-bg border border-app-border rounded-xl p-4">
      ...
      <div className="space-y-2">
        ...
        {/* Friend bar */}
        <div className="flex items-center gap-3">
          <span className="text-[10px] text-white/50 w-12 text-right whitespace-nowrap">B?n bè</span>
          <div className="flex-1 h-2 bg-app-card/50 rounded-full overflow-hidden">
...
export default function CompareFriendsPage() {
  ...
  const handleLoadFriend = useCallback(async () => {
    ...
    setLoadError("");
    try {
      // H? tr? c? /member/:id (legacy) và /public-profile/:id
      const match = linkInput.match(/\/(?:member|public-profile)\/([a-zA-Z0-9-]+)/);
      if (!match) {
        setLoadError("Link không h?p l?. Dùng link d?ng /public-profile/... ho?c /member/...");
        setLoading(false);
        return;
      }
      ...
      if (!profile) {
        setLoadError("Không tìm th?y h? so ngu?i dùng này.");
        setLoading(false);
        return;
      }
      ...
      // Badges: cùng công th?c v?i public-profile d? nh?t quán
      const badges =
      ...
      const friend: FriendData = {
        id: userId,
        name: profile.display_name || "H?c viên",
  ...
  return (
    <DashboardLayout
      title="So sánh ti?n d? b?n bè"
      subtitle="Nh?p link h? so công khai d? so sánh c?p b?c, streak và di?m EPS"
    >
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <h3 className="text-white font-semibold text-sm mb-3">Nh?p link h? so b?n bè</h3>
            ...
            <p className="text-app-text-muted text-xs mt-2">
              ...
              L?y link h? so công khai t? trang Profile ? nút "Chia s? h? so"
            </p>
          </div>
          ...
          {selectedFriend ? (
              ...
              <div className="bg-app-bg border border-app-border rounded-2xl p-5">
                ...
                <div className={`mt-4 p-3 rounded-xl text-center ${winsCount > COMPARE_METRICS.length / 2 ? "bg-emerald-500/10 border border-emerald-500/20" : winsCount === COMPARE_METRICS.length / 2 ? "bg-app-accent-primary/10 border border-app-accent-primary/20" : "bg-red-500/10 border border-red-500/20"}`}>
                  <p className="font-bold text-sm" style={{ color: winsCount > COMPARE_METRICS.length / 2 ? "#34d399" : winsCount === COMPARE_METRICS.length / 2 ? "app-accent-primary" : "#f87171" }}>
                    {winsCount > COMPARE_METRICS.length / 2 ? "?? B?n dang d?n tru?c!" : winsCount === COMPARE_METRICS.length / 2 ? "?? Ngang nhau!" : "?? C?n c? g?ng thêm!"}
                  </p>
                  <p className="text-app-text-muted text-xs mt-0.5">
                    {winsCount > COMPARE_METRICS.length / 2 ? "Ti?p t?c duy trì phong d? t?t!" : "Hãy h?c thêm d? vu?t qua b?n bè!"}
                  </p>
                </div>
              </div>
            ...
            <div className="bg-app-bg border border-app-border rounded-2xl p-12 text-center">
              ...
              <p className="text-app-text-secondary text-base font-medium mb-2">Chua có b?n bè d? so sánh</p>
              <p className="text-app-text-muted text-sm">Nh?p link h? so công khai c?a b?n bè ? trên d? b?t d?u so sánh</p>
        ...
        <div className="space-y-4">
          ...
          {savedFriends.length > 0 && (
            <div className="bg-app-bg border border-app-border rounded-2xl p-5">
              <h3 className="text-white font-semibold text-sm mb-3">B?n bè dã so sánh</h3>
              <div className="space-y-2">
                {savedFriends.map(f => (
                  <button
                    key={f.id}
          ...
          <div className="bg-gradient-to-br from-app-surface to-[#0f1117] border border-app-accent-primary/15 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-white font-semibold text-sm">Cách chia s? h? so</h3>
            </div>
            <div className="space-y-2 text-app-text-secondary text-xs leading-relaxed">
              <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Vào trang <strong className="text-white/60">H? so</strong></p>
              <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Nh?n nút <strong className="text-white/60">"Chia s? h? so"</strong></p>
              <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>G?i link cho b?n bè</p>
              <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>B?n bè dán link vào dây d? so sánh</p>
            </div>
          </div>
        </div>
      </div>
