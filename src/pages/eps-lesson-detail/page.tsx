...
function VocabTab({ lesson }: { lesson: EpsLesson }) {
  ...
  return (
    <div className="space-y-4">
      <p className="text-app-text-secondary text-xs">{lesson.vocabulary.length} t? v?ng — Click vào th? d? xem ví d?</p>
        ...
        {lesson.vocabulary.map((v, i) => (
          ...
          >
            {!flipped[i] ? (
              ...
              <>
                <p className="text-white/50 text-xs mb-2 font-medium">Ví d?:</p>
  ...
  if (finished) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-white font-bold text-lg mb-1">Ôn t?p xong!</p>
        <div className="flex gap-6 my-4">
          <div className="text-center"><p className="text-app-accent-success font-bold text-2xl">{known.size}</p><p className="text-app-text-secondary text-xs">Ðã nh?</p></div>
          <div className="w-px bg-app-card/70"></div>
          <div className="text-center"><p className="text-red-400 font-bold text-2xl">{unknown.size}</p><p className="text-app-text-secondary text-xs">C?n ôn</p></div>
        </div>
    ...
    <div className="space-y-4 max-w-lg mx-auto">
      ...
      <div onClick={() => setFlipped(f => !f)} className="relative w-full cursor-pointer select-none" style={{ perspective: "1000px", minHeight: "220px" }}>
        <div className="relative w-full transition-transform duration-500" style={{ transformStyle: "preserve-3d", transform: flipped ? "rotateY(180deg)" : "rotateY(0deg)", minHeight: "220px" }}>
          <div className="absolute inset-0 flex flex-col items-center justify-center p-6 rounded-2xl border border-app-border bg-app-surface/50" style={{ backfaceVisibility: "hidden" }}>
            <p className="text-app-text-muted text-xs mb-3">Ti?ng Hàn</p>
      ...
      <div className="flex gap-3">
        ...
        <button onClick={handleKnow} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border border-emerald-500/25 bg-emerald-500/5 hover:bg-emerald-500/10 text-app-accent-success text-sm font-semibold cursor-pointer whitespace-nowrap">
          <i className="ri-check-line"></i>Ðã nh?
        </button>
      </div>
    </div>
  );
          ...
          {open[i] && (
            <div className="px-5 pb-5 space-y-4">
              ...
              <div className="space-y-2">
                <p className="text-app-text-muted text-xs font-medium tracking-normal">Ví d?</p>
                {g.examples.map((ex, j) => (
...
function StoryTab({ lesson }: { lesson: EpsLesson }) {
  ...
  const stories = hasRealDialogues
    ? [
        { title: "H?i tho?i chính", lines: lesson.dialogues.map(d => ({ speaker: d.speaker, korean: d.korean, vietnamese: d.vietnamese, keyword: "" })) },
  ...
  if (finished) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-white font-bold text-lg mb-1">Ghép c?p hoàn thành!</p>
        <div className="flex gap-6 my-4">
          <div className="text-center"><p className="text-app-accent-primary font-bold text-2xl">{score}</p><p className="text-app-text-secondary text-xs">C?p dúng</p></div>
          <div className="w-px bg-app-card/70"></div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-app-text-secondary text-xs">Ghép t? ti?ng Hàn v?i nghia ti?ng Vi?t</p>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-2">
          <p className="text-app-text-muted text-[10px] font-semibold tracking-normal text-center mb-2">Ti?ng Hàn</p>
          {koreanOrder.map(v => {
    ...
    <div className="space-y-5">
        ...
        <div key={ex.id} className="bg-white/2 border border-app-border rounded-xl p-5">
          ...
          {ex.type === "fill_blank" && (
            <div className="ml-10 space-y-2">
              <input type="text" disabled={submitted} value={(answers[ex.id] as string) || ""} onChange={e => setAnswers(p => ({ ...p, [ex.id]: e.target.value }))} placeholder="Nh?p câu tr? l?i..." className="w-full bg-app-card/50 border border-app-border rounded-lg px-4 py-2.5 text-white text-sm placeholder-white/20 focus:outline-none focus:border-app-accent-primary/40 disabled:opacity-60" />
              {submitted && <p className={`text-xs font-medium ${(answers[ex.id] as string)?.trim().toLowerCase() === ex.correctAnswer?.toLowerCase() ? "text-app-accent-success" : "text-red-400"}`}>Ðáp án dúng: <span className="font-bold">{ex.correctAnswer}</span></p>}
            </div>
          )}
          ...
          {ex.type === "translate" && (
            <div className="ml-10 space-y-2">
              ...
              {submitted && <p className="text-app-text-secondary text-xs">G?i ý: <span className="text-app-accent-primary">{ex.correctAnswer}</span></p>}
            </div>
          )}
        </div>
      ))}
      ...
      {!submitted ? (
        <button onClick={handleSubmit} disabled={Object.keys(answers).length < lesson.exercises.length} className="w-full py-3 rounded-xl bg-app-accent-primary hover:bg-[#d4b43a] text-app-bg font-bold text-sm cursor-pointer whitespace-nowrap disabled:opacity-40 disabled:cursor-not-allowed">
          N?p bài ({Object.keys(answers).length}/{lesson.exercises.length} câu)
        </button>
      ) : (
        <div className={`p-4 rounded-xl border text-center ${score === lesson.exercises.length ? "border-emerald-500/30 bg-emerald-500/8" : "border-app-accent-primary/30 bg-app-accent-primary/8"}`}>
          <p className="text-white font-bold text-lg">{score}/{lesson.exercises.length} câu dúng</p>
          <p className="text-white/50 text-sm mt-1">{score === lesson.exercises.length ? "Hoàn h?o! B?n dã n?m v?ng bài này!" : "Hãy ôn l?i ph?n chua dúng nhé!"}</p>
        </div>
      )}
    </div>
  );
...
export default function EpsLessonDetailPage() {
  ...
  const handleMarkComplete = (score: number) => {
    ...
    addXP(xpGain, `Hoàn thành bài EPS s? ${lesson.id}`);
  };

  if (!lesson) {
    return (
      <DashboardLayout title="Không tìm th?y bài h?c" subtitle="">
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <i className="ri-error-warning-line text-4xl text-app-text-muted mb-4"></i>
          <p className="text-white/50 text-lg mb-4">Bài h?c #{id} không t?n t?i</p>
  ...
  const tabs = [
    ...
    { key: "matching", label: "Ghép c?p", icon: "ri-links-line" },
    { key: "review", label: "Ôn nhanh", icon: "ri-stack-line" },
    { key: "grammar", label: "Ng? pháp", icon: "ri-book-2-line" },
    { key: "exercise", label: "Bài t?p", icon: "ri-pencil-line" },
  ] as const;
  ...
  return (
    ...
    >
            ...
            <div className="flex items-center gap-2 mb-3 flex-wrap">
              ...
              {isCompleted && (
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-app-accent-success/15 text-app-accent-success">
                  <i className="ri-checkbox-circle-fill mr-1"></i>Ðã hoàn thành
                </span>
              )}
            </div>
          ...
          <div className="flex-shrink-0">
            {isCompleted ? (
              <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                ...
                <span className="text-app-accent-success text-sm font-semibold whitespace-nowrap">Ðã h?c xong</span>
              </div>
            ) : (
              <button
      ...
      <div className="flex items-center justify-between mt-6 pt-6 border-t border-app-border">
        ...
        >
          <i className="ri-arrow-left-line"></i>
          {prevLesson ? `Bài ${prevLesson.id}` : "Bài tru?c"}
        </button>
        ...
        >
          {nextLesson ? `Bài ${nextLesson.id}` : "Bài ti?p"}
