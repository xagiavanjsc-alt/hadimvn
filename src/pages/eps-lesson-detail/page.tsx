...
function VocabTab({ lesson }: { lesson: EpsLesson }) {
  ...
  return (
    <div className="space-y-4">
      <p className="text-white/40 text-xs">{lesson.vocabulary.length} từ vựng — Click vào thẻ để xem ví dụ</p>
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        {lesson.vocabulary.map((v, i) => (
          <button
            key={i}
            onClick={() => setFlipped(p => ({ ...p, [i]: !p[i] }))}
            className="text-left p-4 rounded-xl border border-white/8 bg-white/2 hover:border-white/15 hover:bg-white/4 transition-all cursor-pointer"
          >
            {!flipped[i] ? (
              <>
                <div className="flex items-start justify-between mb-2">
                  ...
                  <button
                    ...
                    className="w-7 h-7 flex items-center justify-center rounded-lg bg-[#e8c84a]/10 hover:bg-[#e8c84a]/20 transition-colors cursor-pointer"
                  >
                    <i className="ri-volume-up-line text-[#e8c84a] text-xs"></i>
                  </button>
                </div>
                <p className="text-white/40 text-xs mb-1">[{v.pronunciation}]</p>
                <p className="text-[#e8c84a] text-sm font-medium">{v.vietnamese}</p>
              </>
            ) : (
              <>
                ...
                <div className="flex items-start gap-1.5 mb-1">
                  <button
                    ...
                    className="w-5 h-5 flex items-center justify-center rounded-md bg-[#e8c84a]/10 hover:bg-[#e8c84a]/20 transition-colors cursor-pointer flex-shrink-0 mt-0.5"
                  >
                    <i className="ri-volume-up-line text-[#e8c84a] text-[10px]"></i>
                  </button>
                  <p className="text-white text-sm font-medium">{v.example}</p>
                </div>
                <p className="text-white/40 text-xs italic">{v.exampleVi}</p>
              </>
            )}
          </button>
        ))}
  ...
  if (finished) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        <div className="w-16 h-16 flex items-center justify-center rounded-full bg-[#e8c84a]/15 mb-4">
          <i className="ri-trophy-line text-[#e8c84a] text-2xl"></i>
        </div>
        <p className="text-white font-bold text-lg mb-1">Ôn tập xong!</p>
        <div className="flex gap-6 my-4">
          <div className="text-center"><p className="text-emerald-400 font-bold text-2xl">{known.size}</p><p className="text-white/40 text-xs">Đã nhớ</p></div>
          <div className="w-px bg-white/10"></div>
          <div className="text-center"><p className="text-red-400 font-bold text-2xl">{unknown.size}</p><p className="text-white/40 text-xs">Cần ôn</p></div>
        </div>
        <button onClick={restart} className="px-6 py-2.5 rounded-xl border border-[#e8c84a]/30 bg-[#e8c84a]/8 text-[#e8c84a] text-sm font-semibold cursor-pointer whitespace-nowrap">
          Ôn lại từ đầu
        </button>
      </div>
    );
    ...
    <div className="space-y-4 max-w-lg mx-auto">
      <div className="flex items-center justify-between text-xs text-white/40">
        ...
        <div className="flex gap-3">
          <span className="text-emerald-400">{known.size} nhớ</span>
          <span className="text-red-400">{unknown.size} chưa</span>
        </div>
      </div>
      <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
        <div className="h-full rounded-full bg-[#e8c84a] transition-all" style={{ width: `${((known.size + unknown.size) / vocab.length) * 100}%` }} />
      </div>
      ...
      <div onClick={() => setFlipped(f => !f)} className="relative w-full cursor-pointer select-none" style={{ perspective: "1000px", minHeight: "220px" }}>
        <div className="relative w-full transition-transform duration-500" style={{ transformStyle: "preserve-3d", transform: flipped ? "rotateY(180deg)" : "rotateY(0deg)", minHeight: "220px" }}>
          <div className="absolute inset-0 flex flex-col items-center justify-center p-6 rounded-2xl border border-white/10 bg-white/3" style={{ backfaceVisibility: "hidden" }}>
            <p className="text-white/30 text-xs mb-3">Tiếng Hàn</p>
            <p className="text-white font-bold text-4xl mb-2">{current.korean}</p>
            <p className="text-white/40 text-sm">[{current.pronunciation}]</p>
            <button onClick={e => { e.stopPropagation(); speakKorean(current.korean); }} className="mt-4 w-9 h-9 flex items-center justify-center rounded-xl bg-[#e8c84a]/10 hover:bg-[#e8c84a]/20 cursor-pointer">
              <i className="ri-volume-up-line text-[#e8c84a]"></i>
            </button>
            <p className="text-white/20 text-xs mt-4">Nhấn để xem nghĩa</p>
          </div>
          <div className="absolute inset-0 flex flex-col items-center justify-center p-6 rounded-2xl border border-[#e8c84a]/20 bg-[#e8c84a]/5" style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}>
            <p className="text-[#e8c84a]/60 text-xs mb-3">Tiếng Việt</p>
            <p className="text-[#e8c84a] font-bold text-2xl mb-3">{current.vietnamese}</p>
            <div className="border-t border-white/8 pt-3 w-full text-center">
              <p className="text-white/50 text-sm italic">{current.example}</p>
              <p className="text-white/30 text-xs mt-1">{current.exampleVi}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-3">
        ...
        <button onClick={handleKnow} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border border-emerald-500/25 bg-emerald-500/5 hover:bg-emerald-500/10 text-emerald-400 text-sm font-semibold cursor-pointer whitespace-nowrap">
      ...
      {lesson.grammarPoints.map((g, i) => (
        <div key={i} className="border border-white/8 rounded-xl overflow-hidden">
          ...
          >
            ...
            <i className={open[i] ? "ri-arrow-up-s-line text-white/30" : "ri-arrow-down-s-line text-white/30"}></i>
          </button>
          {open[i] && (
            <div className="px-5 pb-5 space-y-4">
              <p className="text-white/60 text-sm leading-relaxed bg-white/3 rounded-lg p-3">{g.explanation}</p>
              <div className="space-y-2">
                <p className="text-white/30 text-xs font-medium uppercase tracking-wider">Ví dụ</p>
                {g.examples.map((ex, j) => (
                  <div key={j} className="flex items-start gap-3 p-3 rounded-lg bg-[#e8c84a]/5 border border-[#e8c84a]/10">
                    ...
                    <div>
                      ...
                      <p className="text-white/40 text-xs mt-0.5">{ex.vietnamese}</p>
                    </div>
                  </div>
                ))}
              </div>
...
function StoryTab({ lesson }: { lesson: EpsLesson }) {
      ...
      <>
        {parts.map((part, i) => (
          <span key={i}>
            {part}
            {i < parts.length - 1 && (
              <span className="text-[#e8c84a] font-bold bg-[#e8c84a]/10 px-0.5 rounded cursor-pointer" onClick={() => setActiveWord(keyword)}>{keyword}</span>
            )}
          </span>
        ))}
      </>
  ...
  return (
    <div className="space-y-5">
          ...
          <div className="p-4 space-y-3">
            {story.lines.map((line, li) => (
              <div key={li} className={`flex gap-3 ${line.speaker === "B" ? "flex-row-reverse" : ""}`}>
                <div className={`w-7 h-7 flex items-center justify-center rounded-full flex-shrink-0 text-xs font-bold ${line.speaker === "A" ? "bg-[#e8c84a]/20 text-[#e8c84a]" : "bg-[#34d399]/20 text-[#34d399]"}`}>{line.speaker}</div>
                <div className={`flex-1 p-3 rounded-xl text-sm ${line.speaker === "A" ? "bg-white/4 rounded-tl-none" : "bg-[#34d399]/5 rounded-tr-none"}`}>
                  <div className="flex items-start gap-2">
                    <button onClick={() => speakKorean(line.korean)} className="w-5 h-5 flex items-center justify-center rounded-md bg-[#e8c84a]/10 hover:bg-[#e8c84a]/20 flex-shrink-0 mt-0.5 cursor-pointer">
                      <i className="ri-volume-up-line text-[#e8c84a] text-[10px]"></i>
                    </button>
                    <div>
                      <p className="text-white font-medium leading-relaxed">{highlightKeyword(line.korean, line.keyword)}</p>
                      <p className="text-white/40 text-xs mt-0.5 italic">{line.vietnamese}</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {activeWord && (
        <div className="p-4 rounded-xl border border-[#e8c84a]/25 bg-[#e8c84a]/5">
          <div className="flex items-center justify-between mb-2">
            <p className="text-[#e8c84a] font-bold text-lg">{activeWord}</p>
            <button onClick={() => setActiveWord(null)} className="text-white/30 hover:text-white/60 cursor-pointer"><i className="ri-close-line"></i></button>
          </div>
...
function MatchingTab({ lesson }: { lesson: EpsLesson }) {
  ...
  const getCardClass = (side: "korean" | "viet", idx: number) => {
    if (matched.has(idx)) return "border-emerald-500/40 bg-emerald-500/8 text-emerald-400 cursor-default";
    if (wrong && wrong.side === side && wrong.idx === idx) return "border-red-500/40 bg-red-500/8 text-red-400 cursor-pointer";
    if (selected && selected.side === side && selected.idx === idx) return "border-[#e8c84a]/50 bg-[#e8c84a]/10 text-[#e8c84a] cursor-pointer";
    return "border-white/8 bg-white/2 text-white/70 hover:border-white/20 hover:bg-white/4 cursor-pointer";
  };
  ...
  if (finished) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <div className="flex gap-6 my-4">
          <div className="text-center"><p className="text-[#e8c84a] font-bold text-2xl">{score}</p><p className="text-white/40 text-xs">Cặp đúng</p></div>
          <div className="w-px bg-white/10"></div>
          <div className="text-center"><p className="text-white font-bold text-2xl">{attempts}</p><p className="text-white/40 text-xs">Lần thử</p></div>
        </div>
        <button onClick={restart} className="px-6 py-2.5 rounded-xl border border-[#e8c84a]/30 bg-[#e8c84a]/8 text-[#e8c84a] text-sm font-semibold cursor-pointer whitespace-nowrap">
          Chơi lại
        </button>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-white/40 text-xs">Ghép từ tiếng Hàn với nghĩa tiếng Việt</p>
        <div className="flex items-center gap-3 text-xs">
          <span className="text-emerald-400 font-bold">{matched.size}/{PAIR_COUNT}</span>
          <button onClick={restart} className="text-white/30 hover:text-white/60 cursor-pointer"><i className="ri-refresh-line"></i></button>
        </div>
      </div>
      <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
        <div className="h-full rounded-full bg-[#e8c84a] transition-all" style={{ width: `${(matched.size / PAIR_COUNT) * 100}%` }} />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-2">
          <p className="text-white/30 text-[10px] font-semibold uppercase tracking-wider text-center mb-2">Tiếng Hàn</p>
          {koreanOrder.map(v => {
        ...
        <div className="space-y-2">
          <p className="text-white/30 text-[10px] font-semibold uppercase tracking-wider text-center mb-2">Tiếng Việt</p>
  ...
  return (
    <div className="space-y-5">
      {lesson.exercises.map((ex: EpsExercise, idx) => (
        <div key={ex.id} className="bg-white/2 border border-white/8 rounded-xl p-5">
          <div className="flex items-start gap-3 mb-4">
            <div className="w-7 h-7 flex items-center justify-center rounded-lg bg-white/5 flex-shrink-0 text-white/40 text-xs font-bold">{idx + 1}</div>
            <div>
              <p className="text-white text-sm font-medium">{ex.question}</p>
              <p className="text-white/40 text-xs mt-0.5 italic">{ex.questionVi}</p>
            </div>
          </div>
          ...
          {ex.type === "multiple_choice" && ex.options && (
            <div className="space-y-2 ml-10">
              {ex.options.map((opt, i) => {
                let cls = "border-white/8 bg-white/2 hover:border-white/15 cursor-pointer";
                if (submitted) {
                ...
                } else if (answers[ex.id] === i) cls = "border-[#e8c84a]/40 bg-[#e8c84a]/8 cursor-pointer";
                return (
                  <button key={i} disabled={submitted} onClick={() => setAnswers(p => ({ ...p, [ex.id]: i }))} className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg border transition-all text-left ${cls}`}>
                    <span className="text-white/30 text-xs font-bold w-4">{["A","B","C","D"][i]}</span>
                    <div>
                      <p className="text-white/70 text-sm">{opt}</p>
                      {ex.optionsVi && <p className="text-white/30 text-xs">{ex.optionsVi[i]}</p>}
                    </div>
                    {submitted && i === ex.correctIndex && <i className="ri-checkbox-circle-fill text-emerald-400 ml-auto"></i>}
          ...
          {ex.type === "fill_blank" && (
            <div className="ml-10 space-y-2">
              <input type="text" disabled={submitted} value={(answers[ex.id] as string) || ""} onChange={e => setAnswers(p => ({ ...p, [ex.id]: e.target.value }))} placeholder="Nhập câu trả lời..." className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-white text-sm placeholder-white/20 focus:outline-none focus:border-[#e8c84a]/40 disabled:opacity-60" />
              {submitted && <p className={`text-xs font-medium ${(answers[ex.id] as string)?.trim().toLowerCase() === ex.correctAnswer?.toLowerCase() ? "text-emerald-400" : "text-red-400"}`}>Đáp án đúng: <span className="font-bold">{ex.correctAnswer}</span></p>}
            </div>
          )}

          {ex.type === "translate" && (
            <div className="ml-10 space-y-2">
              <textarea disabled={submitted} value={(answers[ex.id] as string) || ""} onChange={e => setAnswers(p => ({ ...p, [ex.id]: e.target.value }))} placeholder="Nhập bản dịch..." rows={2} className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-white text-sm placeholder-white/20 focus:outline-none focus:border-[#e8c84a]/40 resize-none disabled:opacity-60" />
              {submitted && <p className="text-white/40 text-xs">Gợi ý: <span className="text-[#e8c84a]">{ex.correctAnswer}</span></p>}
            </div>
          )}
        </div>
      ))}
      ...
      {!submitted ? (
        <button onClick={handleSubmit} disabled={Object.keys(answers).length < lesson.exercises.length} className="w-full py-3 rounded-xl bg-[#e8c84a] hover:bg-[#d4b43a] text-[#0f1117] font-bold text-sm cursor-pointer whitespace-nowrap disabled:opacity-40 disabled:cursor-not-allowed">
          Nộp bài ({Object.keys(answers).length}/{lesson.exercises.length} câu)
        </button>
      ) : (
        <div className={`p-4 rounded-xl border text-center ${score === lesson.exercises.length ? "border-emerald-500/30 bg-emerald-500/8" : "border-[#e8c84a]/30 bg-[#e8c84a]/8"}`}>
    ...
    return (
      <DashboardLayout title="Không tìm thấy bài học" subtitle="">
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <i className="ri-error-warning-line text-4xl text-white/20 mb-4"></i>
          <p className="text-white/50 text-lg mb-4">Bài học #{id} không tồn tại</p>
          <button onClick={() => navigate("/eps-lessons")} className="px-6 py-2.5 rounded-xl bg-[#e8c84a] text-[#0f1117] font-bold text-sm cursor-pointer whitespace-nowrap">
            Quay lại danh sách
          </button>
        </div>
      </DashboardLayout>
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-white/30 text-xs px-2">{lesson.id} / {epsLessons.length}</span>
          <button
      ...
      <div className="bg-[#0f1117] border border-white/8 rounded-2xl p-5 mb-6">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            ...
            {/* Key phrase */}
            <div className="flex items-center gap-3 p-3 rounded-xl bg-[#e8c84a]/5 border border-[#e8c84a]/10">
              <button onClick={() => speakKorean(lesson.keyPhrase)} className="w-9 h-9 flex items-center justify-center rounded-lg bg-[#e8c84a]/15 hover:bg-[#e8c84a]/25 cursor-pointer flex-shrink-0">
                <i className="ri-volume-up-line text-[#e8c84a] text-sm"></i>
              </button>
              <div>
                <p className="text-[#e8c84a] font-bold text-base">{lesson.keyPhrase}</p>
                <p className="text-white/40 text-xs">{lesson.keyPhraseVi}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex-shrink-0">
            {isCompleted ? (
              <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                ...
                <span className="text-emerald-400 text-sm font-semibold whitespace-nowrap">Đã học xong</span>
              </div>
            ) : (
              <button
                onClick={() => handleMarkComplete(0)}
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-emerald-500/25 bg-emerald-500/5 hover:bg-emerald-500/10 text-emerald-400 text-sm font-semibold cursor-pointer whitespace-nowrap transition-colors"
              >
      ...
      <div className="flex border-b border-white/8 mb-6 overflow-x-auto">
        {tabs.map(t => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-3 text-sm font-semibold transition-colors cursor-pointer whitespace-nowrap flex-shrink-0 ${tab === t.key ? "text-[#e8c84a] border-b-2 border-[#e8c84a]" : "text-white/40 hover:text-white/60"}`}
          >
      ...
      {/* Tab content */}
      <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5 md:p-6">
      ...
      <div className="flex items-center justify-between mt-6 pt-6 border-t border-white/5">
        <button
          onClick={() => prevLesson && navigate(`/eps-lesson/${prevLesson.id}`)}
          disabled={!prevLesson}
          className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-white/8 bg-white/3 text-white/50 hover:text-white/80 hover:bg-white/6 text-sm font-semibold transition-all cursor-pointer whitespace-nowrap disabled:opacity-30 disabled:cursor-not-allowed"
        >
          <i className="ri-arrow-left-line"></i>
        ...
        <button
          onClick={() => navigate("/eps-lessons")}
          className="text-white/30 hover:text-white/60 text-xs cursor-pointer whitespace-nowrap transition-colors"
        >
        ...
        <button
          onClick={() => nextLesson && navigate(`/eps-lesson/${nextLesson.id}`)}
          disabled={!nextLesson}
          className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-[#e8c84a]/20 bg-[#e8c84a]/5 text-[#e8c84a] hover:bg-[#e8c84a]/10 text-sm font-semibold transition-all cursor-pointer whitespace-nowrap disabled:opacity-30 disabled:cursor-not-allowed"
        >
