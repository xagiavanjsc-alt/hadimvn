  ...
  return (
    <div className="fixed top-6 right-6 z-50 flex items-center gap-3 px-5 py-3 rounded-2xl shadow-lg animate-bounce"
      ...
      <div>
        ...
        <p className="text-[11px] font-medium opacity-70">Th? thách hoàn thành!</p>
      </div>
    </div>
  );
}
    ...
    return (
      <div className="flex flex-col items-center gap-6 py-10">
        ...
        <div className="text-center">
          ...
          <p className="text-white/50 text-sm">{score}/{totalQ} câu dúng</p>
        </div>
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <div className="rounded-2xl p-6 mb-5 text-center" style={{ backgroundColor: `${challenge.color}08`, border: `1px solid ${challenge.color}15` }}>
        <p className="text-app-text-secondary text-xs mb-2">Câu {current + 1}/{totalQ} — Ch?n nghia dúng</p>
...
function WriteChallenge({ challenge, onComplete }: { challenge: Challenge; onComplete: (score: number) => void }) {
  ...
  if (done) {
    ...
    return (
      <div className="flex flex-col items-center gap-6 py-10">
        ...
        <div className="text-center">
          <p className="text-white font-black text-3xl mb-1">{pct}%</p>
          <p className="text-white/50 text-sm">{score}/{challenge.questions.length} câu dúng</p>
        </div>
        <div className="flex items-center gap-2 px-6 py-3 rounded-2xl" style={{ backgroundColor: `${challenge.color}15`, border: `1px solid ${challenge.color}30` }}>
          <i className="ri-star-fill" style={{ color: challenge.color }}></i>
          <span className="font-black text-xl" style={{ color: challenge.color }}>+{earned} XP</span>
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <div className="rounded-2xl p-6 mb-5 text-center" style={{ backgroundColor: `${challenge.color}08`, border: `1px solid ${challenge.color}15` }}>
        <p className="text-app-text-secondary text-xs mb-2">Vi?t nghia ti?ng Vi?t c?a t? sau</p>
        <p className="text-white font-black text-4xl mb-2">{q.korean}</p>
        <p className="text-app-text-secondary text-sm font-mono">{q.pronunciation}</p>
      </div>
      <div className="flex gap-3">
        <input
          ...
          placeholder="Nh?p nghia ti?ng Vi?t..."
      ...
      {result === "wrong" && (
        <p className="text-app-text-secondary text-xs mt-2 text-center">Ðáp án dúng: <span className="text-[#34d399] font-bold">{q.correctAnswer}</span></p>
      )}
    </div>
  );
}
...
export default function DailyChallengePageComponent() {
        ...
        setChallenges([
          {
            ...
            type: "quiz",
            title: "Quiz T? V?ng",
            description: "5 câu h?i tr?c nghi?m — ch?n nghia dúng c?a t? ti?ng Hàn",
            xpReward: 50,
            timeLimit: 90,
          ...
          },
          {
            id: "listen",
            type: "listen",
            title: "Nghe & Nh?n Bi?t",
            description: "Nghe phát âm và ch?n t? dúng — luy?n tai nghe ti?ng Hàn",
            xpReward: 60,
            timeLimit: 120,
          ...
          },
          {
            ...
            type: "write",
            title: "Vi?t Nghia",
            description: "Nhìn t? ti?ng Hàn và vi?t nghia ti?ng Vi?t — ki?m tra trí nh?",
            xpReward: 80,
            timeLimit: 150,
            icon: "ri-edit-line",
            color: "#a78bfa",
  ...
  return (
    <DashboardLayout
      title="Th? thách hàng ngày"
      subtitle="M?i ngày 3 th? thách m?i — hoàn thành d? nh?n XP"
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: "Ðã hoàn thành", value: `${isToday ? dailyRecord.completed.length : 0}/3`, icon: "ri-checkbox-circle-line", color: "#34d399" },
          { label: "Th? thách còn l?i", value: `${challenges.filter(c => !dailyRecord.completed.includes(c.id)).length}`, icon: "ri-timer-line", color: "#a78bfa" },
        ].map(s => (
      ...
      {allDone && (
        <div className="mb-6 rounded-2xl p-5 flex items-center gap-4" style={{ background: "linear-gradient(135deg, rgba(232,200,74,0.15), rgba(52,211,153,0.1))", border: "1px solid rgba(232,200,74,0.2)" }}>
          ...
          <div>
            <p className="text-white font-bold text-base">Xu?t s?c! B?n dã hoàn thành t?t c? th? thách hôm nay ??</p>
            <p className="text-white/50 text-sm">T?ng XP nh?n du?c hôm nay: <span className="text-app-accent-primary font-bold">+{dailyRecord.totalXP} XP</span></p>
          </div>
        </div>
      )}
      ...
      {challenges.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-app-text-muted text-sm">Ðang t?o th? thách hôm nay...</p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <div>
                      <p className="text-app-text-muted text-[10px] tracking-normal">Th? thách {idx + 1}</p>
                  ...
                  >
                    {isDone ? "? Ðã hoàn thành" : "B?t d?u th? thách"}
                  </button>
                </div>
              </div>
            );
      ...
      <div className="mt-8 p-4 rounded-xl border border-app-border bg-white/2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-white/50 text-sm">Th? thách du?c t?o m?i m?i ngày lúc 00:00</p>
            <p className="text-app-text-muted text-xs">Hoàn thành c? 3 th? thách d? nh?n t?i da <span className="text-app-accent-primary">190 XP</span> m?i ngày</p>
          </div>
        </div>
      </div>
    </DashboardLayout>
