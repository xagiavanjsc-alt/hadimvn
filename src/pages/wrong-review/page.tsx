...
function ReviewCard({
  ...
  return (
    <div className={`border rounded-xl overflow-hidden transition-all ${item.mastered ? "border-emerald-500/20 bg-emerald-500/3 opacity-60" : "border-app-border bg-white/2"}`}>
      {/* Header */}
      <button
        onClick={() => setExpanded(e => !e)}
        className="w-full flex items-start gap-3 p-4 hover:bg-app-surface/50 transition-colors cursor-pointer text-left"
      >
        <div className={`w-8 h-8 flex items-center justify-center rounded-lg flex-shrink-0 ${item.mastered ? "bg-app-accent-success/15" : "bg-red-500/10"}`}>
          <i className={`text-sm ${item.mastered ? "ri-checkbox-circle-fill text-app-accent-success" : "ri-close-circle-line text-red-400"}`}></i>
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-app-card/50 text-app-text-secondary">
              Bài {item.lessonId}
            </span>
            <span className="text-[10px] text-app-text-muted">{item.lessonTitle}</span>
            <span className="text-[10px] text-app-text-muted">{daysSince === 0 ? "Hôm nay" : `${daysSince} ngày trước`}</span>
          </div>
          <p className="text-white/70 text-sm font-medium truncate">{item.question}</p>
          <p className="text-app-text-muted text-xs mt-0.5 truncate italic">{item.questionVi}</p>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          {item.reviewCount > 0 && (
            <span className="text-[10px] text-app-accent-primary/60 font-bold">×{item.reviewCount}</span>
          )}
          <i className={`text-app-text-muted text-sm ${expanded ? "ri-arrow-up-s-line" : "ri-arrow-down-s-line"}`}></i>
        </div>
      </button>
      ...
      {expanded && !item.mastered && (
        <div className="px-4 pb-4 space-y-3 border-t border-app-border pt-3">
          ...
          <div className="flex items-start gap-2 p-3 rounded-lg bg-red-500/5 border border-red-500/15">
            ...
            <div>
              <p className="text-app-accent-error/60 text-[10px] font-semibold mb-0.5">Câu trả lời sai của bạn</p>
          ...
          <div className="flex items-start gap-2 p-3 rounded-lg bg-emerald-500/5 border border-emerald-500/15">
            ...
            <div className="flex-1">
              <p className="text-app-accent-success/60 text-[10px] font-semibold mb-0.5">Đáp án đúng</p>
              <div className="flex items-center gap-2">
                <p className="text-app-accent-success text-sm font-bold">{item.correctAnswer}</p>
                {item.correctAnswer.match(/[\u3131-\uD79D]/) && (
                  ...
                  >
                    <i className="ri-volume-up-line text-app-accent-success text-xs"></i>
                  </button>
                )}
              </div>
            </div>
          ...
          {!checked && (
            <div className="space-y-2">
              <p className="text-app-text-secondary text-xs font-medium">Thử lại — nhập đáp án đúng:</p>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-app-card/50 border border-app-border rounded-lg px-3 py-2 text-white text-sm placeholder-white/20 focus:outline-none focus:border-app-accent-primary/40"
                />
                <button
                  onClick={handleCheck}
                  disabled={!userInput.trim()}
                  className="px-4 py-2 rounded-lg bg-app-accent-primary/15 hover:bg-app-accent-primary/25 text-app-accent-primary text-sm font-semibold transition-colors cursor-pointer whitespace-nowrap disabled:opacity-40"
                >
                  Kiểm tra
                </button>
              </div>
          ...
          {checked && (
            <div className={`p-3 rounded-lg border text-center ${isCorrect ? "border-emerald-500/30 bg-emerald-500/8" : "border-red-500/20 bg-red-500/5"}`}>
              <p className={`text-sm font-bold ${isCorrect ? "text-app-accent-success" : "text-red-400"}`}>
          ...
          <div className="flex gap-2 pt-1">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg border border-emerald-500/20 bg-emerald-500/5 hover:bg-emerald-500/10 text-app-accent-success text-xs font-semibold transition-colors cursor-pointer whitespace-nowrap"
            >
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg border border-app-border bg-white/2 hover:bg-app-card/50 text-app-text-secondary text-xs font-semibold transition-colors cursor-pointer whitespace-nowrap"
            >
      ...
      {expanded && item.mastered && (
        <div className="px-4 pb-4 border-t border-app-border pt-3">
          <div className="flex items-center gap-2 text-app-accent-success text-sm">
...
export default function WrongReviewPage() {
  ...
  return (
    ...
    >
      ...
      {wrongItems.length > 0 && (
        <div className="bg-app-bg border border-app-border rounded-xl p-4 mb-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <p className="text-app-accent-success text-sm font-bold">{masteredCount}/{wrongItems.length}</p>
          </div>
          ...
          {masteredCount === wrongItems.length && wrongItems.length > 0 && (
            <p className="text-app-accent-success text-xs font-semibold mt-2 text-center">
      ...
      {pendingCount > 0 && (
        <div className="flex gap-2 mb-4">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-lg border border-emerald-500/20 bg-emerald-500/5 hover:bg-emerald-500/10 text-app-accent-success text-xs font-semibold transition-colors cursor-pointer whitespace-nowrap"
          >
          ...
          {masteredCount > 0 && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg border border-app-border bg-white/2 hover:bg-app-card/50 text-app-text-secondary text-xs font-semibold transition-colors cursor-pointer whitespace-nowrap"
            >
      ...
      <p className="text-app-text-muted text-xs mb-3">{filtered.length} câu</p>
        ...
        <div className="text-center py-16">
          ...
          {filterStatus === "pending" && (
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-app-accent-primary hover:bg-[#d4b43a] text-app-bg font-bold text-sm transition-colors cursor-pointer whitespace-nowrap"
            >
              Đi thi thử theo bài
            </button>
          )}
      ...
      {/* Tips */}
      <div className="mt-6 bg-app-accent-primary/5 border border-app-accent-primary/15 rounded-xl p-4">
        <p className="text-app-accent-primary text-xs font-semibold mb-2">Mẹo ôn tập hiệu quả</p>
