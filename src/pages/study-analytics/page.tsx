  ...
  return (
    <div className="bg-app-bg border border-app-border rounded-2xl p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-white font-semibold text-sm">L?ch h?c t?p (12 tu?n)</h3>
...
function WeeklyChart({
  ...
  const metricLabel = metric === "minutes" ? "phút" : metric === "xp" ? "XP" : "t?";
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex justify-between text-xs text-app-text-muted">
        ...
        <span>T?ng: {values.reduce((a, b) => a + b, 0)} {metricLabel}</span>
      </div>
    </div>
  );
}
...
function VocabTopicChart({ completedLessons }: { completedLessons: Record<number, { score: number }> }) {
  const topics = [
    { label: "Chào h?i", range: [1, 5], color: "app-accent-primary" },
    { label: "Sinh ho?t", range: [6, 12], color: "#a78bfa" },
    { label: "Cu?i tu?n", range: [13, 19], color: "#34d399" },
...
export default function StudyAnalyticsPage() {
  ...
  return (
    <DashboardLayout
      title="Th?ng kê h?c t?p"
      subtitle="Phân tích chi ti?t ti?n d? và thói quen h?c t?p c?a b?n"
    >
      {/* Top KPI cards */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6">
        {[
          { label: "Streak hi?n t?i", value: `${streak} ngày`, icon: "ri-fire-line", color: "#fb923c", sub: "Liên ti?p" },
          { label: "Ngày h?c (30 ngày)", value: `${activeDays30} ngày`, icon: "ri-calendar-check-line", color: "#34d399", sub: `${Math.round((activeDays30 / 30) * 100)}% tháng này` },
          { label: "T?ng XP", value: totalXP.toLocaleString(), icon: "ri-star-line", color: "app-accent-primary", sub: "Ði?m kinh nghi?m" },
          { label: "T? v?ng dã h?c", value: `~${totalVocabLearned}`, icon: "ri-book-2-line", color: "#a78bfa", sub: `${totalLessons} bài hoàn thành` },
        ].map(s => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="lg:col-span-2 bg-app-bg border border-app-border rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
            <h3 className="text-white font-semibold text-sm">Bi?u d? 7 ngày qua</h3>
            <div className="flex rounded-lg border border-app-border overflow-hidden">
              {(["minutes", "xp", "words"] as const).map(m => (
                ...
                >
                  {m === "minutes" ? "Th?i gian" : m === "xp" ? "XP" : "T? v?ng"}
                </button>
              ))}
            </div>
          </div>
        ...
        {/* Quick stats */}
        <div className="bg-app-bg border border-app-border rounded-2xl p-5">
          <h3 className="text-white font-semibold text-sm mb-4">Tóm t?t tu?n này</h3>
          <div className="space-y-4">
            {[
              { label: "Ngày h?c", value: `${activeDays7}/7`, icon: "ri-calendar-line", color: "#34d399" },
              { label: "T?ng th?i gian", value: `${totalMinutes7} phút`, icon: "ri-time-line", color: "app-accent-primary" },
              { label: "TB m?i ngày h?c", value: `${avgMinutes7} phút`, icon: "ri-bar-chart-line", color: "#a78bfa" },
              { label: "T? v?ng ôn", value: `~${totalWords7} t?`, icon: "ri-book-open-line", color: "#06b6d4" },
              { label: "Ngày h?c t?t nh?t", value: bestDay.date ? `${formatDate(bestDay.date)} (${bestDay.minutes}p)` : "—", icon: "ri-trophy-line", color: "#fb923c" },
            ].map(s => (
      ...
      {/* Module progress */}
      <div className="mb-6">
        <h3 className="text-white font-semibold text-sm mb-4">Ti?n d? theo module</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <ModuleProgressCard
            ...
            total={60}
            details={[
              { label: "Bài h?c", value: `${totalLessons}/60` },
              { label: "T? v?ng", value: `~${totalVocabLearned}` },
          ...
          <ModuleProgressCard
            ...
            total={20}
            details={[
              { label: "Bài h?c", value: `${Math.min(Math.floor(totalLessons / 3), 20)}/20` },
              { label: "C?m t?", value: "—" },
              { label: "Bài thi", value: "—" },
            ]}
            href="/seoul-textbook"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        {/* Vocab by topic */}
        <div className="bg-app-bg border border-app-border rounded-2xl p-5">
          <h3 className="text-white font-semibold text-sm mb-4">T? v?ng EPS theo ch? d?</h3>
        ...
        {/* Study habits */}
        <div className="bg-app-bg border border-app-border rounded-2xl p-5">
          <h3 className="text-white font-semibold text-sm mb-4">Thói quen h?c t?p</h3>
          <div className="space-y-3">
            ...
            <div className="p-3 rounded-xl bg-app-surface/50 border border-app-border">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-white/60 text-xs font-medium">Th?i di?m h?c t?t nh?t</p>
              </div>
              <p className="text-white font-semibold text-sm">Bu?i t?i (19:00 – 22:00)</p>
              <p className="text-app-text-muted text-xs mt-0.5">D?a trên l?ch s? h?c t?p</p>
            </div>
            ...
            <div className="p-3 rounded-xl bg-app-surface/50 border border-app-border">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-white/60 text-xs font-medium">Tính nh?t quán</p>
              </div>
            ...
            <div className="p-3 rounded-xl bg-app-surface/50 border border-app-border">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-white/60 text-xs font-medium">Th?i gian h?c trung bình</p>
              </div>
              <p className="text-white font-semibold text-sm">{avgMinutes7} phút / bu?i</p>
              <p className="text-app-text-muted text-xs mt-0.5">
                {avgMinutes7 >= 30 ? "T?t! Duy trì nhé" : avgMinutes7 >= 15 ? "Khá t?t, c? g?ng thêm" : "Hãy h?c thêm m?i ngày"}
              </p>
            </div>
            ...
            <div className="p-3 rounded-xl bg-app-accent-primary/5 border border-app-accent-primary/15">
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-app-accent-primary text-xs font-semibold">G?i ý hôm nay</p>
              </div>
              <p className="text-white/60 text-xs leading-relaxed">
                {totalLessons < 5
                  ? "B?t d?u v?i bài 1–5 EPS d? xây d?ng n?n t?ng v?ng ch?c!"
                  : totalLessons < 15
                  ? "Ti?p t?c h?c bài EPS và ôn l?i t? v?ng b?ng flashcard!"
                  : "Th? thi th? EPS d? ki?m tra trình d? c?a b?n!"}
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Quick actions */}
      <div className="bg-app-bg border border-app-border rounded-2xl p-5">
        <h3 className="text-white font-semibold text-sm mb-4">Hành d?ng nhanh</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: "Ti?p t?c h?c EPS", icon: "ri-book-open-line", color: "app-accent-primary", href: "/eps-lessons" },
            { label: "Ôn flashcard", icon: "ri-stack-line", color: "#a78bfa", href: "/eps-flashcard" },
            { label: "Thi th? EPS", icon: "ri-file-text-line", color: "#34d399", href: "/eps-exam" },
            { label: "L? trình h?c", icon: "ri-map-2-line", color: "#fb923c", href: "/learning-path" },
          ].map(a => (
            <button
              key={a.label}
