    ...
    <div className="max-w-2xl mx-auto">
        ...
        {flipped && (
          <>
            ...
            <button onClick={() => next("known")}
              ...
              <i className="ri-check-line"></i> Ðã bi?t
            </button>
          </>
        )}
        {!flipped && (
      ...
      {done && (
        <div className="mt-6 p-5 rounded-2xl text-center" style={{ background: "linear-gradient(135deg, #ecfdf5, #f0fdf4)" }}>
          ...
          <p className="font-bold text-emerald-700">Hoàn thành! {known.size}/{shuffled.length} t? dã bi?t</p>
    ...
    return (
      <div className="max-w-lg mx-auto text-center">
        <div className="p-8 rounded-3xl border-2 mb-6" style={{ borderColor: pct >= 80 ? "#34d399" : pct >= 60 ? "app-accent-primary" : "#f87171" }}>
          ...
          <p className="text-xl font-bold text-gray-700 mb-1">{score}/{questions.length} câu dúng</p>
          <p className="text-gray-500">{pct >= 80 ? "Xu?t s?c! ??" : pct >= 60 ? "Khá t?t! ??" : "C?n ôn thêm ??"}</p>
        </div>
    ...
    <div className="max-w-2xl mx-auto">
      ...
      {done ? (
        <div className="text-center p-8 rounded-3xl" style={{ background: "linear-gradient(135deg, #ecfdf5, #f0fdf4)" }}>
          ...
          <p className="text-emerald-600 mb-4">B?n dã ghép dúng t?t c? {pairs.length} c?p t?!</p>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            <p className="text-xs font-bold text-gray-400 tracking-normal mb-3">Ti?ng Hàn</p>
            {koreans.map(k => {
    ...
    return (
      <div className="max-w-lg mx-auto text-center">
        <div className="p-8 rounded-3xl border-2 mb-6" style={{ borderColor: pct >= 80 ? "#34d399" : "#f87171" }}>
          ...
          <p className="text-xl font-bold text-gray-700">{score}/{questions.length} câu dúng</p>
        </div>
        <button onClick={onBack} className="px-8 py-3 rounded-2xl font-bold text-white cursor-pointer"
          style={{ background: "linear-gradient(135deg, #f87171, #fb923c)" }}>
          Ch?n bài khác
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <div className="text-center mb-6 p-6 rounded-3xl border-2 border-gray-100">
        <p className="text-sm text-gray-400 mb-2">Ði?n t? ti?ng Hàn tuong ?ng</p>
      ...
      <div className="relative mb-4">
        ...
        {result === "wrong" && (
          <p className="text-center text-sm text-emerald-600 mt-2 font-semibold">
            Ðáp án dúng: <span className="font-black">{current.korean}</span> [{current.pronunciation}]
          </p>
        )}
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Flashcard · Quiz · Ghép c?p · Ði?n t? theo t?ng bài h?c</p>
          </div>
        </div>
          ...
          <>
              ...
              <div className="rounded-3xl border-2 p-6" style={{ borderColor: "#e5e7eb" }}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    ...
                    { mode: "quiz" as Mode, icon: "ri-question-line", label: "Quiz", desc: "Tr?c nghi?m 4 dáp án", color: "#a78bfa", minWords: 4 },
                    { mode: "matching" as Mode, icon: "ri-links-line", label: "Ghép c?p", desc: "N?i t? v?i nghia", color: "app-accent-primary", minWords: 4 },
                    { mode: "fill" as Mode, icon: "ri-edit-line", label: "Ði?n t?", desc: "Nh?p t? ti?ng Hàn", color: "#f87171", minWords: 1 },
                  ].map(m => {
            ...
            {selectedLesson && loading && (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-500">Ðang t?i t? v?ng...</p>
              </div>
            )}
            ...
            {selectedLesson && !loading && words.length === 0 && (
              <div className="text-center py-8 text-gray-400">
                ...
                <p>Bài h?c này chua có t? v?ng</p>
              </div>
            )}
          </>
        )}
