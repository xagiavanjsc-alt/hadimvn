...
const EXAM_TYPE_LABELS: Record<string, string> = {
  eps_mock: "Thi mô ph?ng th?t",
  eps_topic: "Thi theo ch? d?",
  eps_quick: "Ôn t?p nhanh",
  eps: "Thi EPS",
};
      ...
      {bestScore !== null ? (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="text-center">
            ...
            <p className="text-app-text-secondary text-xs">Ði?m cao nh?t</p>
          </div>
          <div className="text-center">
          ...
          <div className="text-center">
            ...
            <p className="text-app-text-secondary text-xs">L?n dã thi</p>
          </div>
        </div>
      ) : (
        <div className="text-center py-3">
          <p className="text-app-text-secondary text-sm">B?n chua thi l?n nào</p>
          <p className="text-app-text-muted text-xs mt-1">Thi th? d? xem di?m c?a b?n trên BXH</p>
        </div>
      )}
    </div>
  );
    ...
    (async () => {
      ...
      try {
        ...
        const { data: rows } = await query
          ...
          .limit(100); // l?y nhi?u d? tính rank c?a user
        ...
        const combined: LeaderboardEntry[] = userIds
          .map(uid => {
            ...
            const name = nameMap[uid] || "H?c viên";
            return {
              rank: 0,
              userId: uid,
              name,
      ...
      <div className="p-6 md:p-8 max-w-3xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-app-text-secondary text-sm">So sánh di?m thi th? v?i c?ng d?ng</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 mb-5 flex-wrap">
          ...
          >
            ...
            <option value="mock">Thi mô ph?ng th?t</option>
            <option value="topic">Thi theo ch? d?</option>
            <option value="quick">Ôn t?p nhanh</option>
          </select>
        </div>
        ...
        <div className="bg-app-surface/50 border border-app-border rounded-2xl overflow-hidden">
          ...
          {loading ? (
            <div className="py-10 text-center text-app-text-muted text-sm">Ðang t?i b?ng x?p h?ng...</div>
          ) : entries.length === 0 ? (
            <div className="py-10 text-center">
              <i className="ri-trophy-line text-app-text-muted text-4xl mb-2 block"></i>
              <p className="text-app-text-secondary text-sm">Chua có ai thi EPS trong {periodLabels[period].toLowerCase()}</p>
              <p className="text-app-text-muted text-xs mt-1">Hãy là ngu?i d?u tiên!</p>
            </div>
          ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-app-text-muted text-xs">{entry.examType} • {entry.correct}/{entry.total} câu dúng</p>
                  </div>
