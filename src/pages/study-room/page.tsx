...
const LEADERBOARD_DATA: LeaderboardEntry[] = [
  { rank: 1, name: "Minh Tu?n", color: "app-accent-primary", level: "B2", studyMinutes: 342, wordsLearned: 187, streak: 47, room: "EPS-TOPIK Cùng Chinh Ph?c", badge: "ri-vip-crown-fill" },
  { rank: 2, name: "Thu Hà", color: "#fb923c", level: "B1", studyMinutes: 298, wordsLearned: 154, streak: 32, room: "EPS-TOPIK Cùng Chinh Ph?c", badge: "ri-medal-fill" },
  { rank: 3, name: "H?i Ðang", color: "#a78bfa", level: "C1", studyMinutes: 276, wordsLearned: 201, streak: 28, room: "TOPIK II Nâng Cao", badge: "ri-medal-fill" },
  ...
  { rank: 5, name: "Van Ð?c", color: "#60a5fa", level: "B1", studyMinutes: 198, wordsLearned: 132, streak: 23, room: "EPS-TOPIK Cùng Chinh Ph?c", badge: "ri-star-fill" },
  { rank: 6, name: "Th?o Nguyên", color: "#f472b6", level: "A2", studyMinutes: 187, wordsLearned: 89, streak: 19, room: "K-pop Lyrics Club", badge: "ri-star-fill" },
  { rank: 7, name: "Bích Ng?c", color: "#fbbf24", level: "B2", studyMinutes: 165, wordsLearned: 143, streak: 11, room: "T? V?ng Hán Hàn", badge: "ri-star-line" },
    ...
    <div className="bg-app-bg border border-app-border rounded-2xl overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-app-border">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-white font-bold text-sm">B?ng x?p h?ng phòng h?c</h3>
            <p className="text-app-text-muted text-[10px]">Tu?n này · C?p nh?t realtime</p>
          </div>
        </div>
...
function RoomCard({ room, onJoin }: RoomCardProps) {
  ...
  return (
    <div className="bg-app-bg border border-app-border rounded-2xl p-5 hover:border-app-border transition-all group">
      ...
      >
        {room.memberCount >= room.maxMembers ? "Phòng d?y" : "Tham gia"}
      </button>
    </div>
  );
}
...
function StudyRoomChat({ room, onLeave, profile }: StudyRoomChatProps) {
  ...
  const [members] = useState<RoomMember[]>(profile ? [{ id: "me", name: profile.display_name, level: "", color: "app-accent-primary", joinedAt: Date.now(), activity: "Ðang h?c", streak: 0, isHost: true }] : []);
    ...
    <div className="flex gap-4 h-[calc(100vh-160px)]">
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-app-border flex-shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={onLeave} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs text-app-text-secondary hover:text-white/70 cursor-pointer whitespace-nowrap transition-colors" style={{ backgroundColor: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
              <i className="ri-logout-box-line"></i>R?i phòng
            </button>
          </div>
        </div>
        ...
        <div className="px-4 py-3 border-t border-app-border flex-shrink-0">
          <div className="flex gap-2">
            ...
            <input
              ...
              placeholder="Nh?n tin v?i nhóm h?c... (Enter d? g?i)"
      ...
      <div className="w-64 flex flex-col gap-3">
        ...
        <div className="bg-app-bg border border-app-border rounded-2xl p-4">
          <h4 className="text-white/50 text-[10px] tracking-normal mb-3">Phiên h?c này</h4>
...
export default function StudyRoomPage() {
  ...
  if (activeRoom) {
    return (
      <DashboardLayout title={activeRoom.name} subtitle="Phòng h?c nhóm realtime">
  ...
  return (
    <DashboardLayout
      title="H?c cùng nhau"
      subtitle="Tham gia phòng h?c nhóm — cùng ti?n b? nhanh hon"
      actions={
        <button
        ...
        >
          <i className="ri-add-line"></i>T?o phòng h?c
        </button>
      }
    >
      {/* Stats */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Phòng dang m?", value: rooms.filter(r => r.memberCount < r.maxMembers).length, icon: "ri-door-open-line", color: "#34d399" },
          { label: "H?c viên online", value: rooms.reduce((s, r) => s + r.memberCount, 0), icon: "ri-group-line", color: "app-accent-primary" },
        ...
        <>
          ...
          <div className="flex items-center gap-3 mb-5">
            <div className="flex-1 flex items-center gap-2 bg-app-bg border border-app-border rounded-xl px-4 py-2.5">
              ...
              <input value={searchRoom} onChange={e => setSearchRoom(e.target.value)} placeholder="Tìm phòng h?c..." className="flex-1 bg-transparent text-white/70 text-sm outline-none placeholder-white/20" />
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowCreate(false)}>
          <div className="bg-app-bg border border-app-border rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-white font-bold text-base">T?o phòng h?c m?i</h3>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-app-text-secondary text-xs mb-1.5 block">Tên phòng h?c</label>
                <input value={newRoomName} onChange={e => setNewRoomName(e.target.value)} placeholder="VD: EPS-TOPIK Cùng Chinh Ph?c" className="w-full bg-app-card/50 border border-app-border rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-app-accent-primary/30 placeholder-white/20" />
              </div>
              <div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-app-text-muted text-xs">Ai cung có th? tham gia</p>
                </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={handleCreate} disabled={!newRoomName.trim()} className="flex-1 py-2.5 rounded-xl bg-app-accent-primary hover:bg-[#d4b43a] disabled:opacity-40 text-app-bg font-bold text-sm cursor-pointer whitespace-nowrap">T?o phòng</button>
              </div>
            </div>
          </div>
        </div>
