      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-white/5">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-white/30 text-[10px]">Tuần này · Cập nhật realtime</p>
          </div>
        </div>
        <div className="flex items-center gap-1 bg-white/4 border border-white/8 rounded-xl p-1">
          {([
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-[11px] font-medium transition-all cursor-pointer whitespace-nowrap ${sortKey === s.key ? "bg-[#e8c84a]/15 text-[#e8c84a]" : "text-white/35 hover:text-white/60"}`}
            >
              {s.label}
            </button>
          ))}
          ...
          <div key={entry.name} className={`flex items-center gap-4 px-5 py-3.5 transition-colors hover:bg-white/2 ${entry.rank <= 3 ? "bg-white/1" : ""}`}>
            <div className="w-7 flex-shrink-0 text-center">
              {entry.rank <= 3 ? (
                ...
                <span className="text-white/25 text-sm font-bold">{entry.rank}</span>
              )}
            </div>
            ...
            <div className="flex items-center gap-5 flex-shrink-0">
              ...
              <div className="text-right">
                ...
                <p className="text-white/25 text-[10px]">ngày</p>
              </div>
            </div>
          </div>
        ))}
    ...
    <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5 hover:border-white/10 transition-all group">
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex items-center gap-1.5">
          {!room.isPublic && <i className="ri-lock-line text-white/25 text-xs"></i>}
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          <span className="text-white/30 text-[10px]">{room.memberCount}/{room.maxMembers} thành viên</span>
...
function StudyRoomChat({ room, onLeave, profile }: { room: StudyRoom; onLeave: () => void; profile: { display_name: string } | null }) {
  ...
  return (
    <div className="flex gap-4 h-[calc(100vh-160px)]">
      {/* Left: Chat */}
      <div className="flex-1 flex flex-col bg-[#0f1117] border border-white/5 rounded-2xl overflow-hidden">
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-white/30 text-[10px]">{members.length} thành viên online</span>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl" style={{ backgroundColor: "rgba(232,200,74,0.08)", border: "1px solid rgba(232,200,74,0.15)" }}>
              <i className="ri-timer-line text-[#e8c84a] text-xs"></i>
              <span className="text-[#e8c84a] text-xs font-mono font-bold">{formatTimer(studyTimer)}</span>
              <button onClick={() => setTimerRunning(v => !v)} className="cursor-pointer">
                <i className={`${timerRunning ? "ri-pause-line" : "ri-play-line"} text-[#e8c84a]/60 text-xs`}></i>
              </button>
            </div>
            <button onClick={onLeave} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs text-white/40 hover:text-white/70 cursor-pointer whitespace-nowrap transition-colors" style={{ backgroundColor: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
        ...
        {/* Quick phrases */}
        {showPhrases && (
          <div className="px-4 py-2 border-t border-white/5 flex flex-wrap gap-1.5 flex-shrink-0">
            {QUICK_PHRASES.map(p => (
        ...
        <div className="px-4 py-3 border-t border-white/5 flex-shrink-0">
          <div className="flex gap-2">
            ...
            <button onClick={() => sendMessage(inputText)} disabled={!inputText.trim()} className="w-9 h-9 flex items-center justify-center rounded-xl cursor-pointer flex-shrink-0 disabled:opacity-40 transition-colors" style={{ backgroundColor: "#e8c84a" }}>
              <i className="ri-send-plane-fill text-sm text-[#0f1117]"></i>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="w-64 flex flex-col gap-3">
        <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-4 flex-1">
          <h4 className="text-white/50 text-[10px] uppercase tracking-widest mb-3">Thành viên ({members.length})</h4>
              ...
              <div key={m.id} className="flex items-center gap-2.5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    {m.isHost && <i className="ri-vip-crown-fill text-[#e8c84a] text-[9px]"></i>}
                  ...
                  <p className="text-white/25 text-[10px] truncate">{m.activity}</p>
                </div>
                <div className="flex items-center gap-0.5 flex-shrink-0">
                  <i className="ri-fire-line text-[#fb923c] text-[9px]"></i>
                  <span className="text-[9px] text-white/25">{m.streak}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-4">
          <h4 className="text-white/50 text-[10px] uppercase tracking-widest mb-3">Phiên học này</h4>
    ...
    >
      ...
      <div className="flex items-center gap-1 bg-white/4 border border-white/8 rounded-xl p-1 mb-5 w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all cursor-pointer whitespace-nowrap ${activeTab === "rooms" ? "bg-[#e8c84a]/15 text-[#e8c84a]" : "text-white/40 hover:text-white/70"}`}
        >
          <i className="ri-door-open-line text-sm"></i>Danh sách phòng
        </button>
        <button
          onClick={() => setActiveTab("leaderboard")}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all cursor-pointer whitespace-nowrap ${activeTab === "leaderboard" ? "bg-[#e8c84a]/15 text-[#e8c84a]" : "text-white/40 hover:text-white/70"}`}
        >
      ...
      {activeTab === "leaderboard" ? (
        ...
        <>
          ...
          <div className="flex items-center gap-3 mb-5">
            <div className="flex-1 flex items-center gap-2 bg-[#0f1117] border border-white/8 rounded-xl px-4 py-2.5">
            ...
            <span className="text-white/25 text-xs whitespace-nowrap">{filteredRooms.length} phòng</span>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowCreate(false)}>
          <div className="bg-[#0f1117] border border-white/10 rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setShowCreate(false)} className="w-7 h-7 flex items-center justify-center rounded-lg bg-white/5 text-white/40 cursor-pointer"><i className="ri-close-line text-sm"></i></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-white/40 text-xs mb-1.5 block">Tên phòng học</label>
                <input value={newRoomName} onChange={e => setNewRoomName(e.target.value)} placeholder="VD: EPS-TOPIK Cùng Chinh Phục" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-[#e8c84a]/30 placeholder-white/20" />
              </div>
              <div>
                <label className="text-white/40 text-xs mb-1.5 block">Chủ đề</label>
                <div className="flex flex-wrap gap-2">
                  {["EPS-TOPIK", "TOPIK I/II", "Seoul", "K-pop", "Hangul", "Hán Hàn"].map(t => (
                    <button key={t} onClick={() => setNewRoomTopic(t)} className={`px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer whitespace-nowrap transition-all ${newRoomTopic === t ? "bg-[#e8c84a] text-[#0f1117]" : "bg-white/5 text-white/40 hover:text-white/60"}`}>{t}</button>
                  ))}
                </div>
              </div>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-white/30 text-xs">Ai cũng có thể tham gia</p>
                </div>
                <button onClick={() => setNewRoomPublic(v => !v)} className={`w-12 h-6 rounded-full transition-all cursor-pointer relative ${newRoomPublic ? "bg-[#e8c84a]" : "bg-white/10"}`}>
                  <div className={`absolute top-0.5 w-5 h-5 rounded-full bg-white transition-all ${newRoomPublic ? "left-6" : "left-0.5"}`}></div>
                </button>
              </div>
              <div className="flex gap-3 pt-2">
                <button onClick={() => setShowCreate(false)} className="flex-1 py-2.5 rounded-xl border border-white/10 text-white/50 text-sm cursor-pointer whitespace-nowrap">Hủy</button>
                <button onClick={handleCreate} disabled={!newRoomName.trim()} className="flex-1 py-2.5 rounded-xl bg-[#e8c84a] hover:bg-[#d4b43a] disabled:opacity-40 text-[#0f1117] font-bold text-sm cursor-pointer whitespace-nowrap">Tạo phòng</button>
              </div>
            </div>
          </div>
        </div>
