...
export default function SeoulProgressPage() {
  ...
  // Ð?ng b? v?i key c?a trang giáo trình (kts_seoul_progress)
  const [completedMap] = useLocalStorage<Record<string, boolean>>("kts_seoul_progress", {});
  // Chuy?n d?i sang format LessonProgress d? tuong thích
      ...
      <div className="p-6 max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-app-text-secondary text-sm">Theo dõi % hoàn thành t?ng bài h?c theo t?ng cu?n sách</p>
          </div>
        </div>
        ...
        <div className="bg-app-surface/50 border border-app-border rounded-2xl p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-app-text-secondary text-sm">Toàn b? giáo trình Seoul 1A–4B</p>
            </div>
          ...
          <div className="h-3 bg-white/8 rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all duration-700 bg-gradient-to-r from-[app-accent-primary] to-[#34d399]" style={{ width: `${totalProg.pct}%` }}></div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
            {LEVEL_GROUPS.map(g => {
              ...
              return (
                <div key={g.group} className="bg-app-surface/50 rounded-xl p-3 text-center">
                  <p className="text-xs font-bold mb-1" style={{ color: g.color }}>{g.label}</p>
                  <p className="text-white text-xl font-black">{groupPct}%</p>
                  <p className="text-app-text-muted text-xs">{groupDone}/{groupLessons.length}</p>
                </div>
              );
            })}
          </div>
        ...
        {activeBook ? (
          <div className="space-y-4">
            ...
            <div className="bg-app-surface/50 border border-app-border rounded-xl p-5">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-app-text-secondary text-xs">{activeBook.description}</p>
                </div>
                <div className="text-right">
              ...
              <div className="space-y-2">
                {activeBook.lessons.map(lesson => {
                  ...
                  return (
                    <div key={lesson.id} className={`flex items-center gap-4 p-3 rounded-xl border transition-all ${prog.studied ? "border-opacity-30" : "border-app-border"}`}
                      style={prog.studied ? { borderColor: activeBook.color + "50", backgroundColor: activeBook.color + "08" } : {}}>
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${prog.studied ? "" : "bg-app-card/50"}`}
                        style={prog.studied ? { backgroundColor: activeBook.color + "20" } : {}}>
                        {prog.studied
                          ? <i className="ri-check-line text-sm" style={{ color: activeBook.color }}></i>
                          : <span className="text-app-text-muted text-xs font-bold">{lesson.lessonNumber}</span>
                        }
                      </div>
            ...
            <div className="flex gap-3">
              <button onClick={() => navigate("/seoul-textbook")} className="flex-1 py-3 rounded-xl border border-app-border text-white/60 hover:bg-app-card/50 text-sm cursor-pointer transition-all">
                <i className="ri-book-3-line mr-2"></i>M? giáo trình
              </button>
              <button onClick={() => navigate("/seoul-lesson-quiz")} className="flex-1 py-3 rounded-xl font-semibold text-sm cursor-pointer transition-all hover:opacity-90" style={{ backgroundColor: activeBook.color, color: "#000" }}>
                <i className="ri-file-list-3-line mr-2"></i>Thi th? theo bài
              </button>
            </div>
          </div>
        ) : (
          ...
          <div className="space-y-6">
            {LEVEL_GROUPS.map(g => (
              <div key={g.group}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-app-text-muted text-xs">CEFR {g.group === 1 ? "A1" : g.group === 2 ? "A2" : g.group === 3 ? "B1" : "B2"}</span>
                </div>
                  ...
                  {g.books.map(bookId => {
                    ...
                    return (
                      <button
                        ...
                        className="bg-app-surface/50 border border-app-border rounded-xl p-5 text-left hover:border-white/15 hover:bg-app-card/50 transition-all cursor-pointer group"
                      >
                        ...
                        <p className="text-app-text-muted text-xs mb-3 line-clamp-2">{book.description}</p>
                        ...
                        <div className="flex flex-wrap gap-1 mt-3">
                          ...
                          {book.lessons.length > 15 && <span className="text-app-text-muted text-xs">+{book.lessons.length - 15}</span>}
                        </div>
                      </button>
                    );
                  })}
            ...
            <div className="bg-app-surface/50 border border-app-border rounded-xl p-4">
              <h3 className="text-white font-semibold text-sm mb-3">Hành d?ng nhanh</h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <button onClick={() => navigate("/seoul-lesson-quiz")} className="flex flex-col items-center gap-2 p-3 rounded-xl bg-app-surface/50 hover:bg-white/6 cursor-pointer transition-all">
                  ...
                  <span className="text-white/50 text-xs text-center">Thi th? bài</span>
                </button>
            ...
            <div className="bg-app-accent-primary/5 border border-app-accent-primary/15 rounded-xl p-4">
              <div className="flex items-start gap-3">
                ...
                <p className="text-app-text-secondary text-xs">
                  Ti?n d? du?c t? d?ng c?p nh?t khi b?n h?c flashcard, làm bài thi th? ho?c luy?n t?p t?ng bài. Hãy h?c d?u d?n d? hoàn thành toàn b? giáo trình!
                </p>
              </div>
            </div>
          </div>
