      ...
      >
        ...
        <div className="flex items-center gap-4 mb-5">
          ...
          <div className="flex-1">
            ...
            <p className="text-app-text-secondary text-sm">{questions.length} câu h?i</p>
          </div>
        ...
        <div className="bg-app-surface/50 rounded-xl p-4 mb-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-center">
            <div>
              ...
              <p className="text-app-text-muted text-[10px]">Ðã làm</p>
            </div>
            <div>
              <p className="text-app-accent-success font-bold text-lg">{correct}</p>
              <p className="text-app-text-muted text-[10px]">Ðúng</p>
            </div>
            <div>
...
function TopicCard({
  ...
  return (
    ...
    >
      ...
      <p className="text-app-text-muted text-xs mb-4">{progress.total} câu h?i</p>
      ...
      <div className="flex items-center justify-between text-[10px]">
        <span className="text-app-text-muted">{progress.done}/{progress.total} dã làm</span>
        {progress.done > 0 && (
          <span className="text-app-accent-success/70">{progress.correct} dúng</span>
        )}
      </div>
    </button>
  );
...
const LEARNING_PATHS = [
  {
    ...
    color: "#34d399",
    desc: "Giao ti?p co b?n, van hóa, sinh ho?t hàng ngày",
    topics: ["greeting", "culture", "daily"],
    duration: "2-3 tu?n",
  },
  {
    id: "worker",
    label: "Lao d?ng ph? thông",
    icon: "ri-tools-line",
    color: "#fb923c",
    desc: "An toàn lao d?ng, noi làm vi?c, tình hu?ng kh?n c?p",
  ...
  },
  {
    ...
    desc: "Pháp lu?t, nghe hi?u, d?c hi?u — chu?n b? thi",
...
export default function EpsTopicsPage() {
  ...
  return (
    <DashboardLayout
      ...
      subtitle="L? trình h?c có h? th?ng — t? giao ti?p co b?n d?n pháp lu?t lao d?ng"
    >
      {/* Overall stats */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          { label: "T?ng câu h?i", value: epsQuestions.length, icon: "ri-survey-line", color: "app-accent-primary" },
          { label: "Ðã hoàn thành", value: `${overallPct}%`, icon: "ri-pie-chart-line", color: "#34d399" },
          { label: "Câu dúng", value: totalCorrect, icon: "ri-checkbox-circle-line", color: "#a78bfa" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6">
        ...
        <div className="space-y-4">
          <div>
            <h3 className="text-white font-semibold text-sm mb-3">Ch?n l? trình h?c</h3>
          ...
          <div className="bg-app-accent-primary/5 border border-app-accent-primary/15 rounded-xl p-4">
            <p className="text-app-accent-primary/80 text-xs font-semibold mb-2">G?i ý h?c hi?u qu?</p>
            <div className="space-y-2">
              {[
                { icon: "ri-time-line", text: "H?c 20-30 phút/ngày, d?u d?n hon h?c d?n" },
                { icon: "ri-repeat-line", text: "Ôn l?i câu sai ngay sau khi làm xong" },
                { icon: "ri-volume-up-line", text: "Nghe và d?c to câu h?i d? nh? lâu hon" },
                { icon: "ri-trophy-line", text: "Hoàn thành 1 ch? d? tru?c khi sang ch? d? khác" },
              ].map((tip, i) => (
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <div className="text-left">
                <p className="text-white/60 text-xs font-medium">Thi th? EPS (40 câu)</p>
                <p className="text-app-text-muted text-[10px]">Mô ph?ng d? thi th?t</p>
              </div>
            ...
            >
              ...
              <div className="text-left">
                <p className="text-white/60 text-xs font-medium">Ki?m tra d?u vào</p>
                <p className="text-app-text-muted text-[10px]">AI phân tích trình d?</p>
              </div>
              <i className="ri-arrow-right-line text-app-text-muted ml-auto"></i>
            </button>
          </div>
