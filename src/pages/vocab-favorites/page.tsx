  ...
  return (
    <div className="max-w-lg mx-auto space-y-5">
      <div className="bg-gradient-to-br from-app-surface to-[#0f1117] border border-app-accent-primary/20 rounded-2xl p-8 text-center">
        ...
        <h2 className="text-white font-bold text-2xl mb-1">{correct}/{results.length} t? dã nh?</h2>
        <p className="text-app-text-secondary text-sm mb-4">Ð?t {pct}%</p>
      ...
      {wrongItems.length > 0 && (
        <div className="bg-app-bg border border-app-border rounded-2xl p-5">
          <h3 className="text-white font-semibold text-sm mb-3">
            ...
            C?n ôn l?i ({wrongItems.length} t?)
          </h3>
      ...
      <div className="flex gap-3">
        {wrongItems.length > 0 && (
          <button onClick={() => onRestartWrong(wrongItems)} className="flex-1 py-3 rounded-xl bg-red-400/10 border border-red-400/20 text-red-400 font-bold text-sm cursor-pointer whitespace-nowrap hover:bg-red-400/15 transition-colors">
            <i className="ri-refresh-line mr-2"></i>Ôn t? chua nh? ({wrongItems.length})
          </button>
        )}
...
export default function VocabFavoritesPage() {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("study_progress").select("vocab_favorites").eq("user_id", user.id).maybeSingle().then(({ data }) => {
  ...
  return (
    <DashboardLayout
      title="Ôn t?p t? yêu thích"
      subtitle="Flashcard riêng cho các t? dã bookmark — luy?n d?n khi thu?c h?t"
      actions={
        <button onClick={() => navigate("/vocabulary")} className="flex items-center gap-2 bg-app-card/50 border border-app-border hover:bg-white/8 text-white/60 text-sm px-4 py-2.5 rounded-xl cursor-pointer whitespace-nowrap transition-colors">
          <i className="ri-bookmark-line"></i>Qu?n lý t? yêu thích
        </button>
      }
    >
      {favoriteItems.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h2 className="text-white font-bold text-xl mb-2">Chua có t? yêu thích</h2>
          <p className="text-app-text-secondary text-sm mb-6 max-w-sm leading-relaxed">
            Nh?n nút bookmark <i className="ri-bookmark-line text-app-accent-primary"></i> trên trang T? v?ng d? luu các t? khó vào dây
          </p>
          <button onClick={() => navigate("/vocabulary")} className="flex items-center gap-2 bg-app-accent-primary hover:bg-[#d4b43a] text-app-bg font-bold px-6 py-3 rounded-xl cursor-pointer whitespace-nowrap transition-colors">
            <i className="ri-translate-2"></i>Ð?n trang T? v?ng
          </button>
        </div>
      ) : sessionState === "idle" ? (
        /* Setup screen */
        <div className="max-w-lg mx-auto space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { label: "T? yêu thích", value: favoriteItems.length, icon: "ri-bookmark-fill", color: "app-accent-primary" },
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              {([["korean_to_vi", "ri-translate-2", "Hàn ? Vi?t", "Xem ti?ng Hàn, doán nghia"], ["vi_to_korean", "ri-font-size", "Vi?t ? Hàn", "Xem ti?ng Vi?t, doán ti?ng Hàn"]] as [CardMode, string, string, string][]).map(([m, icon, label, desc]) => (
                <button
                  key={m}
                  onClick={() => setMode(m)}
          ...
          >
            <i className="ri-play-fill mr-2"></i>B?t d?u ôn t?p ({favoriteItems.length} t?)
          </button>

          {/* Word list */}
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-white font-semibold text-sm">Danh sách t? yêu thích</h3>
