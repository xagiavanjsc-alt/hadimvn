  ...
  return (
    <DashboardLayout>
      <div className="min-h-screen bg-[#0f1117] text-white">
        {/* Header */}
        <div className="bg-gradient-to-r from-[#1a1d2e] to-[#0f1117] border-b border-white/5 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-white/40 text-sm">Roadmap 60 bài học trực quan</p>
              </div>
            </div>
              ...
              {(["roadmap", "grid", "topic"] as ViewMode[]).map(mode => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer whitespace-nowrap ${
                    viewMode === mode ? "bg-[#e8c84a]/20 text-[#e8c84a] border border-[#e8c84a]/30" : "text-white/40 hover:text-white/70 hover:bg-white/5"
                  }`}
                >
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-white/3 border border-white/8 rounded-xl p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[#e8c84a] font-bold">{stats.percent}%</span>
            </div>
            <div className="h-3 bg-white/8 rounded-full overflow-hidden">
              <div
                className="h-full rounded-full transition-all duration-700"
                style={{ width: `${stats.percent}%`, background: "linear-gradient(90deg, #e8c84a, #34d399)" }}
              ></div>
            </div>
            <div className="flex justify-between mt-2">
              <span className="text-white/30 text-xs">Bài 1</span>
              <span className="text-white/30 text-xs">Bài 60</span>
            </div>
          </div>
                  ...
                  <div key={book.label}>
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div>
                        ...
                        <p className="text-white/40 text-xs">Bài {book.range[0]}–{book.range[1]} • {bookDone}/{bookLessons.length} hoàn thành</p>
                      </div>
                      ...
                      {bookLessons.map((lesson, idx) => {
                        ...
                        return (
                          <div key={lesson.id} className="relative">
                            ...
                            {/* Tooltip */}
                            {isHovered && (
                              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 w-48 bg-[#1a1d2e] border border-white/10 rounded-lg p-2.5 shadow-xl pointer-events-none">
                                <p className="text-white text-xs font-semibold leading-tight mb-1">{lesson.titleVi}</p>
                                <p className="text-white/40 text-[10px] mb-1.5">{lesson.title}</p>
                                ...
                                {progress?.score > 0 && (
                                  <p className="text-white/40 text-[10px] mb-1.5">Điểm: {Math.round(progress.score)}%</p>
                                )}
                                <div className="flex items-center gap-1 bg-[#e8c84a]/15 rounded-md px-2 py-1">
                                  <i className="ri-play-circle-line text-[#e8c84a] text-[10px]"></i>
                                  <span className="text-[#e8c84a] text-[10px] font-medium">Bắt đầu học</span>
                                </div>
                              </div>
                            )}
                          </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all cursor-pointer whitespace-nowrap ${selectedTopic === "all" ? "bg-white/15 text-white" : "bg-white/5 text-white/40 hover:text-white/70"}`}
                >
                ...
                {EPS_LESSON_TOPICS.map(topic => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all cursor-pointer whitespace-nowrap ${selectedTopic === topic.id ? "text-white border" : "text-white/40 hover:text-white/70 bg-white/5"}`}
                ...
                {filteredLessons.map(lesson => {
                  ...
                  return (
                    <button
                      key={lesson.id}
                      onClick={() => navigate("/eps-lessons", { state: { openLessonId: lesson.id } })}
                      className="bg-white/3 border border-white/8 rounded-xl p-3 text-left hover:border-white/20 hover:bg-white/5 transition-all cursor-pointer group"
                    >
                      ...
                      <div className="flex items-center gap-1 mb-2">
                        ...
                        <span className="text-white/30 text-[10px]">{lesson.estimatedMinutes} phút</span>
                      </div>
                      {progress?.score > 0 && (
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <i className="ri-play-circle-line text-[#e8c84a] text-[10px]"></i>
                        <span className="text-[#e8c84a] text-[10px]">Bắt đầu học</span>
                      </div>
                    </button>
                  );
                })}
          ...
          {viewMode === "topic" && (
            <div className="space-y-6">
              {EPS_LESSON_TOPICS.map(topic => {
                ...
                return (
                  <div key={topic.id} className="bg-white/3 border border-white/8 rounded-xl p-4">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-white/40 text-xs">{topicDone}/{topicLessons.length} bài hoàn thành</p>
                      </div>
                      <div className="flex items-center gap-2">
                      ...
                      {topicLessons.map(lesson => {
                        ...
                        return (
                          <div key={lesson.id} className="relative group">
                            ...
                            {/* Mini tooltip */}
                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 z-50 w-36 bg-[#1a1d2e] border border-white/10 rounded-lg p-2 shadow-xl pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity">
                              <p className="text-white text-[10px] font-medium leading-tight mb-1">{lesson.titleVi}</p>
                              <div className="flex items-center gap-1">
                                <i className="ri-play-circle-line text-[#e8c84a] text-[9px]"></i>
                                <span className="text-[#e8c84a] text-[9px]">Bắt đầu học</span>
                              </div>
                            </div>
                          </div>
                        );
                ...
                return (
                  <>
                    <div className="bg-emerald-500/8 border border-emerald-500/15 rounded-lg p-3 text-center">
                      <p className="text-xl font-bold text-emerald-400">{completedVocab}</p>
                      <p className="text-emerald-400/60 text-[10px] mt-0.5">Đã hoàn thành</p>
                    </div>
                    <div className="bg-[#e8c84a]/8 border border-[#e8c84a]/15 rounded-lg p-3 text-center">
                      <p className="text-xl font-bold text-[#e8c84a]">{studiedVocab}</p>
                      <p className="text-[#e8c84a]/60 text-[10px] mt-0.5">Đã tiếp xúc</p>
                    </div>
                    <div className="bg-white/5 border border-white/10 rounded-lg p-3 text-center">
                      <p className="text-xl font-bold text-white/60">{totalVocab - studiedVocab}</p>
                      <p className="text-white/30 text-[10px] mt-0.5">Chưa học</p>
                    </div>
                  </>
                );
              })()}
                  ...
                  <div key={lesson.id} className="flex items-center gap-2 group">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-0.5">
                        ...
                        <span className="text-white/30 text-[10px] flex-shrink-0 ml-2">{vocabCount} từ</span>
                      </div>
                    ...
                    >
                      <i className="ri-play-circle-line text-[#e8c84a] text-xs"></i>
                    </button>
                  </div>
                );
              })}
          ...
          <div className="flex flex-wrap items-center gap-4 pt-2 border-t border-white/5">
            <span className="text-white/30 text-xs">Chú thích:</span>
            {[
