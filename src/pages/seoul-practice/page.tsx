    ...
    <div className="flex flex-col items-center gap-6">
      <div className="w-full max-w-lg">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-3">
            <span className="text-app-accent-success text-xs font-semibold">{known.size} thu?c</span>
            <span className="text-red-400 text-xs font-semibold">{unknown.size} chua thu?c</span>
          </div>
        </div>
      ...
      <div className="w-full max-w-lg cursor-pointer" onClick={() => setFlipped(f => !f)} style={{ perspective: "1000px" }}>
        <div className="relative w-full transition-transform duration-500" style={{ transformStyle: "preserve-3d", transform: flipped ? "rotateY(180deg)" : "rotateY(0deg)", minHeight: "240px" }}>
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8 rounded-2xl border border-app-border bg-app-bg" style={{ backfaceVisibility: "hidden" }}>
            ...
            <p className="text-app-text-muted text-xs mt-4">Click d? xem nghia</p>
          </div>
      ...
      <div className="flex items-center gap-4 w-full max-w-lg">
        <button onClick={handleUnknow} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border border-red-500/30 bg-red-500/10 text-red-400 font-semibold text-sm hover:bg-red-500/20 transition-colors cursor-pointer whitespace-nowrap">
          <i className="ri-close-line text-lg"></i>Chua thu?c
        </button>
        ...
        <button onClick={handleKnow} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 text-app-accent-success font-semibold text-sm hover:bg-emerald-500/20 transition-colors cursor-pointer whitespace-nowrap">
          <i className="ri-check-line text-lg"></i>Ðã thu?c
        </button>
      </div>
    </div>
  );
    ...
    <div className="flex flex-col gap-6 max-w-lg mx-auto">
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-app-accent-primary text-xs font-semibold">{score} di?m</span>
        </div>
        <div className="h-1.5 bg-app-card/50 rounded-full overflow-hidden">
      ...
      <div className="bg-app-bg border border-app-border rounded-2xl p-8 text-center">
        <p className="text-app-text-secondary text-xs mb-3">Nghia c?a t? này là gì?</p>
...
function ListenMode({ words, onFinish }: { words: VocabItem[]; onFinish: (score: number) => void }) {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-lg mx-auto">
      ...
      <div className="bg-app-bg border border-app-border rounded-2xl p-8 text-center">
        <p className="text-app-text-secondary text-xs mb-4">Nghe và di?n t? ti?ng Hàn</p>
        <p className="text-white/60 text-sm mb-2">Nghia: <span className="text-white font-semibold">{current.vietnamese}</span></p>
        ...
        <p className="text-app-text-muted text-xs mt-3">Ðã nghe {listenCount} l?n</p>
        {listenCount >= 2 && !showHint && <button onClick={() => setShowHint(true)} className="mt-2 text-app-accent-primary/60 text-xs underline cursor-pointer">Xem g?i ý</button>}
        {showHint && <p className="mt-2 text-app-accent-primary/70 text-sm font-medium">[{current.pronunciation}]</p>}
      </div>
      <div className="flex gap-3">
        <input type="text" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && !submitted && handleSubmit()} placeholder="Nh?p t? ti?ng Hàn..." disabled={submitted}
      ...
      {submitted && (
        <div className={`p-4 rounded-xl text-center ${isCorrect ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-red-500/10 border border-red-500/20"}`}>
          {isCorrect ? <p className="text-app-accent-success font-semibold">Chính xác!</p> : (
            <div><p className="text-red-400 font-semibold mb-1">Chua dúng</p><p className="text-white/60 text-sm">Ðáp án: <span className="text-white font-bold">{current.korean}</span></p></div>
          )}
        </div>
      )}
    </div>
...
function MatchingMode({ words, onFinish, onWrong }: { words: VocabItem[]; onFinish: (score: number) => void; onWrong?: (word: VocabItem) => void }) {
  ...
  return (
    <div className="flex flex-col gap-5 max-w-2xl mx-auto">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            ...
            <span className="text-app-text-secondary text-xs">{batchMatchedCount}/{batchWords.length} c?p</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-app-accent-primary text-xs font-semibold">{totalScore} di?m</span>
      ...
      <p className="text-app-text-secondary text-xs text-center">Ch?n t? ti?ng Hàn và nghia tuong ?ng d? ghép c?p</p>

      {/* Cards grid */}
      <div className="grid grid-cols-2 gap-4">
        {/* Korean column */}
        <div className="space-y-2">
          <p className="text-app-text-muted text-[10px] tracking-normal text-center mb-3">Ti?ng Hàn</p>
          {koreanCards.map(k => {
        ...
        {/* Vietnamese column */}
        <div className="space-y-2">
          <p className="text-app-text-muted text-[10px] tracking-normal text-center mb-3">Ti?ng Vi?t</p>
          {vietCards.map(v => {
      ...
      <div className="text-center">
        <p className="text-app-text-muted text-xs">Click t? ti?ng Hàn ? Click nghia tuong ?ng d? ghép c?p</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-6 max-w-lg mx-auto">
      ...
      <div className="bg-app-bg border border-app-border rounded-2xl p-6">
        <p className="text-app-text-secondary text-xs mb-4 text-center">Ði?n t? ti?ng Hàn vào ch? tr?ng</p>

        {/* Meaning hint */}
        <div className="flex items-center justify-center gap-2 mb-4">
          <span className="text-white/50 text-sm">Nghia:</span>
        ...
        {blankSentence ? (
          <div className="bg-app-surface/50 rounded-xl p-4 mb-4">
            <p className="text-app-text-muted text-[10px] tracking-normal mb-2">Câu ví d?</p>
      ...
      {submitted && (
        <div className={`p-4 rounded-xl text-center ${isCorrect ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-red-500/10 border border-red-500/20"}`}>
          {isCorrect ? (
            ...
            <div>
              <p className="text-red-400 font-semibold mb-1">Chua dúng</p>
              <p className="text-white/60 text-sm">Ðáp án: <span className="text-white font-bold">{current.korean}</span></p>
            </div>
          )}
        </div>
      )}
...
function ResultScreen({ score, total, mode, onRetry, onBack, bookColor }: { score: number; total: number; mode: PracticeMode; onRetry: () => void; onBack: () => void; bookColor: string }) {
  ...
  const grade = pct >= 90 ? "Xu?t s?c!" : pct >= 70 ? "T?t l?m!" : pct >= 50 ? "C? g?ng thêm!" : "C?n ôn l?i!";
  ...
  return (
    <div className="flex flex-col items-center gap-6 py-8 max-w-md mx-auto text-center">
      ...
      <div>
        ...
        <p className="text-white/50 text-sm">Ðúng {score}/{total} t?</p>
      </div>
      <div className="flex gap-3 w-full">
        ...
        <button onClick={onBack} className="flex-1 py-3 rounded-xl font-semibold text-sm transition-colors cursor-pointer whitespace-nowrap text-app-bg" style={{ backgroundColor: bookColor }}>
          <i className="ri-arrow-left-line mr-2"></i>Ch?n bài khác
        </button>
      </div>
    </div>
  );
...
function LeaderboardPanel() {
  ...
  const mockLeaderboard = [
    { rank: 1, name: "Nguy?n Th? Lan", xp: 2840, streak: 45, accuracy: 94, avatar: "ri-user-smile-line", color: "app-accent-primary" },
    { rank: 2, name: "Tr?n Van Minh", xp: 2310, streak: 32, accuracy: 89, avatar: "ri-user-3-line", color: "#34d399" },
    { rank: 3, name: "Lê Th? Hoa", xp: 1980, streak: 28, accuracy: 91, avatar: "ri-user-heart-line", color: "#fb923c" },
    { rank: 4, name: "Ph?m Qu?c B?o", xp: 1650, streak: 21, accuracy: 85, avatar: "ri-user-star-line", color: "#a78bfa" },
    { rank: 5, name: "B?n", xp: stats.totalSessions * 50 + stats.accuracy * 10, streak: 0, accuracy: stats.accuracy, avatar: "ri-user-line", color: "#06b6d4", isMe: true },
  ...
  return (
    <div className="bg-app-bg border border-app-border rounded-2xl p-5">
      <div className="flex items-center gap-2 mb-5">
        ...
        <p className="text-white font-semibold text-sm">B?ng x?p h?ng Seoul</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
        <div className="bg-app-surface/50 rounded-xl p-3 text-center">
          ...
          <p className="text-app-text-muted text-[10px] mt-0.5">Bu?i h?c</p>
        </div>
        <div className="bg-app-surface/50 rounded-xl p-3 text-center">
          <p className="text-app-accent-success text-xl font-bold">{stats.accuracy}%</p>
          <p className="text-app-text-muted text-[10px] mt-0.5">Ð? chính xác</p>
        </div>
        <div className="bg-app-surface/50 rounded-xl p-3 text-center">
          <p className="text-red-400 text-xl font-bold">{stats.wrongCount}</p>
          <p className="text-app-text-muted text-[10px] mt-0.5">T? c?n ôn</p>
        </div>
      </div>
...
export default function SeoulPracticePage() {
  ...
  const modeInfo: Record<PracticeMode, { icon: string; label: string; desc: string }> = {
    flashcard: { icon: "ri-stack-line", label: "Flashcard", desc: "L?t th? h?c t? v?ng" },
    quiz: { icon: "ri-survey-line", label: "Quiz", desc: "Tr?c nghi?m 4 dáp án" },
    listen: { icon: "ri-headphone-line", label: "Nghe & Ði?n", desc: "Nghe và gõ t? ti?ng Hàn" },
    matching: { icon: "ri-links-line", label: "Ghép c?p", desc: "N?i t? v?i nghia" },
    fillblank: { icon: "ri-edit-box-line", label: "Ði?n ch? tr?ng", desc: "Ði?n t? vào câu ví d?" },
  };

  return (
    <DashboardLayout
      title="Luy?n t?p Giáo trình Seoul"
      subtitle="Flashcard, Quiz, Nghe & Ði?n và Ghép c?p theo t?ng bài h?c"
      actions={
        <div className="flex items-center gap-2">
          {wrongWords.length > 0 && (
            <button onClick={() => navigate("/seoul-wrong-review")} className="flex items-center gap-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 text-sm px-4 py-2.5 rounded-xl transition-colors cursor-pointer whitespace-nowrap">
              <i className="ri-error-warning-line"></i>Ôn t? sai ({wrongWords.length})
            </button>
          )}
    ...
    >
      {!started ? (
        <div className="space-y-6">
          {/* Book selector */}
          <div>
            <p className="text-white/50 text-xs font-semibold tracking-normal mb-3">Ch?n cu?n sách</p>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {availableBooks.map(book => (
                <button key={book.id} onClick={() => { setSelectedBook(book); setSelectedLesson(null); }}
                  ...
                  <p className="text-app-text-muted text-xs mt-1">{book.lessons.length} bài có d? li?u</p>
                </button>
              ))}
            </div>
          </div>
          ...
          {selectedBook && (
            <div>
              <p className="text-white/50 text-xs font-semibold tracking-normal mb-3">Ch?n bài h?c</p>
                  ...
                  return (
                    <button key={lesson.id} onClick={() => setSelectedLesson(lesson)}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-app-text-muted text-xs truncate">{lesson.vocabulary.length} t? v?ng</p>
                      </div>
                      {best !== null && (
          ...
          {selectedLesson && (
            <div>
              <p className="text-white/50 text-xs font-semibold tracking-normal mb-3">Ch?n ch? d? luy?n t?p</p>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          {selectedLesson && selectedBook && (
            <div className="flex justify-center pt-2">
              <button onClick={handleStart} className="flex items-center gap-3 px-8 py-4 rounded-2xl font-bold text-base transition-all cursor-pointer whitespace-nowrap text-app-bg hover:opacity-90" style={{ backgroundColor: selectedBook.color }}>
                ...
                B?t d?u luy?n t?p ({selectedLesson.vocabulary.length} t?)
              </button>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {Object.keys(practiceResults).length > 0 && (
              <div className="bg-app-bg border border-app-border rounded-2xl p-5">
                <p className="text-white/50 text-xs font-semibold tracking-normal mb-4">K?t qu? g?n dây</p>
                <div className="space-y-2">
