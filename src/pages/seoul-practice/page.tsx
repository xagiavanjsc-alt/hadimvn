    ...
    <div className="flex flex-col items-center gap-6">
      <div className="w-full max-w-lg">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-3">
            <span className="text-emerald-400 text-xs font-semibold">{known.size} thuộc</span>
            <span className="text-red-400 text-xs font-semibold">{unknown.size} chưa thuộc</span>
          </div>
        </div>
        <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
          <div className="h-full rounded-full bg-gradient-to-r from-emerald-500 to-[#e8c84a] transition-all duration-500" style={{ width: `${progress}%` }} />
        </div>
      </div>
      ...
      <div className="w-full max-w-lg cursor-pointer" onClick={() => setFlipped(f => !f)} style={{ perspective: "1000px" }}>
        <div className="relative w-full transition-transform duration-500" style={{ transformStyle: "preserve-3d", transform: flipped ? "rotateY(180deg)" : "rotateY(0deg)", minHeight: "240px" }}>
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8 rounded-2xl border border-white/10 bg-[#0f1117]" style={{ backfaceVisibility: "hidden" }}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8 rounded-2xl border border-white/10" style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)", backgroundColor: `${current.bookColor}08`, borderColor: `${current.bookColor}30` }}>
    ...
    <div className="flex flex-col gap-6 max-w-lg mx-auto">
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-[#e8c84a] text-xs font-semibold">{score} điểm</span>
        </div>
        <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
          <div className="h-full rounded-full bg-[#e8c84a] transition-all duration-500" style={{ width: `${Math.round((index / total) * 100)}%` }} />
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col gap-6 max-w-lg mx-auto">
      ...
      <div className="bg-[#0f1117] border border-white/10 rounded-2xl p-8 text-center">
        ...
        {listenCount >= 2 && !showHint && <button onClick={() => setShowHint(true)} className="mt-2 text-[#e8c84a]/60 text-xs underline cursor-pointer">Xem gợi ý</button>}
        {showHint && <p className="mt-2 text-[#e8c84a]/70 text-sm font-medium">[{current.pronunciation}]</p>}
      </div>
...
function MatchingMode({ words, onFinish, onWrong }: { words: VocabItem[]; onFinish: (score: number) => void; onWrong?: (word: VocabItem) => void }) {
  ...
  const stateClass = (state: string, color: string) => {
    switch (state) {
      case "matched": return "opacity-30 cursor-default border-white/5 bg-white/2";
      case "correct": return `border-emerald-500/60 bg-emerald-500/15 text-emerald-300`;
      case "wrong": return `border-red-500/60 bg-red-500/15 text-red-300`;
      case "selected": return `border-white/40 bg-white/8 text-white`;
      default: return `border-white/10 bg-white/3 text-white/70 hover:border-white/20 hover:bg-white/6 cursor-pointer`;
    }
  };
  ...
  return (
    <div className="flex flex-col gap-5 max-w-2xl mx-auto">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            <span className="text-white/40 text-xs">Vòng {batchIndex + 1}/{totalBatches}</span>
            <span className="text-white/40 text-xs">·</span>
            <span className="text-white/40 text-xs">{batchMatchedCount}/{batchWords.length} cặp</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-[#e8c84a] text-xs font-semibold">{totalScore} điểm</span>
            <span className="text-white/30 text-xs">{Math.floor(elapsed / 60)}:{String(elapsed % 60).padStart(2, "0")}</span>
          </div>
        </div>
        <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
      ...
      <p className="text-white/40 text-xs text-center">Chọn từ tiếng Hàn và nghĩa tương ứng để ghép cặp</p>

      {/* Cards grid */}
      <div className="grid grid-cols-2 gap-4">
        {/* Korean column */}
        <div className="space-y-2">
          <p className="text-white/30 text-[10px] uppercase tracking-widest text-center mb-3">Tiếng Hàn</p>
          {koreanCards.map(k => {
            ...
            return (
              ...
              >
                ...
                {state === "matched" && <i className="ri-check-line text-emerald-400 ml-2"></i>}
              </button>
            );
          })}
        </div>
        ...
        {/* Vietnamese column */}
        <div className="space-y-2">
          <p className="text-white/30 text-[10px] uppercase tracking-widest text-center mb-3">Tiếng Việt</p>
          {vietCards.map(v => {
      ...
      <div className="text-center">
        <p className="text-white/20 text-xs">Click từ tiếng Hàn → Click nghĩa tương ứng để ghép cặp</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-6 max-w-lg mx-auto">
      ...
      <div className="bg-[#0f1117] border border-white/10 rounded-2xl p-6">
        <p className="text-white/40 text-xs mb-4 text-center">Điền từ tiếng Hàn vào chỗ trống</p>
        ...
        {blankSentence ? (
          <div className="bg-white/3 rounded-xl p-4 mb-4">
            <p className="text-white/30 text-[10px] uppercase tracking-widest mb-2">Câu ví dụ</p>
            <p className="text-white text-base leading-relaxed font-medium">
              {blankSentence.split("___").map((part, i, arr) => (
                <span key={i}>
                  {part}
                  {i < arr.length - 1 && (
                    <span className="inline-block px-3 py-0.5 mx-1 rounded-lg border-b-2 text-[#e8c84a] font-bold" style={{ borderColor: current.bookColor, backgroundColor: `${current.bookColor}10`, minWidth: "60px" }}>
            ...
            <p className="text-white/30 text-xs italic mt-2">{current.exampleVi}</p>
          </div>
        ) : (
          <div className="bg-white/3 rounded-xl p-4 mb-4 text-center">
            <p className="text-white/30 text-[10px] uppercase tracking-widest mb-2">Phiên âm</p>
        ...
        {/* Hint toggle */}
        {!showHint && !submitted && (
          <button onClick={() => setShowHint(true)} className="text-white/25 text-xs underline cursor-pointer hover:text-white/40 transition-colors block mx-auto">
            Xem gợi ý (phiên âm)
          </button>
        )}
        {showHint && !submitted && (
          <p className="text-center text-[#e8c84a]/60 text-sm">[{current.pronunciation}]</p>
        )}
      </div>
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          className="px-5 py-3 rounded-xl font-semibold text-sm transition-colors cursor-pointer whitespace-nowrap disabled:opacity-40 text-[#0f1117]"
      ...
      {submitted && (
        <div className={`p-4 rounded-xl text-center ${isCorrect ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-red-500/10 border border-red-500/20"}`}>
          {isCorrect ? (
            <p className="text-emerald-400 font-semibold">Chính xác! <span className="text-white font-bold">{current.korean}</span></p>
          ) : (
            <div>
  ...
  return (
    <div className="flex flex-col items-center gap-6 py-8 max-w-md mx-auto text-center">
      ...
      <div className="flex gap-3 w-full">
        <button onClick={onRetry} className="flex-1 py-3 rounded-xl border border-white/15 text-white/60 font-semibold text-sm hover:bg-white/5 transition-colors cursor-pointer whitespace-nowrap">
          <i className="ri-refresh-line mr-2"></i>Làm lại
        </button>
        <button onClick={onBack} className="flex-1 py-3 rounded-xl font-semibold text-sm transition-colors cursor-pointer whitespace-nowrap text-[#0f1117]" style={{ backgroundColor: bookColor }}>
    ...
    <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
      <div className="flex items-center gap-2 mb-5">
        <div className="w-8 h-8 flex items-center justify-center">
          <i className="ri-trophy-line text-[#e8c84a] text-lg"></i>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        ...
        <div className="bg-white/3 rounded-xl p-3 text-center">
          <p className="text-emerald-400 text-xl font-bold">{stats.accuracy}%</p>
          <p className="text-white/30 text-[10px] mt-0.5">Độ chính xác</p>
        </div>
        <div className="bg-white/3 rounded-xl p-3 text-center">
          <p className="text-red-400 text-xl font-bold">{stats.wrongCount}</p>
          <p className="text-white/30 text-[10px] mt-0.5">Từ cần ôn</p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {mockLeaderboard.map(user => (
          <div key={user.rank} className={`flex items-center gap-3 p-3 rounded-xl transition-colors ${(user as { isMe?: boolean }).isMe ? "bg-[#06b6d4]/8 border border-[#06b6d4]/20" : "bg-white/2"}`}>
            <div className={`w-7 h-7 flex items-center justify-center rounded-full flex-shrink-0 text-xs font-bold ${user.rank === 1 ? "bg-[#e8c84a]/20 text-[#e8c84a]" : user.rank === 2 ? "bg-white/10 text-white/60" : user.rank === 3 ? "bg-[#fb923c]/20 text-[#fb923c]" : "bg-white/5 text-white/30"}`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-white/25 text-[10px]">{user.accuracy}% chính xác</p>
            </div>
            <div className="text-right flex-shrink-0">
              <p className="text-white/60 text-xs font-bold">{user.xp.toLocaleString()} XP</p>
              {user.streak > 0 && <p className="text-[#e8c84a] text-[10px]"><i className="ri-fire-line"></i> {user.streak}</p>}
            </div>
          </div>
        ))}
      </div>
    ...
    <DashboardLayout
      ...
      actions={
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => navigate("/seoul-textbook")} className="flex items-center gap-2 bg-white/5 hover:bg-white/10 text-white/60 text-sm px-4 py-2.5 rounded-xl transition-colors cursor-pointer whitespace-nowrap">
    ...
    >
      {!started ? (
        <div className="space-y-6">
          {/* Book selector */}
          <div>
            <p className="text-white/50 text-xs font-semibold uppercase tracking-widest mb-3">Chọn cuốn sách</p>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {availableBooks.map(book => (
                <button key={book.id} onClick={() => { setSelectedBook(book); setSelectedLesson(null); }}
                  className={`p-4 rounded-xl border text-left transition-all cursor-pointer ${selectedBook?.id === book.id ? "border-white/25 bg-white/5" : "border-white/8 bg-white/2 hover:border-white/15"}`}>
                  ...
                  <p className="text-white/30 text-xs mt-1">{book.lessons.length} bài có dữ liệu</p>
                </button>
              ))}
            </div>
          </div>
          ...
          {selectedBook && (
            <div>
              <p className="text-white/50 text-xs font-semibold uppercase tracking-widest mb-3">Chọn bài học</p>
                ...
                {selectedBook.lessons.map(lesson => {
                  ...
                  return (
                    <button key={lesson.id} onClick={() => setSelectedLesson(lesson)}
                      className={`p-4 rounded-xl border text-left transition-all cursor-pointer flex items-center gap-4 ${selectedLesson?.id === lesson.id ? "border-white/25 bg-white/5" : "border-white/8 bg-white/2 hover:border-white/15"}`}>
                      <div className={`w-10 h-10 flex items-center justify-center rounded-xl flex-shrink-0 text-sm font-bold ${isCompleted ? "bg-emerald-500/15 text-emerald-400" : "bg-white/5 text-white/30"}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-white/30 text-xs truncate">{lesson.vocabulary.length} từ vựng</p>
                      </div>
                      {best !== null && (
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full flex-shrink-0 ${best >= 80 ? "bg-emerald-500/15 text-emerald-400" : "bg-[#e8c84a]/15 text-[#e8c84a]"}`}>{best}%</span>
                      )}
                    </button>
                  );
                })}
          ...
          {selectedLesson && (
            <div>
              <p className="text-white/50 text-xs font-semibold uppercase tracking-widest mb-3">Chọn chế độ luyện tập</p>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {(Object.entries(modeInfo) as [PracticeMode, typeof modeInfo[PracticeMode]][]).map(([m, info]) => (
                  <button key={m} onClick={() => setMode(m)}
                    className={`p-4 rounded-xl border text-center transition-all cursor-pointer ${mode === m ? "border-white/25 bg-white/5" : "border-white/8 bg-white/2 hover:border-white/15"}`}>
          ...
          {selectedLesson && selectedBook && (
            <div className="flex justify-center pt-2">
              <button onClick={handleStart} className="flex items-center gap-3 px-8 py-4 rounded-2xl font-bold text-base transition-all cursor-pointer whitespace-nowrap text-[#0f1117] hover:opacity-90" style={{ backgroundColor: selectedBook.color }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {Object.keys(practiceResults).length > 0 && (
              <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
                <p className="text-white/50 text-xs font-semibold uppercase tracking-widest mb-4">Kết quả gần đây</p>
                <div className="space-y-2">
                    ...
                    return (
                      <div key={lessonId} className="flex items-center gap-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-white/25 text-[10px]">{modeInfo[last.mode as PracticeMode]?.label} · {new Date(last.date).toLocaleDateString("vi-VN")}</p>
                        </div>
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${pct >= 80 ? "bg-emerald-500/15 text-emerald-400" : "bg-[#e8c84a]/15 text-[#e8c84a]"}`}>{pct}%</span>
                      </div>
                    );
                  })}
                </div>
        ...
        <div className="max-w-2xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <button onClick={handleBack} className="flex items-center gap-2 text-white/40 hover:text-white/70 text-sm cursor-pointer transition-colors">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-white/40 text-xs">{selectedLesson?.titleVi}</span>
            </div>
          </div>
