...
function StarBadge({ stars }: { stars?: number }) {
  const s = stars ?? 0;
  if (s === 0) return <span className="text-app-text-muted text-xs">Chua dánh giá</span>;
  return (
    <div className="flex items-center gap-0.5">
      ...
      <span className="text-app-text-secondary text-[10px] ml-1">
        {s === 5 ? "Xu?t s?c" : s === 4 ? "R?t t?t" : s === 3 ? "Khá" : s === 2 ? "Trung bình" : "Y?u"}
      </span>
    </div>
  );
}
        ...
        <>
          ...
          <div className="absolute left-0 right-0 top-full mt-1.5 z-50 bg-[#1a1d27] border border-app-border rounded-xl overflow-hidden max-h-72 flex flex-col">
            ...
            <div className="overflow-y-auto flex-1">
              {filtered.length === 0 ? (
                <div className="text-center py-6 text-app-text-muted text-xs">Không tìm th?y</div>
              ) : (
                filtered.map((l) => (
                  <button
                    key={l.song.rank}
...
export default function ComparePage() {
  ...
  return (
    <DashboardLayout
      title="So sánh bài h?c"
      subtitle="Ð?t 2 bài c?nh nhau — xem t? v?ng trùng và ch?n bài t?t hon cho ebook"
    >
      {approvedLessons.length < 2 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-app-text-secondary text-sm font-medium">C?n ít nh?t 2 bài h?c dã duy?t</p>
          <p className="text-app-text-muted text-xs mt-1 mb-5">Hi?n có {approvedLessons.length} bài — duy?t thêm trong K-pop Lesson</p>
          <a
            href="/melon"
        ...
        <div className="space-y-5">
          ...
          {!canCompare && (
            <div className="flex flex-col items-center justify-center py-16 bg-app-bg border border-app-border rounded-2xl text-center">
              ...
              <p className="text-app-text-muted text-sm">Ch?n 2 bài h?c d? b?t d?u so sánh</p>
              <p className="text-white/15 text-xs mt-1">Xem t? v?ng trùng nhau, ch?t lu?ng và g?i ý bài t?t hon</p>
            </div>
          )}
            ...
            <>
              ...
              {comparison.winner !== "tie" ? (
                <div className={`flex items-center gap-4 rounded-2xl px-6 py-4 border ${
                  ...
                  <div className="flex-1">
                    <p className={`font-bold text-sm ${comparison.winner === "A" ? "text-app-accent-primary" : "text-sky-400"}`}>
                      Bài {comparison.winner} du?c g?i ý cho ebook
                    </p>
                    <p className="text-app-text-secondary text-xs mt-0.5">
                      {comparison.winner === "A"
                        ? `"${lessonA.song.title}" — di?m t?ng h?p cao hon (sao, t? v?ng, d? dài truy?n)`
                        : `"${lessonB.song.title}" — di?m t?ng h?p cao hon (sao, t? v?ng, d? dài truy?n)`}
                    </p>
                  </div>
                ...
                <div className="flex items-center gap-4 bg-app-surface/50 border border-app-border rounded-2xl px-6 py-4">
                  ...
                  <div>
                    <p className="text-white/60 font-semibold text-sm">Hai bài tuong duong nhau</p>
                    <p className="text-app-text-muted text-xs mt-0.5">Ði?m chênh l?ch không dáng k? — ch?n theo c?m nh?n cá nhân</p>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: "T? v?ng trùng", value: comparison.shared.length, color: "text-app-accent-primary", icon: "ri-links-line" },
                  { label: "Ch? có ? A", value: comparison.onlyA.length, color: "text-app-accent-primary", icon: "ri-subtract-line" },
                  { label: "Ch? có ? B", value: comparison.onlyB.length, color: "text-sky-400", icon: "ri-subtract-line" },
                  { label: "T? l? trùng", value: `${comparison.overlapPct}%`, color: comparison.overlapPct > 50 ? "text-amber-400" : "text-app-accent-success", icon: "ri-percent-line" },
                ].map((stat) => (
              ...
              <div className="flex items-center gap-1 bg-app-bg border border-app-border rounded-xl p-1 w-fit">
                {([
                  ...
                  ["story", "Truy?n Chêm", "ri-book-open-line"],
                      ...
                      <div key={label} className={`${bg} border ${border} rounded-2xl p-5 space-y-4`}>
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between">
                            <span className="text-app-text-secondary text-xs">Ðánh giá</span>
                          ...
                          <div className="flex items-center justify-between">
                            <span className="text-app-text-secondary text-xs">Ð? dài truy?n</span>
                            <span className="text-white/60 text-xs">{lesson.story.length} ký t?</span>
                          </div>
                          <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center justify-between">
                            <span className="text-app-text-secondary text-xs">Ðã dang</span>
                            <span className={`text-xs ${lesson.publishedAt ? "text-app-accent-success" : "text-app-text-muted"}`}>
                              {lesson.publishedAt ? "Ðã dang" : "Chua dang"}
                            </span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-app-text-secondary text-xs">Ði?m t?ng h?p</span>
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-app-bg border border-app-border rounded-2xl p-5">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <p className="text-white/60 text-xs font-semibold">
                          T? v?ng trùng nhau ({comparison.shared.length} t?)
                        </p>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-app-bg border border-app-accent-primary/10 rounded-2xl p-5">
                      <div className="flex items-center gap-2 mb-3">
                        <span className="w-5 h-5 flex items-center justify-center bg-app-accent-primary/15 rounded text-app-accent-primary text-[10px] font-bold">A</span>
                        <p className="text-white/50 text-xs">Ch? có ? bài A ({comparison.onlyA.length} t?)</p>
                      </div>
                      {comparison.onlyA.length === 0 ? (
                        <p className="text-app-text-muted text-xs">Không có t? riêng</p>
                      ) : (
                        <div className="flex flex-wrap gap-1.5">
                    ...
                    <div className="bg-app-bg border border-sky-500/10 rounded-2xl p-5">
                      <div className="flex items-center gap-2 mb-3">
                        <span className="w-5 h-5 flex items-center justify-center bg-sky-500/15 rounded text-sky-400 text-[10px] font-bold">B</span>
                        <p className="text-white/50 text-xs">Ch? có ? bài B ({comparison.onlyB.length} t?)</p>
                      </div>
                    ...
                    ([label, lesson, border, color]) => (
                      <div key={label} className={`bg-app-bg border ${border} rounded-2xl p-5`}>
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <span className="text-app-text-muted text-[10px] ml-auto flex-shrink-0">{lesson.story.length} ký t?</span>
                        </div>
