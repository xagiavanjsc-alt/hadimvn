...
const WEEKLY_CHALLENGES: Challenge[] = [
  {
    ...
    description: "H?c 50 t? v?ng qua flashcard trong tu?n này",
    icon: "ri-stack-line",
    ...
    xpReward: 100,
    bonusXP: 50,
    bonusLabel: "Hoàn thành tru?c th? 5",
    category: "flashcard",
  },
  {
    id: "exam_3",
    title: "Chi?n binh EPS",
    description: "Hoàn thành 3 bài thi th? EPS d?y d? (40 câu)",
    icon: "ri-timer-line",
    color: "#34d399",
    target: 3,
    unit: "bài thi",
    xpReward: 150,
    bonusXP: 75,
    bonusLabel: "Ð?t 80%+ c? 3 bài",
    category: "exam",
  },
  {
    id: "streak_7",
    title: "Không b? ngày nào",
    description: "Duy trì streak h?c t?p 7 ngày liên ti?p trong tu?n",
    icon: "ri-fire-line",
    color: "#fb923c",
    target: 7,
    unit: "ngày",
    xpReward: 200,
    bonusXP: 100,
    bonusLabel: "Streak không b? gián do?n",
    category: "streak",
  },
  {
    id: "drill_5",
    title: "Luy?n ch? d? EPS",
    description: "Hoàn thành 5 bu?i luy?n thi theo ch? d?",
  ...
  },
  {
    ...
    description: "Ðang 3 bài chia s? kinh nghi?m trong c?ng d?ng",
    icon: "ri-group-line",
    color: "#f472b6",
    target: 3,
    unit: "bài dang",
    xpReward: 90,
    bonusXP: 30,
    bonusLabel: "Nh?n 10+ lu?t thích",
    category: "community",
  },
  {
    id: "vocab_topic",
    title: "T? v?ng theo ch? d?",
    description: "Hoàn thành 2 ch? d? flashcard EPS (y t?, giao thông...)",
    icon: "ri-translate-2",
    color: "#a78bfa",
    target: 2,
    unit: "ch? d?",
  ...
  },
  {
    id: "quiz_10",
    title: "Quiz Master",
    description: "Hoàn thành 10 câu quiz hàng ngày trong tu?n",
    icon: "ri-survey-line",
    color: "#84cc16",
    target: 10,
    unit: "câu quiz",
    ...
    <div className={`bg-app-bg border rounded-2xl p-5 transition-all ${completed ? "border-emerald-500/25" : "border-app-border hover:border-app-border"}`}>
      ...
      {completed ? (
        <button
          ...
          disabled
          title="Tính nang dang du?c nâng c?p - XP s? du?c c?ng t? d?ng t? ho?t d?ng h?c th?c t?"
        ...
        <div className="bg-app-card/30 border border-app-border rounded-xl px-3 py-2.5 text-center">
          <p className="text-app-text-muted text-[11px]">
            ...
            Ti?n d? t? d?ng c?p nh?t khi b?n hoàn thành ho?t d?ng h?c th?c t?
          </p>
        </div>
      )}
    </div>
...
export default function WeeklyChallengePage() {
  ...
  return (
    <DashboardLayout
      title="Th? thách hàng tu?n"
      subtitle={`Tu?n ${getWeekDates()} · Còn ${daysLeft} ngày`}
    >
      {/* Celebration overlay */}
      {showCelebration && (
        <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
          <div className="bg-app-bg border border-emerald-500/30 rounded-2xl px-8 py-6 text-center animate-bounce shadow-2xl">
            ...
            <p className="text-white font-bold text-lg">Th? thách hoàn thành!</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Th? thách hoàn thành", value: `${completedCount}/${WEEKLY_CHALLENGES.length}`, icon: "ri-checkbox-circle-line", color: "#34d399" },
          { label: "XP dã nh?n tu?n này", value: `${earnedXP}`, icon: "ri-star-line", color: "app-accent-primary" },
          { label: "XP có th? nh?n", value: `${totalXPAvailable}`, icon: "ri-gift-line", color: "#a78bfa" },
          { label: "Ngày còn l?i", value: daysLeft, icon: "ri-time-line", color: "#fb923c" },
        ].map(s => (
      ...
      {allDone && !currentProgress.claimedBonus && (
        <div className="mb-6 p-5 bg-gradient-to-r from-[app-accent-primary]/10 to-[#fb923c]/10 border border-app-accent-primary/25 rounded-2xl flex items-center gap-4">
          ...
          <div className="flex-1">
            <p className="text-white font-bold text-base">Hoàn thành t?t c? th? thách!</p>
            <p className="text-white/50 text-sm mt-0.5">B?n dã chinh ph?c toàn b? th? thách tu?n này. Nh?n thu?ng d?c bi?t!</p>
          </div>
          <button
            onClick={() => {
              awardXP({ type: "streak_bonus_7", amount: 300 });
              setWeekProgress(prev => ({ ...prev, claimedBonus: true }));
              setShowCelebration({ title: "Hoàn thành t?t c? th? thách!", xp: 300 });
              setTimeout(() => setShowCelebration(null), 3000);
            }}
      ...
      <div className="bg-app-bg border border-app-border rounded-2xl p-5 mb-6">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-white font-semibold text-sm">Ti?n d? t?ng th? tu?n này</h3>
