...
// ─── Main Admin Page ──────────────────────────────────────────────────────────
const WD_STATUS_CFG = {
  pending:  { label: "Chờ xử lý",  cls: "text-amber-400 bg-amber-500/10 border-amber-500/20" },
  approved: { label: "Đã duyệt",   cls: "text-blue-400 bg-blue-500/10 border-blue-500/20" },
  rejected: { label: "Từ chối",    cls: "text-rose-400 bg-rose-500/10 border-rose-500/20" },
  paid:     { label: "Đã thanh toán", cls: "text-app-accent-success bg-emerald-500/10 border-emerald-500/20" },
};
...
export default function AdminCTVPage() {
  ...
  const [tab, setTab] = useState<"ctv" | "withdrawals">("ctv");
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [{ data: ctvData }, { data: comData }, { data: wdData }] = await Promise.all([
      ...
      supabase.from("ctv_withdrawals").select("*").order("created_at", { ascending: false }),
    ]);
    setCtvs(ctvData || []);
  ...
  const handleUpdateWithdrawal = async (id: string, status: Withdrawal["status"], admin_note?: string) => {
    // "paid" goes through the atomic RPC (row lock + commission update +
    // paid_commission bump, all in one tx; idempotent on double-click).
    if (status === "paid") {
  ...
  const pendingWdAmount = withdrawals.filter(w => w.status === "pending").reduce((s, w) => s + w.amount, 0);
    ...
    <AdminLayout>
      <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 mb-6">
          {[
            { label: "Chờ duyệt CTV", value: totalPending, color: "text-amber-400", icon: "ri-time-line" },
            { label: "CTV hoạt động", value: totalActive, color: "text-app-accent-success", icon: "ri-user-star-line" },
            { label: "Tổng hoa hồng", value: fmt(totalCommission), color: "text-app-accent-primary", icon: "ri-coins-line" },
            { label: "Chờ thanh toán HH", value: fmt(unpaidCommission), color: "text-rose-400", icon: "ri-wallet-3-line" },
            { label: "YC rút chờ duyệt", value: pendingWithdrawals + " yêu cầu", color: "text-purple-400", icon: "ri-bank-card-line", onClick: () => setTab("withdrawals") },
            { label: "Tổng cần chuyển", value: fmt(pendingWdAmount), color: "text-blue-400", icon: "ri-money-dollar-circle-line", onClick: () => setTab("withdrawals") },
          ].map((s, i) => (
            <div key={i} onClick={(s as any).onClick} className={`bg-app-card/50 border border-app-border rounded-2xl p-4 ${(s as any).onClick ? "cursor-pointer hover:border-purple-500/30 transition-colors" : ""}`}>
        ...
        {/* Tabs */}
        <div className="flex gap-1 mb-5 bg-app-card/30 border border-app-border rounded-xl p-1 w-fit">
          {([
            { key: "ctv",         label: "Danh sách CTV",    icon: "ri-team-line" },
            { key: "withdrawals", label: `Yêu cầu rút tiền${pendingWithdrawals > 0 ? ` (${pendingWithdrawals})` : ""}`, icon: "ri-bank-card-line" },
          ] as const).map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-semibold cursor-pointer transition-all ${tab === t.key ? "bg-rose-500 text-white" : "text-app-text-muted hover:text-white/70"}`}>
              <i className={t.icon}></i>{t.label}
            </button>
          ))}
        </div>

        {/* Filters (CTV tab only) */}
        {tab === "ctv" && <></> /* spacer — filters below */}
        {tab === "withdrawals" && (
          <div className="bg-app-card/50 border border-app-border rounded-2xl overflow-hidden mb-4">
            <div className="px-5 py-3.5 border-b border-app-border flex items-center justify-between">
              <h3 className="text-sm font-bold text-white/80">Yêu cầu rút tiền</h3>
              <span className="text-xs text-app-text-muted">{withdrawals.length} yêu cầu</span>
            </div>
            {withdrawals.length === 0 ? (
              <div className="py-12 text-center text-app-text-muted">
                <i className="ri-bank-card-line text-3xl mb-2 block opacity-30"></i>
                <p className="text-sm">Không có yêu cầu nào</p>
              </div>
            ) : (
              <div className="divide-y divide-white/5">
                {withdrawals.map(w => {
                  const ctv = ctvs.find(c => c.id === w.ctv_id);
                  const cfg = WD_STATUS_CFG[w.status];
                  return (
                    <div key={w.id} className="px-5 py-4 flex items-center gap-4 flex-wrap">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-white/90">{ctv?.display_name || "—"}</p>
                        <p className="text-xs text-app-text-muted">{ctv?.phone} · {new Date(w.created_at).toLocaleDateString("vi-VN")}</p>
                        {w.note && <p className="text-xs text-white/50 mt-0.5 italic">"{w.note}"</p>}
                      </div>
                      <div className="text-right">
                        <p className="text-base font-black text-white/90">{fmt(w.amount)}</p>
                        <div className="text-xs text-app-text-muted space-y-0.5 mt-0.5">
                          <p>{w.bank_info?.bank} · <span className="font-mono">{w.bank_info?.account_number}</span></p>
                          <p>{w.bank_info?.account_name}</p>
                        </div>
                      </div>
                      <div className="flex flex-col gap-1.5 items-end flex-shrink-0">
                        <span className={`text-[10px] px-2 py-0.5 rounded-full border font-semibold ${cfg.cls}`}>{cfg.label}</span>
                        {w.status === "pending" && (
                          <div className="flex gap-1.5">
                            <button onClick={() => handleUpdateWithdrawal(w.id, "paid")}
                              className="px-3 py-1.5 rounded-lg text-xs bg-emerald-500/15 border border-emerald-500/30 text-app-accent-success hover:bg-emerald-500/25 cursor-pointer transition-all font-semibold">
                              <i className="ri-check-line mr-1"></i>Đã CK
                            </button>
                            <button onClick={() => handleUpdateWithdrawal(w.id, "rejected", "Yêu cầu bị từ chối")}
                              className="px-3 py-1.5 rounded-lg text-xs bg-rose-500/15 border border-rose-500/30 text-rose-400 hover:bg-rose-500/25 cursor-pointer transition-all font-semibold">
                              <i className="ri-close-line mr-1"></i>Từ chối
                            </button>
                          </div>
                        )}
                        {w.admin_note && <p className="text-[10px] text-rose-400">{w.admin_note}</p>}
                      </div>
                    </div>
                  );
                })}
