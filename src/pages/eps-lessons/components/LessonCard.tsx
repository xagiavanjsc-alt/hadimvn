...
export default function LessonCard({ lesson, isCompleted, score, onClick }: LessonCardProps) {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left p-3 md:p-4 rounded-xl border transition-all cursor-pointer group ${isCompleted ? "border-emerald-500/20 bg-emerald-500/3 hover:border-emerald-500/35" : "border-app-border bg-white/2 hover:border-white/15 hover:bg-white/4"}`}
    >
      <div className="flex items-center gap-3">
        <div className={`w-8 h-8 md:w-9 md:h-9 flex items-center justify-center rounded-xl flex-shrink-0 text-xs md:text-sm font-bold ${isCompleted ? "bg-app-accent-success/15 text-app-accent-success" : "bg-app-card/50 text-app-text-muted"}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="hidden sm:block text-app-text-muted text-xs truncate mt-0.5">{lesson.title}</p>
          <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
            ...
            <span className="hidden sm:flex text-[9px] text-app-text-muted items-center gap-0.5">
              <i className="ri-time-line"></i>{lesson.estimatedMinutes}p
            </span>
            {isCompleted && score !== undefined && (
              <span className="text-[9px] text-app-accent-success font-bold">{score}/{lesson.exercises.length} đúng</span>
            )}
          </div>
        </div>
