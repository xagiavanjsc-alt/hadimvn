...
export interface EbookMeta {
  ...
  contactInfo?: string; // Thông tin liên h? trang k?t
...
const DEFAULT_META: EbookMeta = {
  ...
  description: "Tuy?n t?p bài h?c ti?ng Hàn du?c biên so?n t? các bài hát K-pop dang hot, giúp b?n h?c t? v?ng và ng? pháp m?t cách t? nhiên và thú v?.",
  foreword: "Chào b?n d?c thân m?n!\n\nCu?n ebook này du?c biên so?n v?i tình yêu dành cho ti?ng Hàn và K-pop. M?i bài h?c là m?t câu chuy?n nh? — noi ngôn ng? và âm nh?c hòa quy?n d? giúp b?n h?c ti?ng Hàn m?t cách t? nhiên nh?t.\n\nHãy d?c ch?m, c?m nh?n t?ng t?, và d?ng quên nghe l?i bài hát sau m?i bài h?c nhé!\n\nChúc b?n h?c vui!",
...
export default function EbookPage() {
  ...
  const handleSelectHighQuality = () => {
    ...
    showToast(`Ðã ch?n ${highRanks.length} bài 4-5 sao`);
  };
  ...
  const handleExportPDF = useCallback(async () => {
    if (selectedLessons.length === 0) {
      showToast("Ch?n ít nh?t 1 bài h?c d? xu?t PDF", "error");
      return;
    }
    if (!canExport) {
    ...
    setExporting(true);
    consume();
    showToast("Ðang chu?n b? xu?t PDF... Trình duy?t s? m? h?p tho?i in");
    setTimeout(() => {
      window.print();
      setExporting(false);
    }, 800);
  ...
  const handleBatchExportGroup = useCallback((groupLessons: ApprovedLesson[], groupName: string) => {
    ...
    const groupMeta = { ...meta, title: `${meta.title} — ${groupName}`, subtitle: `${groupLessons.length} bài h?c` };
    ...
    showToast(`Ðang xu?t ebook: ${groupName}`);
  ...
  const handleAddFreeLesson = useCallback((lesson: ApprovedLesson) => {
    ...
    showToast(`Ðã thêm "${lesson.song.title}" vào ebook!`);
  ...
  const tabs: { id: EbookTab; label: string; icon: string }[] = [
    ...
    { id: "lessons", label: "Ch?n bài h?c", icon: "ri-list-check-2" },
    ...
    >
      ...
      {showLimitModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowLimitModal(false)}>
          <div className="bg-app-bg border border-app-border rounded-2xl p-7 max-w-sm w-full mx-4" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="text-white font-bold text-lg text-center mb-2">Ðã h?t lu?t xu?t PDF</h3>
            <p className="text-app-text-secondary text-sm text-center mb-5 leading-relaxed">
              {isVip
                ? `Gói VIP cho phép xu?t ${VIP_LIMIT} l?n/tháng. B?n dã dùng h?t lu?t tháng này. Lu?t m?i s? du?c reset vào d?u tháng sau.`
                : `Gói Free ch? cho phép xu?t ${FREE_LIMIT} l?n/tháng. Nâng c?p VIP d? xu?t ${VIP_LIMIT} l?n/tháng!`
              }
            </p>
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="bg-app-surface/50 rounded-xl p-3 text-center">
                ...
                <p className="text-app-text-muted text-[10px]">m?i tháng</p>
              </div>
              <div className="bg-app-accent-primary/8 border border-app-accent-primary/20 rounded-xl p-3 text-center">
                ...
                <p className="text-app-accent-primary/40 text-[10px]">m?i tháng</p>
              </div>
            </div>
            {!isVip && (
      ...
      {approvedLessons.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-app-text-secondary text-sm font-medium">Chua có bài h?c nào</p>
          <p className="text-app-text-muted text-xs mt-1 mb-5">Duy?t bài h?c trong trang K-pop Lesson tru?c</p>
          <a
            href="/melon"
