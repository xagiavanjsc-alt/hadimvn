...
function StarBadge({ stars }: { stars: number }) {
  ...
  return (
    <span className={`flex items-center gap-0.5 text-[10px] font-medium ${stars >= 4 ? "text-app-accent-primary" : "text-app-text-muted"}`}>
    ...
    <div className="space-y-4">
      ...
      <div className="bg-app-bg border border-app-border rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-app-text-muted text-xs">{selectedRanks.length}/{lessons.length} dã ch?n</span>
        </div>
      ...
      {orderedSelected.length > 0 && (
        <div className="bg-app-bg border border-app-border rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-app-text-muted text-[10px] ml-auto">Kéo d? s?p x?p</span>
          </div>
          <div className="space-y-1.5">
            {orderedSelected.map((lesson, idx) => (
              <div
                key={lesson.song.rank}
                className="flex items-center gap-2 bg-app-accent-primary/5 border border-app-accent-primary/15 rounded-lg px-3 py-2.5 group"
              >
                <span className="text-app-accent-primary/60 text-[10px] font-bold w-5 text-center">{idx + 1}</span>
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    disabled={idx === 0}
                    className="w-5 h-5 flex items-center justify-center text-app-text-muted hover:text-white/70 disabled:opacity-20 cursor-pointer transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="w-5 h-5 flex items-center justify-center text-app-text-muted hover:text-red-400 cursor-pointer transition-colors"
                  >
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg border transition-all cursor-pointer text-left ${
                  isSelected
                    ...
                    : "bg-app-surface/50 border-app-border hover:bg-app-card/50 hover:border-app-border"
                }`}
              >
                ...
                <span className="text-app-text-muted text-[10px] w-5">#{lesson.song.rank}</span>
                <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {lesson.publishedAt && (
                    <span className="text-app-accent-success/50 text-[9px]">
