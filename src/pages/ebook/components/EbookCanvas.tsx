...
/** Parse vocab word: tách ti?ng Hàn, phiên âm, nghia */
...
/** Tính s? trang th?c c?a m?i bài (1 ho?c 2 trang) */
function calcLessonPageCount(lesson: ApprovedLesson): number {
  ...
  // Heuristic: n?u story dài > 400 ký t? ho?c vocab > 6 t? ho?c có grammar ? 2 trang
  if (storyLen > 400 || vocabCount > 6 || hasGrammar) return 2;
  return 1;
}

/** Tính page offset cho t?ng bài */
      ...
      <div style={{ backgroundColor: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "12px", overflow: "hidden" }}>
        <div style={{ backgroundColor: meta.coverColor, padding: "24px", minHeight: "200px", display: "flex", flexDirection: "column", justifyContent: "space-between", position: "relative" }}>
          ...
          <div>
            ...
            <h2 style={{ fontSize: "14px", fontWeight: 700, lineHeight: 1.3, marginBottom: "4px", color: accent }}>{meta.title || "Tiêu d? ebook"}</h2>
          ...
          <div style={{ marginTop: "16px" }}>
            ...
            <div style={{ marginTop: "12px", fontSize: "9px", fontWeight: 600, color: accent }}>{lessons.length} bài h?c</div>
          </div>
        </div>
        {lessons.length > 0 && (
        ...
        {lessons.length === 0 && (
          <div style={{ padding: "16px", textAlign: "center", color: "rgba(255,255,255,0.2)", fontSize: "10px" }}>Ch?n bài h?c d? xem m?c l?c</div>
        )}
      </div>
      ...
      <div id="ebook-print-area" ref={printRef} style={{ display: "none" }}>
        ...
        <div className="ebook-page" style={{ ...PAGE_STYLE, backgroundColor: meta.coverColor, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <p style={{ color: "rgba(255,255,255,0.2)", fontSize: "9pt" }}>{lessons.length} bài h?c</p>
        ...
        {/* TABLE OF CONTENTS — tính sau khi bi?t page offsets */}
        ...
        {/* LESSON PAGES — m?i bài có th? 1 ho?c 2 trang */}
        {lessons.map((lesson, idx) => {
              ...
              <div key={`${lesson.song.rank}-p1`} className="ebook-page" style={{ ...PAGE_STYLE }}>
                ...
                <div style={{ marginBottom: "20px" }}>
                  <div className="section-label">
                    ...
                    <span className="section-title">Truy?n Chêm</span>
                  </div>
                  <div
                ...
                {vocabPage1.length > 0 && (
                  <div style={{ marginBottom: lessonPages === 1 ? "20px" : "0" }}>
                    <div className="section-label">
                      ...
                      <span className="section-title">T? v?ng c?t lõi</span>
                    </div>
              ...
              {lessonPages === 2 && (
                <div key={`${lesson.song.rank}-p2`} className="ebook-page" style={{ ...PAGE_STYLE }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "18px" }}>
                    <p style={{ fontSize: "8pt", fontWeight: 700, textTransform: "", letterSpacing: "0.14em", color: accent }}>Bài {idx + 1} — ti?p theo</p>
                    <div style={{ flex: 1, height: "1px", backgroundColor: "#f0f0f0" }} />
                    <p style={{ fontSize: "8pt", color: "#ccc" }}>{lesson.song.title}</p>
                  </div>

                  {/* Vocab trang 2 (n?u còn) */}
                  {vocabPage2.length > 0 && (
                    <div style={{ marginBottom: "22px" }}>
                      <div className="section-label">
                        <div className="section-bar" style={{ backgroundColor: "#2c7a4b" }} />
                        <span className="section-title">T? v?ng c?t lõi (ti?p)</span>
                      </div>
                      <div className="vocab-grid">
                        {vocabPage2.map((v, i) => {
                          const parsed = parseVocab(v);
        ...
        <div className="ebook-page" style={{ ...PAGE_STYLE, backgroundColor: meta.coverColor, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center" }}>
          ...
          <h2 style={{ fontSize: "24pt", fontWeight: 700, color: accent, marginBottom: "12px" }}>C?m on b?n dã d?c!</h2>
          <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "12pt", marginBottom: "40px", maxWidth: "320px", lineHeight: 1.8 }}>
            Hy v?ng ebook này giúp b?n ti?n b? ti?ng Hàn m?i ngày. H?n g?p l?i ? t?p ti?p theo!
          </p>
          {(meta.contactInfo || meta.website) && (
            <div style={{ backgroundColor: "rgba(255,255,255,0.05)", borderRadius: "12px", padding: "24px 32px", border: "1px solid rgba(255,255,255,0.08)", maxWidth: "360px", width: "100%" }}>
              <p style={{ color: accent, fontSize: "9pt", fontWeight: 700, textTransform: "", letterSpacing: "0.12em", marginBottom: "14px" }}>Liên h? & Theo dõi</p>
