...
function parseGrammar(raw: string): GrammarPoint[] {
  ...
  for (const line of lines) {
    // Detect example lines: starts with "?:" / "??:" / "VD:" / "Ví d?:" / "Ex:"
    const isExample = /^(?\s*[::]|??\s*[::]|VD\s*[::]|Ví d?\s*[::]|Ex\s*[::])/i.test(line);
    ...
    if (isHeader) {
      ...
      current.example = line.replace(/^(?\s*[::]|??\s*[::]|VD\s*[::]|Ví d?\s*[::]|Ex\s*[::])\s*/i, "").trim();
    } else {
      if (current) {
...
/** Tính page offset cho t?ng bài — kh?p v?i EbookCanvas */
function calcPageOffsets(lessons: ApprovedLesson[], hasForeword: boolean): number[] {
  // Cover=1, Foreword=2(n?u có), TOC=ti?p theo, bài h?c b?t d?u sau TOC
...
function buildHtmlContent(meta: EbookMeta, lessons: ApprovedLesson[], template: EbookTemplate): string {
      ...
      <div style="margin-bottom:${gi < points.length - 1 ? "18px" : "0"};padding-bottom:${gi < points.length - 1 ? "18px" : "0"};border-bottom:${gi < points.length - 1 ? `1px solid ${borderColor}` : "none"};">
        ...
        ${gp.example ? `
          <div style="background:${darkMode ? "rgba(255,255,255,0.04)" : "#f9f6ee"};border-left:3px solid ${accent};border-radius:0 6px 6px 0;padding:8px 12px;margin-top:6px;">
            <div style="font-size:8pt;font-weight:700;text-transform:;letter-spacing:0.08em;color:${subColor};margin-bottom:4px;">Ví d?</div>
  ...
  const lessonPagesHtml = lessons.map((lesson, idx) => {
    ...
    if (isDark) {
      ...
      <div class="page" style="background:#0f1117;color:#e8e8e8;">
        ...
        <div style="margin-bottom:20px;">
          ${sectionLabel("Truy?n Chêm", accent, true)}
        ...
        <div style="margin-bottom:${lessonPageCount === 1 ? "20px" : "0"};">
          ${sectionLabel("T? v?ng c?t lõi", "#e05555", true)}
        ...
        <div style="margin-bottom:20px;">
          ${sectionLabel("Ði?m ng? pháp", "#d4a017", true)}
      ...
      <div class="page" style="background:#0f1117;color:#e8e8e8;">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:18px;">
          <span style="font-size:8pt;font-weight:700;text-transform:;letter-spacing:0.12em;color:${accent};">Bài ${idx + 1} — ti?p theo</span>
          <div style="flex:1;height:1px;background:rgba(255,255,255,0.06);"></div>
          <span style="font-size:8pt;color:rgba(255,255,255,0.2);">${lesson.song.title}</span>
        </div>
        ${vocabPage2.length > 0 ? `
        <div style="margin-bottom:22px;">
          ${sectionLabel("T? v?ng c?t lõi (ti?p)", "#e05555", true)}
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
    ...
    <div class="page">
      ...
      ${lessonPageCount === 1 && grammarPoints.length > 0 ? `
      <div style="margin-bottom:20px;">
        ${sectionLabel("Ði?m ng? pháp", "#d4a017")}
    ...
    <div class="page">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:18px;">
        <span style="font-size:8pt;font-weight:700;text-transform:;letter-spacing:0.14em;color:${accent};">Bài ${idx + 1} — ti?p theo</span>
        <div style="flex:1;height:1px;background:#f0f0f0;"></div>
        <span style="font-size:8pt;color:#ccc;">${lesson.song.title}</span>
      </div>
      ${vocabPage2.length > 0 ? `
      <div style="margin-bottom:22px;">
        ${sectionLabel("T? v?ng c?t lõi (ti?p)", "#2c7a4b")}
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
  ...
  <div class="page" style="background:${meta.coverColor};display:flex;flex-direction:column;justify-content:center;align-items:center;text-align:center;">
    ...
    <h2 style="font-size:24pt;font-weight:700;color:${accent};margin-bottom:12px;">C?m on b?n dã d?c!</h2>
    <p style="color:rgba(255,255,255,0.5);font-size:12pt;margin-bottom:40px;max-width:320px;line-height:1.8;">Hy v?ng ebook này giúp b?n ti?n b? ti?ng Hàn m?i ngày.</p>
    ...
    <div style="background:rgba(255,255,255,0.05);border-radius:12px;padding:24px 32px;border:1px solid rgba(255,255,255,0.08);max-width:360px;width:100%;">
      <div style="color:${accent};font-size:9pt;font-weight:700;text-transform:;letter-spacing:0.12em;margin-bottom:14px;">Liên h? & Theo dõi</div>
...
<div class="toolbar no-print">
  <div>
    ...
    <div class="toolbar-sub">${meta.author} · ${lessons.length} bài h?c</div>
  </div>
...
<div class="content">
  ...
  <div class="page" style="background:${meta.coverColor};display:flex;flex-direction:column;justify-content:space-between;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;">
      <span style="color:rgba(255,255,255,0.2);font-size:9pt;">${lessons.length} bài h?c</span>
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-black/90 backdrop-blur-sm">
          ...
          <div className="flex items-center justify-between px-6 py-3.5 bg-app-bg border-b border-app-border flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-app-text-secondary text-xs">{lessons.length} bài h?c · Xem tru?c tru?c khi in</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button
          ...
          <div className="flex items-center gap-3 px-6 py-2.5 bg-sky-500/5 border-b border-sky-500/10 flex-shrink-0">
            ...
            <p className="text-sky-400/70 text-xs">
              Ðây là b?n xem tru?c d?y d?. Nh?n <strong className="text-sky-400">In / Luu PDF</strong> d? in ho?c luu thành file PDF. Nh?n <strong className="text-sky-400">T?i file HTML</strong> d? chia s? online.
            </p>
          </div>
