...
export default function EpsSmartFlashcardPage() {
  ...
  if (phase === "select") {
    return (
      <DashboardLayout title="Flashcard thông minh EPS" subtitle="T? d?ng uu tiên t? chua thu?c · Phát âm ti?ng Hàn · Spaced Repetition">
            ...
            <div className="bg-gradient-to-r from-app-surface to-[#0f1117] border border-app-accent-primary/15 rounded-2xl p-6">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <h3 className="text-white font-bold text-base">H?c thông minh hôm nay</h3>
                  <p className="text-app-text-secondary text-sm mt-0.5">
                    <span className="text-app-accent-primary font-semibold">{stats.dueToday}</span> t? c?n ôn hôm nay
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                {selectedTopics.length > 0 && (
                  ...
                  >
                    ...
                    H?c {selectedTopics.length} ch? d? dã ch?n
                  </button>
                )}
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between text-[10px] mb-1.5">
                        ...
                        {topic.due > 0 && <span className="text-app-accent-primary font-semibold">{topic.due} c?n ôn</span>}
                      </div>
                      ...
                      <p className="text-app-text-muted text-[9px] mt-1">{masteredPct}% thu?c lòng</p>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="space-y-4">
            <div className="bg-app-bg border border-app-border rounded-2xl p-5">
              ...
              <div className="space-y-3">
                {[
                  { label: "C?n ôn hôm nay", value: stats.dueToday, color: "#f87171", icon: "ri-alarm-line" },
                  { label: "Ðang h?c", value: stats.learning, color: "app-accent-primary", icon: "ri-book-open-line" },
                  { label: "Thu?c lòng", value: stats.mastered, color: "#34d399", icon: "ri-checkbox-circle-line" },
            ...
            <div className="bg-app-bg border border-app-border rounded-2xl p-5">
              <h3 className="text-white font-semibold text-sm mb-3"><i className="ri-lightbulb-line text-app-accent-primary mr-2"></i>Cách ho?t d?ng</h3>
              <div className="space-y-2.5 text-app-text-secondary text-xs leading-relaxed">
                <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>T? chua thu?c du?c uu tiên hi?n th? tru?c</p>
                <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Nh?n loa d? nghe phát âm ti?ng Hàn</p>
                <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Nh?n th? d? l?t và xem nghia + ví d?</p>
                <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Spaced Repetition: t? dã thu?c s? ôn l?i sau nhi?u ngày hon</p>
                <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>T? sai s? du?c dua v? d?u hàng d?i</p>
              </div>
            </div>

            <div className="bg-app-bg border border-app-border rounded-2xl p-5">
              <h3 className="text-white font-semibold text-sm mb-3">L?ch ôn t?p SR</h3>
                ...
                {SR_INTERVALS.map((days, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-2">
                      ...
                      <span className="text-app-text-muted text-[10px] flex-1">{days === 0 ? "M?i" : `${days} ngày`}</span>
  ...
  if (phase === "study" && currentItem) {
    ...
    return (
      <DashboardLayout
        ...
        subtitle={`${currentIdx + 1}/${studyQueue.length} · ${sessionStats.known} dã nh? · ${sessionStats.unknown} chua nh?`}
        actions={
      ...
      >
        <div className="max-w-xl mx-auto space-y-5">
          ...
          <div className="flex items-center gap-2">
            ...
            {!isNew && isDue && (
              <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-app-accent-primary/15 text-app-accent-primary border border-app-accent-primary/20">
                <i className="ri-alarm-line"></i>Ð?n h?n ôn
              </span>
            )}
            ...
            {cardInfo && cardInfo.interval >= 3 && (
              <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-app-accent-success/15 text-app-accent-success border border-emerald-500/20">
                <i className="ri-checkbox-circle-line"></i>Ðang thu?c
              </span>
            )}
          </div>
              ...
              >
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div className="text-center">
                    <p className="text-app-text-muted text-xs tracking-normal mb-1">Ti?ng Hàn</p>
          ...
          <div className="flex gap-3">
            ...
            >
              <i className="ri-check-line text-lg"></i>Ðã nh?
            </button>
          </div>
          ...
          {cardInfo && (
            <div className="flex items-center justify-center gap-4 text-[10px] text-app-text-muted">
              <span><i className="ri-check-line text-app-accent-success/50 mr-1"></i>{cardInfo.successCount} l?n dúng</span>
              <span><i className="ri-close-line text-red-400/50 mr-1"></i>{cardInfo.failCount} l?n sai</span>
              <span><i className="ri-calendar-line mr-1"></i>Ôn l?i sau {SR_INTERVALS[Math.min(cardInfo.interval + 1, SR_INTERVALS.length - 1)]} ngày</span>
            </div>
          )}
        </div>
      </DashboardLayout>
  ...
  if (phase === "done") {
    ...
    return (
      <DashboardLayout title="Hoàn thành phiên h?c!" subtitle="K?t qu? phiên flashcard thông minh">
        <div className="max-w-md mx-auto text-center py-8 space-y-6">
          ...
          <div>
            ...
            <p className="text-app-text-secondary text-sm mt-1">T? l? nh? trong phiên này</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-emerald-500/5 border border-emerald-500/15 rounded-xl p-4">
              <p className="text-app-accent-success text-2xl font-bold">{sessionStats.known}</p>
              <p className="text-app-text-secondary text-xs mt-1">Ðã nh?</p>
            </div>
            ...
            <div className="bg-app-accent-primary/5 border border-app-accent-primary/15 rounded-xl p-4">
              ...
              <p className="text-app-text-secondary text-xs mt-1">T?ng thu?c lòng</p>
            </div>
          </div>

          <div className="bg-app-bg border border-app-border rounded-xl p-4 text-left">
            <p className="text-white/50 text-xs font-semibold mb-2">L?ch ôn t?p ti?p theo</p>
            <p className="text-app-text-muted text-xs">
              T? dã nh? s? du?c ôn l?i sau {SR_INTERVALS[1]}-{SR_INTERVALS[3]} ngày. T? chua nh? s? xu?t hi?n l?i ngay hôm nay.
            </p>
          </div>
