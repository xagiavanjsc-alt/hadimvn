...
export default function StudyStatsDetailPage() {
  ...
  return (
    <DashboardLayout title="Thống kê học tập chi tiết" subtitle="Biểu đồ tiến độ theo tuần/tháng, thời gian học và từ vựng đã học">
      ...
      <div className="bg-app-bg border border-app-border rounded-2xl p-5 mb-6">
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-app-text-secondary text-xs mt-0.5">Phút học mỗi {view === "week" ? "ngày" : "tháng"}</p>
          </div>
          <div className="flex rounded-lg border border-app-border overflow-hidden">
            {(["week", "month"] as const).map(v => (
              <button key={v} onClick={() => setView(v)}
                className={`px-3 py-1.5 text-xs font-medium transition-colors cursor-pointer whitespace-nowrap ${view === v ? "bg-app-accent-primary/15 text-app-accent-primary" : "text-app-text-secondary hover:text-white/60"}`}>
                {v === "week" ? "Tuần" : "Tháng"}
              </button>
            ))}
          </div>
          ...
          <div className="flex items-end gap-2 h-40">
            {weeklyData.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <p className="text-app-text-secondary text-[10px]">{d.minutes}p</p>
                <div className="w-full rounded-t-md bg-app-accent-primary/20 hover:bg-app-accent-primary/35 transition-colors cursor-default"
                  style={{ height: `${(d.minutes / maxMinutes) * 100}px`, minHeight: "4px" }} />
                <p className="text-app-text-secondary text-[10px]">{d.day}</p>
              </div>
            ))}
          </div>
        ) : (
          <div className="flex items-end gap-1.5 h-40">
            {monthlyData.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                ...
                <p className="text-app-text-muted text-[9px]">{d.month}</p>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="space-y-2">
            {weeklyData.map((d, i) => (
              <div key={i} className="flex items-center gap-3">
                <p className="text-app-text-secondary text-xs w-6">{d.day}</p>
                ...
                <p className="text-app-text-secondary text-xs w-8 text-right">{d.words}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          {recentActivity.length === 0 ? (
            ...
            <div className="space-y-2">
              {recentActivity.map(([id, data]) => (
                <div key={id} className="flex items-center gap-3 p-2.5 rounded-lg bg-white/2 border border-app-border">
                  <div className="w-8 h-8 flex items-center justify-center rounded-lg bg-app-accent-primary/10 flex-shrink-0">
                    <span className="text-app-accent-primary text-xs font-bold">{id}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-white/70 text-xs font-medium">Bài {id}</p>
                    <p className="text-app-text-muted text-[10px]">{new Date(data.completedAt).toLocaleDateString("vi-VN")}</p>
                  </div>
                  <span className="text-app-accent-success text-xs font-bold">{data.score} đúng</span>
                </div>
              ))}
            </div>
          )}
      ...
      {/* Monthly summary */}
      <div className="bg-app-bg border border-app-border rounded-2xl p-5 mt-6">
            ...
            <tbody>
              {monthlyData.map((d, i) => (
                <tr key={i} className="border-b border-white/3 hover:bg-white/2 transition-colors">
                  ...
                  <td className="py-2 text-right text-app-accent-primary">{d.lessons}</td>
                </tr>
              ))}
            </tbody>
          </table>
