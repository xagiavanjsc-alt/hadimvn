        ...
        <div className="absolute right-0 top-full mt-2 w-80 bg-white border border-gray-200 rounded-2xl z-50 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
            ...
            {notifications.length > 0 && (
              <button onClick={onClear} className="text-gray-400 hover:text-gray-600 text-xs cursor-pointer">Xóa t?t c?</button>
            )}
          </div>
          <div className="max-h-80 overflow-y-auto">
            {notifications.length === 0 ? (
              <div className="py-8 text-center">
                ...
                <p className="text-gray-400 text-sm">Chua có thông báo nào</p>
              </div>
            ) : (
              notifications.slice(0, 20).map(notif => {
                const cfg = typeConfig[notif.type];
                const timeAgo = Math.floor((Date.now() - notif.timestamp) / 60000);
                const timeStr = timeAgo < 1 ? "V?a xong" : timeAgo < 60 ? `${timeAgo} phút tru?c` : `${Math.floor(timeAgo / 60)} gi? tru?c`;
                return (
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            {isJoined && (
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-600 font-bold flex-shrink-0">Ðã tham gia</span>
            )}
          </div>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <span className="flex items-center gap-1 text-xs text-gray-500 bg-gray-50 px-2 py-1 rounded-lg">
          ...
          M?c tiêu: {group.weeklyGoal} l?n/tu?n
        </span>
      </div>
    ...
    <div className="p-6 md:p-8 max-w-2xl mx-auto">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {member.isOwner && <span className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-500">Tru?ng nhóm</span>}
                  </div>
      ...
      {tab === "activity" && (
        <div className="space-y-3">
          {[
            { member: sorted[0], action: "d?t di?m cao nh?t tu?n này", score: sorted[0].score, time: "2 gi? tru?c", icon: "ri-trophy-line", color: "app-accent-primary" },
            { member: sorted[1], action: "hoàn thành bài thi EPS 40 câu", score: sorted[1].score, time: "5 gi? tru?c", icon: "ri-file-list-3-line", color: "#06b6d4" },
            { member: sorted[2], action: "duy trì streak 8 ngày", score: null, time: "Hôm qua", icon: "ri-fire-line", color: "#fb923c" },
            { member: sorted[0], action: "ôn t?p câu sai thông minh", score: null, time: "Hôm qua", icon: "ri-brain-line", color: "#a78bfa" },
          ].map((item, i) => (
...
function SendNotifModal({ group, myName, onClose, onSend }: SendNotifModalProps) {
  ...
  const TEMPLATES = [
    { type: "exam_done" as const, icon: "ri-file-list-3-line", color: "app-accent-primary", label: "V?a thi xong", placeholder: "VD: V?a d?t 85% trong bài thi EPS! ??" },
    { type: "streak" as const, icon: "ri-fire-line", color: "#ea580c", label: "Streak m?i", placeholder: "VD: Ðã h?c 15 ngày liên ti?p! ??" },
    { type: "top_score" as const, icon: "ri-trophy-line", color: "#d97706", label: "Ði?m cao m?i", placeholder: "VD: Phá k? l?c cá nhân v?i 92%! ??" },
    { type: "joined" as const, icon: "ri-megaphone-line", color: "#10b981", label: "Nh?c nh? nhóm", placeholder: "VD: Nh? ôn bài hôm nay nhé m?i ngu?i! ??" },
  ];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-gray-800 font-bold text-lg">G?i thông báo nhóm</h3>
        ...
        {sent ? (
          <div className="text-center py-6">
            ...
            <p className="text-gray-700 font-semibold">Ðã g?i thông báo!</p>
          </div>
        ) : (
          <>
            <div className="mb-4">
              <p className="text-gray-600 text-xs font-medium mb-2">Lo?i thông báo</p>
            ...
            <div className="mb-5">
              <p className="text-gray-600 text-xs font-medium mb-1.5">N?i dung thông báo</p>
              <textarea
                value={customMsg}
...
function CreateGroupModal({ onClose, onCreate, myName, myId }: CreateGroupModalProps) {
  ...
  const handleCreate = () => {
    ...
    const group: StudyGroup = {
      ...
      name: name.trim(),
      code,
      description: description.trim() || "Nhóm h?c EPS-TOPIK",
      members: [{
        id: myId,
        name: myName,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-gray-800 font-bold text-lg">T?o nhóm h?c m?i</h3>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="VD: Nhóm EPS Hà N?i 2026"
          ...
          <div>
            <label className="text-gray-600 text-xs font-medium mb-1.5 block">Mô t? nhóm</label>
            <textarea
              value={description}
              onChange={e => setDescription(e.target.value)}
              placeholder="M?c tiêu, l?ch h?c, yêu c?u thành viên..."
              rows={3}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-gray-600 text-xs font-medium mb-1.5 block">M?c tiêu thi/tu?n</label>
              <select
                value={weeklyGoal}
            ...
            <div>
              ...
              >
                ...
                <option>Thi mô ph?ng th?t</option>
                <option>Ôn t?p nhanh</option>
              </select>
            </div>
          </div>
        </div>
...
function JoinGroupModal({ onClose, onJoin, groups }: { onClose: () => void; onJoin: (code: string) => void; groups: StudyGroup[] }) {
  ...
  const handleJoin = () => {
    ...
    if (!found) {
      setError("Không tìm th?y nhóm v?i mã này. Ki?m tra l?i mã nhóm.");
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="mb-4">
          <label className="text-gray-600 text-xs font-medium mb-1.5 block">Nh?p mã nhóm</label>
          <input
            value={code}
...
export default function EpsStudyGroupPage() {
  ...
  // Notifications s? du?c thêm th?t khi có Supabase Realtime (không còn fake events)
  ...
  const handleCreate = useCallback((group: StudyGroup) => {
    ...
    showSuccess(`Ðã t?o nhóm "${group.name}" — Mã: ${group.code}`);
  ...
  const handleJoinByCode = useCallback((code: string) => {
    ...
    setShowJoin(false);
    setSelectedGroup(group);
    showSuccess(`Ðã tham gia nhóm "${group.name}"!`);
  }, [groups, setJoinedIds]);
  ...
  if (selectedGroup) {
    return (
      <DashboardLayout title="Chi ti?t nhóm h?c">
        <GroupDetail
          group={selectedGroup}
  ...
  return (
    <DashboardLayout title="Nhóm h?c EPS" subtitle="Thi dua di?m s? v?i b?n bè cùng l?p">
      <div className="p-6 md:p-8 max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-6 flex-wrap">
          <div className="flex-1 relative min-w-48">
            ...
            <input
              ...
              placeholder="Tìm nhóm theo tên ho?c mã..."
        ...
        <div>
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-2xl border border-gray-200">
              ...
              <p className="text-gray-500 text-sm">Không tìm th?y nhóm nào</p>
        ...
        <div className="mt-8 bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { icon: "ri-add-circle-line", color: "app-accent-primary", title: "T?o ho?c tham gia", desc: "T?o nhóm m?i ho?c nh?p mã nhóm t? b?n bè" },
              { icon: "ri-timer-line", color: "#06b6d4", title: "Thi dua cùng nhau", desc: "Làm bài thi EPS, di?m t? d?ng c?p nh?t BXH nhóm" },
              { icon: "ri-trophy-line", color: "app-accent-primary", title: "So sánh ti?n b?", desc: "Xem ai dang d?n d?u, ai c?n c? g?ng thêm" },
            ].map((item, i) => (
