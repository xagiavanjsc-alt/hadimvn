...
const OFFICIAL_EXAM_SAMPLES: UploadedExam[] = [
  {
    id: "eps2023",
    name: "Ð? thi EPS-TOPIK 2023 (M?u)",
    year: "2023",
    source: "B? Lao d?ng Hàn Qu?c",
    uploadedAt: new Date().toISOString(),
    questions: Array.from({ length: 40 }, (_, i) => ({
      id: `q${i + 1}`,
      number: i + 1,
      text: `Câu ${i + 1}: [Ð? thi EPS-TOPIK 2023] Ch?n dáp án dúng nh?t cho câu h?i sau v? ${["giao ti?p co b?n", "an toàn lao d?ng", "van hóa Hàn Qu?c", "pháp lu?t lao d?ng"][i % 4]}.`,
      options: ["Ðáp án A", "Ðáp án B", "Ðáp án C", "Ðáp án D"],
  ...
  },
  {
    ...
    questions: Array.from({ length: 40 }, (_, i) => ({
      id: `q${i + 1}`,
      number: i + 1,
      text: `Câu ${i + 1}: [Ð? thi EPS-TOPIK 2022] Câu h?i v? ${["nghe hi?u", "d?c hi?u", "sinh ho?t hàng ngày", "noi làm vi?c"][i % 4]}.`,
      options: ["Ðáp án A", "Ðáp án B", "Ðáp án C", "Ðáp án D"],
      correctIndex: Math.floor(Math.random() * 4),
      userAnswer: null,
    })),
    ...
    >
      ...
      {processing ? (
        <div className="space-y-3">
          ...
          <p className="text-gray-600 font-medium">Ðang x? lý file...</p>
          <p className="text-gray-400 text-sm">H? th?ng dang phân tích d? thi</p>
        </div>
      ) : (
        <div className="space-y-3">
          ...
          <div>
            <p className="text-gray-700 font-semibold text-base">Kéo th? ho?c click d? upload d? thi</p>
            <p className="text-gray-400 text-sm mt-1">H? tr?: PDF, Word (.doc/.docx), TXT</p>
          </div>
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-[#f4f6fb]">
      ...
      <div className="flex-shrink-0 bg-white border-t border-gray-200 px-4 py-3">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          ...
          >
            {currentIndex === exam.questions.length - 1 ? "N?p bài" : "Ti?p"}
    ...
    <div className="p-6 md:p-8 max-w-2xl mx-auto">
      <div className={`rounded-2xl p-6 text-center mb-6 ${passed ? "bg-emerald-50 border border-emerald-200" : "bg-rose-50 border border-rose-200"}`}>
        ...
        <h2 className={`text-xl font-bold mb-1 ${passed ? "text-emerald-700" : "text-rose-600"}`}>
          {passed ? "Ð?U! Xu?t s?c!" : "Chua d?u — C? lên!"}
        </h2>
        <p className="text-gray-500 text-sm">{correct}/{exam.questions.length} câu dúng • {answered} câu dã tr? l?i</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
        {[
          { label: "Câu dúng", value: correct, color: "text-emerald-600", bg: "bg-emerald-50" },
          { label: "Câu sai", value: answered - correct, color: "text-rose-500", bg: "bg-rose-50" },
          { label: "Chua tr? l?i", value: exam.questions.length - answered, color: "text-gray-500", bg: "bg-gray-50" },
        ].map((item, i) => (
...
export default function EpsOfficialExamPage() {
  ...
  const handleFileUpload = useCallback(async (file: File) => {
    ...
    if (!["pdf", "doc", "docx", "txt"].includes(ext || "")) {
      setUploadMsg("Ð?nh d?ng file không du?c h? tr?. Vui lòng dùng PDF, Word ho?c TXT.");
      setProcessing(false);
      return;
    }
    ...
    const newExam: UploadedExam = {
      ...
      source: "Upload t? ngu?i dùng",
      uploadedAt: new Date().toISOString(),
      questions: Array.from({ length: 40 }, (_, i) => ({
        id: `uq${i + 1}`,
        number: i + 1,
        text: `Câu ${i + 1}: [${file.name}] Câu h?i du?c trích xu?t t? file d? thi. Trong phiên b?n d?y d?, h? th?ng AI s? t? d?ng phân tích và trích xu?t câu h?i t? file PDF/Word c?a b?n.`,
        options: [
          "Ðáp án A — Ðu?c trích xu?t t? file",
          "Ðáp án B — Ðu?c trích xu?t t? file",
          "Ðáp án C — Ðu?c trích xu?t t? file",
          "Ðáp án D — Ðu?c trích xu?t t? file",
        ],
    ...
    setUploadMsg(`Ðã upload thành công "${file.name}" — ${newExam.questions.length} câu h?i`);
    setProcessing(false);
  }, []);
  ...
  if (phase === "result" && selectedExam) {
    return (
      <DashboardLayout title="K?t qu? thi th?">
        <ResultScreen
          exam={selectedExam}
          answers={answers}
  ...
  return (
    <DashboardLayout title="Thi th? EPS b? d? chính th?c" subtitle="Upload d? thi PDF/Word t? B? Lao d?ng Hàn Qu?c">
      <div className="p-6 md:p-8 max-w-3xl mx-auto">
        {/* Info banner */}
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 mb-6 flex items-start gap-3">
          <i className="ri-information-line text-blue-500 text-lg flex-shrink-0 mt-0.5"></i>
          <div>
            <p className="text-blue-700 font-semibold text-sm">Cách s? d?ng</p>
            <p className="text-blue-600 text-xs mt-1 leading-relaxed">
              Upload file d? thi EPS-TOPIK chính th?c t? website B? Lao d?ng Hàn Qu?c (eps.hrdkorea.or.kr) ho?c các ngu?n uy tín. 
              H? th?ng s? t? d?ng phân tích và t?o bài thi tuong tác. Ho?c ch?n d? m?u có s?n bên du?i d? luy?n t?p ngay.
            </p>
          </div>
        </div>
        ...
        <div>
          <h2 className="text-gray-700 font-semibold text-sm mb-3 flex items-center gap-2">
            ...
            Thu vi?n d? thi ({exams.length} b? d?)
          </h2>
          <div className="space-y-3">
            {exams.map(exam => (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1 flex-wrap">
                    ...
                    {exam.id.startsWith("upload_") && (
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-600 font-medium">Ðã upload</span>
                    )}
                  </div>
                </div>
                <button
        ...
        <div className="mt-6 bg-gray-50 border border-gray-200 rounded-2xl p-5">
          <h3 className="text-gray-700 font-semibold text-sm mb-3 flex items-center gap-2">
            ...
            M?o luy?n thi hi?u qu?
          </h3>
          <div className="space-y-2">
            {[
              "T?i d? thi chính th?c t? eps.hrdkorea.or.kr (website chính th?c c?a B? Lao d?ng Hàn Qu?c)",
              "Luy?n t?p v?i d? thi t? 3-5 nam g?n nh?t d? n?m format câu h?i",
              "K?t h?p v?i tính nang Ôn t?p câu sai thông minh d? t?p trung vào di?m y?u",
              "M?c tiêu d?t 80%+ trong 3 l?n thi liên ti?p tru?c khi thi th?t",
            ].map((tip, i) => (
