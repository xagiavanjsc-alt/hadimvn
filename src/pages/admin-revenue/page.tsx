    ...
    <div className="rounded-2xl border p-5" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
      <div className="flex items-start justify-between mb-5 flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: "var(--admin-text-muted)" }}>
            T? l? gi? chân VIP theo tháng dang ký — xanh = gi? t?t, d? = churn cao
          </p>
        </div>
      ...
      <div className="mt-4 pt-4 border-t" style={{ borderColor: "var(--admin-border)" }}>
        ...
        <p className="text-[9px] mt-2" style={{ color: "var(--admin-text-faint)" }}>
          * D? li?u retention u?c tính d?a trên ngày dang ký và tr?ng thái VIP hi?n t?i.
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-1 p-1 rounded-xl w-fit" style={{ backgroundColor: "var(--admin-hover)" }}>
        {([{ val: "quarter" as const, label: "So sánh theo Quý" }, { val: "year" as const, label: "So sánh theo Nam" }]).map(m => (
        ...
        <>
          ...
          <div className="rounded-2xl border overflow-hidden" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
            <div className="px-5 py-3 border-b" style={{ borderColor: "var(--admin-border)" }}>
              <p className="font-semibold text-sm" style={{ color: "var(--admin-text)" }}>Chi ti?t theo Quý</p>
            </div>
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b" style={{ borderColor: "var(--admin-border)" }}>
                  {["Quý", "Doanh thu", "VIP m?i", "T?ng VIP", "Churn", "Tang tru?ng"].map(h => (
      ...
      {compareMode === "year" && (
        <>
                ...
                <div key={y.label} className="rounded-2xl border p-5" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      { label: "ARR u?c tính", value: formatVND(y.arr), color: "app-accent-primary" },
                      ...
                      { label: "S? tháng d? li?u", value: `${y.months} tháng`, color: "#6b7280" },
                    ].map(m => (
          ...
          <div className="rounded-2xl border p-5" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
            <p className="font-semibold text-sm mb-5" style={{ color: "var(--admin-text)" }}>So sánh doanh thu nam</p>
...
export default function AdminRevenuePage() {
  ...
  const monthlyData: MonthlyData[] = useMemo(() => {
    ...
    for (let i = months - 1; i >= 0; i--) {
      ...
      // churnedVip tính t? s? VIP th?c t? b? h?t h?n trong tháng
  ...
  if (loading) {
    return (
      <AdminLayout title="Phân tích Doanh thu" subtitle="MRR, ARR, Churn Rate t? d? li?u th?c">
  ...
  return (
    <AdminLayout
      ...
      subtitle="MRR, ARR, Churn Rate và phân tích VIP t? d? li?u Supabase th?c"
      actions={
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
        {[
          { label: "MRR", value: formatVND(mrr), sub: `${mrrGrowth >= 0 ? "+" : ""}${mrrGrowth.toFixed(1)}% so v?i tháng tru?c`, icon: "ri-money-dollar-circle-line", color: "#34d399", positive: mrrGrowth >= 0 },
          { label: "ARR (u?c tính)", value: formatVND(arr), sub: "Doanh thu nam d? ki?n", icon: "ri-bar-chart-grouped-line", color: "app-accent-primary", positive: true },
          { label: "Churn Rate", value: `${churnRate.toFixed(1)}%`, sub: `${churnRisk.length} VIP s?p h?t h?n 30 ngày`, icon: "ri-user-unfollow-line", color: churnRate > 20 ? "#f87171" : "#fb923c", positive: churnRate < 10 },
          { label: "ARPU", value: formatVND(arpu), sub: `LTV u?c tính: ${formatVND(ltv)}`, icon: "ri-user-star-line", color: "#a78bfa", positive: true },
        ].map(s => (
      ...
      <div className="flex items-center gap-1 rounded-xl p-1 w-fit mb-5" style={{ backgroundColor: "var(--admin-hover)" }}>
        {([
          ...
          { id: "quarterly" as const, label: "Quý / Nam", icon: "ri-bar-chart-grouped-line" },
          { id: "churn" as const, label: "Churn Risk", icon: "ri-user-unfollow-line" },
          { id: "cohort" as const, label: "Phân b? VIP", icon: "ri-pie-chart-line" },
        ]).map(tab => (
      ...
      {activeTab === "overview" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="rounded-2xl border p-5" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs mt-0.5" style={{ color: "var(--admin-text-muted)" }}>{period === "6m" ? "6 tháng" : "12 tháng"} g?n nh?t</p>
              </div>
          ...
          <div className="rounded-2xl border p-5" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs mt-0.5" style={{ color: "var(--admin-text-muted)" }}>S? VIP m?i m?i tháng</p>
              </div>
            </div>
          ...
          {/* Revenue breakdown */}
          <div className="rounded-2xl border p-5" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
            <p className="font-semibold text-sm mb-4" style={{ color: "var(--admin-text)" }}>Phân b? doanh thu</p>
            <div className="space-y-3">
              {[
          ...
          <div className="rounded-2xl border p-5" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
            ...
            <div className="space-y-3">
              {[
                { label: "T?ng thành viên", count: users.length, color: "#6b7280", pct: 100 },
                { label: "Ðã t?ng VIP", count: vipMonthUsers.length + vipYearUsers.length, color: "#a78bfa", pct: users.length > 0 ? (vipMonthUsers.length + vipYearUsers.length) / users.length * 100 : 0 },
      ...
      {activeTab === "mrr" && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div className="rounded-2xl border p-5" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
              <p className="font-semibold text-sm mb-4" style={{ color: "var(--admin-text)" }}>MRR t? VIP Tháng</p>
          ...
          <div className="rounded-2xl border overflow-hidden" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
            <div className="px-5 py-3 border-b" style={{ borderColor: "var(--admin-border)" }}>
              <p className="font-semibold text-sm" style={{ color: "var(--admin-text)" }}>Chi ti?t theo tháng</p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b" style={{ borderColor: "var(--admin-border)" }}>
                    {["Tháng", "VIP m?i", "T?ng VIP", "MRR Tháng", "MRR Nam", "T?ng MRR", "Tang tru?ng"].map(h => (
                      <th key={h} className="text-left px-4 py-2.5 text-[10px] tracking-normal font-semibold" style={{ color: "var(--admin-text-faint)" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              { label: "Doanh thu có nguy co m?t", value: formatVND(churnRisk.reduce((s, u) => s + (u.vipType === "year" ? YEAR_PRICE_MONTHLY : MONTH_PRICE), 0)), color: "#f87171", icon: "ri-money-dollar-circle-line" },
            ].map(s => (
          ...
          <div className="rounded-2xl border overflow-hidden" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
            <div className="flex items-center justify-between px-5 py-3 border-b" style={{ borderColor: "var(--admin-border)" }}>
              <p className="font-semibold text-sm" style={{ color: "var(--admin-text)" }}>Danh sách VIP s?p h?t h?n</p>
            ...
            {churnRisk.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-sm" style={{ color: "var(--admin-text-muted)" }}>Không có VIP nào s?p h?t h?n trong 30 ngày!</p>
              </div>
            ) : (
                  ...
                  <div key={u.id} className="flex items-center gap-4 px-5 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px]" style={{ color: "var(--admin-text-faint)" }}>
                        VIP {u.vipType === "year" ? "Nam" : "Tháng"} · H?t h?n {new Date(u.vip_expires_at).toLocaleDateString("vi-VN")}
                      </p>
                    </div>
                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${u.daysLeft <= 7 ? "bg-rose-500/15 text-rose-400" : "bg-amber-500/15 text-amber-400"}`}>
                      {u.daysLeft <= 0 ? "Ðã h?t" : `${u.daysLeft} ngày`}
                    </span>
      ...
      {activeTab === "cohort" && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {/* VIP distribution */}
            <div className="rounded-2xl border p-5" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
              <p className="font-semibold text-sm mb-4" style={{ color: "var(--admin-text)" }}>Phân b? thành viên</p>
              <div className="space-y-3">
                {[
            ...
            <div className="rounded-2xl border p-5" style={{ backgroundColor: "var(--admin-card)", borderColor: "var(--admin-border)" }}>
              ...
              <div className="space-y-3">
                {[
                  ...
                  { label: "ARPU (trung bình/user VIP)", value: formatVND(arpu), desc: "Doanh thu trung bình m?i VIP", color: "#34d399" },
                  { label: "LTV u?c tính", value: formatVND(ltv), desc: "Giá tr? vòng d?i (8 tháng)", color: "app-accent-primary" },
                  ...
                  { label: "Churn Rate (30 ngày)", value: `${churnRate.toFixed(1)}%`, desc: `${churnRisk.length} ngu?i s?p h?t h?n`, color: churnRate > 20 ? "#f87171" : "#fb923c" },
                ].map(m => (
