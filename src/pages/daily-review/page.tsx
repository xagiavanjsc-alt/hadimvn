...
function generateDailyQuestions(): ReviewQuestion[] {
  ...
  for (let i = 4; i < 7; i++) {
    ...
    questions.push({
      ...
      questionSub: `Ch?n t? ti?ng Hàn dúng`,
      options: opts,
      answer: item.korean,
...
function QuestionCard({
  ...
  const typeLabel: Record<QuestionType, string> = {
    ...
    vocab_korean: "T? ti?ng Hàn",
    grammar_choose: "Ng? pháp",
    grammar_fill: "Ði?n vào ch? tr?ng",
  };
...
function ResultScreen({
  ...
  return (
    <div className="max-w-xl mx-auto space-y-5">
      ...
      <div className="bg-gradient-to-br from-app-surface to-[#0f1117] border border-app-accent-primary/20 rounded-2xl p-8 text-center">
        <div className="relative inline-flex items-center justify-center mb-4">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-app-text-secondary text-[10px]">dúng</span>
          </div>
        </div>
        <h2 className="text-white font-bold text-xl mb-1">
          {pct >= 80 ? "Xu?t s?c!" : pct >= 60 ? "T?t l?m!" : "C? g?ng hon nhé!"}
        </h2>
        <p className="text-app-text-secondary text-sm mb-4">B?n d?t {pct}% hôm nay</p>
      ...
      {questions.some(q => answers[q.id] !== q.answer) && (
        <div className="bg-app-bg border border-app-border rounded-2xl p-5">
          <h3 className="text-white font-semibold text-sm mb-4">
            ...
            Câu c?n ôn l?i ({questions.filter(q => answers[q.id] !== q.answer).length})
          </h3>
          <div className="space-y-3">
            {questions.filter(q => answers[q.id] !== q.answer).map(q => (
              <div key={q.id} className="p-3 bg-red-400/5 border border-red-400/10 rounded-xl">
                ...
                <p className="text-app-accent-success text-xs">
                  <i className="ri-check-line mr-1"></i>Ðáp án dúng: <strong>{q.answer}</strong>
                </p>
                {answers[q.id] && (
      ...
      >
        <i className="ri-refresh-line mr-2"></i>Ôn t?p l?i
      </button>
    </div>
  );
}
...
export default function DailyReviewPage() {
  ...
  return (
    <DashboardLayout
      title="Ôn t?p hàng ngày"
      subtitle="10 câu m?i ngày — duy trì streak và c?ng c? ki?n th?c"
    >
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-6">
                ...
                <div className="flex justify-center">
                  ...
                  >
                    {isLastQuestion ? (
                      ...
                      <><i className="ri-arrow-right-line"></i>Câu ti?p theo</>
                    )}
                  </button>
                </div>
              )}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-app-text-secondary text-xs">câu dã làm</p>
                {showResult && session && (
          ...
          {/* Heatmap */}
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <h3 className="text-white font-semibold text-sm mb-3">14 ngày g?n dây</h3>
          ...
          {history.length > 0 && (
            <div className="bg-app-bg border border-app-border rounded-2xl p-5">
              <h3 className="text-white font-semibold text-sm mb-3">L?ch s? g?n dây</h3>
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            ...
            <ul className="space-y-2 text-app-text-secondary text-xs leading-relaxed">
              <li className="flex gap-2"><span className="text-app-accent-primary flex-shrink-0">•</span>H?c 10 câu m?i ngày hi?u qu? hon h?c 100 câu m?t l?n</li>
              <li className="flex gap-2"><span className="text-app-accent-primary flex-shrink-0">•</span>Câu sai hôm nay s? xu?t hi?n l?i ngày mai</li>
              <li className="flex gap-2"><span className="text-app-accent-primary flex-shrink-0">•</span>Streak 21 ngày giúp hình thành thói quen h?c t?p</li>
            </ul>
          </div>
        </div>
      </div>
