...
const ROOT_MEANINGS: Record<string, string> = {
  "?": "Ngu?i", "?": "L?n", "?": "Qu?c gia", "?": "H?c", "?": "Tâm/Lòng",
};
    ...
    <div className="relative mb-6">
      <div className={`flex items-center gap-3 border-2 rounded-2xl px-4 py-3 transition-all ${focused ? "border-rose-500/60 bg-app-card/50" : "border-app-border bg-app-card/50"}`}>
        ...
        <input
          ...
          placeholder="Tìm ki?m Hán Hàn... (ti?ng Hàn, Hán t?, nghia ti?ng Vi?t)"
          ...
          {results.map(node => (
            ...
            >
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                ...
                }`}>{node.difficulty === 1 ? "D?" : node.difficulty === 2 ? "TB" : "Khó"}</span>
              </div>
            </button>
          ))}
        </div>
      ...
      {focused && query.trim() && results.length === 0 && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-[#1a1d27] border border-app-border rounded-2xl p-6 text-center z-50">
          ...
          <p className="text-sm text-app-text-secondary">Không tìm th?y t? nào cho &ldquo;{query}&rdquo;</p>
        </div>
      )}
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                }`}>{selected.difficulty === 1 ? "D?" : selected.difficulty === 2 ? "Trung bình" : "Khó"}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
        <div className="bg-[#1a1d27] border border-app-border rounded-2xl p-8 max-w-sm w-full text-center" onClick={e => e.stopPropagation()}>
          ...
          <h3 className="text-lg font-bold text-white/80 mb-2">Chua có t? dã h?c</h3>
          <p className="text-sm text-app-text-secondary mb-4">Hãy vào Hán Hàn hình cây và dánh d?u m?t s? t? dã h?c tru?c nhé!</p>
          <button onClick={onClose} className="w-full py-2.5 bg-rose-500 text-white font-bold rounded-xl cursor-pointer whitespace-nowrap">Ðóng</button>
        </div>
      </div>
    );
  }
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
        <div className="bg-[#1a1d27] border border-app-border rounded-2xl p-8 max-w-sm w-full text-center" onClick={e => e.stopPropagation()}>
          ...
          <p className="text-sm text-white/50 mb-4">
            {score.correct}/{total} câu dúng
            {pct >= 80 ? " — Xu?t s?c!" : pct >= 60 ? " — Khá t?t!" : " — C?n ôn thêm!"}
          </p>
          <div className="grid grid-cols-2 gap-3 mb-5">
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-3">
              <p className="text-2xl font-bold text-app-accent-success">{score.correct}</p>
              <p className="text-xs text-emerald-500/70">Ðúng</p>
            </div>
      ...
      <div className="bg-[#1a1d27] border border-app-border rounded-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 pt-5 pb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-white/80">Ôn t?p t? dã h?c</span>
          </div>
        ...
        <div className="px-5 pb-3">
          <div className="flex items-center justify-between text-xs text-app-text-secondary mb-1.5">
            ...
            <span className="text-app-accent-success font-medium">{score.correct} dúng</span>
          </div>
          ...
          <div className="bg-rose-500/10 border border-rose-500/20 rounded-2xl p-6 text-center mb-4">
            {quizMode === "kr-to-vi" ? (
              <>
                ...
                <p className="text-xs text-app-text-muted mt-2">Nghia ti?ng Vi?t là gì?</p>
              </>
            ) : (
              <>
                <p className="text-2xl font-bold text-white/80 mb-1">{currentCard.vietnamese}</p>
                <p className="text-xs text-app-text-muted mt-2">T? ti?ng Hàn tuong ?ng là gì?</p>
              </>
            )}
          </div>
    ...
    <div className="bg-app-card/50 border border-app-border rounded-2xl p-5 mb-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-white/80">H?c t? m?i hôm nay</h3>
            <p className="text-xs text-app-text-secondary">{learnedToday}/{dailyWords.length} t? dã h?c · Nh?n th? d? xem nghia</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={refreshDaily} className="w-7 h-7 flex items-center justify-center rounded-lg bg-white/8 hover:bg-white/12 text-app-text-secondary cursor-pointer transition-all" title="Làm m?i danh sách">
          ...
          return (
            <div key={node.id} className="relative">
              ...
              <button
                ...
                title={isLearned ? "B? dánh d?u" : "Ðánh d?u dã h?c"}
              >
...
export default function HanjaDashboardPage() {
  ...
  return (
    <DashboardLayout title="T?ng quan Hán Hàn" subtitle="Theo dõi ti?n d? h?c t? v?ng Hán Hàn và qu?n lý d? li?u">
      <div className="p-6">
        {/* Global Search */}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            ...
            { label: "Ðã h?c", value: totalLearned, icon: "ri-checkbox-circle-line", color: "#10b981" },
            { label: "Ti?n d?", value: `${totalPct}%`, icon: "ri-pie-chart-2-line", color: "#f59e0b" },
            { label: "Streak hi?n t?i", value: `${streak.current} ngày`, icon: "ri-fire-line", color: "#f97316" },
          ].map(s => (
        ...
        <div className="bg-app-card/50 border border-app-border rounded-xl p-4 mb-4">
            ...
            <div className="flex items-center gap-2 flex-wrap justify-end">
              ...
              {totalLearned > 0 && (
                <button onClick={() => setShowReview(true)}
                  ...
                  <i className="ri-brain-line"></i>Ôn t?p ({totalLearned} t?)
                </button>
              )}
            </div>
          </div>
          ...
          <p className="text-xs text-app-text-muted mt-1.5">Còn {nodes.length - totalLearned} t? chua h?c</p>
        </div>
        ...
        <div className="bg-orange-500/8 border border-orange-500/20 rounded-xl p-4 mb-6">
          <div className="flex items-center gap-4 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-xs text-orange-400 font-semibold">Streak h?c Hán Hàn</p>
                <p className="text-xl font-bold text-orange-300">{streak.current} <span className="text-sm font-normal text-orange-400/70">ngày liên ti?p</span></p>
              </div>
            </div>
            <div className="flex items-center gap-6 ml-auto flex-wrap">
              <div className="text-center">
                <p className="text-lg font-bold text-white/70">{streak.longest}</p>
                <p className="text-[10px] text-app-text-muted">Dài nh?t</p>
              </div>
              <div className="text-center">
                <p className="text-lg font-bold text-white/70">{streak.total}</p>
                <p className="text-[10px] text-app-text-muted">T?ng ngày</p>
              </div>
              <div className="text-center">
                <p className="text-lg font-bold text-white/70">{totalLearned}</p>
                <p className="text-[10px] text-app-text-muted">Ðã h?c</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-5">
            <div className="bg-app-card/50 border border-app-border rounded-2xl p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-bold text-white/80">Ti?n d? theo cây</h3>
                <button onClick={() => navigate("/hanja-tree")} className="flex items-center gap-1.5 text-xs text-rose-400 hover:text-rose-300 cursor-pointer font-medium">
                  Vào h?c <i className="ri-arrow-right-line"></i>
                </button>
              </div>
              {loading ? (
                ...
                <div className="text-center py-8 text-app-text-muted">
                  ...
                  <p className="text-sm">Chua có d? li?u. Import CSV d? b?t d?u!</p>
                </div>
              ) : (
            ...
            <div className="bg-app-card/50 border border-app-border rounded-2xl p-4 sm:p-5">
              <h3 className="text-sm font-bold text-white/80 mb-3 sm:mb-4">Phân b? theo d? khó</h3>
            ...
            {recentLearned.length > 0 && (
              <div className="bg-app-card/50 border border-app-border rounded-2xl p-5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-bold text-white/80">T? v?a h?c g?n dây</h3>
                  <button onClick={() => setShowReview(true)} className="text-xs text-rose-400 hover:text-rose-300 cursor-pointer font-medium flex items-center gap-1">
                    <i className="ri-brain-line"></i> Ôn t?p ngay
                  </button>
                </div>
          ...
          <div>
            <div className="bg-app-card/50 border border-app-border rounded-2xl p-4 space-y-2">
              ...
              {[
                { label: "H?c theo cây", icon: "ri-git-merge-line", path: "/hanja-tree", color: "#f43f5e" },
                { label: "T? v?ng Hán Hàn", icon: "ri-translate-2", path: "/hanja-vocab", color: "#a78bfa" },
                { label: "Hán Hàn chi ti?t", icon: "ri-character-recognition-line", path: "/hanja-detail", color: "#f59e0b" },
              ].map(a => (
            ...
            <div className="mt-4 bg-amber-500/8 border border-amber-500/20 rounded-xl p-3">
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-amber-400/60 mt-0.5">Ch?c nang import CSV/Excel ch? dành cho qu?n tr? viên. Truy c?p trang Admin d? th?c hi?n.</p>
                </div>
              </div>
            </div>
          </div>
