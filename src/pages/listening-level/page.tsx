...
const LEVEL_CONFIG: Record<string, { color: string; bg: string; border: string; label: string; desc: string }> = {
  ...
  B1: { color: "app-accent-primary", bg: "bg-yellow-500/10", border: "border-yellow-500/20", label: "Trung c?p 1", desc: "Trung bình" },
  B2: { color: "#fb923c", bg: "bg-orange-500/10", border: "border-orange-500/20", label: "Trung c?p 2", desc: "Trên trung bình" },
  C1: { color: "#f87171", bg: "bg-red-500/10", border: "border-red-500/20", label: "Cao c?p", desc: "Nâng cao" },
};
        ...
        <div className="p-6">
          ...
          {!track.audio_url && (
            <div className="flex items-center gap-2 p-3 rounded-xl bg-app-accent-primary/8 border border-app-accent-primary/15 mb-4">
              ...
              <p className="text-app-accent-primary/80 text-xs">Audio th?t s? du?c tích h?p cho thành viên VIP. Hi?n t?i b?n có th? d?c transcript và dùng TTS.</p>
            </div>
          )}
              ...
              {!submitted ? (
                ...
                <div className="p-4 rounded-2xl text-center" style={{ backgroundColor: `${cfg.color}10`, border: `1px solid ${cfg.color}25` }}>
                  ...
                  <p className="text-sm" style={{ color: cfg.color }}>
                    {score === track.questions.length ? "Xu?t s?c! B?n hi?u bài r?t t?t!" : "Hãy nghe l?i và th? l?n n?a!"}
                  </p>
                </div>
              )}
            </div>
...
export default function ListeningLevelPage() {
  ...
  return (
    <DashboardLayout
      ...
      subtitle="Bài nghe t? A1 d?n C1 v?i transcript, t? v?ng và bài ki?m tra — d? li?u t? Supabase"
    >
      {/* Level cards */}
      ...
      <div className="flex items-center gap-3 mb-5">
        <div className="flex-1 relative">
          ...
          <input
            type="text"
            placeholder="Tìm bài nghe theo tên, ch? d?, tag..."
            value={searchQuery}
      ...
      {loading ? (
        ...
        <div className="bg-app-bg border border-app-border rounded-2xl p-16 text-center">
          ...
          <p className="text-app-text-muted text-sm">Không tìm th?y bài nghe phù h?p</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 gap-4">
          {filtered.map(track => (
      ...
      <div className="mt-6 bg-app-bg border border-app-border rounded-2xl p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs text-app-text-secondary leading-relaxed">
          <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Nghe l?n d?u không xem transcript d? ki?m tra kh? nang nghe hi?u</p>
          <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Nghe l?n 2 v?i transcript d? hi?u rõ t?ng câu và t? v?ng m?i</p>
          <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Làm bài ki?m tra sau khi nghe d? c?ng c? ki?n th?c</p>
        </div>
      </div>
