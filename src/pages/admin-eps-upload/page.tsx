...
function DropZone({ onFiles }: { onFiles: (files: File[]) => void }) {
  ...
  return (
    <div
      ...
      className={`border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer transition-all ${dragging ? "border-[#e8c84a]/60 bg-[#e8c84a]/5" : "border-white/10 hover:border-white/20 hover:bg-white/2"}`}
    >
      ...
      <div className="w-14 h-14 flex items-center justify-center rounded-2xl bg-white/5 mx-auto mb-4">
        <i className={`ri-upload-cloud-2-line text-3xl ${dragging ? "text-[#e8c84a]" : "text-white/30"}`}></i>
      </div>
  ...
  return (
    <div className={`bg-[#0f1117] border rounded-xl p-4 transition-all ${item.status === "done" ? "border-emerald-500/20" : item.status === "error" ? "border-red-500/20" : "border-white/5"}`}>
      <div className="w-full h-36 rounded-lg overflow-hidden bg-white/3 mb-3 relative">
        ...
        {item.status === "uploading" && <div className="absolute inset-0 bg-black/50 flex items-center justify-center"><div className="w-6 h-6 border-2 border-[#e8c84a]/30 border-t-[#e8c84a] rounded-full animate-spin"></div></div>}
      ...
      <div className="mb-3">
        ...
        <select value={item.questionId} onChange={e => onAssign(item.id, e.target.value)}
          className="w-full bg-white/5 border border-white/10 rounded-lg px-2.5 py-1.5 text-white/70 text-xs focus:outline-none focus:border-[#e8c84a]/30 cursor-pointer">
      ...
      {assignedQ && (
        <div className="bg-white/3 rounded-lg px-2.5 py-2 mb-3">
          <p className="text-white/25 text-[9px] mb-0.5">URL đích trên VPS:</p>
          <p className="text-[#e8c84a]/60 text-[9px] font-mono break-all">{VPS_IMAGE_BASE}/{assignedQ.topic}/{item.file.name}</p>
        </div>
      )}
      ...
      <button onClick={() => onUpload(item.id)} disabled={!item.questionId || item.status === "uploading" || item.status === "done"}
        className={`w-full py-2 rounded-lg text-xs font-semibold transition-colors cursor-pointer whitespace-nowrap ${item.status === "done" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : !item.questionId ? "bg-white/3 text-white/20 cursor-not-allowed" : "bg-[#e8c84a]/10 hover:bg-[#e8c84a]/20 text-[#e8c84a] border border-[#e8c84a]/20"}`}>
...
function AudioGenerator() {
  ...
  return (
    <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
      <h3 className="text-white font-semibold text-sm mb-4"><i className="ri-volume-up-line text-[#e8c84a] mr-2"></i>Tạo âm thanh TTS</h3>
      ...
      {romanized && (
        <div className="bg-white/3 rounded-lg px-3 py-2 mb-3">
          ...
          <p className="text-[#e8c84a]/70 text-xs font-mono">{VPS_AUDIO_BASE}/{romanized}.mp3</p>
        </div>
      )}
  ...
  return (
    <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
      ...
      <div className="space-y-3">
        {configs.map(cfg => (
          <div key={cfg.key} className="bg-white/3 rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-3 py-2 border-b border-white/5">
              <span className="text-white/50 text-xs font-medium">{cfg.label}</span>
              <button onClick={() => copy(cfg.code, cfg.key)} className="flex items-center gap-1 text-[10px] text-white/30 hover:text-[#e8c84a]/70 cursor-pointer whitespace-nowrap transition-colors">
                <i className={copied === cfg.key ? "ri-check-line text-emerald-400" : "ri-clipboard-line"}></i>
                {copied === cfg.key ? "Đã copy" : "Copy"}
              </button>
            </div>
            <pre className="px-3 py-2.5 text-[10px] text-white/40 font-mono overflow-x-auto leading-relaxed">{cfg.code}</pre>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5">
      <div className="space-y-4">
        <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
          <h3 className="text-white font-semibold text-sm mb-3"><i className="ri-list-unordered text-[#e8c84a] mr-2"></i>Nhập danh sách từ vựng</h3>
          <p className="text-white/30 text-xs mb-3">Mỗi dòng một từ. Định dạng: <code className="bg-white/5 px-1 rounded text-[#e8c84a]">tiếng_hàn,nghĩa_việt</code></p>
          <textarea value={bulkInput} onChange={e => setBulkInput(e.target.value)}
            placeholder={"안전모,mũ bảo hộ\n비상구,lối thoát hiểm\n작업복,đồng phục lao động\n소화기,bình chữa cháy"}
            rows={8} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-[#e8c84a]/30 placeholder-white/20 resize-none font-mono" />
          <div className="flex gap-2 mt-3">
            ...
            {items.length > 0 && (
              <button onClick={runGeneration} disabled={running || !apiKey.trim()}
                className="flex items-center gap-2 px-4 py-2 bg-[#e8c84a] hover:bg-[#d4b43a] disabled:opacity-40 text-[#0f1117] font-bold text-sm rounded-xl cursor-pointer whitespace-nowrap transition-colors">
          ...
          <div className="bg-[#0f1117] border border-white/5 rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 border-b border-white/5">
              <div className="flex items-center gap-3">
                ...
                <span className="text-emerald-400 text-xs">{doneCount} xong</span>
            ...
            <div className="divide-y divide-white/3 max-h-96 overflow-y-auto">
              {items.map(item => (
                <div key={item.id} className="grid grid-cols-[1fr_1fr_1fr_32px] gap-3 px-5 py-3 items-center">
                  ...
                  <div>
                    {item.status === "done" && <span className="text-[#e8c84a] text-sm font-medium">{item.phonetic}</span>}
                    {item.status === "generating" && <span className="flex items-center gap-1.5 text-white/30 text-xs"><i className="ri-loader-4-line animate-spin text-[#e8c84a]"></i>Đang tạo...</span>}
                  ...
                  <div className="flex items-center justify-center">
                    ...
                    {item.status === "generating" && <div className="w-3 h-3 border border-[#e8c84a]/30 border-t-[#e8c84a] rounded-full animate-spin"></div>}
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="space-y-4">
        <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
          <h3 className="text-white font-semibold text-sm mb-1"><i className="ri-key-2-line text-[#e8c84a] mr-2"></i>OpenRouter API Key</h3>
          <p className="text-white/30 text-[10px] mb-3 leading-relaxed">
            ...
            <a href="https://openrouter.ai/keys" target="_blank" rel="noopener noreferrer" className="text-[#e8c84a]/70 hover:underline">openrouter.ai/keys</a>
          </p>
          <div className="relative mb-3">
            <input type={showKey ? "text" : "password"} value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder="sk-or-v1-..."
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-[#e8c84a]/30 placeholder-white/20 pr-10 font-mono" />
          ...
          {apiKey ? (
            <div className="flex items-center gap-1.5 text-emerald-400 text-[10px]"><i className="ri-shield-check-line"></i>API key đã lưu (chỉ trong trình duyệt này)</div>
          ) : (
            <div className="flex items-center gap-1.5 text-white/25 text-[10px]"><i className="ri-information-line"></i>Nhập API key để bắt đầu tạo phiên âm</div>
          )}
        </div>
        <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
          ...
          <div className="space-y-2">
            {OPENROUTER_MODELS.map(m => (
              <button key={m.id} onClick={() => setModel(m.id)}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border text-left transition-all cursor-pointer ${model === m.id ? "border-[#e8c84a]/30 bg-[#e8c84a]/5" : "border-white/5 hover:border-white/10 hover:bg-white/3"}`}>
                <div className={`w-3 h-3 rounded-full border-2 flex-shrink-0 ${model === m.id ? "border-[#e8c84a] bg-[#e8c84a]" : "border-white/20"}`}></div>
                <div>
                  <p className={`text-xs font-medium ${model === m.id ? "text-[#e8c84a]" : "text-white/60"}`}>{m.label}</p>
          ...
          <div className="space-y-2.5">
            {[
              ...
              <div key={i} className="flex items-start gap-2.5">
                ...
                <p className="text-white/40 text-[10px] leading-relaxed">{tip.text}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        activeTab === "upload" && uploads.length > 0 ? (
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleUploadAll} disabled={pendingCount === 0 || uploadAll}
              className="flex items-center gap-2 bg-[#e8c84a] hover:bg-[#d4b43a] disabled:opacity-40 text-[#0f1117] font-bold text-sm px-4 py-2.5 rounded-xl transition-colors cursor-pointer whitespace-nowrap">
    ...
    >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                  ...
                  <div key={s.step} className="bg-white/3 rounded-xl p-4 text-center">
                    <div className="w-10 h-10 flex items-center justify-center rounded-xl bg-[#e8c84a]/10 mx-auto mb-3">
                      <i className={`${s.icon} text-[#e8c84a] text-lg`}></i>
                    </div>
      ...
      {activeTab === "audio" && (
        <div className="grid grid-cols-2 gap-5">
          <AudioGenerator />
          <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
            <h3 className="text-white font-semibold text-sm mb-4"><i className="ri-information-line text-[#e8c84a] mr-2"></i>Quy trình âm thanh TTS</h3>
            <div className="space-y-3">
              {[
