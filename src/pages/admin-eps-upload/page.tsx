...
function DropZone({ onFiles }: { onFiles: (files: File[]) => void }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-white/60 font-semibold text-sm mb-1">{dragging ? "Th? ?nh vào dây!" : "Kéo th? ?nh ho?c click d? ch?n"}</p>
      <p className="text-app-text-muted text-xs">H? tr? JPG, PNG, WebP · Nhi?u file cùng lúc</p>
    </div>
  );
}
...
function UploadCard({ item, questions, onAssign, onRemove, onUpload }: {
  ...
  return (
    <div className={`bg-app-bg border rounded-xl p-4 transition-all ${item.status === "done" ? "border-emerald-500/20" : item.status === "error" ? "border-red-500/20" : "border-app-border"}`}>
      ...
      <div className="mb-3">
        <label className="text-app-text-muted text-[10px] block mb-1">Gán vào câu h?i EPS</label>
      ...
      {assignedQ && (
        <div className="bg-app-surface/50 rounded-lg px-2.5 py-2 mb-3">
          <p className="text-app-text-muted text-[9px] mb-0.5">URL dích trên VPS:</p>
      ...
      <button onClick={() => onUpload(item.id)} disabled={!item.questionId || item.status === "uploading" || item.status === "done"}
        ...
        {item.status === "uploading" ? "Ðang upload..." : item.status === "done" ? "Ðã upload" : !item.questionId ? "Ch?n câu h?i tru?c" : "Upload lên VPS"}
      </button>
    </div>
  );
}
...
function AudioGenerator() {
  ...
  return (
    <div className="bg-app-bg border border-app-border rounded-2xl p-5">
      <h3 className="text-white font-semibold text-sm mb-4"><i className="ri-volume-up-line text-app-accent-primary mr-2"></i>T?o âm thanh TTS</h3>
      <div className="flex gap-2 mb-3">
        <input value={word} onChange={e => setWord(e.target.value)} placeholder="Nh?p t? ti?ng Hàn..."
      ...
      {status === "error" && <p className="text-red-400 text-xs">L?i t?o TTS. Th? l?i ho?c dùng Web Speech API.</p>}
    </div>
  );
}
...
function VPSGuide() {
  ...
  const configs = [
    { label: "C?u trúc thu m?c ?nh", key: "dir", code: `/var/www/img.hanquocoi.vn/\n+-- eps/\n¦   +-- safety/\n¦   +-- greeting/\n¦   +-- workplace/\n¦   +-- daily/\n¦   +-- emergency/\n¦   +-- culture/\n¦   +-- law/\n¦   +-- listening/\n¦   +-- reading/` },
  ...
  return (
    <div className="bg-app-bg border border-app-border rounded-2xl p-5">
      <h3 className="text-white font-semibold text-sm mb-4"><i className="ri-server-line text-[#fb923c] mr-2"></i>Hu?ng d?n c?u trúc VPS</h3>
...
const OPENROUTER_MODELS = [
  { id: "google/gemma-3-12b-it:free", label: "Gemma 3 12B (Mi?n phí)" },
  { id: "anthropic/claude-3-haiku", label: "Claude 3 Haiku (Nhanh)" },
  { id: "openai/gpt-4o-mini", label: "GPT-4o Mini" },
  { id: "meta-llama/llama-3.1-8b-instruct:free", label: "Llama 3.1 8B (Mi?n phí)" },
];

async function generatePhonetic(korean: string, meaning: string, apiKey: string, model: string): Promise<string> {
  const prompt = `B?n là chuyên gia ti?ng Hàn. Hãy t?o phiên âm ti?ng Vi?t (cách d?c) cho t? ti?ng Hàn sau theo chu?n phiên âm ti?ng Vi?t d? d?c (không dùng IPA, không dùng Romanization Latin).\n\nT? ti?ng Hàn: ${korean}\nNghia ti?ng Vi?t: ${meaning}\n\nCh? tr? v? phiên âm ti?ng Vi?t, không gi?i thích thêm. Ví d?: ????? ? "an-nhong-ha-xê-yo"`;
  const res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
    method: "POST",
    headers: { "Authorization": `Bearer ${apiKey}`, "Content-Type": "application/json", "HTTP-Referer": "https://hanquocoi.vn", "X-Title": "Hàn Qu?c Oi! Admin" },
...
function PhoneticGenerator() {
  ...
  const runGeneration = async () => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        ...
        const msg = err instanceof Error ? err.message : "L?i không xác d?nh";
  ...
  const exportResult = () => {
    ...
    navigator.clipboard.writeText(`Ti?ng Hàn\tNghia ti?ng Vi?t\tPhiên âm\n${lines}`).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); });
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5">
      <div className="space-y-4">
        <div className="bg-app-bg border border-app-border rounded-2xl p-5">
          <h3 className="text-white font-semibold text-sm mb-3"><i className="ri-list-unordered text-app-accent-primary mr-2"></i>Nh?p danh sách t? v?ng</h3>
          <p className="text-app-text-muted text-xs mb-3">M?i dòng m?t t?. Ð?nh d?ng: <code className="bg-app-card/50 px-1 rounded text-app-accent-primary">ti?ng_hàn,nghia_vi?t</code></p>
          ...
          <div className="flex gap-2 mt-3">
            ...
            {items.length > 0 && (
              <button onClick={runGeneration} disabled={running || !apiKey.trim()}
                ...
                {running ? `Ðang t?o... (${doneCount}/${items.length})` : `T?o phiên âm (${items.length} t?)`}
              </button>
            )}
            {running && (
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 border-b border-app-border">
              <div className="flex items-center gap-3">
                <span className="text-white font-semibold text-sm">K?t qu?</span>
                <span className="text-app-accent-success text-xs">{doneCount} xong</span>
                {errorCount > 0 && <span className="text-red-400 text-xs">{errorCount} l?i</span>}
              </div>
              {doneCount > 0 && (
                <button onClick={exportResult}
                  ...
                  <i className={copied ? "ri-check-line" : "ri-clipboard-line"}></i>{copied ? "Ðã copy!" : "Copy k?t qu?"}
                </button>
              )}
            </div>
              ...
              {items.map(item => (
                <div key={item.id} className="grid grid-cols-[1fr_1fr_1fr_32px] gap-3 px-5 py-3 items-center">
                  ...
                  <div>
                    ...
                    {item.status === "generating" && <span className="flex items-center gap-1.5 text-app-text-muted text-xs"><i className="ri-loader-4-line animate-spin text-app-accent-primary"></i>Ðang t?o...</span>}
                    {item.status === "pending" && <span className="text-app-text-muted text-xs">Ch? x? lý</span>}
                    {item.status === "error" && <span className="text-red-400 text-xs" title={item.error}>L?i</span>}
                  </div>
      ...
      <div className="space-y-4">
        <div className="bg-app-bg border border-app-border rounded-2xl p-5">
          ...
          <p className="text-app-text-muted text-[10px] mb-3 leading-relaxed">
            Key luu trong localStorage — không g?i lên server. L?y key mi?n phí t?i{" "}
          ...
          {apiKey ? (
            <div className="flex items-center gap-1.5 text-app-accent-success text-[10px]"><i className="ri-shield-check-line"></i>API key dã luu (ch? trong trình duy?t này)</div>
          ) : (
            <div className="flex items-center gap-1.5 text-app-text-muted text-[10px]"><i className="ri-information-line"></i>Nh?p API key d? b?t d?u t?o phiên âm</div>
          )}
        </div>
        <div className="bg-app-bg border border-app-border rounded-2xl p-5">
          <h3 className="text-white font-semibold text-sm mb-3"><i className="ri-robot-line text-[#a78bfa] mr-2"></i>Ch?n model AI</h3>
          <div className="space-y-2">
        ...
        <div className="bg-app-bg border border-app-border rounded-2xl p-5">
          <h3 className="text-white font-semibold text-sm mb-3"><i className="ri-lightbulb-line text-[#fb923c] mr-2"></i>Hu?ng d?n s? d?ng</h3>
          <div className="space-y-2.5">
            {[
              { icon: "ri-file-text-line", color: "app-accent-primary", text: "Nh?p t? v?ng: ti?ng_hàn,nghia_vi?t (m?i dòng 1 t?)" },
              { icon: "ri-magic-line", color: "#a78bfa", text: "AI t?o phiên âm ti?ng Vi?t d? d?c, không ph?i Romanization Latin" },
              { icon: "ri-clipboard-line", color: "#34d399", text: "Copy k?t qu? và paste vào file epsVocabulary.ts d? c?p nh?t" },
              { icon: "ri-shield-line", color: "#fb923c", text: "API key ch? luu trong trình duy?t, không g?i lên server c?a app" },
            ].map((tip, i) => (
...
export default function AdminEpsUploadPage() {
  ...
  const tabs = [
    ...
    { id: "audio", label: "T?o âm thanh", icon: "ri-volume-up-line" },
    { id: "guide", label: "Hu?ng d?n VPS", icon: "ri-server-line" },
  ] as const;

  return (
    <DashboardLayout
      title="Admin Upload EPS"
      subtitle="Upload ?nh, t?o phiên âm AI và âm thanh TTS cho câu h?i EPS"
      actions={
        activeTab === "upload" && uploads.length > 0 ? (
          <div className="flex items-center gap-3">
            <span className="text-app-text-secondary text-xs">{doneCount}/{uploads.length} dã upload</span>
            <button onClick={handleUploadAll} disabled={pendingCount === 0 || uploadAll}
              ...
              {uploadAll ? "Ðang upload..." : `Upload t?t c? (${pendingCount})`}
            </button>
          </div>
        ) : null
      }
          ...
          {uploads.length > 0 && (
            <div className="flex items-center gap-4 px-4 py-3 bg-app-bg border border-app-border rounded-xl">
              {[
                ...
                { label: "Ðã upload", value: doneCount, color: "#34d399" },
              ...
              <button onClick={() => setUploads([])} className="ml-auto text-app-text-muted hover:text-white/50 text-xs cursor-pointer whitespace-nowrap transition-colors">
                <i className="ri-delete-bin-line mr-1"></i>Xóa t?t c?
              </button>
            </div>
          )}
          {uploads.length > 0 ? (
            ...
            <div className="bg-app-bg border border-app-border rounded-2xl p-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                  { step: "1", icon: "ri-drag-drop-line", title: "Kéo th? ?nh", desc: "Kéo nhi?u ?nh cùng lúc vào vùng upload phía trên" },
                  { step: "2", icon: "ri-link-m", title: "Gán câu h?i", desc: "Ch?n câu h?i EPS tuong ?ng cho t?ng ?nh" },
                  { step: "3", icon: "ri-upload-cloud-2-line", title: "Upload VPS", desc: "Click Upload ho?c Upload t?t c? d? d?y lên server" },
                ].map(s => (
      ...
      {activeTab === "audio" && (
        <div className="grid grid-cols-2 gap-5">
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            ...
            <div className="space-y-3">
              {[
                { icon: "ri-translate-2", color: "app-accent-primary", title: "Phiên âm latinh", desc: "??? ? anjeonmo.mp3 (tên file an toàn cho m?i VPS)" },
                { icon: "ri-google-line", color: "#34d399", title: "Google TTS mi?n phí", desc: "T?o MP3 ch?t lu?ng t?t, không c?n API key" },
                { icon: "ri-save-line", color: "#fb923c", title: "Cache vinh vi?n", desc: "L?n d?u nghe ? cache browser. L?n sau phát t? cache" },
                { icon: "ri-server-line", color: "#a78bfa", title: "Upload VPS", desc: "T?i file MP3 v? ? upload lên audio.hanquocoi.vn/tts/" },
              ].map(s => (
