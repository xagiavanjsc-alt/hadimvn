  ...
  return (
    <div className="bg-[#0f1117] border border-rose-500/20 rounded-2xl p-5 space-y-4">
      ...
      <div className="space-y-3">
        ...
        <div>
          <label className="text-white/40 text-xs mb-1 block">Alt text</label>
          <input type="text" value={imageAlt} onChange={e => setImageAlt(e.target.value)}
            placeholder="Mô tả ngắn về ảnh..."
            className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-white/70 text-sm outline-none focus:border-rose-400/40 placeholder-white/20" />
        </div>
        <div>
          <label className="text-white/40 text-xs mb-1 block">Chú thích ảnh</label>
    ...
    <div className="space-y-5">
      <div className="bg-[#0f1117] border border-white/8 rounded-2xl p-5">
        ...
        <div className="bg-white/3 rounded-xl p-3 mb-4 border border-white/5">
          <p className="text-white/30 text-[10px] mb-2 font-medium">Format chuẩn:</p>
          <pre className="text-rose-400/70 text-[10px] font-mono leading-relaxed overflow-x-auto">{sampleCSV}</pre>
          <button onClick={() => { setImportText(sampleCSV); setParseResult(null); }} className="mt-2 text-[10px] text-white/30 hover:text-white/60 cursor-pointer whitespace-nowrap">
        ...
        <textarea value={importText} onChange={e => { setImportText(e.target.value); setParseResult(null); }}
          ...
          className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-white/70 text-xs font-mono outline-none focus:border-rose-400/40 placeholder-white/20 resize-none" />
        ...
        <div className="space-y-3">
          ...
          {parseResult.dupes.length > 0 && (
            <div className="bg-[#e8c84a]/5 border border-[#e8c84a]/20 rounded-xl p-4">
              <p className="text-[#e8c84a] text-sm font-semibold mb-2"><i className="ri-skip-forward-line mr-2"></i>{parseResult.dupes.length} từ trùng lặp</p>
...
function VpsGuide() {
  ...
  return (
    <div className="space-y-5">
      {[
        { label: "Cấu trúc thư mục", key: "folder", code: folderStructure, color: "text-emerald-400" },
        { label: "Nginx config (CORS)", key: "nginx", code: nginxConfig, color: "text-sky-400" },
      ].map(cfg => (
        <div key={cfg.key} className="bg-[#0f1117] border border-white/8 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-white font-semibold text-sm">{cfg.label}</h3>
            <button onClick={() => copy(cfg.code, cfg.key)} className="flex items-center gap-1 text-[10px] text-white/30 hover:text-rose-400/70 cursor-pointer whitespace-nowrap transition-colors">
      ...
      <div className="flex items-center gap-1 bg-white/5 rounded-xl p-1 mb-6 w-fit">
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-medium transition-all cursor-pointer whitespace-nowrap ${activeTab === tab.id ? "bg-rose-500 text-white" : "text-white/40 hover:text-white/60"}`}>
        ...
        <div>
          ...
          <div className="space-y-3 mb-5">
            <div className="flex items-center gap-2 bg-white/5 border border-white/8 rounded-xl px-3 py-2.5">
              ...
              {searchQ && <button onClick={() => setSearchQ("")} className="text-white/30 hover:text-white/60 cursor-pointer"><i className="ri-close-line text-sm"></i></button>}
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-white/25 text-xs ml-auto">{filteredQuestions.length}/{epsQuestions.length} câu</span>
            </div>
          </div>
          ...
          <VirtualList
            ...
            renderItem={(q: unknown) => {
              ...
              return (
                <div className="bg-[#0f1117] border border-white/5 rounded-2xl overflow-hidden h-full">
                  <div className="flex items-start gap-4 p-4 h-full">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <span className="text-white/25 text-[10px] font-mono">#{question.id}</span>
                        ...
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${question.difficulty === "easy" ? "bg-emerald-500/10 text-emerald-400" : question.difficulty === "medium" ? "bg-[#f59e0b]/10 text-[#f59e0b]" : "bg-red-500/10 text-red-400"}`}>
                      ...
                      <p className="text-white/30 text-xs italic mt-0.5 line-clamp-1">{question.questionVi}</p>
                    </div>
                    <button onClick={() => setEditingId(isEditing ? null : question.id)}
                      className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium cursor-pointer whitespace-nowrap transition-colors flex-shrink-0 ${isEditing ? "bg-rose-500/15 text-rose-400 border border-rose-500/25" : "bg-white/5 text-white/40 hover:text-white/60 border border-white/8"}`}>
