...
function QuestionItem({
  ...
  const sectionLabel = q.section === "listening" ? "Nghe" : "Ð?c";
...
function ResultScreen({
  ...
  const getGrade = () => {
    ...
    if (totalScore >= 80) return { label: "Ð?t", color: "#a78bfa" };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-app-bg border border-app-border rounded-2xl p-8 text-center">
        <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-bold mb-4`} style={{ backgroundColor: `${grade.color}15`, color: grade.color }}>
          ...
          {grade.label} — {passed ? "Ð?T TOPIK I" : "CHUA Ð?T"}
        </div>
        ...
        <p className="text-app-text-muted text-xs">Ði?u ki?n d?t: T?ng = 80 di?m, m?i ph?n = 40 di?m</p>
      ...
      <div className="grid grid-cols-2 gap-4">
        {[
          ...
          { label: "Ph?n Ð?c (??)", score: readingScore, max: maxReading, correct: readingCorrect, total: READING_COUNT, color: "#a78bfa", icon: "ri-book-open-line" },
        ].map(sec => {
            ...
            <div key={sec.label} className={`bg-app-bg border rounded-2xl p-5 ${secPassed ? "border-app-border" : "border-red-500/20"}`}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <div className={`ml-auto text-[10px] font-bold px-2 py-0.5 rounded-full ${secPassed ? "bg-emerald-500/10 text-app-accent-success" : "bg-red-500/10 text-red-400"}`}>
                  {secPassed ? "Ð?T" : "CHUA Ð?T"}
                </div>
              </div>
              <div className="text-2xl font-bold mb-1" style={{ color: sec.color }}>{sec.score}</div>
              <p className="text-app-text-muted text-xs mb-3">/ {sec.max} di?m · {sec.correct}/{sec.total} câu dúng</p>
...
export default function TopikTestPage() {
  ...
  if (phase === "intro") {
    return (
      <DashboardLayout title="Thi th? TOPIK I" subtitle="Format chu?n — 50 câu, 100 phút, phân tích k?t qu? chi ti?t">
        <div className="max-w-2xl mx-auto space-y-5">
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-8 text-center">
            ...
            <p className="text-app-text-secondary text-sm mb-6">??????? — K? thi nang l?c ti?ng Hàn</p>
            {bestScore > 0 && (
              <div className="inline-flex items-center gap-2 bg-app-accent-primary/8 border border-app-accent-primary/15 rounded-xl px-4 py-2 mb-4">
                ...
                <span className="text-app-accent-primary text-sm font-semibold">Ði?m cao nh?t: {bestScore} di?m</span>
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              { icon: "ri-time-line", label: "Th?i gian", value: "100 phút", color: "app-accent-primary" },
              { icon: "ri-question-line", label: "S? câu h?i", value: "50 câu", color: "#34d399" },
              { icon: "ri-headphone-line", label: "Ph?n Nghe", value: "30 câu", color: "#38bdf8" },
              { icon: "ri-book-open-line", label: "Ph?n Ð?c", value: "20 câu", color: "#a78bfa" },
              { icon: "ri-bar-chart-line", label: "Ði?m t?i da", value: "200 di?m", color: "#fb923c" },
              { icon: "ri-checkbox-circle-line", label: "Ði?m d?t", value: "= 80 di?m", color: "#f472b6" },
            ].map(info => (
          ...
          {/* Rules */}
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <p className="text-white/50 text-sm font-semibold mb-3">Luu ý tru?c khi thi</p>
            <ul className="space-y-2">
              {[
                "M?i câu có 4 l?a ch?n, ch? có 1 dáp án dúng",
                "Ph?n nghe: d?c k? h?i tho?i/do?n van du?c cung c?p",
                "Ði?u ki?n d?t: T?ng = 80 di?m VÀ m?i ph?n = 40 di?m",
                "Có th? chuy?n qua l?i gi?a ph?n Nghe và Ð?c",
                "K?t qu? s? du?c phân tích chi ti?t sau khi n?p bài",
              ].map((rule, i) => (
  ...
  if (phase === "result") {
    return (
      <DashboardLayout title="K?t qu? TOPIK I" subtitle="Phân tích chi ti?t theo t?ng ph?n">
  ...
  return (
    <DashboardLayout
      ...
      subtitle={`${answeredCount}/${TOTAL_QUESTIONS} câu dã tr? l?i`}
      actions={
        <button
        ...
        >
          ...
          N?p bài ({answeredCount}/{TOTAL_QUESTIONS})
        </button>
      }
    >
      {/* Timer bar */}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-1">
              <span className="text-app-text-muted text-[10px]">Th?i gian còn l?i</span>
          ...
          <div className="flex items-center gap-3 text-xs">
            ...
            <span className="text-[#a78bfa]/70">Ð?c: {readingAnswered}/{READING_COUNT}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center bg-app-card/50 rounded-xl p-1 mb-5 w-fit">
        {(["listening", "reading"] as const).map(sec => (
          ...
          >
            ...
            {sec === "listening" ? `Ph?n Nghe (${listeningAnswered}/${LISTENING_COUNT})` : `Ph?n Ð?c (${readingAnswered}/${READING_COUNT})`}
          </button>
        ))}
      </div>
      ...
      <div className="mt-8 flex justify-center">
        ...
        >
          <i className="ri-send-plane-fill"></i>
          N?p bài ({answeredCount}/{TOTAL_QUESTIONS} câu)
        </button>
      </div>
    </DashboardLayout>
  );
