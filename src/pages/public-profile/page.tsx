    ...
    <div className="bg-gradient-to-br from-[#1a1f2e] to-[#0f1117] rounded-2xl p-6 border border-app-border mb-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { icon: "ri-trophy-line", color: "app-accent-primary", label: "Ði?m EPS cao nh?t", value: `${stats.epsBestScore}%` },
...
export default function PublicProfilePage() {
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      // Load leaderboard stats (c?t dúng: xp, streak — theo schema leaderboard)
      ...
      // Flashcard dã bi?t — d?m t? study_progress.flashcard_known (jsonb)
      ...
      // Huy hi?u — t?m tính d?a trên các milestone streak + XP + words_learned
      const badgeCount =
        (streak >= 7 ? 1 : 0) +
        (streak >= 30 ? 1 : 0) +
        (streak >= 100 ? 1 : 0) +
  ...
  if (loading) {
    return (
      <DashboardLayout title="H? so h?c viên">
        <div className="p-8 flex items-center justify-center min-h-64">
          <div className="text-center">
            ...
            <p className="text-gray-400 text-sm">Ðang t?i h? so...</p>
          </div>
        </div>
      </DashboardLayout>
    );
    ...
    return (
      <DashboardLayout title="H? so h?c viên">
        <div className="p-8 text-center">
          ...
          <h3 className="text-gray-700 font-bold text-lg mb-2">Không tìm th?y h? so</h3>
          <p className="text-gray-400 text-sm mb-6">H? so này không t?n t?i ho?c dã b? xóa</p>
          <button
            onClick={() => navigate("/")}
  ...
  return (
    <DashboardLayout title="H? so h?c viên công khai">
      <div className="p-6 md:p-8 max-w-2xl mx-auto">
        ...
        {isOwnProfile && (
          <div className="mb-4 flex items-center gap-3 px-4 py-3 bg-app-accent-primary/10 border border-app-accent-primary/20 rounded-xl">
            ...
            <p className="text-app-accent-primary text-sm flex-1">Ðây là h? so công khai c?a b?n — ngu?i khác th?y giao di?n này khi click link</p>
            <button
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-4 shadow-sm">
          ...
          {stats && (
            <div className="grid grid-cols-2 gap-3">
              {[
                { icon: "ri-fire-line", color: "#fb923c", bg: "bg-orange-50", label: "Streak hi?n t?i", value: `${stats.streak} ngày` },
                { icon: "ri-star-line", color: "app-accent-primary", bg: "bg-yellow-50", label: "T?ng XP", value: stats.totalXP.toLocaleString() },
                { icon: "ri-trophy-line", color: "#34d399", bg: "bg-emerald-50", label: "Ði?m EPS cao nh?t", value: `${stats.epsBestScore}%` },
                { icon: "ri-file-list-3-line", color: "#06b6d4", bg: "bg-cyan-50", label: "L?n thi EPS", value: stats.epsExamCount },
                { icon: "ri-stack-line", color: "#a78bfa", bg: "bg-purple-50", label: "T? v?ng dã thu?c", value: stats.flashcardKnown },
