...
const QUALITY_LABELS = [
  ...
  { q: 3 as const, label: "Được", color: "#e8c84a", bg: "bg-[#e8c84a]/10 border-[#e8c84a]/20 hover:bg-[#e8c84a]/20", icon: "ri-emotion-normal-line" },
    ...
    <div className="max-w-lg mx-auto">
      <div className="flex items-center gap-3 mb-5">
        <div className="flex-1 h-2 bg-white/5 rounded-full overflow-hidden">
          <div className="h-full bg-[#e8c84a] rounded-full transition-all duration-500" style={{ width: `${(sessionIdx / sessionTotal) * 100}%` }} />
        </div>
        <span className="text-white/30 text-xs whitespace-nowrap">{sessionIdx}/{sessionTotal}</span>
      </div>
      <div className="bg-[#0f1117] border border-white/8 rounded-2xl overflow-hidden mb-4">
        <div className="p-8 text-center border-b border-white/5">
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            {srCard.repetitions > 0 && <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/5 text-white/30">Lần {srCard.totalReviews + 1} · {nextReviewText()}</span>}
          </div>
          <p className="text-5xl font-bold text-white mb-3">{item.korean}</p>
          <p className="text-white/25 text-sm font-mono">[{item.reading}]</p>
          <button onClick={() => playKorean(item.korean)} className="mt-4 flex items-center gap-2 mx-auto px-4 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-white/40 hover:text-white/70 text-sm transition-colors cursor-pointer whitespace-nowrap">
            <i className="ri-volume-up-line"></i>Nghe phát âm
          </button>
        </div>
        {!revealed ? (
          <div className="p-6 text-center">
            <button onClick={() => setRevealed(true)} className="px-8 py-3 bg-[#e8c84a] hover:bg-[#d4b43a] text-[#0f1117] font-bold text-sm rounded-xl transition-colors cursor-pointer whitespace-nowrap">Xem nghĩa</button>
            <p className="text-white/20 text-xs mt-3">Thử nhớ nghĩa trước khi xem đáp án</p>
          </div>
        ) : (
          <div className="p-6">
            <div className="text-center mb-5">
              <p className="text-[#e8c84a] text-2xl font-bold mb-2">{item.vietnamese}</p>
              {item.example && (
                <div className="bg-white/3 rounded-xl p-3 mt-3 text-left">
                  <p className="text-white/50 text-xs">{item.example}</p>
                  <p className="text-white/25 text-[10px] italic mt-1">{item.exampleVi}</p>
                </div>
              )}
            </div>
      ...
      <div className="bg-[#0f1117] border border-white/8 rounded-2xl overflow-hidden mb-4">
        ...
        <div className="p-5 border-b border-white/5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/5 text-white/30">Ôn lần {srCard.totalReviews + 1}</span>
          </div>
          <p className="text-white font-semibold text-base leading-relaxed mb-1">{question.question}</p>
          <p className="text-white/40 text-sm italic">{question.questionVi}</p>
        </div>

        {/* Options */}
        <div className="p-5 space-y-2.5">
          {question.options.map((opt, idx) => {
            let cls = "border-white/8 bg-white/3 text-white/70 hover:bg-white/8";
            if (revealed) {
              if (idx === question.correctIndex) cls = "border-emerald-500/40 bg-emerald-500/10 text-emerald-400";
            ...
            return (
              ...
              >
                ...
                {revealed && idx === question.correctIndex && <i className="ri-checkbox-circle-fill text-emerald-400 ml-auto flex-shrink-0 mt-0.5"></i>}
          ...
          <div className="px-5 pb-5 border-t border-white/5 pt-4">
            <div className={`rounded-xl p-4 mb-4 ${isCorrect ? "bg-emerald-500/8 border border-emerald-500/15" : "bg-red-500/8 border border-red-500/15"}`}>
              <div className="flex items-center gap-2 mb-2">
                <i className={`${isCorrect ? "ri-checkbox-circle-fill text-emerald-400" : "ri-close-circle-fill text-red-400"} text-base`}></i>
                <span className={`text-sm font-bold ${isCorrect ? "text-emerald-400" : "text-red-400"}`}>{isCorrect ? "Chính xác!" : "Chưa đúng"}</span>
              </div>
...
function SessionComplete({ results, onRestart, onClose }: {
  ...
  return (
    <div className="max-w-md mx-auto text-center py-8">
      <div className="w-20 h-20 flex items-center justify-center rounded-2xl bg-[#e8c84a]/10 mx-auto mb-5">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-[#0f1117] border border-white/5 rounded-xl p-4">
          <p className="text-[#e8c84a] font-bold text-2xl">{pct}%</p>
          <p className="text-white/40 text-xs mt-1">Tỷ lệ đúng</p>
        </div>
        <div className="bg-[#0f1117] border border-white/5 rounded-xl p-4">
          <p className="text-[#a78bfa] font-bold text-2xl">{results.newCards}</p>
          <p className="text-white/40 text-xs mt-1">Thẻ mới</p>
        </div>
      </div>
      <div className="flex gap-3">
        <button onClick={onRestart} className="flex-1 py-3 bg-[#e8c84a] hover:bg-[#d4b43a] text-[#0f1117] font-bold text-sm rounded-xl transition-colors cursor-pointer whitespace-nowrap">
          <i className="ri-refresh-line mr-2"></i>Ôn tiếp
        </button>
        <button onClick={onClose} className="flex-1 py-3 bg-white/5 hover:bg-white/10 border border-white/8 text-white/60 text-sm rounded-xl transition-colors cursor-pointer whitespace-nowrap">Xong</button>
      </div>
    </div>
  );
}
    ...
    <>
      ...
      {wrongQuestions.length === 0 && (
        <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-2xl p-6 mb-5 text-center">
          ...
          <p className="text-emerald-400 font-semibold text-sm mb-1">Chưa có câu sai nào!</p>
          <p className="text-white/40 text-xs">Hãy làm bài thi EPS trước. Các câu sai sẽ tự động xuất hiện ở đây.</p>
        </div>
      )}
          ...
          <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-all cursor-pointer whitespace-nowrap ${selectedTopic === "all" ? "bg-[#f87171] text-white" : "bg-white/5 text-white/40 hover:text-white/60"}`}
              >
              ...
              {topicsWithWrong.map(topic => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium transition-all cursor-pointer whitespace-nowrap ${selectedTopic === topic.id ? "text-white" : "bg-white/5 text-white/40 hover:text-white/60"}`}
          ...
          <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
            ...
            <div className="flex items-center gap-4 mb-5">
              ...
              <div className="flex items-center bg-white/5 rounded-xl p-1">
                {[10, 20, 30, 50].map(n => (
                  <button key={n} onClick={() => setMaxCards(n)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer whitespace-nowrap ${maxCards === n ? "bg-[#f87171] text-white" : "text-white/40 hover:text-white/60"}`}>{n}</button>
                ))}
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-5">
              <div className="bg-white/3 rounded-xl p-3">
                <p className="text-white/40 text-xs mb-1">Cần ôn lại</p>
                <p className="text-[#f87171] font-bold text-xl">{dueCards.length}</p>
                <p className="text-white/25 text-[10px]">câu đến hạn hôm nay</p>
              </div>
              <div className="bg-white/3 rounded-xl p-3">
                <p className="text-white/40 text-xs mb-1">Trong hàng đợi</p>
                <p className="text-[#e8c84a] font-bold text-xl">{Object.keys(srQData).length}</p>
                <p className="text-white/25 text-[10px]">câu đã thêm vào SR</p>
              </div>
            </div>
            <button
      ...
      <div className="grid grid-cols-[1fr_300px] gap-6">
        <div className="space-y-5">
          <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={() => setSelectedTopic("all")} className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-all cursor-pointer whitespace-nowrap ${selectedTopic === "all" ? "bg-[#e8c84a] text-[#0f1117]" : "bg-white/5 text-white/40 hover:text-white/60"}`}>
              ...
              {EPS_VOCAB_TOPICS.map(topic => {
                ...
                return (
                  <button key={topic.id} onClick={() => setSelectedTopic(topic.id)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium transition-all cursor-pointer whitespace-nowrap ${selectedTopic === topic.id ? "text-[#0f1117]" : "bg-white/5 text-white/40 hover:text-white/60"}`} style={selectedTopic === topic.id ? { backgroundColor: topic.color } : {}}>
          ...
          <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
            ...
            <div className="flex items-center gap-4 mb-5">
              ...
              <div className="flex items-center bg-white/5 rounded-xl p-1">
                {[10, 20, 30, 50].map(n => (
                  <button key={n} onClick={() => setMaxCards(n)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer whitespace-nowrap ${maxCards === n ? "bg-[#e8c84a] text-[#0f1117]" : "text-white/40 hover:text-white/60"}`}>{n}</button>
                ))}
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-5">
              <div className="bg-white/3 rounded-xl p-3">
                <p className="text-white/40 text-xs mb-1">Cần ôn lại</p>
                <p className="text-[#f87171] font-bold text-xl">{dueCards.length}</p>
                <p className="text-white/25 text-[10px]">từ đến hạn hôm nay</p>
              </div>
              <div className="bg-white/3 rounded-xl p-3">
                <p className="text-white/40 text-xs mb-1">Từ mới</p>
                <p className="text-[#e8c84a] font-bold text-xl">{newCards.length}</p>
                <p className="text-white/25 text-[10px]">chưa học lần nào</p>
              </div>
            </div>
            <button onClick={startSession} disabled={dueCards.length === 0 && newCards.length === 0} className="w-full flex items-center justify-center gap-2 py-3.5 bg-[#e8c84a] hover:bg-[#d4b43a] disabled:opacity-40 text-[#0f1117] font-bold text-sm rounded-xl transition-colors cursor-pointer whitespace-nowrap">
            ...
            {dueCards.length === 0 && newCards.length === 0 && (
              <p className="text-emerald-400 text-xs text-center mt-3"><i className="ri-checkbox-circle-line mr-1"></i>Tuyệt vời! Bạn đã ôn hết từ vựng hôm nay.</p>
            )}
          </div>
        </div>
        <div className="space-y-4">
                ...
                return (
                  <div key={b.label}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-white/30 text-xs">{b.count} từ</span>
                    </div>
          ...
          <div className="bg-[#e8c84a]/5 border border-[#e8c84a]/15 rounded-xl p-4">
            <p className="text-[#e8c84a] text-xs font-semibold mb-2">Thuật toán SM-2 hoạt động thế nào?</p>
            <div className="space-y-2 text-[10px] text-white/35 leading-relaxed">
              <p><i className="ri-check-line text-emerald-400 mr-1"></i>Từ bạn nhớ tốt → ôn lại sau nhiều ngày hơn</p>
              <p><i className="ri-close-line text-red-400 mr-1"></i>Từ bạn quên → ôn lại ngay ngày mai</p>
              <p><i className="ri-time-line text-[#e8c84a] mr-1"></i>Khoảng cách tăng dần: 1 → 6 → 15 → 30 ngày...</p>
...
export default function EpsSpacedReviewPage() {
  ...
  return (
    ...
    >
      {/* Tab switcher */}
      <div className="flex items-center bg-white/5 rounded-xl p-1 mb-6 w-fit">
        {tabs.map(tab => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold transition-all cursor-pointer whitespace-nowrap ${activeTab === tab.id ? "text-[#0f1117]" : "text-white/40 hover:text-white/70"}`}
