  ...
  return (
    <div className="flex flex-col items-center gap-5 w-full max-w-xl mx-auto">
      ...
      {swipeHint && (
        <div className="fixed inset-0 pointer-events-none z-10 flex items-center justify-center">
          <div className={`px-6 py-3 rounded-2xl text-white font-bold text-lg ${swipeHint === "right" ? "bg-emerald-500/80" : "bg-red-500/80"}`}>
            {swipeHint === "right" ? <><i className="ri-check-line mr-2"></i>Ðã thu?c</> : <><i className="ri-close-line mr-2"></i>Chua thu?c</>}
          </div>
        </div>
      )}
      ...
      <div className="w-full flex items-center gap-3">
        <p className="text-app-text-muted text-xs whitespace-nowrap">{current}/{total}</p>
        <div className="flex-1 h-1.5 bg-app-card/50 rounded-full overflow-hidden">
          <div className="h-full rounded-full bg-app-accent-primary transition-all" style={{ width: `${(current / total) * 100}%` }} />
        </div>
      </div>
      ...
      >
        ...
        >
          {/* Front */}
          <div
            className="absolute inset-0 rounded-2xl border border-app-border flex flex-col items-center justify-center p-8 text-center bg-app-bg"
          ...
          >
            ...
            <div className="flex items-center gap-2 mb-5">
              ...
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: `${diffColor}15`, color: diffColor }}>
                {card.difficulty === "easy" ? "D?" : card.difficulty === "medium" ? "TB" : "Khó"}
              </span>
            </div>
...
function SessionResult({
  ...
  const label = pct >= 80 ? "Xu?t s?c!" : pct >= 60 ? "Khá t?t!" : "C?n ôn thêm!";
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center max-w-sm mx-auto">
      ...
      <p className="text-app-text-secondary text-sm mb-5">
        Ðã thu?c <span className="font-bold" style={{ color }}>{known}/{total}</span> t? ({pct}%)
      </p>
...
export default function EpsFlashcardPage() {
  ...
  return (
    <DashboardLayout
      ...
      subtitle="H?c t? v?ng EPS có h? th?ng — an toàn, pháp lu?t, giao ti?p"
      actions={
        mode === "browse" ? (
          <button
            onClick={startStudy}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mb-4 flex-wrap">
              ...
              <div className="flex items-center gap-2 bg-app-card/50 border border-app-border rounded-xl px-3 py-2 flex-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {filteredCards.map(card => {
                  ...
                  return (
                    <div
                      key={card.id}
                      className={`p-4 rounded-xl border transition-all ${isMastered ? "border-emerald-500/20 bg-emerald-500/5" : "border-app-border bg-app-bg hover:border-app-border"}`}
                    >
                      ...
                      <p className="text-app-text-muted text-[10px] font-mono mb-1.5">[{card.reading}]</p>
                      <p className="text-app-accent-primary text-sm font-semibold mb-2">{card.vietnamese}</p>
                      <p className="text-app-text-muted text-xs leading-relaxed line-clamp-2">{card.exampleVi}</p>
                      {topic && (
            ...
            {EPS_VOCAB_TOPICS.map(topic => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-xl border transition-all cursor-pointer ${isSelected ? "border-white/15 bg-app-card/50" : "border-app-border hover:border-app-border"}`}
                >
