    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-[#0f1117] border border-white/10 rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-5 space-y-4">
          {/* Coupon type */}
          <div>
            <label className="text-white/40 text-xs font-medium block mb-1.5">Loại coupon *</label>
            <div className="flex bg-white/5 border border-white/10 rounded-lg overflow-hidden">
              <button
                onClick={() => setCouponType("ebook")}
                className={`flex-1 px-4 py-2.5 text-xs font-bold transition-colors cursor-pointer whitespace-nowrap ${couponType === "ebook" ? "bg-rose-500 text-white" : "text-white/40 hover:text-white/70"}`}
              >
                <i className="ri-book-line mr-1" />Ebook
              </button>
              <button
                onClick={() => setCouponType("vip")}
                className={`flex-1 px-4 py-2.5 text-xs font-bold transition-colors cursor-pointer whitespace-nowrap ${couponType === "vip" ? "bg-[#e8c84a] text-[#0f1117]" : "text-white/40 hover:text-white/70"}`}
              >
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="20"
                className="flex-1 bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-white text-sm placeholder-white/20 focus:outline-none focus:border-rose-500/40 transition-colors"
              />
              <div className="flex bg-white/5 border border-white/10 rounded-lg overflow-hidden">
                <button
                  onClick={() => setDiscountType("percent")}
                  className={`px-4 py-2.5 text-xs font-bold transition-colors cursor-pointer whitespace-nowrap ${discountType === "percent" ? "bg-rose-500 text-white" : "text-white/40 hover:text-white/70"}`}
                >
                  %
                </button>
                <button
                  onClick={() => setDiscountType("fixed")}
                  className={`px-4 py-2.5 text-xs font-bold transition-colors cursor-pointer whitespace-nowrap ${discountType === "fixed" ? "bg-rose-500 text-white" : "text-white/40 hover:text-white/70"}`}
                >
                  VNĐ
                </button>
              </div>
          ...
          {couponType === "vip" && (
            <div>
              <label className="text-white/40 text-xs font-medium block mb-1.5">Áp dụng cho gói VIP</label>
              <div className="flex bg-white/5 border border-white/10 rounded-lg overflow-hidden">
                <button
                  onClick={() => setVipPlan("month")}
                  className={`flex-1 px-4 py-2.5 text-xs font-bold transition-colors cursor-pointer whitespace-nowrap ${vipPlan === "month" ? "bg-[#e8c84a]/20 text-[#e8c84a]" : "text-white/40 hover:text-white/70"}`}
                >
                  Tháng
                </button>
                <button
                  onClick={() => setVipPlan("year")}
                  className={`flex-1 px-4 py-2.5 text-xs font-bold transition-colors cursor-pointer whitespace-nowrap ${vipPlan === "year" ? "bg-[#e8c84a]/20 text-[#e8c84a]" : "text-white/40 hover:text-white/70"}`}
                >
                  Năm
                </button>
              </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-white/40 text-xs font-medium block mb-1.5">Giới hạn dùng</label>
              <input
                type="number"
                value={maxUsage}
      ...
      actions={
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-2 text-xs font-medium px-3 py-2 rounded-lg transition-colors cursor-pointer whitespace-nowrap bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20"
          >
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        {[
          ...
          <div key={stat.label} className="bg-[#111318] border border-white/5 rounded-xl p-4 flex items-center gap-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
        ...
        {/* Coupon list */}
        <div className="col-span-2 bg-[#111318] border border-white/5 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
            <div>
              <h3 className="text-white font-semibold text-sm">Danh sách coupon</h3>
              <p className="text-white/30 text-xs mt-0.5">{filtered.length} coupon · Supabase</p>
            </div>
            <div className="flex items-center gap-2">
              <select
                value={filterType}
                ...
                return (
                  <div
                    className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-colors group h-full ${
                      c.active ? "bg-white/3 border-white/5 hover:border-white/8" : "bg-white/1 border-white/3 opacity-50"
                    }`}
                  >
                    <button
                    ...
                    >
                      ...
                      <i className={`text-[10px] ${copiedCode === c.code ? "ri-check-line text-emerald-400" : "ri-clipboard-line text-white/20"}`}></i>
                    </button>

                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap ${
                      c.couponType === "vip" ? "bg-[#e8c84a]/10 text-[#e8c84a]" : "bg-rose-500/10 text-rose-400"
                    }`}>
                    ...
                    <span className="text-white/40 text-[10px] flex-1 truncate">{c.channel}</span>
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button
                        ...
                        className="w-6 h-6 flex items-center justify-center bg-white/5 hover:bg-white/10 rounded-lg transition-colors cursor-pointer"
                      >
                        <i className={`text-[10px] ${c.active ? "ri-pause-line text-white/40" : "ri-play-line text-emerald-400"}`}></i>
                      </button>
                      <button
