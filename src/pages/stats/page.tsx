...
function StatCard({ icon, label, value, sub, color, onClick }: {
  ...
  return (
    <div
      onClick={onClick}
      className={`bg-[#0f1117] border border-white/5 rounded-2xl p-5 ${onClick ? "cursor-pointer hover:border-white/10 transition-all" : ""}`}
    >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-[#0f1117] border border-white/10 rounded-2xl w-full max-w-md">
        ...
        <div className="p-5 space-y-4">
          <div>
            <label className="text-white/40 text-xs font-medium block mb-1.5">Series ebook</label>
            <select
              value={seriesId}
              onChange={e => setSeriesId(e.target.value)}
              className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-emerald-400/40 transition-colors cursor-pointer"
            >
              {series.map(s => (
                <option key={s.id} value={s.id} className="bg-[#0f1117]">{s.name}</option>
              ))}
              <option value="" className="bg-[#0f1117]">Khác (không thuộc series)</option>
            </select>
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-white/40 text-xs font-medium block mb-1.5">Số tiền (VNĐ) *</label>
              <input
                type="number"
                value={amount}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-white/40 text-xs font-medium block mb-1.5">Ngày bán</label>
              <input
                type="date"
                value={date}
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-5 mb-5">
        ...
        <div className="col-span-2 bg-[#0f1117] border border-white/5 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-white/30 text-xs mt-0.5">Theo tháng (VNĐ)</p>
            </div>
          ...
          {revenues.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-white/25 text-sm mb-1">Chưa có doanh thu nào</p>
              ...
              <div className="flex gap-3">
                {stats.monthlyChart.map((m, i) => (
                  <div key={i} className="flex-1 text-center">
                    <span className="text-white/25 text-[10px]">{m.label}</span>
                  </div>
                ))}
              </div>
            </>
        ...
        <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
          ...
          <p className="text-white/30 text-xs mb-4">Doanh thu từng ebook</p>
                  ...
                  return (
                    <div key={id}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-white/30 text-[10px]">{data.count} đơn</span>
                          <span className="text-emerald-400 text-xs font-bold">{pct}%</span>
                        </div>
                      </div>
                      <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                        <div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: s?.coverAccent ?? "#34d399" }} />
                      </div>
                      <p className="text-white/30 text-[10px] mt-0.5">{formatVND(data.revenue)}</p>
                    </div>
                  );
                })}
            </div>
      ...
      <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-white/30 text-xs mt-0.5">{revenues.length} đơn hàng</p>
          </div>
          <select
            value={filterSeries}
            onChange={e => setFilterSeries(e.target.value)}
            className="bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-white/60 text-xs focus:outline-none focus:border-white/20 cursor-pointer"
          >
        ...
        {filteredRevenues.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-white/25 text-sm">Chưa có giao dịch nào</p>
            <button
              onClick={() => setShowAddRevenue(true)}
              className="mt-4 flex items-center gap-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 text-xs font-medium px-4 py-2 rounded-lg transition-colors cursor-pointer whitespace-nowrap"
            >
              <i className="ri-add-line"></i>
          ...
          <div className="space-y-2">
            {filteredRevenues.map(r => {
              const s = seriesList.find(s => s.id === r.seriesId);
              return (
                <div key={r.id} className="flex items-center gap-4 px-4 py-3 bg-white/3 rounded-xl border border-white/5 hover:border-white/8 transition-colors group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-white/30 text-[10px]">{s?.name ?? "Khác"}{r.note ? ` · ${r.note}` : ""}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="text-emerald-400 text-sm font-bold">{formatVND(r.amount)}</p>
                    <p className="text-white/25 text-[10px]">{new Date(r.date).toLocaleDateString("vi-VN")}</p>
                  </div>
                  <button
      ...
      {/* Tips */}
      <div className="mt-5 bg-[#e8c84a]/5 border border-[#e8c84a]/15 rounded-xl p-4 flex items-start gap-3">
        <i className="ri-lightbulb-line text-[#e8c84a] text-sm mt-0.5 flex-shrink-0"></i>
        <div>
          <p className="text-[#e8c84a]/80 text-xs font-semibold mb-1">Chiến lược bán ebook hiệu quả</p>
