        ...
        <div className="p-5 space-y-4">
          <div>
            ...
            >
              ...
              <option value="" className="bg-app-bg">Khác (không thu?c series)</option>
            </select>
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-app-text-secondary text-xs font-medium block mb-1.5">S? ti?n (VNÐ) *</label>
              <input
                type="number"
                value={amount}
...
export default function StatsPage() {
  ...
  return (
    <DashboardLayout
      title="Th?ng kê & Doanh thu"
      subtitle="Theo dõi hi?u qu? kinh doanh ebook"
      actions={
        <button
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <StatCard icon="ri-money-dollar-circle-line" label="T?ng doanh thu" value={formatVND(stats.totalRevenue)} sub={`${revenues.length} don hàng`} color="#34d399" />
        <StatCard icon="ri-calendar-line" label="Tháng này" value={formatVND(stats.monthRevenue)} sub={`D? ki?n: ${formatVND(stats.projectedMonthly)}`} color="app-accent-primary" />
        <StatCard icon="ri-stack-line" label="T?ng series" value={seriesList.length} sub="ebook dang bán" color="#fb923c" onClick={() => navigate("/series")} />
        <StatCard icon="ri-book-open-line" label="Bài h?c s?n sàng" value={approvedLessons.filter(l => (l.stars ?? 0) >= 4).length} sub="4-5 sao" color="#a78bfa" onClick={() => navigate("/ebook")} />
      </div>
        ...
        <div className="col-span-2 bg-app-bg border border-app-border rounded-2xl p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-app-text-muted text-xs mt-0.5">Theo tháng (VNÐ)</p>
            </div>
          ...
          {revenues.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-app-text-muted text-sm mb-1">Chua có doanh thu nào</p>
              <p className="text-white/15 text-xs">Nh?n "Ghi nh?n doanh thu" d? b?t d?u theo dõi</p>
            </div>
          ) : (
            <>
      ...
      <div className="bg-app-bg border border-app-border rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-app-text-muted text-xs mt-0.5">{revenues.length} don hàng</p>
          </div>
          <select
            value={filterSeries}
        ...
        {filteredRevenues.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-app-text-muted text-sm">Chua có giao d?ch nào</p>
            <button
              onClick={() => setShowAddRevenue(true)}
      ...
      <div className="mt-5 bg-app-accent-primary/5 border border-app-accent-primary/15 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-app-accent-primary/80 text-xs font-semibold mb-1">Chi?n lu?c bán ebook hi?u qu?</p>
          <p className="text-app-text-secondary text-xs leading-relaxed">
            Bán qua <strong className="text-white/60">Zalo OA</strong> (mi?n phí, ti?p c?n h?c viên cu) · 
            <strong className="text-white/60"> Gumroad</strong> (thanh toán qu?c t?, t? d?ng giao file) · 
            <strong className="text-white/60"> Facebook Group</strong> (viral trong c?ng d?ng K-pop). 
            Giá lý tu?ng: 29.000d–79.000d/ebook, bundle 3 ebook gi?m 20%.
          </p>
        </div>
      </div>
