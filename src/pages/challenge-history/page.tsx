...
function WeekCard({ record, isExpanded, onToggle }: {
  ...
  return (
    <div className={`bg-[#0f1117] border rounded-2xl overflow-hidden transition-all ${isFullComplete ? "border-emerald-500/20" : "border-white/5"}`}>
      {/* Header row */}
      <button
        onClick={onToggle}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3">
            <span className="text-white/40 text-xs">{record.completedCount}/{record.totalCount} thử thách</span>
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            ...
            <p className="text-white/30 text-[10px]">XP nhận được</p>
          </div>
          ...
          {isExpanded ? <i className="ri-arrow-up-s-line text-white/30 text-lg"></i> : <i className="ri-arrow-down-s-line text-white/30 text-lg"></i>}
        </div>
      </button>
      ...
      {isExpanded && (
        <div className="px-5 pb-5 border-t border-white/5 pt-4">
          <p className="text-white/30 text-xs mb-3">Thử thách đã hoàn thành:</p>
          <div className="grid grid-cols-2 gap-2">
            {Object.entries(CHALLENGE_META).map(([id, meta]) => {
              ...
              return (
                <div
                  key={id}
                  className={`flex items-center gap-2.5 p-2.5 rounded-xl ${done ? "bg-white/3" : "opacity-30"}`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-xs font-medium truncate ${done ? "text-white/70" : "text-white/25"}`}>{meta.title}</p>
          ...
          {record.bonusClaimed && (
            <div className="mt-3 flex items-center gap-2 p-2.5 rounded-xl bg-[#e8c84a]/8 border border-[#e8c84a]/15">
              <i className="ri-vip-crown-fill text-[#e8c84a] text-sm"></i>
              <p className="text-[#e8c84a] text-xs font-semibold">Bonus hoàn thành tất cả: +300 XP</p>
            </div>
          )}
        </div>
      )}
          ...
          <div className="flex items-center gap-2 mb-4">
            {([["all", "Tất cả"], ["perfect", "Hoàn hảo"], ["incomplete", "Chưa hoàn thành"]] as const).map(([key, label]) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer whitespace-nowrap ${filterView === key ? "bg-[#e8c84a]/15 text-[#e8c84a] border border-[#e8c84a]/25" : "bg-white/3 text-white/40 border border-white/8 hover:text-white/60"}`}
              >
                {label}
                {key === "perfect" && perfectWeeks > 0 && (
                  <span className="ml-1.5 px-1.5 py-0.5 bg-[#e8c84a]/20 text-[#e8c84a] text-[9px] font-bold rounded-full">{perfectWeeks}</span>
                )}
              </button>
            ))}
          </div>
          ...
          <div className="space-y-3">
            {filteredHistory.length === 0 ? (
              <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-12 text-center">
                ...
                <p className="text-white/30 text-sm">Không có tuần nào phù hợp</p>
              </div>
            ) : (
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-5">
              ...
              {history.slice(0, 8).reverse().map((r, i) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      backgroundColor: isPerfect ? "#e8c84a20" : isGood ? "#fb923c15" : "rgba(255,255,255,0.03)",
                      color: isPerfect ? "#e8c84a" : isGood ? "#fb923c" : "rgba(255,255,255,0.2)",
                      border: `1px solid ${isPerfect ? "#e8c84a30" : isGood ? "#fb923c20" : "rgba(255,255,255,0.05)"}`,
                    }}
                    title={r.weekLabel}
                  >
                    {isPerfect ? "★" : isGood ? "✓" : "·"}
            ...
            <div className="flex items-center gap-3 mt-3 text-[10px] text-white/25">
              <span className="flex items-center gap-1"><span className="text-[#e8c84a]">★</span> Hoàn hảo</span>
          ...
          <div className="bg-gradient-to-br from-[#1a1600] to-[#0f1117] border border-[#e8c84a]/15 rounded-2xl p-5">
            ...
            <div className="space-y-2 text-white/40 text-xs leading-relaxed">
              <p><i className="ri-arrow-right-s-line text-[#e8c84a] mr-1"></i>Ưu tiên thử thách Streak 7 ngày trước</p>
              <p><i className="ri-arrow-right-s-line text-[#e8c84a] mr-1"></i>Flashcard Marathon dễ hoàn thành nhất</p>
              <p><i className="ri-arrow-right-s-line text-[#e8c84a] mr-1"></i>Cập nhật tiến độ mỗi ngày để không quên</p>
              <p><i className="ri-arrow-right-s-line text-[#e8c84a] mr-1"></i>Hoàn thành 4+ thử thách để giữ streak</p>
            </div>
          </div>
        </div>
      </div>
