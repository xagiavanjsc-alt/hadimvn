...
const CHALLENGE_META: Record<string, { title: string; icon: string; color: string; xpReward: number }> = {
  ...
  streak_7:    { title: "Không b? ngày nào",     icon: "ri-fire-line",       color: "#fb923c", xpReward: 200 },
...
function WeekCard({ record, isExpanded, onToggle }: {
  ...
  const label = isFullComplete ? "Hoàn h?o!" : pct >= 70 ? "T?t" : pct >= 40 ? "Trung bình" : "C?n c? g?ng";
    ...
    <div className={`bg-app-bg border rounded-2xl overflow-hidden transition-all ${isFullComplete ? "border-emerald-500/20" : "border-app-border"}`}>
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            {isFullComplete && (
              <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-app-accent-success/15 text-app-accent-success">
                <i className="ri-vip-crown-fill"></i>Hoàn h?o
              </span>
            )}
          </div>
          <div className="flex items-center gap-3">
            <span className="text-app-text-secondary text-xs">{record.completedCount}/{record.totalCount} th? thách</span>
      ...
      {isExpanded && (
        <div className="px-5 pb-5 border-t border-app-border pt-4">
          <p className="text-app-text-muted text-xs mb-3">Th? thách dã hoàn thành:</p>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px]" style={{ color: done ? meta.color : "rgba(255,255,255,0.2)" }}>
                      {done ? `+${meta.xpReward} XP` : "Chua hoàn thành"}
                    </p>
                  </div>
                </div>
              );
          ...
          {record.bonusClaimed && (
            <div className="mt-3 flex items-center gap-2 p-2.5 rounded-xl bg-app-accent-primary/8 border border-app-accent-primary/15">
              ...
              <p className="text-app-accent-primary text-xs font-semibold">Bonus hoàn thành t?t c?: +300 XP</p>
            </div>
          )}
        </div>
      )}
...
export default function ChallengeHistoryPage() {
  ...
  return (
    <DashboardLayout
      title="L?ch s? th? thách"
      subtitle="Xem l?i k?t qu? các tu?n tru?c và streak th? thách c?a b?n"
    >
      {/* Summary stats */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Streak th? thách", value: `${challengeStreak} tu?n`, icon: "ri-fire-line", color: "#fb923c", sub: "Liên ti?p =50% th? thách" },
          { label: "Tu?n hoàn h?o", value: perfectWeeks, icon: "ri-vip-crown-line", color: "app-accent-primary", sub: `/${history.length} tu?n dã ghi nh?n` },
          { label: "T?ng XP t? th? thách", value: totalXPFromChallenges.toLocaleString(), icon: "ri-star-line", color: "#34d399", sub: "Tích luy t? t?t c? tu?n" },
          { label: "T? l? hoàn thành TB", value: `${avgCompletion}%`, icon: "ri-bar-chart-line", color: "#a78bfa", sub: "Trung bình m?i tu?n" },
        ].map(s => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mb-4">
            {([["all", "T?t c?"], ["perfect", "Hoàn h?o"], ["incomplete", "Chua hoàn thành"]] as const).map(([key, label]) => (
              <button
                key={key}
          ...
          <div className="space-y-3">
            {filteredHistory.length === 0 ? (
              <div className="bg-app-bg border border-app-border rounded-2xl p-12 text-center">
                ...
                <p className="text-app-text-muted text-sm">Không có tu?n nào phù h?p</p>
              </div>
            ) : (
        ...
        <div className="space-y-4">
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <h3 className="text-white font-semibold text-sm mb-4">Streak th? thách</h3>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-app-text-secondary text-xs">tu?n liên ti?p</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 mt-3 text-[10px] text-app-text-muted">
              <span className="flex items-center gap-1"><span className="text-app-accent-primary">?</span> Hoàn h?o</span>
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <h3 className="text-white font-semibold text-sm mb-4">XP theo lo?i th? thách</h3>
          ...
          <div className="bg-gradient-to-br from-app-surface to-[#0f1117] border border-app-accent-primary/15 rounded-2xl p-5">
            ...
            <div className="space-y-2 text-app-text-secondary text-xs leading-relaxed">
              <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Uu tiên th? thách Streak 7 ngày tru?c</p>
              <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Flashcard Marathon d? hoàn thành nh?t</p>
              <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>C?p nh?t ti?n d? m?i ngày d? không quên</p>
              <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Hoàn thành 4+ th? thách d? gi? streak</p>
            </div>
          </div>
        </div>
      </div>
