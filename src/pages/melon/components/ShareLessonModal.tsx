...
export default function ShareLessonModal({ song, result, onClose }: ShareLessonModalProps) {
  ...
  function drawCard() {
    ...
    ctx.fillText("?? Hàn Qu?c Oi!", 44, 42);
    ...
    ctx.fillStyle = theme.accent;
    ctx.font = "bold 13px system-ui";
    ctx.fillText("H?c ti?ng Hàn qua K-pop · hanquocoi.app", 48, bottomY + 25);
  }
  ...
  const handleCopyText = async () => {
    const text = [
      `?? H?c ti?ng Hàn qua K-pop — "${song.title}" (${song.artist})`,
      "",
      "?? T? v?ng n?i b?t:",
      ...topVocab.map((v) => `• ${v.word} = ${v.meaning}`),
      "",
      "H?c thêm t?i Hàn Qu?c Oi! ????",
    ].join("\n");
  ...
  const handleShare = async () => {
    const text = `?? H?c ti?ng Hàn qua "${song.title}" — ${song.artist}\n\n` +
      topVocab.map((v) => `• ${v.word} = ${v.meaning}`).join("\n") +
      "\n\n#HànQu?cOi #KoreanLearning #Kpop";
      ...
      <div className="relative w-full sm:max-w-xl bg-app-bg sm:rounded-2xl rounded-t-2xl border border-app-border flex flex-col max-h-[95vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-app-border flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <p className="text-white font-semibold text-sm">Chia s? bài h?c</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {/* Theme selector */}
          <div className="mb-5">
            <p className="text-app-text-secondary text-xs tracking-normal mb-2">Ch? d? màu s?c</p>
          ...
          <div className="grid grid-cols-2 gap-2 mb-3">
            ...
            >
              ...
              {copied ? "Ðã sao chép!" : "Sao chép van b?n"}
            </button>
          </div>
          ...
          >
            ...
            {shareSuccess ? "Ðã chia s?!" : "Chia s? ngay"}
          </button>
