  ...
  const todaySuggestions = useMemo(() => {
    ...
    if (epsDone < 10)
      suggestions.push({
        ...
        path: "/eps",
        desc: "Chua làm nhi?u — b?t d?u ngay!",
      });
    if (knownCount < 20)
      suggestions.push({
        icon: "ri-stack-line",
        ...
        path: "/flashcard-hub",
        desc: "Tang v?n t? v?ng m?i ngày",
      });
    suggestions.push({
      icon: "ri-sun-line",
      color: "app-accent-primary",
      label: "H?c t? m?i hôm nay",
      path: "/daily-words",
      desc: `${todayLearned.length}/8 t? dã h?c`,
    });
    if (examResults.length === 0)
      suggestions.push({
        icon: "ri-timer-line",
        color: "#fb923c",
        label: "Thi th? EPS (40 câu)",
        path: "/eps-exam",
        desc: "Ki?m tra trình d? c?a b?n",
      });
    return suggestions.slice(0, 3);
  }, [epsDone, knownCount, examResults, todayLearned]);

  return (
    <DashboardLayout title="Trang ch?" subtitle="Hàn Qu?c Oi! — H?c ti?ng Hàn hi?u qu?">
      <div className="max-w-5xl mx-auto px-3 sm:px-4 py-3 sm:py-5 space-y-3 sm:space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
          ...
          <StatCard
            ...
            label="EPS chính xác"
            value={`${epsAccuracy}%`}
            sub={`${epsDone} câu dã làm`}
          />
          <StatCard
            icon="ri-trophy-line"
            color="#a78bfa"
            label="Ði?m thi cao nh?t"
            value={bestExamPct > 0 ? `${bestExamPct}%` : "—"}
            sub={`${knownCount} t? thu?c lòng`}
          />
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2 sm:mb-3">
            ...
            >
              L? trình <i className="ri-arrow-right-line" />
            </button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
            <QuickCard
            ...
            <QuickCard
              ...
              color="#fb923c"
              label="L? trình h?c"
              desc="Cá nhân hóa theo m?c tiêu"
        ...
        <div>
          <div className="flex items-center justify-between mb-2 sm:mb-3">
            ...
            >
              Xem th?ng kê <i className="ri-arrow-right-line" />
            </button>
          </div>
          <div className="bg-app-bg border border-app-border rounded-2xl overflow-hidden divide-y divide-white/5">
            <ToolRow
              ...
              stat={`${melonStats.total} bài · TB ${melonStats.avgVocab} t?/bài`}
              path="/melon"
            ...
            <ToolRow
              ...
              color="#38bdf8"
              title="Naver KiN"
              desc="Câu h?i th?c t? ? AI ? Excel"
              stat={`${naverStats.total} Q&A · ${naverStats.overallRate}% x? lý`}
              path="/naver"
            ...
            <ToolRow
              ...
              color="#a78bfa"
              title="Ebook Builder"
              desc="Gom bài ch?n l?c ? Xu?t PDF"
            ...
            <ToolRow
              ...
              desc="Streak, t? dã h?c, so sánh tu?n"
            ...
            {isAdmin && (
              <ToolRow
                ...
                color="#fb923c"
                title="T?i lên d? li?u (Admin)"
                desc="Nh?p bài h?c K-pop và Naver t? CSV"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
            {[
              ...
              },
              {
                ...
                color: "app-accent-primary",
                label: "Hán Hàn VIP",
                desc: "2.691 t? Hán Hàn",
                path: "/hanja-detail",
                sub: "T? di?n chuyên sâu",
              },
            ].map((p) => (
              <button
                key={p.path}
