  ...
  return (
    <div className="bg-[#0f1117] border border-white/5 rounded-2xl p-4 md:p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 flex items-center justify-center bg-[#e8c84a]/10 rounded-lg">
            <i className="ri-sun-line text-[#e8c84a] text-sm" />
          </div>
          <div>
            <h3 className="text-white font-semibold text-sm">Từ điển hôm nay</h3>
            <p className="text-white/25 text-[10px]">{today} · 5 từ</p>
          </div>
        </div>
        <button
          onClick={() => navigate("/dictionary")}
          className="text-[#e8c84a] text-xs hover:text-[#d4b43a] cursor-pointer whitespace-nowrap"
        >
          ...
          return (
            ...
            >
              <div className="flex-1 min-w-0">
                {isFlipped ? (
                  <div>
                    <p className="text-[#e8c84a] text-sm font-bold">{v.word}</p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-white/20 text-[10px] ml-auto">Click để xem nghĩa</span>
                  </div>
                )}
              </div>
              <i
                className={`text-xs flex-shrink-0 ${
                  isFlipped ? "ri-eye-off-line text-[#e8c84a]/40" : "ri-eye-line text-white/15"
                }`}
              />
            </button>
          );
      ...
      <div className="mt-3 pt-3 border-t border-white/5 flex items-center justify-between">
        <p className="text-white/20 text-[10px]">Thay đổi mỗi ngày lúc 0:00</p>
        <button
          onClick={() => setFlipped(new Set([0, 1, 2, 3, 4]))}
          className="text-white/25 hover:text-white/50 text-[10px] cursor-pointer whitespace-nowrap"
        >
          Lật tất cả
        </button>
      </div>
