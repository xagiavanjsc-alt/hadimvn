...
function generateHeatmap(srsData: SRSData): DayActivity[] {
  ...
  for (let i = 363; i >= 0; i--) {
    ...
    const key = d.toISOString().split("T")[0];
    // Ch? dùng data th?t t? SRS — không b?a fake data
...
export default function VocabStatsPage() {
  ...
  const ratingDist = [1, 2, 3, 4, 5].map(r => ({
    ...
    label: ["Không nh?", "Khó", "Nh? du?c", "D?", "R?t d?"][r - 1],
  ...
  return (
    <DashboardLayout
      title="Th?ng kê t? v?ng cá nhân"
      subtitle="Heatmap ôn t?p, t?c d? h?c và d? doán th?i gian thu?c h?t"
    >
      {/* Top stats */}
      <div className="grid grid-cols-5 gap-4 mb-6">
        {[
          { label: "T?ng t? v?ng", value: totalVocab, icon: "ri-book-open-line", color: "app-accent-primary", sub: "trong database" },
          { label: "Ðã thu?c", value: masteredCount, icon: "ri-check-double-line", color: "#34d399", sub: `${totalVocab > 0 ? Math.round((masteredCount / totalVocab) * 100) : 0}% t?ng s?` },
          { label: "Ðã ôn t?p", value: totalReviewed, icon: "ri-refresh-line", color: "#fb923c", sub: "l?n v?i SRS" },
          { label: "Streak hi?n t?i", value: `${currentStreak} ngày`, icon: "ri-fire-line", color: "#f87171", sub: "liên ti?p" },
        ...
        <div className="space-y-5">
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-white font-semibold text-sm">Heatmap ôn t?p (52 tu?n)</h3>
          ...
          {/* Weekly bar chart */}
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <h3 className="text-white font-semibold text-sm mb-4">Ho?t d?ng 7 ngày g?n nh?t</h3>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gradient-to-br from-app-surface to-[#0f1117] border border-app-accent-primary/20 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-white font-semibold text-sm">D? doán hoàn thành</h3>
            </div>
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-app-text-secondary text-xs">Còn l?i</span>
                <span className="text-white font-bold">{remaining} t?</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-app-text-secondary text-xs">T?c d? TB</span>
                <span className="text-app-accent-primary font-bold">{avgWordsPerDay} t?/ngày</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-app-text-secondary text-xs">Th?i gian u?c tính</span>
                <span className="text-white font-bold">{daysToFinish} ngày</span>
              </div>
              <div className="h-px bg-white/8 my-2"></div>
              <div className="text-center">
                <p className="text-app-text-muted text-xs mb-1">D? ki?n hoàn thành</p>
          ...
          {/* Rating distribution */}
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            <h3 className="text-white font-semibold text-sm mb-4">Phân b? dánh giá SRS</h3>
            {srsEntries.length === 0 ? (
              <p className="text-app-text-muted text-xs text-center py-4">Chua có d? li?u SRS. Hãy h?c flashcard!</p>
            ) : (
              <div className="space-y-2.5">
                {ratingDist.map(r => (
                  <div key={r.rating}>
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-5">
            ...
            <div className="space-y-2 text-xs text-app-text-secondary leading-relaxed">
              {masteredCount < 50 && (
                <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Hãy t?p trung ôn t?p t? co b?n (A1) tru?c d? xây n?n t?ng v?ng ch?c.</p>
              )}
              {avgWordsPerDay < 5 && (
                <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Tang t?c d? h?c lên 10 t?/ngày d? hoàn thành nhanh hon.</p>
              )}
              {currentStreak < 7 && (
                <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Duy trì streak 7 ngày liên ti?p d? tang hi?u qu? ghi nh?.</p>
              )}
              {avgInterval > 10 && (
                <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>Interval trung bình cao — b?n dang ghi nh? t?t! Ti?p t?c duy trì.</p>
              )}
              <p><i className="ri-arrow-right-s-line text-app-accent-primary mr-1"></i>H?c d?u d?n m?i ngày hi?u qu? hon h?c d?n vào cu?i tu?n.</p>
            </div>
          </div>
        </div>
      </div>
