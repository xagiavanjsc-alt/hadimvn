...
function BookCard({
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left p-3 rounded-xl border transition-all cursor-pointer group ${isSelected ? "border-white/20 bg-app-card/50" : "border-app-border bg-white/2 hover:border-app-border hover:bg-app-surface/50"}`}
    >
      <div className="flex items-center gap-3 mb-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-app-text-muted text-[10px]">{LEVEL_GROUP_LABELS[book.levelGroup]}</p>
        </div>
      ...
      <div className="flex items-center justify-between gap-2 text-[10px] text-app-text-secondary">
        ...
        <span>{book.totalGrammar} ng? pháp</span>
      </div>
...
function LessonRow({
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left flex items-center gap-3 md:gap-4 p-3 md:p-4 rounded-xl border transition-all cursor-pointer group ${isCompleted ? "border-emerald-500/20 bg-emerald-500/3 hover:border-emerald-500/30" : "border-app-border bg-white/2 hover:border-white/15 hover:bg-white/4"}`}
    >
      <div className={`w-9 h-9 md:w-10 md:h-10 flex items-center justify-center rounded-xl flex-shrink-0 text-sm font-bold ${isCompleted ? "bg-app-accent-success/15 text-app-accent-success" : "bg-app-card/50 text-app-text-muted"}`}>
      ...
      <i className="ri-arrow-right-s-line text-app-text-muted group-hover:text-white/50 transition-colors flex-shrink-0"></i>
    </button>
  );
}
...
function LessonModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-app-bg border border-app-border rounded-t-2xl md:rounded-2xl w-full md:max-w-2xl max-h-[92vh] md:max-h-[90vh] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-start justify-between p-4 md:p-6 border-b border-app-border flex-shrink-0">
            ...
            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
              ...
              {isCompleted && (
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-app-accent-success/15 text-app-accent-success">
                  <i className="ri-checkbox-circle-fill mr-1"></i>Ðã h?c
                </span>
              )}
            </div>
            <p className="text-white font-bold text-sm md:text-base">{lesson.titleVi}</p>
            <p className="text-app-text-secondary text-xs md:text-sm mt-0.5">{lesson.title}</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/8 text-app-text-secondary hover:text-white/70 transition-colors cursor-pointer flex-shrink-0">
            <i className="ri-close-line text-lg"></i>
          </button>
        </div>

        {/* Objectives */}
        <div className="px-4 md:px-6 py-2.5 bg-white/2 border-b border-app-border flex-shrink-0">
          <div className="flex flex-wrap gap-1.5">
            {lesson.objectives.map((obj, i) => (
              <span key={i} className="text-[10px] px-2 py-0.5 rounded-full bg-app-card/50 text-white/50">{obj}</span>
            ))}
          </div>
        </div>
        ...
        <div className="flex border-b border-app-border flex-shrink-0">
          {(["vocab", "grammar", "dialogue"] as const).map(t => (
            ...
            >
              {t === "vocab" ? `T? v?ng (${lesson.vocabulary.length})` : t === "grammar" ? `Ng? pháp (${lesson.grammarPoints.length})` : "H?i tho?i"}
            </button>
          ))}
        </div>
          ...
          {tab === "vocab" && (
            <div className="space-y-3 md:space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-app-text-secondary text-xs">Nh?n vào th? d? xem ví d?</p>
                {dbVocab.length > 0 && (
              ...
              {vocabLoading && (
                <div className="text-center py-4 text-app-text-muted text-xs">
                  <i className="ri-loader-4-line animate-spin mr-1"></i>Ðang t?i t? v?ng...
                </div>
              )}
              <div className="grid grid-cols-2 gap-2 md:gap-3">
                {(dbVocab.length > 0
                  ...
                  <button
                    ...
                    className="text-left p-3 md:p-4 rounded-xl border border-app-border bg-white/2 hover:border-white/15 hover:bg-white/4 transition-all cursor-pointer min-h-[90px] md:min-h-[100px]"
                  >
                    {!flipped[i] ? (
                      <>
                        ...
                        <p className="text-app-text-muted text-[9px] md:text-[10px] mt-1">{v.partOfSpeech}</p>
                      </>
                    ) : (
                      <>
                        <p className="text-app-text-secondary text-[10px] font-medium mb-1.5">Ví d?:</p>
              ...
              {lesson.culturalTip && (
                <div className="mt-3 p-3 md:p-4 rounded-xl border border-[#84cc16]/20 bg-[#84cc16]/5">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-[#84cc16] text-xs font-semibold mb-1">Ghi chú van hóa</p>
              ...
              {lesson.grammarPoints.map((g, i) => (
                <div key={i} className="border border-app-border rounded-xl overflow-hidden">
                  <button
                    ...
                    className="w-full flex items-center justify-between px-4 md:px-5 py-3 md:py-4 hover:bg-app-surface/50 transition-colors cursor-pointer"
                  >
                    ...
                    <i className={grammarOpen[i] ? "ri-arrow-up-s-line text-app-text-muted" : "ri-arrow-down-s-line text-app-text-muted"}></i>
                  </button>
                  {grammarOpen[i] && (
                    <div className="px-4 md:px-5 pb-4 md:pb-5 space-y-3 md:space-y-4">
                      <p className="text-white/60 text-xs md:text-sm leading-relaxed bg-app-surface/50 rounded-lg p-3">{g.explanation}</p>
                      {g.notes && (
                        <div className="flex items-start gap-2 p-3 rounded-lg bg-app-accent-primary/5 border border-app-accent-primary/10">
                      ...
                      <div className="space-y-2">
                        {g.examples.map((ex, j) => (
                          <div key={j} className="flex items-start gap-2 md:gap-3 p-2.5 md:p-3 rounded-lg bg-app-surface/50 border border-app-border">
                            <button
                            ...
                            <div>
                              ...
                              <p className="text-app-text-secondary text-[10px] md:text-xs mt-0.5">{ex.vietnamese}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                ...
                {lesson.dialogue.map((line, i) => (
                  <div key={i} className={`flex gap-2 md:gap-3 ${i % 2 === 0 ? "" : "flex-row-reverse"}`}>
                    ...
                    <div className={`flex-1 max-w-[80%] ${i % 2 === 0 ? "" : "text-right"}`}>
                      <p className="text-app-text-muted text-[9px] md:text-[10px] mb-1">{line.speaker}</p>
                      <div className={`inline-block p-2.5 md:p-3 rounded-xl text-left ${i % 2 === 0 ? "bg-app-card/50 border border-app-border" : "border"}`} style={i % 2 !== 0 ? { backgroundColor: `${book.color}10`, borderColor: `${book.color}25` } : {}}>
                        <div className="flex items-start gap-1.5 md:gap-2">
                          <div className="flex-1">
                            <p className="text-white text-xs md:text-sm font-medium">{line.text}</p>
                            <p className="text-app-text-secondary text-[10px] md:text-xs mt-1 italic">{line.translation}</p>
                          </div>
                          <button
                            onClick={() => speakKorean(line.text)}
                            className="w-5 h-5 md:w-6 md:h-6 flex items-center justify-center rounded-md flex-shrink-0 cursor-pointer hover:bg-app-card/70 transition-colors"
                          >
                            <i className="ri-volume-up-line text-app-text-muted text-xs"></i>
                          </button>
                        </div>
                      </div>
                    </div>
        ...
        <div className="p-3 md:p-4 border-t border-app-border flex-shrink-0">
          {isCompleted ? (
            <div className="flex items-center justify-center gap-2 py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
              ...
              <span className="text-app-accent-success text-sm font-semibold">Ðã dánh d?u hoàn thành</span>
            </div>
          ) : (
            <button
...
function BookDetailView({
  ...
  return (
    <div>
      ...
      {/* Book header */}
      <div className="rounded-2xl p-4 md:p-6 mb-5 md:mb-6 border border-app-border" style={{ background: `linear-gradient(135deg, ${book.color}15, ${book.color}05)` }}>
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0 pr-3">
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              ...
              <span className="text-xs text-app-text-secondary">{book.cefrLevel}</span>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 md:gap-3 mt-4 md:mt-5">
          {[
            { label: "T?ng bài", value: book.totalLessons, icon: "ri-book-open-line" },
            { label: "T? v?ng", value: `~${book.totalVocab}`, icon: "ri-translate-2" },
            { label: "Ng? pháp", value: book.totalGrammar, icon: "ri-book-2-line" },
          ].map(s => (
      ...
      <h3 className="text-white font-semibold text-sm mb-3">
        Danh sách bài h?c ({validLessons.length} bài chi ti?t)
      </h3>
      <div className="space-y-2">
        ...
        {remainingCount > 0 && (
          <div className="p-4 rounded-xl border border-app-border bg-white/1 text-center">
            ...
            <p className="text-app-text-muted text-sm font-medium">+{remainingCount} bài h?c ti?p theo</p>
            <p className="text-app-text-muted text-xs mt-1">Ðang du?c c?p nh?t thêm n?i dung chi ti?t</p>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="bg-app-bg border border-app-border rounded-2xl p-4 md:p-5 mb-5 md:mb-6">
      <div className="flex items-center justify-between mb-4 md:mb-5">
        <div>
          ...
          <p className="text-app-text-muted text-xs mt-0.5">% hoàn thành t?ng cu?n sách</p>
        </div>
        <button
...
export default function SeoulTextbookPage() {
  ...
  return (
    <DashboardLayout
      ...
      subtitle="H?c ti?ng Hàn bài b?n theo giáo trình chu?n qu?c t?"
    >
      {!selectedBook ? (
        <>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            >
              <i className="ri-translate-2"></i>T? v?ng Hán-Hàn
            </button>
            <button
            ...
            >
              <i className="ri-font-size"></i>H?c Hán t? Seoul
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-5 md:mb-6">
            {[
              { label: "T?ng bài h?c", value: totalLessons, icon: "ri-book-open-line", color: "app-accent-primary" },
              { label: "Ðã hoàn thành", value: totalCompleted, icon: "ri-checkbox-circle-line", color: "#34d399" },
              { label: "T?ng t? v?ng", value: `~${totalVocab}`, icon: "ri-translate-2", color: "#a78bfa" },
              { label: "Ði?m ng? pháp", value: totalGrammar, icon: "ri-book-2-line", color: "#fb923c" },
            ].map(s => (
              <div key={s.label} className="bg-app-bg border border-app-border rounded-xl p-3 md:p-4 flex items-center gap-2 md:gap-3">
                ...
                <div>
                  ...
                  <p className="text-app-text-secondary text-[10px] md:text-xs mt-0.5">{s.label}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center gap-1.5 md:gap-2 mb-4 md:mb-5 flex-wrap">
            ...
            {levelGroups.map(g => (
              <button
                ...
                className={`px-3 md:px-4 py-1.5 md:py-2 rounded-lg text-xs font-semibold transition-colors cursor-pointer whitespace-nowrap ${filterGroup === g.id ? "text-app-bg" : "text-app-text-secondary hover:text-white/60 hover:bg-app-card/50"}`}
          ...
          <div className="bg-app-bg border border-app-border rounded-2xl p-4 md:p-5">
            <h3 className="text-white font-semibold text-sm mb-3 md:mb-4">L? trình h?c t?p</h3>
              ...
              {seoulBooks.map((book, i) => (
                <div key={book.id} className="flex items-center flex-shrink-0">
                  ...
                  >
                    ...
                    <p className="text-app-text-muted text-[9px] text-center whitespace-nowrap">{book.cefrLevel}</p>
                  </button>
                  {i < seoulBooks.length - 1 && (
                    <div className="w-5 md:w-8 h-0.5 bg-app-card/70 mx-0.5 md:mx-1 flex-shrink-0" />
                  )}
                </div>
              ))}
            </div>
