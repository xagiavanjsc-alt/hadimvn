...
export default function ProgressPage() {
  ...
  return (
    <DashboardLayout
      title="So sánh ti?n d?"
      subtitle="Bi?u d? radar phân tích di?m m?nh/y?u theo ch? d? EPS"
      actions={
        <button
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Câu dã làm", value: totalDone, sub: `/ ${epsQuestions.length} t?ng`, color: "app-accent-primary", icon: "ri-survey-line" },
          { label: "T? l? dúng", value: `${overallPct}%`, sub: `${totalCorrect} câu dúng`, color: "#34d399", icon: "ri-checkbox-circle-line" },
          { label: "L?n thi th?", value: examResults.length, sub: "bài thi dã làm", color: "#06b6d4", icon: "ri-timer-line" },
          { label: "Ði?m cao nh?t", value: bestExam > 0 ? `${bestExam}%` : "—", sub: "trong thi th?", color: "#a78bfa", icon: "ri-trophy-line" },
        ].map(s => (
          <div key={s.label} className="bg-app-bg border border-app-border rounded-2xl p-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-app-text-muted text-[10px]">{s.sub}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-[auto_1fr] gap-6 mb-6">
        ...
        <div className="bg-app-bg border border-app-border rounded-2xl p-6 flex flex-col items-center">
          ...
          <p className="text-app-text-muted text-xs mb-4 self-start">D?a trên k?t qu? luy?n t?p th?c t?</p>
          {totalDone === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center w-72">
              ...
              <p className="text-app-text-muted text-sm">Chua có d? li?u</p>
              <p className="text-white/15 text-xs mt-1">Làm câu h?i EPS d? xem bi?u d?</p>
              ...
              return (
                <div key={t.id}>
                  <div className="flex items-center gap-3 mb-1.5">
                    ...
                    <span className="text-app-text-muted text-[10px]">{d.correct}/{d.total} dúng</span>
                  ...
                  <p className="text-white/15 text-[10px] ml-9 mt-0.5">{totalQ} câu trong ngân hàng d?</p>
                </div>
              );
            })}
          </div>
      ...
      <div className="grid grid-cols-2 gap-6">
              ...
              <div className="flex items-center gap-1">
                {trend[trend.length - 1]?.pct > trend[0]?.pct ? (
                  <span className="text-app-accent-success text-xs font-semibold flex items-center gap-1">
                    <i className="ri-arrow-up-line"></i>Ðang ti?n b?
                  </span>
                ) : (
        ...
        <div className="bg-app-bg border border-app-border rounded-2xl p-5">
          <h3 className="text-white font-semibold text-sm mb-1">Ch? d? c?n ôn thêm</h3>
          <p className="text-app-text-muted text-xs mb-4">T?p trung vào nh?ng ch? d? y?u nh?t</p>

          {weakTopics.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <i className="ri-checkbox-circle-line text-white/10 text-3xl mb-2"></i>
              <p className="text-app-text-muted text-sm">Chua có d? li?u luy?n t?p</p>
            </div>
          ) : (
            <div className="space-y-2">
              {weakTopics.slice(0, 5).map((t, i) => (
                <button
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl bg-app-surface/50 hover:bg-app-card/50 transition-colors cursor-pointer text-left"
                >
                  <span className="text-app-text-muted text-xs font-bold w-4 flex-shrink-0">{i + 1}</span>
                  <div className="w-6 h-6 flex items-center justify-center rounded-lg flex-shrink-0" style={{ backgroundColor: `${t.color}15` }}>
                    <i className={`${t.icon} text-xs`} style={{ color: t.color }}></i>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-white/60 text-xs font-medium truncate">{t.label}</p>
                    <p className="text-app-text-muted text-[10px]">{t.done} câu dã làm</p>
                  </div>
