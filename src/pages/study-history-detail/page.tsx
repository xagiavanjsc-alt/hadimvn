...
export default function StudyHistoryDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // G?p quiz theo ngày d? tính trung bình score
      ...
      (historyRows || []).forEach((row: { study_date: string; study_time?: number; vocab_count?: number; grammar_count?: number }) => {
        ...
        records.push({
          ...
          activities: [
            ...
            { type: "grammar", label: "Ng? pháp", count: numGrammar, icon: "ri-stack-line", color: "text-violet-500" },
          ],
        });
      });
      setHistory(records);

      // Load t? dã bi?t t? study_progress.vocab_known / flashcard_known
      ...
      <div className="min-h-screen bg-[#f8f7f4] p-6">
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">Theo dõi chi ti?t t?ng ngày, t?ng t? dã h?c</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
          {[
            { label: "Ngày h?c", value: totalDays, icon: "ri-calendar-check-line", color: "text-teal-500", bg: "bg-teal-50" },
            { label: "T? dã h?c", value: totalWords, icon: "ri-translate-2", color: "text-sky-500", bg: "bg-sky-50" },
            { label: "Phút h?c", value: totalMinutes, icon: "ri-time-line", color: "text-violet-500", bg: "bg-violet-50" },
            { label: "T?ng XP", value: totalXP, icon: "ri-star-line", color: "text-amber-500", bg: "bg-amber-50" },
            { label: "Ði?m TB", value: `${avgScore}%`, icon: "ri-bar-chart-line", color: "text-emerald-500", bg: "bg-emerald-50" },
          ].map((s, i) => (
        ...
        <div className="flex gap-2 mb-5">
          {(["calendar", "list", "words"] as ViewMode[]).map(mode => (
            ...
            >
              ...
              {mode === "calendar" ? "L?ch h?c" : mode === "list" ? "Theo ngày" : "T? dã h?c"}
            </button>
          ))}
        </div>
              ...
              {selectedDay ? (
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    {[
                      { label: "Phút h?c", value: selectedDay.studyMinutes, icon: "ri-time-line", color: "text-violet-500" },
                      { label: "T? h?c", value: selectedDay.wordsLearned.length, icon: "ri-translate-2", color: "text-sky-500" },
                      { label: "Ði?m quiz", value: `${selectedDay.quizScore}%`, icon: "ri-survey-line", color: "text-amber-500" },
                  ...
                  <p className="text-xs font-semibold text-gray-400 tracking-normal mb-2">T? dã h?c ({selectedDay.wordsLearned.length})</p>
                ...
                <div className="flex items-center justify-center h-48 text-gray-400">
                  <div className="text-center">
                    ...
                    <p className="text-sm">Ch?n ngày d? xem chi ti?t</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-4 flex flex-col sm:flex-row gap-3">
              ...
              >
                ...
                <option value="mastery">Ð? thành th?o</option>
                <option value="review">S? l?n ôn</option>
              </select>
            </div>
                ...
                <div key={i} className="bg-white rounded-xl border border-gray-100 p-4">
                  ...
                  <div className="mb-2">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs text-gray-400">Ð? thành th?o</span>
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-400">
                    ...
                    <span><i className="ri-refresh-line mr-1"></i>Ôn: {w.reviewCount} l?n</span>
                  </div>
                </div>
              ))}
            </div>
