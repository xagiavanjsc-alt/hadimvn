...
const LEVELS = [
  { id: "A1", label: "A1 - So c?p 1", color: "#34d399", desc: "T? v?ng co b?n hàng ngày" },
  { id: "A2", label: "A2 - So c?p 2", color: "#38bdf8", desc: "T? v?ng giao ti?p thông d?ng" },
  { id: "B1", label: "B1 - Trung c?p 1", color: "#fb923c", desc: "T? v?ng xã h?i, công vi?c" },
  { id: "B2", label: "B2 - Trung c?p 2", color: "#f87171", desc: "T? v?ng h?c thu?t, nâng cao" },
];
...
function generateQuestions(words: VocabItem[], count: number): ListeningQuestion[] {
  ...
  return pool.map((word, idx) => {
    ...
    if (qType === "word-meaning") {
      ...
      question = `T? v?a nghe có nghia là gì?`;
    } else if (qType === "sentence-fill") {
      audioText = word.example;
      question = `Câu v?a nghe, t? "${word.korean}" có nghia là gì?`;
    } else {
      audioText = `${word.korean}. ${word.example}`;
      question = `Trong do?n h?i tho?i, "${word.korean}" có nghia là gì?`;
    }
    ...
    return {
      ...
      options: allOptions,
      correctIndex,
      explanation: `"${word.korean}" [${word.reading}] = ${word.vietnamese}. Ví d?: ${word.example} — ${word.exampleVi}`,
      word,
    };
  });
}
      ...
      <DashboardLayout>
        <div className="p-6 max-w-3xl mx-auto">
          <div className="mb-8">
            ...
            <p className="text-app-text-secondary text-sm">Nghe câu ti?ng Hàn r?i ch?n dáp án dúng — luy?n k? nang nghe cho k? thi TOPIK</p>
          </div>

          {/* How it works */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-8">
            {[
              { icon: "ri-volume-up-line", color: "#34d399", title: "Nghe", desc: "H? th?ng d?c câu ti?ng Hàn" },
              { icon: "ri-checkbox-multiple-line", color: "app-accent-primary", title: "Ch?n dáp án", desc: "4 l?a ch?n nghia ti?ng Vi?t" },
              { icon: "ri-bar-chart-line", color: "#fb923c", title: "Xem k?t qu?", desc: "Gi?i thích chi ti?t t?ng câu" },
            ].map((step) => (
          ...
          {/* Session size */}
          <div className="mb-5">
            <p className="text-white/60 text-xs tracking-normal mb-3">S? câu</p>
          ...
          <div className="mb-8 flex items-center gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-white/50 text-xs">T? d?ng phát âm</span>
            </label>
            <div className="flex items-center gap-2">
              ...
              {[0.7, 1.0, 1.2].map((r) => (
                ...
                >
                  {r === 0.7 ? "Ch?m" : r === 1.0 ? "Bình thu?ng" : "Nhanh"}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mb-4">
            <p className="text-app-text-muted text-xs">
              {filteredWords.length} t? có s?n ? c?p {selectedLevel}
          ...
          >
            ...
            B?t d?u luy?n nghe ({Math.min(sessionSize, filteredWords.length)} câu)
          </button>
        </div>
      </DashboardLayout>
    );
        ...
        <div className="p-6 max-w-3xl mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-bold text-white mb-1">
              {score >= 80 ? "Xu?t s?c!" : score >= 60 ? "Khá t?t!" : "C?n luy?n thêm!"}
            </h2>
            <p className="text-app-text-secondary text-sm">
              {correctCount}/{results.length} câu dúng · Th?i gian TB: {avgTime}s/câu
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
            {[
              { label: "Ðúng", value: correctCount, color: "#34d399" },
              { label: "Sai", value: results.filter((r) => !r.correct).length, color: "#f87171" },
              { label: "Ði?m", value: `${score}%`, color: "app-accent-primary" },
          ...
          <div className="flex gap-2 mb-4">
            {(["all", "correct", "wrong"] as const).map((tab) => (
              ...
              >
                {tab === "all" ? `T?t c? (${results.length})` : tab === "correct" ? `Ðúng (${correctCount})` : `Sai (${results.filter((r) => !r.correct).length})`}
              </button>
            ))}
          </div>
        ...
        <div className="bg-app-surface/50 border border-app-border rounded-2xl p-6 mb-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-app-text-muted text-xs">
                {currentQ?.type === "word-meaning" ? "Nghe t?" : currentQ?.type === "sentence-fill" ? "Nghe câu" : "Nghe h?i tho?i"}
              </span>
            </div>
            {!showResult && (
          ...
          <div className="flex flex-col items-center py-4">
            ...
            <p className="text-app-text-muted text-xs mb-1">
              {isPlaying ? "Ðang phát..." : `Nh?n d? nghe${playCount > 0 ? ` (dã nghe ${playCount} l?n)` : ""}`}
            </p>
          ...
          <div className={`rounded-xl p-4 mb-4 border ${selectedOption === currentQ?.correctIndex ? "bg-emerald-500/8 border-emerald-500/20" : "bg-red-500/8 border-red-500/20"}`}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className={`text-sm font-bold ${selectedOption === currentQ?.correctIndex ? "text-app-accent-success" : "text-red-400"}`}>
                {selectedOption === currentQ?.correctIndex ? "Chính xác!" : selectedOption === -1 ? "H?t gi?!" : "Chua dúng!"}
              </span>
            </div>
        ...
        {showResult && (
          ...
          >
            {currentIdx + 1 >= questions.length ? (
              ...
              <><i className="ri-arrow-right-line mr-2"></i>Câu ti?p theo</>
            )}
          </button>
        )}
      </div>
