...
const ALL_PERMISSIONS = [
  { id: "content.view", label: "Xem n?i dung", group: "N?i dung", icon: "ri-eye-line", color: "#60a5fa" },
  { id: "content.approve", label: "Duy?t bài vi?t", group: "N?i dung", icon: "ri-checkbox-circle-line", color: "#34d399" },
  { id: "content.delete", label: "Xóa bài vi?t", group: "N?i dung", icon: "ri-delete-bin-line", color: "#f87171" },
  { id: "reports.view", label: "Xem báo cáo", group: "Báo cáo", icon: "ri-flag-line", color: "#fb923c" },
  { id: "reports.resolve", label: "X? lý báo cáo", group: "Báo cáo", icon: "ri-shield-check-line", color: "#34d399" },
  { id: "users.view", label: "Xem thành viên", group: "Thành viên", icon: "ri-user-line", color: "#a78bfa" },
  { id: "users.vip", label: "C?p/H?y VIP", group: "Thành viên", icon: "ri-vip-crown-line", color: "app-accent-primary" },
  { id: "users.ban", label: "Khóa tài kho?n", group: "Thành viên", icon: "ri-user-forbid-line", color: "#f87171" },
  { id: "stats.view", label: "Xem th?ng kê", group: "Th?ng kê", icon: "ri-bar-chart-line", color: "#34d399" },
  { id: "system.broadcast", label: "G?i broadcast", group: "H? th?ng", icon: "ri-broadcast-line", color: "#f87171" },
  { id: "system.roles", label: "Qu?n lý phân quy?n", group: "H? th?ng", icon: "ri-shield-keyhole-line", color: "#f87171" },
  { id: "system.settings", label: "Cài d?t h? th?ng", group: "H? th?ng", icon: "ri-settings-3-line", color: "#f87171" },
  { id: "eps.edit", label: "Ch?nh s?a EPS", group: "N?i dung h?c", icon: "ri-edit-line", color: "app-accent-primary" },
  { id: "eps.upload", label: "Upload EPS", group: "N?i dung h?c", icon: "ri-upload-cloud-2-line", color: "app-accent-primary" },
];
...
const ROLE_PRESETS: Record<Role, { label: string; color: string; bg: string; icon: string; desc: string; permissions: string[] }> = {
  super_admin: {
    ...
    icon: "ri-shield-star-line", desc: "Toàn quy?n truy c?p và qu?n lý h? th?ng",
    permissions: ALL_PERMISSIONS.map(p => p.id),
  },
  smod: {
    label: "SMod", color: "#a78bfa", bg: "rgba(167,139,250,0.12)",
    icon: "ri-shield-keyhole-line", desc: "Qu?n lý c?ng d?ng, duy?t n?i dung, x? lý báo cáo, qu?n lý thành viên",
  ...
  moderator: {
    ...
    icon: "ri-shield-check-line", desc: "Duy?t n?i dung, x? lý báo cáo, qu?n lý thành viên co b?n",
  ...
  member: {
    ...
    icon: "ri-user-line", desc: "Không có quy?n admin",
    permissions: [],
  },
};
...
export default function AdminRolesPage() {
  ...
  const handleSave = async (userId: string, role: Role, permissions: string[]) => {
    ...
    try {
      ...
      if (rpcRes.error) {
        showToast(`L?i: ${rpcRes.error.message}`);
        return;
      }
      ...
      if (!rpcRes.data?.success) {
        showToast("? C?p nh?t th?t b?i - ki?m tra quy?n");
        return;
      }
      ...
      await loadRoles();
      setEditUser(null);
      showToast(`? Ðã c?p nh?t quy?n ? ${ROLE_PRESETS[role].label}`);
    } catch (err) {
      console.error("[handleSave] Exception:", err);
      const msg = err instanceof Error ? err.message : "L?i c?p nh?t quy?n";
      showToast(`L?i: ${msg}`);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <AdminLayout title="Phân quy?n Admin" subtitle={`Qu?n lý vai trò — d? li?u th?c t? Supabase (${roleUsers.length} thành viên)`}>
      {toast && (
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl mb-5 text-xs"
        ...
        D? li?u th?c t? Supabase. Thay d?i vai trò s? c?p nh?t tr?c ti?p vào DB (is_admin field).
      </div>
        ...
        <div className="flex items-center gap-1 p-1 rounded-lg" style={{ backgroundColor: "var(--admin-card2)" }}>
          {(["all", "super_admin", "smod", "moderator", "member"] as const).map(r => (
            <button key={r} onClick={() => setFilterRole(r)}
              ...
              {r === "all" ? "T?t c?" : ROLE_PRESETS[r].label}
            </button>
          ))}
        </div>
      </div>
              ...
              <div key={user.id} className="flex items-center gap-4 px-5 py-4 rounded-xl border"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px]" style={{ color: "var(--admin-text-faint)" }}>
                    ID: {user.id.slice(0, 12)}... · Ðang ký {new Date(user.created_at).toLocaleDateString("vi-VN")}
                  </p>
                </div>
                <div className="text-center flex-shrink-0 hidden lg:block">
                  ...
                  <p className="text-[9px]" style={{ color: "var(--admin-text-faint)" }}>quy?n</p>
                </div>
                <button onClick={() => setEditUser(user)}
                  ...
                  <i className="ri-edit-line"></i>Phân quy?n
                </button>
              </div>
            );
          })}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg rounded-2xl border overflow-hidden"
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: "var(--admin-border)" }}>
              <div>
                <p className="font-bold text-sm" style={{ color: "var(--admin-text)" }}>Phân quy?n: {editUser.display_name}</p>
                <p className="text-[10px]" style={{ color: "var(--admin-text-muted)" }}>Ch?n vai trò và luu vào Supabase</p>
              </div>
            ...
            <div className="px-5 py-4 border-t flex gap-3" style={{ borderColor: "var(--admin-border)" }}>
              ...
              <button onClick={() => handleSave(editUser.id, editUser.role, editUser.permissions)}
                ...
                {saving ? "Ðang luu..." : "Luu vào Supabase"}
              </button>
            </div>
          </div>
        </div>
